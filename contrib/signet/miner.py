...
def get_reward_addr_spk(args, height):
    ...
    reward_spk = unhexlify(args.bcli("getaddressinfo", reward_addr)["scriptPubKey"])
    if args.address is not None:
...
# ITCOIN_SPECIFIC START, new methods for solution independent blockchain miner
class DoGenerateArgs:
    # Util class for passing arguments to signet miner do_generate
    pass

def do_generate_next_block(args) -> tuple([CBlock, dict, str]):
    ...
    if tmpl["previousblockhash"] != bci["bestblockhash"]:
        logger.warning("GBT based off unexpected block (%s not %s), retrying", tmpl["previousblockhash"], bci["bestblockhash"])
        raise
    logger.debug("GBT template: %s", tmpl)
    ...
    block.nNonce = 0
    block.vtx = [cbtx] + [FromHex(CTransaction(), t["data"]) for t in tmpl["transactions"]]
    logger.debug(f"Block merkle root (function which includes signatures) after block creation = { ser_uint256(block.calc_merkle_root()).hex() }")
    ...
    block.vtx[0].vout.append(CTxOut(0, get_witness_script(witroot, witnonce)))
    block.vtx[0].rehash()
    logger.debug(f"Block merkle root (function which includes signatures) after appending witness commitment = { ser_uint256(block.calc_merkle_root()).hex() }")
    ...
    block.vtx[0].rehash()
    logger.debug(f"Block witness commitment after appending signet header = { block.vtx[0].vout[-1].scriptPubKey.hex() }")
    logger.debug(f"Block merkle root (function which includes signatures) after appending signet header = { ser_uint256(block.calc_merkle_root()).hex() }")
    ...
    block.nNonce = newhead.nNonce
    block.rehash()
    logger.debug(f"Block merkle root (function which includes signatures) after mining = { ser_uint256(block.calc_merkle_root()).hex() }")
...
def do_sign_block(args, block: CBlock, signet_spk) -> CBlock:
    ...
    signme, spendme = signet_txs(block, signet_spk_bin)
    logger.debug(f"signme.vin[0] = {signme.vin[0]}")

    # sign block
    psbt = do_createpsbt(block, signme, spendme)
    psbt_signed = args.bcli("walletprocesspsbt", psbt)
    if not psbt_signed.get("complete",False):
        logger.debug("Generated PSBT: %s" % (psbt,))
    ...
    logger.debug(f"Signet solution = { signet_solution.hex() }")
    logger.debug(f"Block merkle root (function which includes signatures) after signature = { ser_uint256(block_copy.calc_merkle_root()).hex() }")
...
def do_sign_block_partial(args: DoGenerateArgs, block: CBlock, signet_spk: str) -> Tuple[str, bool]:
    ...
    partially_signed_psbt = args.bcli("walletprocesspsbt", psbt)
...
def extract_block(args, psbt: str) -> CBlock:
    ...
    logger.debug(f"Signet solution = { signet_solution.hex() }")
    logger.debug(f"Block merkle root after signature = { ser_uint256(block_copy.calc_merkle_root()).hex() }")

    return block_copy

def do_propagate_block(args, block: CBlock):
    logger.debug(f"propagating block = {ToHex(block)}")
    logger.debug(f"with block_header = {CBlockHeader(block).serialize().hex()}")

    # submit block
    r = args.bcli("submitblock", ToHex(block))
    return r
# ITCOIN_SPECIFIC END
...
def do_generate(args):
    if args.max_blocks is not None:
        if args.ongoing:
            logger.error("Cannot specify both --ongoing and --max-blocks")
            return 1
    ...
    if args.set_block_time is not None and max_blocks != 1:
        logger.error("Cannot specify --ongoing or --max-blocks > 1 when using --set-block-time")
        return 1
    if args.set_block_time is not None and args.set_block_time < 0:
        args.set_block_time = time.time()
        logger.info("Treating negative block time as current time (%d)" % (args.set_block_time))

    if args.min_nbits:
        if args.nbits is not None:
            logger.error("Cannot specify --nbits and --min-nbits")
            return 1
        args.nbits = "1e0377ae"
        logger.info("Using nbits=%s" % (args.nbits))

    if args.set_block_time is None:
        if args.nbits is None or len(args.nbits) != 8:
            logger.error("Must specify --nbits (use calibrate command to determine value)")
            return 1
    ...
    if args.multiminer is None:
       my_blocks = (0,1,1)
        ...
        if not args.ongoing:
            logger.error("Cannot specify --multiminer without --ongoing")
            return 1
        m = RE_MULTIMINER.match(args.multiminer)
        if m is None:
            logger.error("--multiminer argument must be k/m or j-k/m")
            return 1
        ...
        if stop < start or start <= 0 or total < stop or total == 0:
            logger.error("Inconsistent values for --multiminer")
            return 1
    ...
    while max_blocks is None or mined_blocks < max_blocks:
        ...
        if lastheader is None:
            ...
            logger.info("Received new block at height %d; next in %s (%s)", bestheader["height"], seconds_to_hms(next_delta), ("mine" if next_is_mine else "backup"))
            lastheader = bestheader["hash"]

        # when is the next block due to be mined?
        now = time.time()
        if args.set_block_time is not None:
            logger.debug("Setting start time to %d", args.set_block_time)
            ...
            logger.info("Backdating time for first block to %d minutes ago" % (time_delta/60))
        ...
        if now < action_time:
            ...
            logger.debug("Sleeping for %s, next block due in %s (%s)" % (seconds_to_hms(sleep_for), seconds_to_hms(mine_time - now), minestr))
            time.sleep(sleep_for)
            continue

        # ITCOIN_SPECIFIC START generate block using the new methods, leave as comment the other lines to highlight the differences
        # gbt
        block, tmpl, reward_addr = do_generate_next_block(args)
        # tmpl = args.bcli("getblocktemplate", '{"rules":["signet","segwit"]}')
        if tmpl["previousblockhash"] != bci["bestblockhash"]:
            logger.warning("GBT based off unexpected block (%s not %s), retrying", tmpl["previousblockhash"], bci["bestblockhash"])
            time.sleep(1)
        ...
        if tmpl["mintime"] > mine_time:
            logger.info("Updating block time from %d to %d", mine_time, tmpl["mintime"])
            mine_time = tmpl["mintime"]
            if mine_time > now:
                logger.error("GBT mintime is in the future: %d is %d seconds later than %d", mine_time, (mine_time-now), now)
                return 1
        ...
        # mine block
        logger.debug("Mining block delta=%s start=%s mine=%s", seconds_to_hms(mine_time-bestheader["time"]), mine_time, is_mine)
        mined_blocks += 1
        ...
        logger.debug("Block hash %s payout to %s", block.hash, reward_addr)
        logger.info("Mined %s at height %d; next in %s (%s)", bstr, tmpl["height"], seconds_to_hms(next_delta), ("mine" if next_is_mine else "backup"))
        if r != "":
            logger.warning("submitblock returned %s for height %d hash %s", r, tmpl["height"], block.hash)
...
def bitcoin_cli(cli_function, command, *args, **kwargs):
    """A generic bitcoin_cli function
    """
    logger.debug("Calling bitcoin-cli: %r %r", command, list(args))
    result = cli_function(command, *args,  **kwargs)
    logger.debug("Result of bitcoin-cli: %r %r", command, result)
    return result

def subprocess_bitcoin_cli(basecmd, args, **kwargs):
    """A bitcoin_cli function that calls subprocess
    """
    cmd = basecmd + ["-signet"] + args
    logger.debug("Calling bitcoin-cli: %r", cmd)
    out = subprocess.run(cmd, stdout=subprocess.PIPE, **kwargs, check=True, 
        env={ "LD_LIBRARY_PATH": f'$LD_LIBRARY_PATH:{ITCOIN_PBFT_ROOT_DIR}/usrlocal/lib' }
    ).stdout
    ...
    # If json, convert string to dict
    try:
        out = json.loads(out)
    except ValueError as e:
        pass

    logger.debug("Result of bitcoin-cli: %r %r", cmd, out)
    return out
...
def main():
    ...
    args.bcli = lambda *a, input=b"", **kwargs: subprocess_bitcoin_cli(args.cli.split(" "), list(a), input=input, **kwargs)

...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing britanniacoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='britanniacoin-cli', help='britanniacoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='BritanniaCoin address to which the faucet should send')
...
parser.add_argument('britanniacoin_cli_args', nargs='*', help='Arguments to pass on to britanniacoin-cli (default: -signet)')

args = parser.parse_args()

if args.britanniacoin_cli_args == []:
    args.britanniacoin_cli_args = ['-signet']


def britanniacoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.britanniacoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = britanniacoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = britanniacoin_cli(['getnewaddress', 'faucet', 'bech32'])

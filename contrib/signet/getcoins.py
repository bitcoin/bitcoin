...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing syscoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='syscoin-cli', help='syscoin-cli command to use')
parser.add_argument('-f', '--faucet', dest='faucet', default=DEFAULT_GLOBAL_FAUCET, help='URL of the faucet')
parser.add_argument('-a', '--addr', dest='addr', default='', help='Syscoin address to which the faucet should send')
parser.add_argument('-p', '--password', dest='password', default='', help='Faucet password, if any')
parser.add_argument('syscoin_cli_args', nargs='*', help='Arguments to pass on to syscoin-cli (default: -signet)')

args = parser.parse_args()

if args.syscoin_cli_args == []:
    args.syscoin_cli_args = ['-signet']

if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    # Get the hash of the block at height 1 of the currently active signet chain
    try:
        curr_signet_hash = subprocess.check_output([args.cmd] + args.syscoin_cli_args + ['getblockhash', '1']).strip().decode()
...
if args.addr == '':
    ...
    try:
        args.addr = subprocess.check_output([args.cmd] + args.syscoin_cli_args + ['getnewaddress', 'faucet', 'bech32']).strip()
    except FileNotFoundError:
        print('The binary', args.cmd, 'could not be found.')
        exit()

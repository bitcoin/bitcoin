...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing snailcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='snailcoin-cli', help='snailcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Snailcoin address to which the faucet should send')
...
parser.add_argument('bitcoin_cli_args', nargs='*', help='Arguments to pass on to snailcoin-cli (default: -signet)')

args = parser.parse_args()

if args.snailcoin_cli_args == []:
    args.snailcoin_cli_args = ['-signet']


def bitcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.snailcoin_cli_args + rpc_command_and_params
    try:

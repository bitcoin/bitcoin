#!/usr/bin/env python3
# Copyright (c) 2020-2021 The Syscoin Core developers
...
signature file SHA256SUMS.asc from syscoincore.org and syscoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/syscoin-core/guix.sigs/tree/main/builder-keys
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Syscoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"syscoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by syscoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the syscoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes syscoin.org lags behind syscoincore.org.)')
    )

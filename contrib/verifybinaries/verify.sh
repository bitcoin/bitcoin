...
WORKINGDIR="/tmp/widecoin_verify_binaries"
TMPFILE="hashes.tmp"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'widecoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "widecoin.org failed to provide signature file, but widecoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "widecoin.org and widecoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Widecoin Core binary release signing key installed?"
   fi

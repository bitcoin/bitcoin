...
###   iopcore.org and iop.org and compares them.
...
WORKINGDIR="/tmp/iop_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://iopcore.org"
HOST2="https://iop.org"
BASEDIR="/bin/"
VERSIONPREFIX="iop-hd-"
RCVERSIONSTRING="rc"
...
if [ $? -ne 0 ]; then
   echo "iop.org failed to provide signature file, but iopcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "iop.org and iopcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the IoP HD binary release signing key installed?"
   fi

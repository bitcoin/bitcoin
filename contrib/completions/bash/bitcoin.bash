...
for target in bitcoind bitcoin_cli bitcoin_tx; do
    file="$(grep -l -E "complete[[:space:]]+-F[[:space:]]+_${target}" "$DIR"/* 2>/dev/null | head -n1)"
    [ -z "$file" ] && file="$(grep -l -E "^[[:space:]]*_${target}.*\(\)" "$DIR"/* 2>/dev/null | head -n1)"
    [ -n "$file" ] && source "$file"
done
...
_bitcoin() {
    ...
    case "${words[1]}" in
        node)
            _delegate bitcoind 2
            return 0
            ;;
        rpc)
            _delegate bitcoin-cli 2
            return 0
            ;;
        tx)
            _delegate bitcoin-tx 2
            return 0
            ;;
        -m|-M)
            if [[ "${words[2]}" == "node" ]]; then
                _delegate bitcoind 3
                return 0
            fi
            ;;
    esac

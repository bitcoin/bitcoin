...
EXAMPLE_CONF_FILE=${EXAMPLE_CONF_FILE:-$SHARE_EXAMPLES_DIR/widecoin.conf}

[ ! -x "$WIDECOIND" ] && echo "$WIDECOIND not found or not executable." && exit 1

DIRTY=""
VERSION_OUTPUT=$($WIDECOIND --version)
if [[ $VERSION_OUTPUT == *"dirty"* ]]; then
  DIRTY="${DIRTY}${WIDECOIND}\n"
fi

if [ -n "$DIRTY" ]
then
  echo -e "WARNING: $WIDECOIND was built from a dirty tree.\n"
  echo -e "To safely generate a widecoin.conf file, please commit your changes to $WIDECOIND, rebuild, then run this script again.\n"
fi

echo 'Generating example widecoin.conf file in share/examples/'
...
## Generated by contrib/devtools/gen-widecoin-conf.sh.
##
...
# parse the output from widecoind --help

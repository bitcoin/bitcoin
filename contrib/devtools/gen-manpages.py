...
VARS_TO_BINARIES = {
"ENABLE_BITCOIND":            "bin/bitcoind",
"ENABLE_CLI":                 "bin/bitcoin-cli",
"BUILD_BITCOIN_TX":           "bin/bitcoin-tx",
"ENABLE_WALLET_TOOL":         "bin/bitcoin-wallet",
"ENABLE_BITCOIN_UTIL":        "bin/bitcoin-util",
"BUILD_GUI":                  "bin/bitcoin-qt",
}
...
# Read config.ini generated by configure.
config = configparser.ConfigParser()
configfile = os.path.join(builddir, "test/config.ini")
config.read_file(open(configfile, encoding="utf8"))
# Get list of binaries that should have been installed, according to config.ini
installed_binaries = [VARS_TO_BINARIES[key] for key in VARS_TO_BINARIES if config["components"].getboolean(key) ]

# Verify that all the required binaries are usable, and extract copyright
# message in a first pass.
versions = []
for relpath in installed_binaries:
...
with tempfile.NamedTemporaryFile('w', suffix='.h2m') as footer:
    ...
    footer.write('\n[SEE ALSO]\n')
    footer.write(', '.join(s.rpartition('/')[2] + '(1)' for s in installed_binaries))
    footer.write('\n')
    footer.flush()

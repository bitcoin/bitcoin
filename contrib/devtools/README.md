...
The optional `coverage_check` argument controls which checks are run: `both`
runs both checks, `single` only checks each input individually, and `combined`
only checks all inputs in one go. The default is `both`.

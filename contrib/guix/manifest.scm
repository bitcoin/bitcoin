...
(define* (make-widecoin-cross-toolchain target
                                       #:key
...
desirable for building Widecoin Core release binaries."
...
(define-public glibc-2.27/widecoin-patched
  (package
    (inherit glibc-2.31)
    (version "2.27")
    (source (origin
    ...
    (cond ((string-suffix? "-mingw32" target)
          ...
          ((string-contains target "-linux-")
           (list (cond ((string-contains target "riscv64-")
                        (make-widecoin-cross-toolchain target
                                                      #:base-libc (make-glibc-with-stack-protector
                                                        (make-glibc-with-bind-now (make-glibc-without-werror glibc-2.27/widecoin-patched)))))
                       (else
                        (make-widecoin-cross-toolchain target)))))

# Bootstrappable BritanniaCoin Core Builds

This directory contains the files necessary to perform bootstrappable BritanniaCoin
Core builds.
...
To build BritanniaCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BritanniaCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/britanniacoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/britanniacoin-core/guix.sigs).

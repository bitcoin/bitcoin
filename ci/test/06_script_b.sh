#!/usr/bin/env bash
#
# Copyright (c) 2018-2022 The Syscoin Core developers
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  grep -v HAVE_SYS_GETRANDOM src/config/syscoin-config.h > src/config/syscoin-config.h.tmp && mv src/config/syscoin-config.h.tmp src/config/syscoin-config.h
fi
...
if [ "${RUN_TIDY}" = "true" ]; then
  ...
  # * secp256k1
  jq 'map(select(.file | test("src/qt/qrc_.*\\.cpp$|/moc_.*\\.cpp$|src/wallet/walletutil|src/secp256k1/src/|src/dashbls/|src/dashbls/depends/|src/dashbls/depends/relic/") | not))' ../compile_commands.json > tmp.json

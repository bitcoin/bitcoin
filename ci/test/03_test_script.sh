...
if [[ "${RUN_IWYU}" == true ]]; then
  ...
  FILES_WITH_ENFORCED_IWYU="/src/(((common|crypto|index|kernel|primitives|univalue/(lib|test)|util|zmq)/.*|node/blockstorage|node/utxo_snapshot|clientversion|core_io|signet)\\.cpp)"

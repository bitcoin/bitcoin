...
if [ -z "$NO_DEPENDS" ]; then
  BITCOIN_CONFIG_ALL="${BITCOIN_CONFIG_ALL} -DCMAKE_TOOLCHAIN_FILE=$DEPENDS_DIR/$HOST/toolchain.cmake -DCMAKE_PROJECT_TOP_LEVEL_INCLUDES=$DEPENDS_DIR/$HOST/dependency_provider.cmake"
fi
if [ -z "$NO_WERROR" ]; then

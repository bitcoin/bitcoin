...
- A new `listrawtransactions` RPC has been added to the wallet. Unlike
  ...
  funded the transaction, the fee paid (`fee`). The results support `count` and
  `skip` pagination, and a `verbose` flag that adds a `decoded` field with the
  full decoded transaction data. (#35813)

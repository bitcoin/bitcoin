...
*Setup instructions for a Gitian build of IoP Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the IoP
...
- [Building IoP Core](#building-iop-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-iop-host/README.md).
...
Clone the git repositories for iop and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/Internet-of-People/iop-core.git
...
in the iop repository under 'Fetch and create inputs' to install sources which require
...
To build IoP Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the iop repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/iop/.git/
...
For example:
```bash
URL=https://github.com/laanwj/iop.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit iop=${COMMIT} --url iop=${URL} ../iop/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit iop=${COMMIT} --url iop=${URL} ../iop/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit iop=${COMMIT} --url iop=${URL} ../iop/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the iop git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../iop/contrib/gitian-descriptors/*|sort|uniq )
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/iop-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/iop-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/iop-osx-build.assert
```
...
[iop-core/gitian.sigs](https://github.com/iop-core/gitian.sigs/) repository, or if that's not possible create a pull

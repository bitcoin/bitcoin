...
- [Building Dash Core](#building-chaincoin-core)
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/chaincoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/chaincoinpay/chaincoin
...
For example:
```bash
URL=https://github.com/crowning-/chaincoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit chaincoin=${COMMIT} --url chaincoin=${URL} ../chaincoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit chaincoin=${COMMIT} --url chaincoin=${URL} ../chaincoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit chaincoin=${COMMIT} --url chaincoin=${URL} ../chaincoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the chaincoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../chaincoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/chaincoin.git
SIGPATH=/some/root/path/chaincoin-detached-sigs.git

./bin/gbuild --url chaincoin=${BTCPATH},signature=${SIGPATH} ../chaincoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    ...
    gpg --detach-sign ${VERSION}-win/${SIGNER}/chaincoin-win-build.assert

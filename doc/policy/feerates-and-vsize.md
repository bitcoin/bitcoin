...
**BIP‑141 virtual size** (`vsize`) is simply the block weight divided
...
the field named `vsize_adjusted` represents the **sigop‑adjusted size**, reflecting the policy‑adjusted estimate
of block space usage. These same RPCs also provide a field called `vsize` which exposes the
pure BIP‑141 virtual size without sigop adjustment. Additionally, `getrawtransaction` now includes
a `vsize_adjusted` field and continues to report the unadjusted BIP-141 virtual size in its `vsize` field.

...
- [Development guidelines](#development-guidelines)
    - [General NdovuCoin Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
NdovuCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
NdovuCoin Core is a multi-threaded application, and deadlocks or other
...
NdovuCoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as NdovuCoin Core, where everyone uses
...
pay attention to for reviewers of NdovuCoin Core code.

General NdovuCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: NdovuCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of NdovuCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against NdovuCoin Core so that they can be integrated
...
file descriptors it will cause problems with NdovuCoin's `select()` loop, because
...
compatibility between nodes. This happened in NdovuCoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to NdovuCoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of NdovuCoin Core, the wallet was always

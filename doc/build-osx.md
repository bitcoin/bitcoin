...
Tortoisecoin Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.zip` containing the Tortoisecoin Core application using `make deploy`.
...
Tortoisecoin Core should now be available at `./src/tortoisecoind`.
If you compiled support for the GUI, it should be available at `./src/qt/tortoisecoin-qt`.
...
./src/qt/tortoisecoin-qt -server # Starts the tortoisecoin-qt server mode, allows tortoisecoin-cli control
```

...
PSBT is an interchange format for Syscoin transactions that are not fully signed
...
Overall, the construction of a fully signed Syscoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Syscoin transaction (in network format)
...
#### Multisig with multiple Syscoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Syscoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `syscoin-cli` is used.
...
Later, when *V* SYS has been received on *Amulti*, and Bob and Carol want to

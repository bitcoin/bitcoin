# PSBT Howto for Tortoisecoin Core

Since Tortoisecoin Core 0.17, an RPC interface exists for Partially Signed Tortoisecoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/tortoisecoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Tortoisecoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://tortoisecointalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed Tortoisecoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Tortoisecoin transaction (in network format)
...
#### Multisig with multiple Tortoisecoin Core instances
...
Tortoisecoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `tortoisecoin-cli` is used.

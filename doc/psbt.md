# PSBT Howto for vBitcoin Core

Since vBitcoin Core 0.17, an RPC interface exists for Partially Signed Bitcoin
...
PSBT is an interchange format for vBitcoin transactions that are not fully signed
...
Overall, the construction of a fully signed vBitcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid vBitcoin transaction (in network format)
...
#### Multisig with multiple vBitcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
vBitcoin Core. We assume their wallets only contain the multisig funds. In case
...
Later, when *V* vBTC has been received on *Amulti*, and Bob and Carol want to

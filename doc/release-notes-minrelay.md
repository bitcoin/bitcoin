...
configuration option. However, it is not recommended to set this value higher than the `-minrelaytxfee`, as some
transactions would be accepted and relayed but not mined.
...
are not guaranteed to propagate or confirm. The wallet feerates remain unchanged.

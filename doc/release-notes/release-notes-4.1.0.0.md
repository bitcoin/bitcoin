...
BEFORE INSTALLING 4.1.0, PLEASE READ: https://syscoin.readme.io/v4.1.0/docs/syscoin-41-upgrade-guide
...
syscoincheckmint
- `syscoincheckmint` Useful for those that want to check if a Syscoin Transaction
...
- In 4.0.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 4.1.0, this flag is now __disabled__ by default. If you want to compile Syscoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- #368 Bounty - CheckSyscoinMint cutoffHeight attacks
- #328 syscoin-geth.exe won't run if folder (windows) has space.
...
- #15683 Comment for seemingly duplicate LIBSyscoin_SERVER (Bushstar)

BritanniaCoin Core version 0.19.0.1 is now available from:

  <https://britanniacoincore.org/bin/britanniacoin-core-0.19.0.1/>
...
BritanniaCoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use BritanniaCoin Core on unsupported systems.

BritanniaCoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, BritanniaCoin Core does not yet change appearance when
macOS "dark mode" is activated.

Users running macOS Catalina may need to "right-click" and then choose "Open"
to open the BritanniaCoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the BritanniaCoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/britanniacoin/britanniacoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running BritanniaCoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of BritanniaCoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `britanniacoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile BritanniaCoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in BritanniaCoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of BritanniaCoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in BritanniaCoin Core,
  ...
  This prevents BritanniaCoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of BritanniaCoin Core and nodes that have manually enabled BIP37 support,
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how BritanniaCoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  BritanniaCoin Core would sequentially attempt to download the transaction from each
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 BritanniaCoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  BRT), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in BRT per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in BritanniaCoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@BritanniaCoin AEAD (jonasschnelli)
...
- #15639 britanniacoin-wallet tool: Drop libbritanniacoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as BRT/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in britanniacoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "britanniacoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of britanniacoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from britanniacoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_britanniacoin to auto generated project files (sipsorcery)
...
- #15773 Add BritanniaCoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_britanniacoin output (practicalswift)
...
- #15896 feature_filelock, interface_britanniacoin_cli: Use PACKAGE_NAME in messages rather than hardcoding BritanniaCoin Core (luke-jr)
...
- #16277 Suppress output in test_britanniacoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as BRT/kB (jonatack)
...
- #16489 log: harmonize britanniacoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (britanniacoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example britanniacoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBBRITANNIACOIN_SERVER (Bushstar)
...
- #16080 build/doc: update britanniacoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in britanniacoin.conf (fanquake)
...
- #16621 add default britanniacoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the britanniacoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, britanniacoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/britanniacoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/britanniacoin/britanniacoin/).

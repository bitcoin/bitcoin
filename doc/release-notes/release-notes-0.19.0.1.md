Buttcoin Core version 0.19.0.1 is now available from:

  <https://Buttcoincore.org/bin/Buttcoin-core-0.19.0.1/>
...
Buttcoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Buttcoin Core on unsupported systems.

Buttcoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Buttcoin Core does not yet change appearance when
macOS "dark mode" is activated.

Users running macOS Catalina may need to "right-click" and then choose "Open"
to open the Buttcoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Buttcoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/Buttcoin/Buttcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Buttcoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Buttcoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `Buttcoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Buttcoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Buttcoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Buttcoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Buttcoin Core,
  ...
  This prevents Buttcoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Buttcoin Core and nodes that have manually enabled BIP37 support,
...
- By default, Buttcoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `Buttcoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Buttcoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Buttcoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Buttcoin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Buttcoin address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Buttcoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Buttcoin AEAD (jonasschnelli)
...
- #15639 Buttcoin-wallet tool: Drop libButtcoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in Buttcoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "Buttcoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of Buttcoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from Buttcoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_Buttcoin to auto generated project files (sipsorcery)
...
- #15773 Add ButtcoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_Buttcoin output (practicalswift)
...
- #15896 feature_filelock, interface_Buttcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Buttcoin Core (luke-jr)
...
- #16277 Suppress output in test_Buttcoin for expected errors (gertjaap)
...
- #16489 log: harmonize Buttcoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (ButtcoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example Buttcoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBButtcoin_SERVER (Bushstar)
...
- #16080 build/doc: update Buttcoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in Buttcoin.conf (fanquake)
...
- #16621 add default Buttcoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the Buttcoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, Buttcoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/Buttcoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/Buttcoin/Buttcoin/).

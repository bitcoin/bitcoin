...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `naviod -help` output for these two options for
...
- The `testnet` field in `navio-cli -getinfo` has been renamed to
...
- #15639 navio-wallet tool: Drop libbitcoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in naviod/qt (MarcoFalke)
...
- #16258 [MSVC]: Create the config.ini as part of naviod build (NicolasDorier)
...
- #16489 log: harmonize naviod logging (jonatack)
...
- #16863 Add a missing closing parenthesis in the navio-wallet's help (darosior)
...
- #16912 Remove Doxygen intro from src/naviod.cpp (ch4ot1c)

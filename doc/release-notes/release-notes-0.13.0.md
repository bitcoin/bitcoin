...
For example:
    ...
    $ src/navio-cli -stdin walletpassphrase
    mysecretcode
    120
...
- #7550 `8b958ab` Input-from-stdin mode for navio-cli (laanwj)
...
- #7613 `3798e5d` Add autocomplete to navio-qt's console window (GamerSg)
...
- #8256 `af2421c` BUG: navio-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from naviod (sipa)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for naviod procs (MarcoFalke)
...
- #7744 `d8e862a` test_framework: detect failure of naviod startup (laanwj)

Syscoin Core version 0.13.1 is now available from:

  <https://syscoin.org/bin/syscoin-core-0.13.1/>
...
randomly crashing on Windows XP. It is [not clear](https://github.com/syscoin/syscoin2/issues/7681#issuecomment-217439891)
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Syscoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Syscoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Syscoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of syscoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of syscoins being spent is exactly
...
- **Increased security for multisig:** Syscoin addresses (both P2PKH addresses
  ...
  believed to be provided by Syscoin's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Syscoin paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Syscoin's current ledger (called the UTXO set),
  ...
  note that Syscoin Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Syscoin users to individually opt-in to almost any change in the Syscoin
  Script language when those users receive new transactions.  Features
  currently being researched by Syscoin Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Syscoin community members are studying several other improvements
...
Since Syscoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `syscoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `syscoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting

...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming IoP wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each IoP block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of iops they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of iops being spent is exactly
...
- **Increased security for multisig:** IoP addresses (both P2PKH addresses
  ...
  believed to be provided by IoP's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  IoP paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of IoP's current ledger (called the UTXO set),
...
- **Script versioning:** Segwit makes it easy for future soft forks to allow
  ...
  Other IoP community members are studying several other improvements
...
[BIP147]: https://github.com/iop/bips/blob/master/bip-0147.mediawiki
...
- `iop-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `iop-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in iopconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.iopstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libiopconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_iop by itself (luke-jr)
...
- #8829 `2a8bca4` Add iop-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` iop-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_iopds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name iop.qrc  (iopsSG)
...
- #8817 `bcf3806` update iop-tx to output witness data (jnewbery)

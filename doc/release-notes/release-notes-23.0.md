...
Snailcoin Core version 23.0 is now available from:
...
- A snailcoind node will no longer rumour addresses to inbound peers by default.
  They will become eligible for address gossip after sending an ADDR, ADDRV2,
  or GETADDR message. (#21528)

- Before this release, Snailcoin Core had a strong preference to try to connect only to peers that listen on port 8333. As a result of that, Snailcoin nodes listening on non-standard ports would likely not get any Snailcoin Core peers connecting to them. This preference has been removed. (#23542)
...
Snailcoin Core release binaries for Linux now include experimental tracepoints which
...
- The top-level fee fields `fee`, `modifiedfee`, `ancestorfees` and `descendantfees`
  ...
  fields in the `fees` object are denominated in SNAIL. (#22689)
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. Snailcoin Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Snailcoin Core in 22.0. (#22544)

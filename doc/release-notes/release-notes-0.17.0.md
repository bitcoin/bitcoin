Snailcoin Core version 0.17.0 is now available from:
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `snailcoin.conf` file, not inside included files or from
  ...
  as snailcoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the snailcoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
be used in V0.17 by starting snailcoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Snailcoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Snailcoin transactions that are not fully signed
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in SNAIL. This new field deprecates previous fee fields, such as
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart snailcoind with
...
- When running snailcoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If snailcoind
...
- #11962 `0a01843` add seed.snailcoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in SNAIL (mryandao)
...
- #13528 `b877c39` Move SnailcoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13604 `75bea05` Add depends 32-bit arm support for snailcoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling snailcoin-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use SnailcoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to snailcoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use SnailcoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use SnailcoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use SnailcoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for SnailcoinTestFramework (WillAyd)
...
- #13687 `9d26b69` travis: Check that ~/.snailcoin is never created (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in snailcoind systemd service file (Flowdalic)
- #12652 `55f490a` snailcoin-cli: Provide a better error message when snailcoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to snailcoind in debian package (ghost)
...
- #13725 `07ce278` Fix snailcoin-cli --version (Empact)
...
- #12877 `2b54155` Use snailcoind in Tor documentation (knoxcard)
...
- #13905 `73a09b4` fixed snailcoin-cli -help output for help2man (hebasto)

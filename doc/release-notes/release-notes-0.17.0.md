...
be used in V0.17 by starting naviod with the '-deprecatedrpc=accounts'
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart naviod with
...
- When running naviod with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If naviod
...
- #13486 `66e1a08` Move rpc/util.cpp from libnavio-util to libbitcoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for navio-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling navio-qt (TheCharlatan)
...
- #12102 `7fb8fb4` Apply hardening measures in naviod systemd service file (Flowdalic)
- #12652 `55f490a` navio-cli: Provide a better error message when naviod is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to naviod in debian package (ghost)
...
- #13603 `453ae5e` navio-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix navio-cli --version (Empact)
...
- #12877 `2b54155` Use naviod in Tor documentation (knoxcard)
...
- #13905 `73a09b4` fixed navio-cli -help output for help2man (hebasto)

...
- A new `-asmap` configuration option has been added to diversify the
  ...
  `naviod help` for more information.  This option is experimental and
...
- The `navio-cli` utility used with the `-getinfo` parameter now
...
- The `navio-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17741 Included `test_navio-qt` in msvc build (sipsorcery)
...
- #18702 Fix ASLR for navio-cli on Windows (fanquake)
...
- #17119 Fix broken navio-cli examples (andrewtoth)
...
- #17503 Remove bitness from navio-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to navio-wallet (hel-o)
...
- #16115 On naviod startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from navio-cli -getinfo (malevolent)

...
BritanniaCoin Core version 0.20.0 is now available from:

  <https://britanniacoincore.org/bin/britanniacoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from BritanniaCoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since BritanniaCoin Core version 0.18.0.
...
- Testing or debugging of implementations of the BritanniaCoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of BritanniaCoin Core. BritanniaCoin Core logs debug messages
...
- OpenSSL is no longer used by BritanniaCoin Core.  (#17265)

- BIP70 support has been fully removed from BritanniaCoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/britanniacoin/britanniacoin/issues/16599),
  [PR #16702](https://github.com/britanniacoin/britanniacoin/pull/16702), and the
  `britanniacoind help` for more information.  This option is experimental and
...
- All custom settings configured when BritanniaCoin Core starts are now
...
- The "Start BritanniaCoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://britanniacoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple BritanniaCoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed BritanniaCoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/britanniacoin-core/HWI) for
  signing.  Future versions of BritanniaCoin Core should support a GUI option
...
- BritanniaCoin Core's automatically-generated source code documentation is
  now available at https://doxygen.britanniacoincore.org.  (#17596)
...
- The `britanniacoin-cli` utility used with the `-getinfo` parameter now
...
- The `britanniacoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `britanniacoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_britanniacoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBBRITANNIACOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_britanniacoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBBRITANNIACOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for britanniacoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate britanniacoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest britanniacoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_britanniacoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (britanniacoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone britanniacoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about BritanniaCoin Core files (hebasto)
...
- #17119 Fix broken britanniacoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from britanniacoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.britanniacoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to britanniacoin-wallet (hel-o)
...
- #18341 Replace remaining literal BRT with `CURRENCY_UNIT` (domob1812)
...
- #16115 On britanniacoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from britanniacoin-cli -getinfo (malevolent)

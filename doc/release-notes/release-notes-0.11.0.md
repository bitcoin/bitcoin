XBit Core version 0.11.0 is now available from:

  <https://xbit.org/bin/xbit-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer XBT/kB fee to be
...
For example, add the following to `xbit.conf`:
...
related to the blockchain in the xbit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows XBit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that XBit Core will 
...
For an example script see [xbit-submittx](https://github.com/laanwj/xbit-submittx).
...
- #5334 `cf87536` libxbitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new xbit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only XBitAmountField
- #5753 `0f44672` Add xbit logo to about screen
...
- #5809 `46bfbe7` Add xbit-cli man page
...
- #5849 `41259ca` contrib/init/xbitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename XBit-Qt.app to XBit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for xbit-tx)

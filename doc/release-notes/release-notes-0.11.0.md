BritanniaCoin Core version 0.11.0 is now available from:

  <https://britanniacoin.org/bin/britanniacoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer BRT/kB fee to be
...
For example, add the following to `britanniacoin.conf`:
...
related to the blockchain in the britanniacoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows BritanniaCoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that BritanniaCoin Core will 
...
For an example script see [britanniacoin-submittx](https://github.com/laanwj/britanniacoin-submittx).
...
- #5334 `cf87536` libbritanniacoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new britanniacoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only BritanniaCoinAmountField
- #5753 `0f44672` Add britanniacoin logo to about screen
...
- #5809 `46bfbe7` Add britanniacoin-cli man page
...
- #5849 `41259ca` contrib/init/britanniacoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename BritanniaCoin-Qt.app to BritanniaCoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for britanniacoin-tx)

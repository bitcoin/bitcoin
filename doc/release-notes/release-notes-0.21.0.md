...
XBit Core version 0.21.0 is now available from:

  <https://xbitcore.org/bin/xbit-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/xbit/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by XBit Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. XBit Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/xbit/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/xbit/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/xbit/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/xbit/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/xbit/bips/blob/master/bip-0342.mediawiki)),
...
- A new `xbit-cli -netinfo` command provides a network peer connections
...
- A new `xbit-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `xbit-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `xbit-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when XBit Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `xbit-wallet` tool which performs the salvage
...
XBit Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`xbit.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
XBit Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `xbit-cli createwallet` or `xbit-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/xbit/xbit/issues).
...
Users who already have a XBit Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of XBit Core.
...
The change to using descriptors changes the default derivation paths used by XBit Core
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in XBT per 1,000
  vbytes (XBT/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from XBT/kB
  to sat/vB and the help documentation in XBT/kB is updated to XBT/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from XBT/kvB to sat/vB and the
  ...
  difference between XBT/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in XBT/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time XBit Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `xbit.conf`
...
- #19284 Add seed.xbit.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in xbit-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add xbit-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in xbit-cli (laanwj)
...
### Build system
- #18504 Drop xbit-tx and xbit-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in XBIT_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for xbit-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz XBit Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitrexbit)
...
- #18817 Document differences in xbitd and xbit-qt locale handling (practicalswift)

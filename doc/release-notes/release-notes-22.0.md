...
- When creating a hex-encoded bitcoin transaction using the `navio-tx` utility
...
- Passing an invalid `-rpcauth` argument now cause naviod to fail to start.  (#20461)
...
- A new `-rpcwaittimeout` argument to `navio-cli` sets the timeout
  in seconds to use with `-rpcwait`. If the timeout expires,
  `navio-cli` will report a failure. (#21056)
...
- bitcoin/bitcoin#21029 navio-cli: Correct docs (no "generatenewaddress" exists) (luke-jr)
...
### RPC and other APIs
- bitcoin/bitcoin#18335, bitcoin/bitcoin#21484 cli: Print useful error if naviod rpc work queue exceeded (LarryRuane)
...
- bitcoin/bitcoin#20549 Support make src/bitcoin-node and src/navio-gui (promag)
...
- bitcoin/bitcoin#20939 fix `RELOC_SECTION` security check for navio-util (fanquake)
...
- bitcoin/bitcoin#22230 Fix MSVC linker /SubSystem option for navio-qt.exe (hebasto)
...
- bitcoin/bitcoin#22270 Add navio-util tests (+refactors) (MarcoFalke)
...
- bitcoin/bitcoin#20715 util: Add argsmanager::getcommand() and use it in navio-wallet (MarcoFalke)
...
- bitcoin/bitcoin#20884 script: Improve robustness of naviod.service on startup (hebasto)
...
- bitcoin/bitcoin#21007 naviod: Add -daemonwait option to wait for initialization (laanwj)
...
- bitcoin/bitcoin#21695 contrib: Remove no longer used contrib/navio-qt.pro (hebasto)
...
- bitcoin/bitcoin#20913 Add manual page generation for navio-util (laanwj)

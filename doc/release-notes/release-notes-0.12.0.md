Snailcoin Core version 0.12.0 is now available from:
...
pre-0.12 versions of Snailcoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Snailcoin Core version 0.11 or
earlier.
...
ECDSA signatures inside Snailcoin transactions now use validation using
...
Previous versions of Snailcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Snailcoin Core was to raise the default minimum
relay fee.

Snailcoin Core 0.12 will have a strict maximum size on the mempool. The
...
Snailcoin Core 0.12 also introduces new default policy limits on the length and
...
Snailcoin Core 0.12 nodes. Snailcoin Core will only allow replacement of
...
Note that the wallet in Snailcoin Core 0.12 does not yet have support for
...
This allows running snailcoind without having to do any manual configuration.
...
Snailcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Snailcoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Snailcoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Snailcoin Core 0.13, and it is at this time undecided whether the
...
Snailcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Snailcoin Core automatically creates a hidden service to listen on, without
manual configuration. Snailcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Snailcoin Core is listening, and
...
Snailcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Snailcoin
Core to use floating fees. By default, Snailcoin Core will use floating
fees.
...
(default: `0.0002` SNAIL/kB).

At all times, Snailcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SNAIL.
Furthermore, Snailcoin Core will never create transactions paying less than
...
`snailcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ...
        # optional bypass snailcoind rpc basic auth
...
Restarting snailcoind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Snailcoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
...
- #6730 `cb4d6d0` build: Remove dependency of snailcoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [snailcoin-cli] improve error output (Jonas Schnelli)
...
- #6504 `b6fee6b` Rationalize currency unit to "SNAIL" (Ross Nicoll)

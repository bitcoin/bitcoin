BritanniaCoin Core version 0.18.0 is now available from:

  <https://britanniacoincore.org/bin/britanniacoin-core-0.18.0/>
...
`/Applications/BritanniaCoin-Qt` (on Mac) or `britanniacoind`/`britanniacoin-qt` (on
Linux).
...
recommended to use BritanniaCoin Core on unsupported systems.
...
`contrib/init/britanniacoind.service`, it has been changed to use
`/var/lib/britanniacoind` as the data directory instead of
`~britanniacoin/.britanniacoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/britanniacoind` will
exist has enough space (check using `df -h /var/lib/britanniacoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/britanniacoin/britanniacoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://britanniacoincore.org/en/contact/) the BritanniaCoin Core
...
The systemd init file (`contrib/init/britanniacoind.service`) has been changed
to use `/var/lib/britanniacoind` as the data directory instead of
`~britanniacoin/.britanniacoin`. This change makes BritanniaCoin Core more consistent with
...
`datadir` option in `/etc/britanniacoin/britanniacoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/britanniacoin/britanniacoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/britanniacoin/britanniacoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/britanniacoin/britanniacoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/britanniacoin/britanniacoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/britanniacoin/britanniacoin/blob/master/doc/britanniacoin-conf.md)
  about the `britanniacoin.conf` file describes how to use it to configure
  BritanniaCoin Core.

- A new document introduces BritanniaCoin Core's BIP174 [Partially-Signed
  BritanniaCoin Transactions
  (PSBT)](https://github.com/britanniacoin/britanniacoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/britanniacoin/britanniacoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent BritanniaCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  BritanniaCoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/britanniacoin/britanniacoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/britanniacoin/britanniacoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when BritanniaCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of BritanniaCoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/britanniacoin/britanniacoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/britanniacoin/britanniacoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart britanniacoind with the `-deprecatedrpc=generate`
...
- A new `britanniacoin-wallet` tool is now distributed alongside BritanniaCoin
...
This section describes planned changes to BritanniaCoin Core that may affect
other BritanniaCoin software and services.

- Since version 0.16.0, BritanniaCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with BritanniaCoin Core 0.20 (expected about
  a year after 0.18), BritanniaCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the BritanniaCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in BritanniaCoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any BritanniaCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, BritanniaCoin Core
...
- A sub-project of BritanniaCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with BritanniaCoin Core.  See their [project
  page](https://github.com/britanniacoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to BritanniaCoin Core's own implementation, although entropy
  gathered by BritanniaCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves BritanniaCoin Core a
...
- On macOS, BritanniaCoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `britanniacoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbritanniacoin_server.a` (jnewbery)
...
- #13926 [Tools] britanniacoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like britanniacoind (MarcoFalke)
...
- #14521 qt, docs: Fix `britanniacoin-qt -version` output formatting (hebasto)
...
- #11625 Add BritanniaCoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbritanniacoin_server` (laanwj)
...
- #15154 Configure: britanniacoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/britanniacoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for BritanniaCoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbritanniacoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in britanniacoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into BritanniaCoin Core (Christewart)
...
- #12246 Bugfix: Only run britanniacoin-tx tests when britanniacoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_britanniacoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_britanniacoin` (MarcoFalke)
...
- #14186 britanniacoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_britanniacoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update britanniacoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/britanniacoin-conf.md (hebasto)
...
- #15354 Add missing `britanniacoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct britanniacoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support britanniacoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/britanniacoin/).

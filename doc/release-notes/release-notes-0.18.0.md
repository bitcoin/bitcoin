Buttcoin Core version 0.18.0 is now available from:

  <https://Buttcoincore.org/bin/Buttcoin-core-0.18.0/>
...
`/Applications/Buttcoin-Qt` (on Mac) or `Buttcoind`/`Buttcoin-qt` (on
Linux).
...
recommended to use Buttcoin Core on unsupported systems.
...
`contrib/init/Buttcoind.service`, it has been changed to use
`/var/lib/Buttcoind` as the data directory instead of
`~Buttcoin/.Buttcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/Buttcoind` will
exist has enough space (check using `df -h /var/lib/Buttcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/Buttcoin/Buttcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://Buttcoincore.org/en/contact/) the Buttcoin Core
...
The systemd init file (`contrib/init/Buttcoind.service`) has been changed
to use `/var/lib/Buttcoind` as the data directory instead of
`~Buttcoin/.Buttcoin`. This change makes Buttcoin Core more consistent with
...
`datadir` option in `/etc/Buttcoin/Buttcoin.conf` will have no effect.
...
- A new short [document](https://github.com/Buttcoin/Buttcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/Buttcoin/Buttcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/Buttcoin/Buttcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/Buttcoin/Buttcoin/blob/master/doc/Buttcoin-conf.md)
  about the `Buttcoin.conf` file describes how to use it to configure
  Buttcoin Core.

- A new document introduces Buttcoin Core's BIP174 [Partially-Signed
  Buttcoin Transactions
  (PSBT)](https://github.com/Buttcoin/Buttcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/Buttcoin/Buttcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Buttcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Buttcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/Buttcoin/Buttcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/Buttcoin/Buttcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Buttcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Buttcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/Buttcoin/Buttcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/Buttcoin/Buttcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart Buttcoind with the `-deprecatedrpc=generate`
...
- A new `Buttcoin-wallet` tool is now distributed alongside Buttcoin
...
This section describes planned changes to Buttcoin Core that may affect
other Buttcoin software and services.

- Since version 0.16.0, Buttcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Buttcoin Core 0.20 (expected about
  a year after 0.18), Buttcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Buttcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Buttcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Buttcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Buttcoin Core
...
- A sub-project of Buttcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Buttcoin Core.  See their [project
  page](https://github.com/Buttcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Buttcoin Core's own implementation, although entropy
  gathered by Buttcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Buttcoin Core a
...
- On macOS, Buttcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `Buttcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libButtcoin_server.a` (jnewbery)
...
- #13926 [Tools] Buttcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like Buttcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `Buttcoin-qt -version` output formatting (hebasto)
...
- #11625 Add ButtcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libButtcoin_server` (laanwj)
...
- #15154 Configure: Buttcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/Buttcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Buttcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libButtcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in Buttcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Buttcoin Core (Christewart)
...
- #12246 Bugfix: Only run Buttcoin-tx tests when Buttcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_Buttcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_Buttcoin` (MarcoFalke)
...
- #14186 Buttcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_Buttcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update Buttcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/Buttcoin-conf.md (hebasto)
...
- #15354 Add missing `Buttcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct Buttcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support Buttcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/Buttcoin/).

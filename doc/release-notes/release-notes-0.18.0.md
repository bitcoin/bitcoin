...
`/Applications/Bitcoin-Qt` (on Mac) or `naviod`/`navio-qt` (on
Linux).
...
`contrib/init/naviod.service`, it has been changed to use
`/var/lib/naviod` as the data directory instead of
`~bitcoin/.bitcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/naviod` will
exist has enough space (check using `df -h /var/lib/naviod`), and
...
The systemd init file (`contrib/init/naviod.service`) has been changed
to use `/var/lib/naviod` as the data directory instead of
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart naviod with the `-deprecatedrpc=generate`
...
- A new `navio-wallet` tool is now distributed alongside Bitcoin
...
- #13926 [Tools] navio-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like naviod (MarcoFalke)
...
- #14521 qt, docs: Fix `navio-qt -version` output formatting (hebasto)
...
- #15154 Configure: navio-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/navio-wallet (Sjors)
...
- #13953 Fix deprecation in navio-util-test.py (isghe)
...
- #12246 Bugfix: Only run navio-tx tests when navio-tx is enabled (luke-jr)
...
- #14186 navio-cli: don't translate command line options (HashUnlimited)
...
- #15354 Add missing `navio-wallet` tool manpages (MarcoFalke)

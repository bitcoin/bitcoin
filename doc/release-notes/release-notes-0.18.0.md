Snailcoin Core version 0.18.0 is now available from:
...
`/Applications/Snailcoin-Qt` (on Mac) or `snailcoind`/`snailcoin-qt` (on
Linux).
...
recommended to use Snailcoin Core on unsupported systems.
...
`contrib/init/snailcoind.service`, it has been changed to use
`/var/lib/snailcoind` as the data directory instead of
`~bitcoin/.snailcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/snailcoind` will
exist has enough space (check using `df -h /var/lib/snailcoind`), and
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Snailcoin Core
...
The systemd init file (`contrib/init/snailcoind.service`) has been changed
to use `/var/lib/snailcoind` as the data directory instead of
`~bitcoin/.snailcoin`. This change makes Snailcoin Core more consistent with
...
- A new [document](https://github.com/bitcoin/bitcoin/blob/master/doc/bitcoin-conf.md)
  about the `snailcoin.conf` file describes how to use it to configure
  Snailcoin Core.

- A new document introduces Snailcoin Core's BIP174 [Partially-Signed
  Snailcoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Snailcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Snailcoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Snailcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Snailcoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart snailcoind with the `-deprecatedrpc=generate`
...
- A new `bitcoin-wallet` tool is now distributed alongside Snailcoin
...
This section describes planned changes to Snailcoin Core that may affect
other Snailcoin software and services.

- Since version 0.16.0, Snailcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Snailcoin Core 0.20 (expected about
  a year after 0.18), Snailcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Snailcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Snailcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Snailcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Snailcoin Core
...
- A sub-project of Snailcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Snailcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Snailcoin Core's own implementation, although entropy
  gathered by Snailcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Snailcoin Core a
...
- On macOS, Snailcoin Core now opts out of application CPU throttling
...
- #14162 Also log and print messages or questions like snailcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `snailcoin-qt -version` output formatting (hebasto)
...
- #11625 Add SnailcoinApplication & RPCConsole tests (ryanofsky)
...
- #11526 Visual Studio build configuration for Snailcoin Core (sipsorcery)
...
- #12775 Integration of property based testing into Snailcoin Core (Christewart)
...
- #14186 snailcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_snailcoin.conf` message on startup if conf file exists (leishman)

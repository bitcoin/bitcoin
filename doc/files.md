...
The data directory is the default location where the Snailcoin Core files are stored.

1. The default data directory paths for supported platforms are:

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.snailcoin/`
macOS    | `$HOME/Library/Application Support/Snailcoin/`
Windows  | `%LOCALAPPDATA%\Snailcoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `snailcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Snailcoin blocks (dumped in network format, 128 MiB per file)
...
`./`               | `snailcoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `snailcoind` or `snailcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `snailcoind.pid`        | Stores the process ID (PID) of `snailcoind` or `snailcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `snailcoind` or `snailcoin-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [snailcoin.conf](bitcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`snailcoin-qt` uses [`QSettings`](https://doc.qt.io/qt-6/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-6/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Snailcoin Core:
...
This table describes the files installed by Snailcoin Core across different platforms.

| **Path**                                                   | **Description**                                                             |
|------------------------------------------------------------|-----------------------------------------------------------------------------|
| [README.md](README.md) or [readme.txt](README_windows.txt) | Project information and instructions                                        |
| snailcoin.conf                                               | [Generated](../contrib/devtools/gen-bitcoin-conf.sh) configuration file     |
| bin/bitcoin                                                | Command-line tool for interacting with Snailcoin. Calls other binaries below. |
| bin/snailcoin-cli                                            | Tool for making node and wallet RPC calls.                                  |
| bin/snailcoin-qt                                             | Snailcoin node and wallet GUI                                                 |
| bin/bitcoin-tx                                             | Tool for creating and modifying transactions                                |
| bin/bitcoin-util                                           | Miscellaneous utilities                                                     |
| bin/bitcoin-wallet                                         | Snailcoin wallet tool                                                         |
| bin/snailcoind                                               | Snailcoin node and wallet daemon                                              |
...
| libexec/bitcoin-gui                                        | IPC-enabled alternative to `snailcoin-qt`                                     |
| libexec/bitcoin-node                                       | IPC-enabled alternative to `snailcoind`                                       |
| libexec/test_bitcoin                                       | Unit test binary                                                            |
| *libexec/test_snailcoin-qt*                                  | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `snailcoin-cli`, `snailcoind`, and others |
...
- README and snailcoin.conf files are included in binary releases but not installed in source builds.
- On Windows, binaries have a `.exe` suffix (e.g., `snailcoin-cli.exe`).
...
There have been multiple reports of database corruption and data loss when using this filesystem with Snailcoin Core, see [Issue #31454](https://github.com/bitcoin/bitcoin/issues/31454) for more details.

# User-space, Statically Defined Tracing (USDT) for Widecoin Core

Widecoin Core includes statically defined tracepoints to allow for more
...
**Note**: Widecoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Widecoin Core
...
Multiple tools can list the available tracepoints in a `widecoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Widecoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/widecoind
stap net        outbound_message 0x0000000000107c05 /src/widecoind
stap validation block_connected  0x00000000002fb10c /src/widecoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Widecoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/widecoind | grep NT_STAPSDT -A 4 -B 2

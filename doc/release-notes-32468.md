...
- The RPC `generateblock` takes `output` as an argument. This argument allows to specify where to distribute the reward of the block. It takes an address, a descriptor or a list of multiple addresses and descriptors. If `output` is not set, the reward is burnt in an OP_RETURN output. The reward is split, in equal parts, among all addresses and descriptors provided.

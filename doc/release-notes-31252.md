...
and the REST endpoint `/rest/block/` (with transaction details) now include two new optional fields in each
transaction input when previous output data is available (i.e. for non-coinbase
transactions in non-pruned blocks):
- `redeemScript`: the decoded redeem script for P2SH, P2SH-P2WPKH and P2SH-P2WSH inputs.

...
Previously, flag `--testnet` would be used to indicate "_testnet_" specifically. Presently, all calls of external-signer include `--chain` argument with value.
...
Usage:
```
$ <cmd> --fingerprint=<fingerprint> signtx <psbt>
<base64-encode (partially-)signed PSBT>
```

With `<psbt>` a Base64-encoded PSBT and updated `<psbt>` result returned.

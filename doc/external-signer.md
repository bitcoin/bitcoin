...
```sh
bitcoin rpc createwallet wallet_name="hww2" disable_private_keys=true passphrase="" descriptors=true external_signer=true
```

One can confirm creation of external wallet via `getwalletinfo`, which will then report `"external_signer": true`.

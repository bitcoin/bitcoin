...
```
Creating `N` distinctly named wallets with the same external signer, will create `N` wallets with the same seed, account, and addresses - just named differently. New wallet creation will not automatically increment the `account` path.

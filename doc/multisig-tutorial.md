...
Currently, it is possible to create a multisig wallet using Snailcoin Core only.
...
```bash
./build/bin/snailcoind -signet -daemon
```
...
At the time of writing, there is no way to extract a specific path from wallets in Snailcoin Core. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./build/bin/snailcoin-cli -signet createwallet "participant_${n}"
done
```

`bitcoin rpc` can also be substituted for `snailcoin-cli`.

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Snailcoin Core single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in Snailcoin Core will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./build/bin/snailcoin-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./build/bin/snailcoin-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
external_desc_sum=$(./build/bin/snailcoin-cli -signet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./build/bin/snailcoin-cli -signet getdescriptorinfo $internal_desc | jq '.descriptor')
...
Documentation for these and other parameters can be found by typing `./build/bin/snailcoin-cli help importdescriptors`.
...
```bash
./build/bin/snailcoin-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/snailcoin-cli  -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/snailcoin-cli  -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/snailcoin-cli -signet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./build/bin/snailcoin-cli -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/snailcoin-cli -a $receiving_address
```
...
```bash
./build/bin/snailcoin-cli -signet -rpcwallet="multisig_wallet_01" getbalances
```

### 1.5 Create a PSBT

Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed Snailcoin Transaction (PSBT).

PSBT is a data format that allows wallets and other tools to exchange information about a Snailcoin transaction and the signatures necessary to complete it. [[source](https://bitcoinops.org/en/topics/psbt/)]
...
```bash
balance=$(./build/bin/snailcoin-cli -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/snailcoin-cli -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/snailcoin-cli -signet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./build/bin/snailcoin-cli -signet decodepsbt $funded_psbt

./build/bin/snailcoin-cli -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/snailcoin-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/snailcoin-cli -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/snailcoin-cli -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/snailcoin-cli -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/snailcoin-cli -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/snailcoin-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/snailcoin-cli -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/snailcoin-cli -signet finalizepsbt $psbt_2 | jq -r '.hex')

...
The basic steps are:
  ...
  2. Create a watch-only descriptor wallet (blank, private keys disabled). Now the multisig is created by importing a single multi-path descriptor:
     `wsh(sortedmulti(<M>,XPUB1/<0;1>/*,XPUB2/<0;1>/*,…,XPUBN/<0;1>/*))`
     (where `<0;1>` expands to both `/0` for receiving addresses and `/1` for change). Every participant does this. All key origin information (master key fingerprint and all derivation steps) should be included with xpubs for proper support of hardware devices / external signers

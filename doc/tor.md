...
### 3.1 Tor files generated by bitcoind

By default, bitcoind generates a file called `onion_v3_private_key` and generates a randomly generated `.onion` address.

For those wanting to preserve their V2 address (which is not advised), you may do the following `cp -f onion_private_key onion_v3_private_key` and then restarting bitcoind.

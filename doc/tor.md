...
It is possible to run Snailcoin Core as a Tor onion service, and connect to such services.
...
- Starting with version 22.0, Snailcoin Core only supports Tor version 3 hidden
  services (Tor v3). Tor v2 addresses are ignored by Snailcoin Core and neither
  relayed nor stored.

- Tor removed v2 support beginning with version 0.4.6.

## How to see information about your Tor configuration via Snailcoin Core

There are several ways to see your local onion address in Snailcoin Core:
...
You can use the `getnodeaddresses` RPC to fetch a number of onion peers known to your node; run `snailcoin-cli help getnodeaddresses` for details.

`bitcoin rpc` can also be substituted for `snailcoin-cli`.

## 1. Run Snailcoin Core behind a Tor proxy

The first step is running Snailcoin Core behind a Tor proxy. This will already anonymize all
...
In a typical situation, this suffices to run behind a Tor proxy:
    ...
    snailcoind -proxy=127.0.0.1:9050

`bitcoin node` or `bitcoin gui` can also be substituted for `snailcoind`.

## 2. Automatically create a Snailcoin Core onion service

Snailcoin Core makes use of Tor's control socket API to create and destroy
ephemeral onion services programmatically. This means that if Tor is running and
proper authentication has been configured, Snailcoin Core automatically creates an
onion service to listen on. The goal is to increase the number of available
onion nodes.

This feature is enabled by default if Snailcoin Core is listening (`-listen`) and
it requires a Tor connection to work. It can be explicitly disabled with
`-listenonion=0`. If it is not disabled, it can be configured using the
`-torcontrol` and `-torpassword` settings.

To see verbose Tor information in the snailcoind debug log, pass `-debug=tor`.
...
methods to be configured: cookie authentication or snailcoind's `-torpassword`
configuration option.

#### Cookie authentication

For cookie authentication, the user running snailcoind must have read access to
the `CookieAuthFile` specified in the Tor configuration. In some cases this is
preconfigured and the creation of an onion service is automatic. Don't forget to
use the `-debug=tor` snailcoind configuration option to enable Tor debug logging.

If a permissions problem is seen in the debug log, e.g. `tor: Authentication
cookie /run/tor/control.authcookie could not be opened (check permissions)`, it
can be resolved by adding both the user running Tor and the user running
snailcoind to the same Tor group and setting permissions appropriately.
...
run snailcoind, run this as root:
...
## 3. Manually create a Snailcoin Core onion service
...
your snailcoind's P2P listen port (8333 by default), and target addresses and ports
...
In a typical situation, where you're only reachable via Tor, this should suffice:
    ...
    snailcoind -proxy=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -listen

(obviously, replace the .onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

    snailcoind ... -bind=127.0.0.1:8334=onion
...
for normal IPv4/IPv6 communication, use:
    ...
    snailcoind -onion=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -discover

## 4. Privacy recommendations

- Do not add anything but Snailcoin Core ports to the onion service created in section 3.

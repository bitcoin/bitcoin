# CJDNS support in Widecoin Core

It is possible to run Widecoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Widecoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Widecoin network and individual nodes.
...
before it will work with your Widecoin Core node. This is described in steps
...
## Run Widecoin Core with CJDNS

Once you are connected to the CJDNS network, the following Widecoin Core
...
When enabled, this option tells Widecoin Core that it is running in an
...
IPv6 local network. This helps Widecoin Core perform better address management:
...
CJDNS support was added to Widecoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `widecoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/widecoin/widecoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Widecoin Core

There are several ways to see your CJDNS address in Widecoin Core:

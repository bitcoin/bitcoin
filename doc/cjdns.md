# CJDNS support in Syscoin Core

It is possible to run Syscoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Syscoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Syscoin network and individual nodes.
...
before it will work with your Syscoin Core node. This is described in steps
...
## Run Syscoin Core with CJDNS

Once you are connected to the CJDNS network, the following Syscoin Core
...
When enabled, this option tells Syscoin Core that it is running in an
...
IPv6 local network. This helps Syscoin Core perform better address management:
...
CJDNS support was added to Syscoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `syscoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/syscoin/syscoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Syscoin Core

There are several ways to see your CJDNS address in Syscoin Core:

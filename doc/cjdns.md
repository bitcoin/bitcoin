# CJDNS support in Snailcoin Core

It is possible to run Snailcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Snailcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Snailcoin network and individual nodes.
...
before it will work with your Snailcoin Core node. This is described in steps
...
## Run Snailcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Snailcoin Core
...
When enabled, this option tells Snailcoin Core that it is running in an
...
IPv6 local network. This helps Snailcoin Core perform better address management:
...
CJDNS support was added to Snailcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `snailcoin-cli -addrinfo` to see the
...
## CJDNS-related information in Snailcoin Core

There are several ways to see your CJDNS address in Snailcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `snailcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `snailcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `snailcoin-cli help getnodeaddresses` for details.

# CJDNS support in BritanniaCoin Core

It is possible to run BritanniaCoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of BritanniaCoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BritanniaCoin network and individual nodes.
...
before it will work with your BritanniaCoin Core node. This is described in steps
...
## Run BritanniaCoin Core with CJDNS

Once you are connected to the CJDNS network, the following BritanniaCoin Core
...
When enabled, this option tells BritanniaCoin Core that it is running in an
...
IPv6 local network. This helps BritanniaCoin Core perform better address management:
...
CJDNS support was added to BritanniaCoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `britanniacoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/britanniacoin/britanniacoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in BritanniaCoin Core

There are several ways to see your CJDNS address in BritanniaCoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `britanniacoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `britanniacoin-cli getpeerinfo`).

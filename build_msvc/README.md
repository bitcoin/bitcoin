...
Qt
---------------------
All the Syscoin Core applications are configured to build with static linking. In order to build the Syscoin Core Qt applications a static build of Qt is required.

The runtime library version (e.g. v141, v142) and platform type (x86 or x64) must also match. OpenSSL must also be linked into the Qt binaries in order to provide full functionality of the Syscoin Core Qt programs. An example of the configure command to build Qtv5.9.7 locally to link with Syscoin Core is shown below (adjust paths accordingly), note it can be expected that the configure and subsequent build will fail numerous times until dependency issues are resolved.

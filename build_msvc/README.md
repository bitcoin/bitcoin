...
Introduction
---------------------
Solution and project files to build Widecoin Core with `msbuild` or Visual Studio can be found in the `build_msvc` directory. The build has been tested with Visual Studio 2019 (building with earlier versions of Visual Studio should not be expected to work).

To build Widecoin Core from the command-line, it is sufficient to only install the Visual Studio Build Tools component.
...
Qt
---------------------
To build Widecoin Core with the GUI, a static build of Qt is required.
...
```cmd
msbuild -property:Configuration=Release -maxCpuCount -verbosity:minimal widecoin.sln
```

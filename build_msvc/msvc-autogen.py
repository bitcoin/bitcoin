...
def parse_config_into_wcn_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Widecoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/widecoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/widecoin_config.h'), "w", encoding="utf8") as wcn_config:
        wcn_config.writelines(template)

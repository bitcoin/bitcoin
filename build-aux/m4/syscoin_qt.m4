...
AC_DEFUN([SYSCOIN_QT_FAIL],[
  if test "$syscoin_qt_want_version" = "auto" && test "$syscoin_qt_force" != "yes"; then
    if test "$syscoin_enable_qt" != "no"; then
...
AC_DEFUN([SYSCOIN_QT_CHECK],[
  if test "$syscoin_enable_qt" != "no" && test "$syscoin_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
dnl SYSCOIN_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
dnl This must be called before any other SYSCOIN_QT* macros to ensure that
...
AC_DEFUN([SYSCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [
     ...
     if test "$syscoin_qt_want_version" = "yes"; then
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); syscoin_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   SYSCOIN_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _SYSCOIN_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: syscoin_enable_qt, syscoin_enable_qt_dbus, syscoin_enable_qt_test
AC_DEFUN([SYSCOIN_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  SYSCOIN_QT_CHECK([_SYSCOIN_QT_FIND_LIBS])
  ...
  dnl the final binary as well. _SYSCOIN_QT_CHECK_STATIC_PLUGIN does
  ...
  if test "$syscoin_cv_static_qt" = "yes"; then
    _SYSCOIN_QT_CHECK_STATIC_LIBS
  ...
  SYSCOIN_QT_PATH_PROGS([LCONVERT], [lconvert-qt5 lconvert5 lconvert], $qt_bin_path, yes)
  ...
  if test $syscoin_enable_qt = "yes"; then
...
dnl
dnl _SYSCOIN_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_SYSCOIN_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); SYSCOIN_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _SYSCOIN_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
dnl Outputs: QT_LIBS is prepended.
AC_DEFUN([_SYSCOIN_QT_CHECK_STATIC_LIBS], [
...
dnl
dnl _SYSCOIN_QT_FIND_LIBS
dnl ---------------------
dnl
dnl Outputs: All necessary QT_* variables are set.
dnl Outputs: have_qt_test and have_qt_dbus are set (if applicable) to yes|no.
AC_DEFUN([_SYSCOIN_QT_FIND_LIBS],[
  SYSCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_CORE_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_CORE_LIBS $QT_LIBS"],
                      [SYSCOIN_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  SYSCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_GUI_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_GUI_LIBS $QT_LIBS"],
                      [SYSCOIN_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  SYSCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_WIDGETS_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_WIDGETS_LIBS $QT_LIBS"],
                      [SYSCOIN_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  SYSCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_NETWORK_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_NETWORK_LIBS $QT_LIBS"],
                      [SYSCOIN_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])

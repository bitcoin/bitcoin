...
class StealthTest(ParticlTestFramework):
    ...
    def run_test(self):
        ...
        bitcoind_processes[2].wait() # wait until encryptwallet has shut down node
        # Restart node 2
        self.nodes[2] = start_node(2, self.options.tmpdir, self.extra_args[2], genfirstkey=False)
        node2 = self.nodes[2]
        connect_nodes_bi(self.nodes, 0, 2)

...
class WalletParticlTest(ParticlTestFramework):
    ...
    def run_test (self):
        ...
        try:
            ...
            assert("No default account set" in e.error['message'])
        ...
        # restart node
        stop_node(nodes[0], 0)
        nodes[0] = start_node(0, self.options.tmpdir, self.extra_args[0], genfirstkey=False)
        ...
        # restart node
        bitcoind_processes[1].wait() # wait until encryptwallet has shut down node
        nodes[1] = start_node(1, self.options.tmpdir, self.extra_args[1], genfirstkey=False)

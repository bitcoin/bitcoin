...
B4ARGS="-datadir=$D4"
$CROWNCOIND $CROWNCOINDARGS $B4ARGS &
B4PID=$!
...
B3ARGS="-datadir=$D3"
$CROWNCOIND $CROWNCOINDARGS $B3ARGS &
B3PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} CRW"
...
function StartThree {
  $CROWNCOIND $CROWNCOINDARGS $B1ARGS &
  B1PID=$!
  $CROWNCOIND $CROWNCOINDARGS $B2ARGS &
  B2PID=$!
  $CROWNCOIND $CROWNCOINDARGS $B3ARGS &
  B3PID=$!
}

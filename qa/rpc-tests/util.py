...
def initialize_chain(test_dir):
    ...
    if not os.path.isdir(os.path.join("cache", "node0")):
        ...
        # Create cache directories, run crownds:
        for i in range(4):
            datadir=initialize_datadir("cache", i)
            args = [ os.getenv("BITCOIND", "crownd"), "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
...
def start_node(i, dirname, extra_args=None, rpchost=None):
    """
    Start a crownd and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    args = [ os.getenv("BITCOIND", "crownd"), "-datadir="+datadir, "-keypool=1", "-discover=0", "-rest" ]
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None):
    """
    Start multiple crownds, return RPC connections to them
    """

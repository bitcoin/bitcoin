...
welcome to contribute towards development through peer review, testing, and patches. 
...
Reviewing and testing are highly valued and the most effective way to contribute
...
recommended first to open a pull request against the GUI repository. When there
is an agreement to proceed with the changes, a pull request with the build system
and interface changes can be submitted to the node repository.
...
Please update the resulting commit message if needed. It should be read as a
...
audibility because merge commits can be assumed not to contain arbitrary code changes. Merge commits should be signed,
...
and maintenance that the feature may require after inclusion. Before proposing a new
...
code behavior within the pull request (bugs must be preserved as is).
...
benefits may be immediately closed by the maintainers to reduce unnecessarily
the workload on reviewing.
...
principles of the project; meet the minimum standards for inclusion; and will
...
maintainers consider the peer review when determining if there is
...
using common sense judgment and may also weigh based on merit. Reviewers that
...
higher regarding discussion and peer review requirements, keeping in mind that
mistakes could be very costly to the wider community. This includes the refactoring
...
As most reviewers are developers with their own projects, the review
...
  - It may be because the changes you suggest do not appeal to people. Rather than

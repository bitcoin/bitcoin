...
def main():
    ...
    tmpdir = "%s/syscoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_syscoind):
        print("No functional tests to run. Wallet, utils, and syscoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False):
    ...
    # Warn if syscoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "syscoind"]) is not None:

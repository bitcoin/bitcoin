...
`test/functional/test_framework/test_framework.py:TortoisecoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_tortoisecoin_cli.py --legacy-wallet',
    'interface_tortoisecoin_cli.py --descriptors',
...
def main():
    ...
    enable_tortoisecoind = config["components"].getboolean("ENABLE_TORTOISECOIND")

    if not enable_tortoisecoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if tortoisecoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "tortoisecoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a tortoisecoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass

...
class AssumeValidTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        with self.nodes[3].assert_debug_log(expected_msgs=[
            f"Enabling script verification at block #1 ({block_1_hash}): block not on best header chain.",
        ]):
        ...
        with self.nodes[4].assert_debug_log(expected_msgs=[
            f"Enabling script verification at block #1 ({alt1.hash_hex}): block not part of assumevalid chain.",
        ]):

...
class IPCInterfaceTest(BitcoinTestFramework):
    ...
    async def parse_and_deserialize_coinbase_tx(self, block_template, ctx):
        coinbase_data = BytesIO((await block_template.result.getCoinbaseRawTx(ctx)).result)
        tx = CTransaction()
        tx.deserialize(coinbase_data)
        return tx

    async def parse_and_deserialize_coinbase(self, block_template, ctx) -> CoinbaseTxTemplateData:
        ...
        template_capnp = (await block_template.result.getCoinbaseTx(ctx)).result
        witness: Optional[bytes] = None
        if template_capnp._has("witness"):
            witness = bytes(template_capnp.witness)
        return CoinbaseTxTemplateData(
    ...
    async def build_coinbase_test(self, template, ctx, miniwallet):
        ...
        coinbase.vout[0].nValue = coinbase_template.blockRewardRemaining
        ...
        assert_equal(coinbase_legacy.vout[0].nValue, coinbase_template.blockRewardRemaining)
    ...
    def run_mining_test(self):
        ...
        async def async_routine():
            ...
            # Inspect dummy coinbase returned by (deprecated) getCoinbaseRawTx()
            coinbase_data = BytesIO((await template.result.getCoinbaseRawTx(ctx)).result)
            coinbase = CTransaction()

...
# TestP2PConn: A peer we use to send messages to xbitd, and store responses.
...
class CompactBlocksTest(XBitTestFramework):
    ...
    # This test actually causes xbitd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that xbitd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for xbitd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # xbitd will not send blocktxn responses for blocks whose height is

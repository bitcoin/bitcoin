...
class TxIndexPruneTest(BitcoinTestFramework):
    ...
    def rest_tx(self, node, txid, status=200):
        ...
        if status == 200:
            assert_equal(resp.getheader("Content-Type"), "application/json")
            return loads(body)
    ...
    def run_test(self):
        ...
        try:
            ...
            assert_equal(error.error["data"]["pruned_block_hashes"], [blockhash])
        ...
        try:
            pruned.gettxoutproof([parent_txid])
            raise AssertionError("Expected pruned transaction lookup failure")
        except JSONRPCException as error:
            assert_equal(error.error["code"], -1)
            assert f"Transaction may be in pruned block {parent_blockhash}" in error.error["message"]
            assert_equal(error.error["data"]["pruned_block_hashes"], [parent_blockhash])

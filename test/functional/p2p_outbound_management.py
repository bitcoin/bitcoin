...
class P2POutboundManagement(BitcoinTestFramework):
    ...
    def test_network_management(self):
        ...
        self.cur_time += self.MOCKTIME_BUMP
        node.setmocktime(self.cur_time)
        self.generate(node, 1)  # Generate a block to disable stale tip extra outbounds

        self.log.info("Check for extra block-relay-only connection")
        # Extra block-relay peers are on a 5min timer and take precedence over network-specific ones.
        self.wait_until(lambda: self.has_extra_block_relay_peer(node))
        # Make sure that the new peer has finished version handshake
        self.all_sync_send_with_ping(self.auto_outbound_destinations)
        self.disconnect_last_block_relay_peer()

        self.log.info("Check for replacement of an ipv4 peer with a network-specific peer")
        self.wait_until(lambda: self.has_extra_full_outbound_peer(node))
        self.all_sync_send_with_ping(self.auto_outbound_destinations)
        ...
        self.log.info("Advance time a second time")
        self.cur_time += self.MOCKTIME_BUMP
        node.setmocktime(self.cur_time)
        ...
        self.log.info("Wait for another replacement with a network-specific peer")
        self.wait_until(lambda: self.has_extra_full_outbound_peer(node))
        self.all_sync_send_with_ping(self.auto_outbound_destinations)
        node.mockscheduler(self.SCHEDULE_EXTRA_PEER_CHECK)
        self.wait_until(lambda: self.has_regular_outbounds(node))

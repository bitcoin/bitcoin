...
class GetBlocksActivityTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.test_no_blockhash_passed(node)
    ...
    def test_no_blockhash_passed(self, node):
        self.log.info("Test that calling getdescriptoractivity with no arguments raises appropriate RPC error with the correct description")
        assert_raises_rpc_error(-8, "At least 1 blockhash or 1 descriptor must be specified.", node.getdescriptoractivity)
        self.log.info("Test that calling getdescriptoractivity with an empty array raises also RPC_INVALID_PARAMETER (-8)")
        assert_raises_rpc_error(-8, "At least 1 blockhash or 1 descriptor must be specified.", node.getdescriptoractivity, [])

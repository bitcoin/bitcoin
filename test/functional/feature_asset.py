...
class AssetTest(SyscoinTestFramework):
    ...
    def basic_asset(self):
        asset = self.nodes[0].assetnew('1', 'TST', 'asset description', '0x', 8, 1000, 10000, 127, '', '', {}, {})['asset_guid']
        self.sync_mempools()
    ...
    def asset_description_too_big(self):
        # 760 + 8 byte overhead for pub data descriptor and json = 768 bytes long (1024 bytes base64 encoded)
        ...
        assert_equal(assetInfo['asset_guid'], asset1)
        # data too big
        assert_raises_rpc_error(-4, 'asset-pubdata-too-big', self.nodes[0].assetnew, '1', 'TST', baddata, '0x', 8, 1000, 10000, 127, '', '', {}, {})
    
    def asset_symbol_size(self):
        gooddata = 'asset description'
        self.nodes[0].assetnew('1', 'T', gooddata, '0x', 8, 1000, 10000, 127, '', '', {}, {})
        assert_raises_rpc_error(-4, 'asset-invalid-symbol', self.nodes[0].assetnew, '1', '', gooddata, '0x', 8, 1000, 10000, 127, '', '', {}, {})
        self.nodes[0].assetnew('1', 'ABCDEFGHI', gooddata, '0x', 8, 1000, 10000, 127, '', '', {}, {})
        assert_raises_rpc_error(-4, 'asset-invalid-symbol', self.nodes[0].assetnew, '1', 'ABCDEFGHIJ', gooddata, '0x', 8, 1000, 10000, 127, '', '', {}, {})
        self.nodes[0].generate(1)
    ...
    def asset_maxsupply(self):
        ...
        self.nodes[0].generate(1)
        self.nodes[0].assetupdate(asset, '', '', 0.9, 127, '', '', {}, {})
        self.nodes[0].generate(1)
        # would go over 2 coins supply
        assert_raises_rpc_error(-4, 'asset-invalid-supply', self.nodes[0].assetupdate, asset, '', '', 0.1, 127, '', {}, {})
        self.nodes[0].generate(1)
        # balance > max supply
        assert_raises_rpc_error(-4, 'asset-invalid-supply', self.nodes[0].assetnew, '1', 'TST', gooddata, '0x', 8, 2, 1, 127, '', {}, {})
        # int64 limits
        # largest decmal amount that we can use, without compression overflow of uint (~1 quintillion)
        # 10^18 - 1
        maxUint = 999999999999999999
        asset = self.nodes[0].assetnew('1', 'TST', gooddata, '0x', 0, maxUint-1, maxUint, 127, '', '', {}, {})['asset_guid']
        self.nodes[0].generate(1)
        assetInfo = self.nodes[0].assetinfo(asset)
        assert_equal(assetInfo['asset_guid'], asset)
        ...
        self.nodes[0].assetupdate(asset, '', '', 1, 127, '', '', {}, {})
        self.nodes[0].generate(1)
        assetInfo = self.nodes[0].assetinfo(asset)
        assert_equal(assetInfo['asset_guid'], asset)
        ...
        assert_equal(assetInfo['max_supply'], maxUint)
        assert_raises_rpc_error(-4, 'asset-amount-outofrange', self.nodes[0].assetupdate, asset, '', '', 1, 127, '', {}, {})
        assert_raises_rpc_error(-4, 'asset-invalid-maxsupply', self.nodes[0].assetnew, '1', 'TST', gooddata, '0x', 0, maxUint, maxUint+1, 127, '', {}, {})
        assert_raises_rpc_error(-4, 'asset-invalid-maxsupply', self.nodes[0].assetnew, '1', 'TST', gooddata, '0x', 0, maxUint+1, maxUint+1, 127, '', {}, {})
    ...
    def asset_transfer(self):
        ...
        self.nodes[0].generate(1)
        self.sync_blocks()
        asset0 = self.nodes[0].assetnew('1', 'TST', 'asset description', '0x', 8, 1000, 10000, 127, '', '', {}, {})['asset_guid']
        asset1 = self.nodes[1].assetnew('1', 'TST', 'asset description', '0x', 8, 1000, 10000, 127, '', '', {}, {})['asset_guid']
        self.sync_mempools()
        self.nodes[0].generate(1)
        self.sync_blocks()
        self.nodes[0].assetupdate(asset0, '', '', 0, 127, '', '', {}, {})
        self.nodes[1].assetupdate(asset1, '', '', 0, 127, '', '', {}, {})
        self.sync_mempools()
        self.nodes[0].generate(1)
        self.sync_blocks()
        ...
        self.nodes[0].assetupdate(asset1, '', '', 0, 127, '', '', {}, {})
        self.nodes[1].assetupdate(asset0, '', '', 0, 127, '', '', {}, {})
        self.sync_mempools()
        self.nodes[0].generate(1)
        self.sync_blocks()

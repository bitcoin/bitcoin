...
class ReconciliationInitiatorTest(ReconciliationTest):
    ...
    def test_reconciliation_initiator_flow_interleaved_txs(self):
        ...
        for peer in peers:
            capacity = estimate_sketch_capacity(len(shared_txs), len(peer.shared_wtxids))
            shared_short_txids = self.send_sketch_from(peer, peer.shared_wtxids, [], capacity)
        ...
        # Only the last peer should have received a transaction announcement. Wait for that one
        # first: it proves the announcement round completed, so an erroneous inv to the others
        # cannot still be in flight when we check them.
        self.wait_for_inv(peers[2], shared_wtxids[:1])
        assert not any(peers[0].last_inv)
        assert not any(peers[1].last_inv)
    ...
    def test_reconciliation_initiator_protocol_violations(self):
        ...
        # Test disconnect on a SKETCH that is not a whole number of sketch elements
        self.log.info('Testing protocol violation: sending a malformed SKETCH')
        peer = self.test_node.add_outbound_p2p_connection(TxReconTestP2PConn(), p2p_idx=0, connection_type="outbound-full-recon")
        self.test_node.bumpmocktime(RECON_REQUEST_INTERVAL)
        peer.sync_with_ping()
        self.wait_for_reqtxrcncl(peer)
        peer.send_sketch([0] * (BYTES_PER_SKETCH_CAPACITY - 1))
        peer.wait_for_disconnect()

    def test_reconciliation_initiator_no_extension(self, n_mininode, n_node, n_shared):
        ...
        mininode_unique_wtxids = [tx.wtxid_int for tx in mininode_unique_txs]
        ...
        # Send the corresponding sketches, of the capacity a real responder would have picked
        capacity = estimate_sketch_capacity(n_node + n_shared, mininode_set_size)
        for peer in peers:
            peer.unique_short_txids = self.send_sketch_from(peer, mininode_unique_wtxids, shared_wtxids, capacity)
        ...
        # The node announces only its unique transactions. The shared ones cancelled out of the
        # sketch, so it knows we have them and must not announce them.
        for peer in peers:
            self.wait_for_inv(peer, node_unique_wtxids)
            self.request_transactions_from(peer, node_unique_wtxids)
            self.wait_for_txs(peer, node_unique_wtxids)
    ...
    def run_test(self):
        ...
        self.test_reconciliation_initiator_no_extension(3, 3, 20)

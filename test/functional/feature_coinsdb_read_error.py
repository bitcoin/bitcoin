...
from test_framework.util import assert_equal, assert_not_equal, sync_txindex
...
class DBReadErrorTest(BitcoinTestFramework):
    ...
    def assert_shutdown_on_read_error(node, rpc_fn, expected_msgs=None):
        with node.assert_debug_log(expected_msgs or []):
            try:
                rpc_fn()
            except RPC_CONN_LOST:
                pass

        node.wait_until(lambda: node.process.poll() is not None, timeout=60)
        assert_not_equal(node.process.returncode, 0)  # died via abort; the exact code is platform-dependent

...
class WalletTaprootTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].createwallet(wallet_name="privs_tr_enabled", blank=True)
        self.privs_tr_enabled = self.nodes[0].get_wallet_rpc("privs_tr_enabled")
        self.nodes[2].createwallet(wallet_name="privs_tr_disabled", blank=True)
        self.privs_tr_disabled=self.nodes[2].get_wallet_rpc("privs_tr_disabled")
        self.nodes[0].createwallet(wallet_name="pubs_tr_enabled", blank=True, disable_private_keys=True)
        self.pubs_tr_enabled = self.nodes[0].get_wallet_rpc("pubs_tr_enabled")
        self.nodes[2].createwallet(wallet_name="pubs_tr_disabled", blank=True, disable_private_keys=True)
        self.pubs_tr_disabled=self.nodes[2].get_wallet_rpc("pubs_tr_disabled")
        self.nodes[0].createwallet(wallet_name="boring")
        self.nodes[0].createwallet(wallet_name="addr_gen", disable_private_keys=True, blank=True)
        self.nodes[0].createwallet(wallet_name="rpc_online", blank=True)
        self.nodes[0].createwallet(wallet_name="psbt_online", disable_private_keys=True, blank=True)
        self.nodes[1].createwallet(wallet_name="psbt_offline", blank=True)

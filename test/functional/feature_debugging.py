    ...
    def attempt_attaching(self, pid, expect_success: bool):
        def verify_attach(name, args, expect_success):
            ...
            if expect_success != (result.returncode == 0):
                if expect_success:
                    raise AssertionError(f"Expected success but {args} failed:\n{result.stderr}")
                else:
        ...
        # inside WaitForDebugger(), it would result in no TracerPid being
        # detected, so the node would not leave that function and start
        # responding to RPCs. This would mean wait_for_rpc_connection() during
        # the test would not complete.
        if self.gdb:

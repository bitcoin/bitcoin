...
class BytesPerSigOpTest(BitcoinTestFramework):
    ...
    def test_legacy_sigops_stdness(self):
        ...
        tx = CTransaction()
        tx.vin = [CTxIn(op, CScript([b"", packed_redeem_script])) for op in outpoints]
        tx.vout = [CTxOut(0, CScript([OP_RETURN, b""]))]
        assert_raises_rpc_error(-26, "bad-txns-nonstandard-inputs", self.nodes[0].sendrawtransaction, tx.serialize().hex())

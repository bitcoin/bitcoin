        ...
        super().__init__(
            ...
            (
                "split", CScript([
                    ...
                    -1,  # use internal key of the current input
                    -1,  # use taptree of the current input
                    CCV_FLAG_DEDUCT_OUTPUT_AMOUNT,   # flag: check output, deduct amount from input
                    OP_CHECKCONTRACTVERIFY,
                    ...
                    0,   # NUMS pubkey
                    0,   # no taptweak
                    0,   # default behavior, check output, all remaining amount must go to this output
                    OP_CHECKCONTRACTVERIFY,
...
def ccv(flags=0, index=0, data=0, pk=0, tree=0):
    return CScript([
        data,
        index,
        pk,
...
class TestCCVSuccess(P2TR):
    """
    A utxo with hardcoded parameters to test the OP_SUCCESS behavior.
    """
...
class TestCCVFail(P2TR):
    """
    A utxo with hardcoded parameters to test the failure cases.
    """
...
class TestCCVBehavior(P2TR):
    """
    A utxo that can be sent anywhere because it triggers an undefined behavior
    of the flags (which has OP_SUCCESS behavior).
    """
...
class CheckContractVerifyTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.test_undefined_flags_opsuccess(node, wallet)
    ...
    def test_deduct_amount(
        ...
        # Tests the behavior of the CCV_FLAG_DEDUCT_OUTPUT_AMOUNT flag
    ...
    def test_undefined_flags_opsuccess(
        self,
        node: TestNode,
        wallet: MiniWallet
    ):
        ...
        # Tests that undefined values for flags immediately terminate the script successfully.

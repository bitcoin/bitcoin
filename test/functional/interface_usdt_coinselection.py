...
class CoinSelectionTracepointTest(BitcoinTestFramework):
    ...
    def clear_tracepoint_queue(self):
        """Clear any pending tracepoints from the BPF queue."""
        while True:
            try:
                self.bpf["coin_selection_events"].pop()
            except KeyError:
                break
    ...
    def run_test(self):
        ...
        self.log.info("Sending a transaction triggers APS tracepoints when partial spend exists")
        # Create a separate wallet with only partial spend UTXOs for controlled testing
        self.nodes[0].createwallet("aps_test_wallet")
        aps_wallet = self.nodes[0].get_wallet_rpc("aps_test_wallet")
        # Create multiple UTXOs at the same address to trigger APS
        partial_spend_addr = aps_wallet.getnewaddress()
        wallet.sendtoaddress(partial_spend_addr, 5)
        wallet.sendtoaddress(partial_spend_addr, 5)
        self.generate(self.nodes[0], 1)
        # Clear tracepoints from setup transactions
        self.clear_tracepoint_queue()
        # We should have 5 tracepoints in the order:
        # 1. selected_coins (type 1)
        # 2. normal_create_tx_internal (type 2)
        # 3. attempting_aps_create_tx (type 3) - APS attempted because partial spend exists
        # 4. selected_coins (type 1)
        # 5. aps_create_tx_internal (type 4)
        aps_wallet.sendtoaddress(wallet.getnewaddress(), 2)
        events = self.get_tracepoints([1, 2, 3, 1, 4], "aps_test_wallet")
        ...
        self.log.info("Change position is -1 if no change is created normally and APS is skipped")
        # When sending entire balance, all UTXOs are spent - no partial spend exists
        # With the APS skip optimization, APS is not attempted since it cannot improve privacy
        # We should have 2 tracepoints in the order:
        # 1. selected_coins (type 1)
        # 2. normal_create_tx_internal (type 2)

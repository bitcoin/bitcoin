...
class WalletTest(SyscoinTestFramework):
    ...
    def run_test(self):
        ...
        for unit, fee_rate in {"SYS/kB": 0.00000999, "sat/B": 0.99999999}.items():
            self.log.info("Test sendmany raises 'fee rate too low' if conf_target {} and estimate_mode {} are passed".format(fee_rate, unit))
            assert_raises_rpc_error(-6, "Fee rate (0.00000999 SYS/kB) is lower than the minimum fee rate setting (0.00001000 SYS/kB)",
        ...
        if not self.options.descriptors:
            ...
            self.log.info("Test case-insensitive explicit fee rate (sendtoaddress as SYS/kB)")
            self.nodes[0].generate(1)
            self.sync_all(self.nodes[0:3])

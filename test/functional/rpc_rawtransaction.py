...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def createrawtransaction_tests(self):
        ...
        tx = tx_from_hex(self.nodes[2].createrawtransaction(inputs=[{'txid': TXID, 'vout': 9}], outputs=multidict([(address, 99), (address2, 99), ('data', '99'), ('data', '99')])))
        assert_equal(len(tx.vout), 4)
        assert_equal(
            tx.serialize().hex(),
            self.nodes[2].createrawtransaction(inputs=[{'txid': TXID, 'vout': 9}], outputs=[{address: 99}, {address2: 99}, {'data': '99'}, {'data': '99'}]),
        )

...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def getrawtransaction_verbosity_tests(self):
        ...
        # Native P2WPKH: no redeem or witness script (nothing is pushed in the scriptSig
        # and the witness stack only holds a signature and a pubkey)
        ...
        # P2SH-P2WPKH: the redeem script is the P2WPKH witness program itself
        # (OP_0 <20-byte keyhash>), as per BIP 141, and no witness script exists for keyhash inputs
        ...
        assert 'redeemScript' in res_p2sh_p2wpkh['vin'][0]
        assert 'witnessScript' not in res_p2sh_p2wpkh['vin'][0]
        # The redeem script is the P2WPKH witness program (OP_0 <20-byte keyhash>)
        assert_equal(sorted(['asm', 'desc', 'address', 'type']), sorted(res_p2sh_p2wpkh['vin'][0]['redeemScript'].keys()))
        assert_equal(res_p2sh_p2wpkh['vin'][0]['redeemScript']['type'], "witness_v0_keyhash")
        assert_equal(res_p2sh_p2wpkh['vin'][0]['redeemScript']['asm'], node.decodescript(key.p2sh_p2wpkh_redeem_script)['asm'])

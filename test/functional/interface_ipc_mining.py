...
class IPCMiningTest(BitcoinTestFramework):
    ...
    def run_submit_block_malformed_test(self):
        """Test that submitting truncated block bytes does not crash the node."""
        ...
        async def async_routine():
            ...
            async with destroying((await mining.createNewBlock(ctx, self.default_block_create_options)).result, ctx) as template:
                ...
                # Deserialization fails in the Cap'n Proto proxy layer before
                # submitBlock is called, so we expect a remote KjException.
                try:
                    await mining.submitBlock(ctx, bad_bytes)
                    assert False, "submitBlock should have raised KjException"
    ...
    def run_test(self):
        ...
        self.run_submit_block_malformed_test()

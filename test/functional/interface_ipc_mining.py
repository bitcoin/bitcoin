...
class IPCMiningTest(BitcoinTestFramework):
    ...
    @staticmethod
    def make_block_variant(block, coinbase, extra_nonce):
    ...
    @staticmethod
    def assert_chain_tip(node, block_hash, *, active):
    ...
    @staticmethod
    async def assert_submit_block(mining, ctx, block, *, result, reason="", debug="", precious=None):
        if precious is None:
    ...
    def run_coinbase_and_submission_test(self):
        ...
        async def async_routine():
            ...
            async with destroying((await mining.createNewBlock(ctx, self.default_block_create_options)).result, ctx) as template:
                ...
                self.log.debug("Deprecated submission methods should direct clients to update")
                await assert_capnp_raises(
                    lambda: mining2.submitBlockOld7(ctx2, block.serialize()),
                    "remote exception: std::exception: Old submitBlock (@7) not supported. Please update your client!",
                )
                await assert_capnp_raises(
                    lambda: template.submitSolutionOld10(ctx, block.nVersion, block.nTime, block.nNonce, coinbase.serialize()),
                    "remote exception: std::exception: Old submitSolution (@10) not supported. Please update your client!",
                )
    ...
    def run_precious_submission_test(self):
        ...
        async def async_routine():
            ...
            async with AsyncExitStack() as stack:
                ...
                self.log.debug("submitBlock with precious=true prefers the stored side block and reports duplicate")
                await self.assert_submit_block(mining, ctx, side_block, result=False, reason="duplicate", precious=True)
                assert_equal(node.getbestblockhash(), side_block.hash_hex)
                self.assert_chain_tip(node, side_block.hash_hex, active=True)

                self.log.debug("submitBlock with precious=true on the active tip reports duplicate")
                await self.assert_submit_block(mining, ctx, side_block, result=False, reason="duplicate", precious=True)
                assert_equal(node.getbestblockhash(), side_block.hash_hex)
                ...
                self.log.debug("submitSolution with precious=true prefers the stored same-work solution and reports duplicate")
                result = await stale_template.submitSolution(
                    ctx,
                    side_solution_block.nVersion,
                    side_solution_block.nTime,
                ...
                self.log.debug("submitSolution with precious=true on the active tip reports duplicate")
                result = await stale_template.submitSolution(
                    ctx,
                    side_solution_block.nVersion,
                    side_solution_block.nTime,
                ...
                self.log.debug("submitSolution with precious=true on a fresh same-work solution succeeds")
                fresh_solution_block, fresh_solution_coinbase = self.make_block_variant(stale_block, stale_coinbase, extra_nonce=5)
                result = await stale_template.submitSolution(
                    ctx,
                    fresh_solution_block.nVersion,
                    fresh_solution_block.nTime,
                    fresh_solution_block.nNonce,
                    fresh_solution_coinbase.serialize(),
                    precious=True,
                )
                assert_equal(result.result, True)
                assert_equal(result.reason, "")
                assert_equal(result.debug, "")
                assert_equal(node.getbestblockhash(), fresh_solution_block.hash_hex)
                self.assert_chain_tip(node, fresh_solution_block.hash_hex, active=True)

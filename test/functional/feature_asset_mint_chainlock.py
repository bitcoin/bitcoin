#!/usr/bin/env python3
# Copyright (c) 2019-2020 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
import time
from test_framework.test_framework import DashTestFramework
from test_framework.util import assert_equal, assert_raises_rpc_error, force_finish_mnsync

class AssetMintTest(DashTestFramework):
    def set_test_params(self):
        self.mocktime = 1594359054
        self.set_dash_test_params(5, 4, [['-assetindex=1','-mocktime=1594359054','-whitelist=noban@127.0.0.1']] * 5, fast_dip3_enforcement=True)

    def skip_test_if_missing_module(self):
        self.skip_if_no_wallet()
    # node0 and node1 will have txroot and accept a minttx, node2 will create a fork and node1-4 will CLSIG in a lock that will prevent tx from a
    def run_test(self):
        self.mocktime = 1594359054
        for i in range(len(self.nodes)):
            if i != 1:
                self.connect_nodes(i, 2)
            force_finish_mnsync(self.nodes[i])

        self.nodes[0].generate(10)
        self.sync_blocks(self.nodes, timeout=60*5)

        self.nodes[0].spork("SPORK_17_QUORUM_DKG_ENABLED", 0)
        self.wait_for_sporks_same()

        self.log.info("Mining 4 quorums")
        for i in range(4):
            self.mine_quorum()

        self.nodes[0].spork("SPORK_19_CHAINLOCKS_ENABLED", 0)
        self.wait_for_sporks_same()

        self.log.info("Mine single block, wait for chainlock")
        self.nodes[0].generate(1)
        self.wait_for_chainlocked_block_all_nodes(self.nodes[0].getbestblockhash())
        self.log.info("Mine many blocks, wait for chainlock")
        self.nodes[0].generate(20)

        self.asset = 2615707979
        spv_tx_root = "a0842ab40a9c4770c8ec74158aadcf943e8158128fdd1ba8cef9c7cb8eda732692"
        spv_tx_parent_nodes = ""
        spv_tx_value = ""
        spv_tx_path = "0b"
        spv_receipt_root = "a0a958499bf48fcce17672b58aa9037bd3dafeb6231880722d909c60bacfaaa8d4"
        spv_receipt_parent_nodes = ""
        spv_receipt_value = ""
        height = 6816449
        blockhash = '0xee524852fb7df5a6c27106f4bc47e740e6a6751e66bce1f98363ff2eecbf8c0d'
        prevblockhash = '0x5f41930a021d1c48a8add5d35aac63f18e085c2ee862990660603058fd6216c0'
        bridgetransferid = 2

        self.basic_asset()
        self.nodes[0].generate(1)
        self.bump_mocktime(5)
        assetInfo = self.nodes[0].assetinfo(self.asset)
        assert_equal(assetInfo['asset_guid'], self.asset)
        self.sync_blocks()

        # Add eth root to DB so it an validate this SPV proof, do it on both nodes so they can verify the tx
        self.nodes[0].syscoinsetethheaders([[6816449, blockhash, prevblockhash, spv_tx_root, spv_receipt_root, 1594359054]])
        self.nodes[0].syscoinsetethstatus('synced', 6816449)
        self.nodes[1].syscoinsetethheaders([[6816449, blockhash, prevblockhash, spv_tx_root, spv_receipt_root, 1594359054]])
        self.nodes[1].syscoinsetethstatus('synced', 6816449)

        newaddress = self.nodes[0].getnewaddress()
        # must wait an hour first
        assert_raises_rpc_error(-26, 'mint-insufficient-confirmations', self.nodes[0].assetallocationmint, self.asset, newaddress, 100, height, bridgetransferid, spv_tx_value, spv_tx_root, spv_tx_parent_nodes, spv_tx_path, spv_receipt_value, spv_receipt_root, spv_receipt_parent_nodes)
        self.bump_mocktime(3600)
        self.nodes[0].generate(10)
        self.bump_mocktime(5)
        # it will be reorged out via chainlocks
        minttx = self.nodes[0].assetallocationmint(self.asset, newaddress, 100, height, bridgetransferid, spv_tx_value, spv_tx_root, spv_tx_parent_nodes, spv_tx_path, spv_receipt_value, spv_receipt_root, spv_receipt_parent_nodes)['txid']
        minttxhex = self.nodes[0].getrawtransaction(minttx)
        # other nodes shouldn't accept the transaction
        time.sleep(0.25)
        # ensure node0 and node1 see the transaction as they have the tx root
        self.sync_mempools(self.nodes[0:1], timeout=30)
        # we didn't add txroot to any other nodes
        assert_raises_rpc_error(-26, 'mint-txroot-missing', self.nodes[2].sendrawtransaction, minttxhex)
        assert_raises_rpc_error(-26, 'mint-txroot-missing', self.nodes[3].sendrawtransaction, minttxhex)
        assert_raises_rpc_error(-26, 'mint-txroot-missing', self.nodes[4].sendrawtransaction, minttxhex)
        self.nodes[0].generate(5)
        self.bump_mocktime(5)
        time.sleep(0.25)
        self.sync_blocks(self.nodes[0:1])
        assert(self.nodes[0].getrawtransaction(minttx, True)['in_active_chain'] == True)
        assert(self.nodes[1].getrawtransaction(minttx, True)['in_active_chain'] == True)
        assert(self.nodes[0].getbestblockhash() == self.nodes[1].getbestblockhash())

        # create fork
        self.nodes[2].generate(6)
        self.bump_mocktime(5)
        time.sleep(0.25)
        self.wait_for_chainlocked_block_all_nodes(self.nodes[2].getbestblockhash())
        assert(self.nodes[0].getbestblockhash() == self.nodes[2].getbestblockhash())
        # node0/node1 shouldn't have the minttx in canonical chain anymore because of the reorg
        assert 'in_active_chain' not in self.nodes[0].getrawtransaction(minttx, True)
        assert 'in_active_chain' not in self.nodes[1].getrawtransaction(minttx, True)
        # these nodes should have rejected the transaction and should not have it
        assert_raises_rpc_error(-5, 'No such mempool transaction', self.nodes[2].getrawtransaction, minttx)
        assert_raises_rpc_error(-5, 'No such mempool transaction', self.nodes[3].getrawtransaction, minttx)
        assert_raises_rpc_error(-5, 'No such mempool transaction', self.nodes[4].getrawtransaction, minttx)
        # now push tx roots on all other nodes
        self.nodes[2].syscoinsetethheaders([[6816449, blockhash, prevblockhash, spv_tx_root, spv_receipt_root, 1594359054]])
        self.nodes[2].syscoinsetethstatus('synced', 6816449)
        self.nodes[3].syscoinsetethheaders([[6816449, blockhash, prevblockhash, spv_tx_root, spv_receipt_root, 1594359054]])
        self.nodes[3].syscoinsetethstatus('synced', 6816449)
        self.nodes[4].syscoinsetethheaders([[6816449, blockhash, prevblockhash, spv_tx_root, spv_receipt_root, 1594359054]])
        self.nodes[4].syscoinsetethstatus('synced', 6816449)
        # send on node1 and confirm all other nodes see it
        self.nodes[2].sendrawtransaction(minttxhex)
        self.bump_mocktime(5)
        time.sleep(0.25)
        # now all nodes should have the mint tx
        self.sync_mempools(self.nodes, timeout=30)
        # duplicate check
        assert_raises_rpc_error(-26, 'mint-duplicate-transfer', self.nodes[0].assetallocationmint, self.asset, newaddress, 100, height, bridgetransferid, spv_tx_value, spv_tx_root, spv_tx_parent_nodes, spv_tx_path, spv_receipt_value, spv_receipt_root, spv_receipt_parent_nodes)
        self.nodes[0].generate(1)
        self.bump_mocktime(5)
        time.sleep(0.25)
        self.sync_blocks()
        # after a block it should show a different exists error
        assert_raises_rpc_error(-26, 'mint-exists', self.nodes[0].assetallocationmint, self.asset, newaddress, 100, height, bridgetransferid, spv_tx_value, spv_tx_root, spv_tx_parent_nodes, spv_tx_path, spv_receipt_value, spv_receipt_root, spv_receipt_parent_nodes)
        # increase time by ~1 week and assetallocationmint should throw timeout error, must send to network by 0.5 week at least
        numBlocks = int(604800 / (2*60*59))
        # need to bump 2 hours at a time max
        for block in range(numBlocks):
            self.bump_mocktime(2*60*59)
            self.nodes[0].generate(1)
        assert_raises_rpc_error(-26, 'mint-too-old', self.nodes[0].assetallocationmint, self.asset, newaddress, 100, height, bridgetransferid, spv_tx_value, spv_tx_root, spv_tx_parent_nodes, spv_tx_path, spv_receipt_value, spv_receipt_root, spv_receipt_parent_nodes)

    def basic_asset(self):
        auxfees = {'auxfee_address': self.nodes[0].getnewaddress(), 'fee_struct': [[0,0.01],[10,0.004],[250,0.002],[2500,0.0007],[25000,0.00007],[250000,0]]}
        self.nodes[0].assetnewtest(self.asset, '1', 'TST', 'asset description', '0x9f90b5093f35aeac5fbaeb591f9c9de8e2844a46', 8, 10000, 127, '', {}, auxfees)

if __name__ == '__main__':
    AssetMintTest().main()

...
class DecodeScriptTest(BitcoinTestFramework):
    ...
    def decodescript_script_sig(self):
        ...
        assert_equal(asm_hex(signature), rpc_result['asm'])

        self.log.info("- P2PKH")
        rpc_result = self.nodes[0].decodescript(push_signature + push_public_key)
        assert_equal(asm_hex(signature) + ' ' + asm_hex(public_key), rpc_result['asm'])

        self.log.info("- multisig")
        # this also tests the leading portion of a P2SH multisig scriptSig
        # 0 <A sig> <B sig>
        rpc_result = self.nodes[0].decodescript('00' + push_signature + push_signature)
        assert_equal('0 ' + asm_hex(signature) + ' ' + asm_hex(signature), rpc_result['asm'])
    ...
    def decodescript_script_pub_key(self):
        ...
        assert_equal(asm_hex(public_key) + ' CHECKSIG', rpc_result['asm'])
        assert_equal('pubkey', rpc_result['type'])
        # P2PK is translated to P2WPKH
        assert_equal('0 ' + asm_hex(public_key_hash), rpc_result['segwit']['asm'])

        self.log.info("- P2PKH")
        # DUP HASH160 <PubKeyHash> EQUALVERIFY CHECKSIG
        rpc_result = self.nodes[0].decodescript('76a9' + push_public_key_hash + '88ac')
        assert_equal('pubkeyhash', rpc_result['type'])
        assert_equal('DUP HASH160 ' + asm_hex(public_key_hash) + ' EQUALVERIFY CHECKSIG', rpc_result['asm'])
        # P2PKH is translated to P2WPKH
        assert_equal('witness_v0_keyhash', rpc_result['segwit']['type'])
        assert_equal('0 ' + asm_hex(public_key_hash), rpc_result['segwit']['asm'])

        self.log.info("- multisig")
        # <m> <A pubkey> <B pubkey> <C pubkey> <n> CHECKMULTISIG
        ...
        assert_equal('2 ' + asm_hex(public_key) + ' ' + asm_hex(public_key) + ' ' + asm_hex(public_key) +  ' 3 CHECKMULTISIG', rpc_result['asm'])
        # multisig in P2WSH
        multisig_script_hash = sha256(bytes.fromhex(multisig_script)).hex()
        assert_equal('witness_v0_scripthash', rpc_result['segwit']['type'])
        assert_equal('0 ' + asm_hex(multisig_script_hash), rpc_result['segwit']['asm'])

        self.log.info ("- P2SH")
        # HASH160 <Hash160(redeemScript)> EQUAL.
        # push_public_key_hash here should actually be the hash of a redeem script.
        # but this works the same for purposes of this test.
        rpc_result = self.nodes[0].decodescript('a9' + push_public_key_hash + '87')
        assert_equal('scripthash', rpc_result['type'])
        assert_equal('HASH160 ' + asm_hex(public_key_hash) + ' EQUAL', rpc_result['asm'])
        ...
        assert_equal('RETURN ' + asm_hex(signature_imposter[2:]), rpc_result['asm'])

        self.log.info("- CLTV redeem script")
        # redeem scripts are in-effect scriptPubKey scripts, so adding a test here.
        # NOP2 is also known as CHECKLOCKTIMEVERIFY.
        # just imagine that the pub keys used below are different.
        # for our purposes here it does not matter that they are the same even though it is unrealistic.
        #
        # IF
        #   <receiver-pubkey> CHECKSIGVERIFY
        # ELSE
        #   <lock-until> CHECKLOCKTIMEVERIFY DROP
        # ENDIF
        # <sender-pubkey> CHECKSIG
        #
        ...
        assert_equal('IF ' + asm_hex(public_key) + ' CHECKSIGVERIFY ELSE 500000 CHECKLOCKTIMEVERIFY DROP ENDIF ' + asm_hex(public_key) + ' CHECKSIG', rpc_result['asm'])
        # CLTV script in P2WSH
        cltv_script_hash = sha256(bytes.fromhex(cltv_script)).hex()
        assert_equal('0 ' + asm_hex(cltv_script_hash), rpc_result['segwit']['asm'])

        self.log.info("- P2PK with uncompressed pubkey")
        # <pubkey> CHECKSIG
        rpc_result = self.nodes[0].decodescript(push_uncompressed_public_key + 'ac')
        assert_equal('pubkey', rpc_result['type'])
        assert_equal(asm_hex(uncompressed_public_key) + ' CHECKSIG', rpc_result['asm'])
        ...
        # <m> <A pubkey> <B pubkey> <n> CHECKMULTISIG
        # just imagine that the pub keys used below are different.
        ...
        assert_equal('multisig', rpc_result['type'])
        assert_equal('2 ' + asm_hex(public_key) + ' ' + asm_hex(uncompressed_public_key) + ' 2 CHECKMULTISIG', rpc_result['asm'])
        # uncompressed pubkeys are invalid for checksigs in segwit scripts.
        # decodescript should not return a P2WPKH equivalent.
        assert 'segwit' not in rpc_result
        ...
        assert_equal('0 ' + asm_hex(public_key_hash), rpc_result['asm'])
        ...
        assert_equal('0 ' + asm_hex(p2wsh_p2pk_script_hash), rpc_result['asm'])
        # segwit scripts do not work nested into each other.
        # a nested segwit script should not be returned in the results.
        assert 'segwit' not in rpc_result
        ...
        assert_equal('1 ' + asm_hex(xonly_public_key), rpc_result['asm'])
        assert 'segwit' not in rpc_result
    ...
    def decoderawtransaction_asm_sighashtype(self):
        ...
        signature_sighash_decoded = der_signature + FormatSigHashHex(SIGHASH_ALL)
        signature_2 = der_signature + FormatSigHashHex(SIGHASH_NONE|SIGHASH_ANYONECANPAY)
        push_signature_2 = '48' + signature_2
        signature_2_sighash_decoded = der_signature + FormatSigHashHex(SIGHASH_NONE|SIGHASH_ANYONECANPAY)

        self.log.info("- P2PK scriptSig")
        txSave.vin[0].scriptSig = bytes.fromhex(push_signature)
        rpc_result = self.nodes[0].decoderawtransaction(txSave.serialize().hex())
        assert_equal(asm_hex(signature_sighash_decoded), rpc_result['vin'][0]['scriptSig']['asm'])

        # make sure that the sighash decodes come out correctly for a more complex / lesser used case.
        txSave.vin[0].scriptSig = bytes.fromhex(push_signature_2)
        rpc_result = self.nodes[0].decoderawtransaction(txSave.serialize().hex())
        assert_equal(asm_hex(signature_2_sighash_decoded), rpc_result['vin'][0]['scriptSig']['asm'])

        self.log.info("- multisig scriptSig")
        txSave.vin[0].scriptSig = bytes.fromhex('00' + push_signature + push_signature_2)
        rpc_result = self.nodes[0].decoderawtransaction(txSave.serialize().hex())
        assert_equal('0 ' + asm_hex(signature_sighash_decoded) + ' ' + asm_hex(signature_2_sighash_decoded), rpc_result['vin'][0]['scriptSig']['asm'])

        self.log.info("- scriptSig that contains more than push operations")
        # in fact, it contains an RETURN with data specially crafted to cause improper decode if the code does not catch it.

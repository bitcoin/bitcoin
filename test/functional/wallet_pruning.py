...
class WalletPruningTest(SyscoinTestFramework):
    ...
    def test_wallet_import_pruned_with_missing_blocks(self, wallet_name):
        ...
        assert_raises_rpc_error(-4, f"Pruned blocks from height {wallet_birthheight - 47} required to import keys. Use RPC call getblockchaininfo to determine your pruned height.", self.nodes[1].importwallet, os.path.join(self.nodes[0].datadir, wallet_file))
        self.log.info("- Done")

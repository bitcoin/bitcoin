...
class FeatureIndexPruneTest(BitcoinTestFramework):
    ...
    def sync_index(self, height):
        expected_filter = {
            'basic block filter index': {'synced': True, 'best_block_height': height, 'first_block_height': 0},
        }
        self.wait_until(lambda: self.nodes[0].getindexinfo() == expected_filter)

        expected_stats = {
            'coinstatsindex': {'synced': True, 'best_block_height': height, 'first_block_height': 0}
        }

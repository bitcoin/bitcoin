...
class SilentPaymentsReceivingTest(BitcoinTestFramework):
    ...
    def test_encrypt_and_decrypt(self):
        ...
        self.log.info("Check persistence of encrypted wallet")
        self.nodes[0].unloadwallet("sp_encrypted")
        self.nodes[0].loadwallet("sp_encrypted")

        self.log.info("Check that we can scan without the freshly loaded wallet being unlocked")
        addr = wallet.getnewaddress(address_type="silent-payments")
        self.def_wallet.sendtoaddress(addr, 10)
        self.generate(self.nodes[0], 1)
    ...
    def test_basic(self):
        ...
        assert change_addr.startswith("sp")
    ...
    def test_import_descriptors(self):
        self.log.info("Check import silent payments descriptor with same scan and spend key")

        self.nodes[0].createwallet(wallet_name="import_same_scan_spend", blank=True, silent_payments=True)
        wallet = self.nodes[0].get_wallet_rpc("import_same_scan_spend")

        descriptor = "sp([eea23daf/352h/1h/0h/0h/0]cUxxbQ67tepsEn3AUKSCWAKrvLwP6MPk55DAptcqxqDiXN6yKkNW,[eea23daf/352h/1h/0h/0h/0]cUxxbQ67tepsEn3AUKSCWAKrvLwP6MPk55DAptcqxqDiXN6yKkNW)#vktsamkr"
        ...
        wallet_wo = self.nodes[0].get_wallet_rpc("wallet_wo")
        assert_equal(wallet_wo.getbalance(), 0)

        wallet_sp_desc = [d["desc"] for d in wallet.listdescriptors()["descriptors"] if d["desc"].startswith("sp(")][0]
        res = wallet_wo.importdescriptors([{
            "desc": wallet_sp_desc,
            "active": True,
            "timestamp": timestamp
        }])

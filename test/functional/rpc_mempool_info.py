...
class RPCMempoolInfoTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(result, [ {'txid' : txidA, 'vout' : 5} ])
        # fails on node 2 as txospenderindex is not activated
        assert_raises_rpc_error(-1, f"No spending tx for the outpoint {txidA}:5 in mempool, and txospenderindex is unavailable", self.nodes[2].gettxspendingprevout, [{'txid' : txidA, 'vout' : 5} ])
        ...
        assert_equal(result, [ {'txid' : confirmed_utxo['txid'], 'vout' : 0, 'spendingtxid' : txidA, 'blockhash' : blockhash, 'spendingtx' : txA['hex']}, {'txid' : txidA, 'vout' : 1, 'spendingtxid' : txidC,  'blockhash' : blockhash, 'spendingtx' : txC['hex']} ])
        assert_raises_rpc_error(-1, "No spending tx for the outpoint " + confirmed_utxo['txid'] + ":0 in mempool, and txospenderindex is unavailable", self.nodes[2].gettxspendingprevout, [{'txid' : confirmed_utxo['txid'], 'vout' : 0}, {'txid' : txidA, 'vout' : 1} ], return_spending_tx=True)

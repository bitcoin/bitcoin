...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the RevoltTestFramework, place overrides for the
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 RVT) in its wallet. Using them is much more
...
The [`TestShell`](test-shell.md) class exposes the RevoltTestFramework

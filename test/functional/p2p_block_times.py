...
The test starts one node and a test framework peer; the test framework
peer generates a block and announces it to the node, then uses the
`getpeerinfo` RPC to verify the new `last_block_announcement` field. The
test then verifies that sending a block announcement without a _new_
block does not update this field. Finally, the test generates and
announces a sibling block (at the same height, 2); since it has the
same chainwork, it is not accepted as the new tip. The test verifies
that the `last_block_announcement` field is not updated.
"""

...
class AssetNotaryTest(SyscoinTestFramework):
    ...
    def complex_notary(self):
        ...
        assert_equal(out[0]['asset_amount'], int(0.5*COIN) or out[1]['asset_amount'], int(0.5*COIN))
        out =  self.nodes[0].listunspent(query_options={'assetGuid': self.asset3})
        assert_equal(len(out), 2)
        assert_equal(out[0]['asset_amount'], int(1*COIN) or out[1]['asset_amount'], int(1*COIN))
        out =  self.nodes[0].listunspent(query_options={'assetGuid': self.asset4})
        assert_equal(len(out), 2)
        assert_equal(out[0]['asset_amount'], int(1.5*COIN) or out[1]['asset_amount'], int(1.5*COIN))

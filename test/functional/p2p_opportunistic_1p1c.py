...
class PackageRelayTest(BitcoinTestFramework):
    ...
    def test_orphanage_dos_many(self):
        ...
        self.wait_until(lambda: any([tx.rehash() in node.getorphantxs() for tx in shared_orphans]))
        ...
        for peernum in range(num_peers_unique):
            ...
            self.wait_until(lambda: any([tx.rehash() in node.getorphantxs() for tx in this_batch_orphans]))

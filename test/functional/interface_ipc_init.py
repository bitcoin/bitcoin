...
class IPCInitTest(BitcoinTestFramework):
    ...
    def test_ipcbind_max_connections(self):
        ...
        node.assert_start_raises_init_error(
            ...
            expected_msg=f"Error: Too many IPC file descriptors requested: {MAX_IPC_CONNECTIONS + 2} -ipcbind listeners and connection slots exceed the limit of {MAX_IPC_CONNECTIONS + 1}. Reduce the number of -ipcbind addresses or their max-connections values",
        )

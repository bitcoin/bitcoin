...
class WalletDescriptorTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Test that the HD master key can be rotated")
        self.nodes[0].createwallet(wallet_name="keyrotationtest", descriptors=True)
        wallet = self.nodes[0].get_wallet_rpc("keyrotationtest")
        xpub = wallet.getxpub()
        wallet.sethdseed()
        assert xpub != wallet.getxpub()

        self.log.info("Test that new HD master keys persist")
        xpub = wallet.getxpub()
        wallet.unloadwallet()
        self.nodes[0].loadwallet("keyrotationtest")
        assert_equal(xpub, wallet.getxpub())

        self.log.info("Test setting of HD master keys")
        assert_raises_rpc_error(-5, "Key must be a BIP 32 extended private key, or the BIP 32 seed as a WIF private key or a hex string.", wallet.sethdseed, True, "not a valid key encoding")
        assert_raises_rpc_error(-5, "Key must be a BIP 32 extended private key, or the BIP 32 seed as a WIF private key or a hex string.", wallet.sethdseed, True, xpub["xpub"])

        keys_and_seeds = [
            "tprv8ZgxMBicQKsPd8juz2Vg1M49cqAFhNBRESispnpXWC4A5kcVHKcQbNZ8foKppdRNtDzqx7ueY8vWo2LHKJW8Pv9CwTNJpaGWACoR8Nsowzf",
            secrets.randbits(128).to_bytes(16, byteorder="big").hex(),
            secrets.randbits(256).to_bytes(32, byteorder="big").hex(),
            secrets.randbits(384).to_bytes(48, byteorder="big").hex(),
            secrets.randbits(512).to_bytes(64, byteorder="big").hex(),
            generate_wif_key(),
        ]

        for ks in keys_and_seeds:
            wallet.sethdseed(True, ks)
            # Setting the same key active should work
            wallet.sethdseed(True, ks)

        # We can also set a previous master as active
        wallet.sethdseed(True, keys_and_seeds[0])
        ...
        for t in addr_types:
            ...
            # Change and receiving addresses of the same type should have different xpubs
            recv_addr = wallet.getnewaddress(address_type=t)
            chng_addr = wallet.getrawchangeaddress(address_type=t)

            recv_addr_info = wallet.getaddressinfo(recv_addr)
            chng_addr_info = wallet.getaddressinfo(chng_addr)

            recv_xpub = re.search(r"tpub.+?\/", recv_addr_info["parent_desc"])
            assert recv_xpub
            chng_xpub = re.search(r"tpub.+?\/", chng_addr_info["parent_desc"])
            assert chng_xpub
            assert recv_xpub.group() != chng_xpub.group()

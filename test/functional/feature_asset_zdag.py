...
class AssetZDAGTest(SyscoinTestFramework):
    ...
    def basic_zdag_doublespend(self):
        ...
        self.nodes[0].generate(1)
        self.sync_blocks()
        out =  self.nodes[0].listunspent(query_options={'assetGuid': self.asset, 'minAmountAsset':0,'maxAmountAsset':0})
        assert_equal(len(out), 1)
        out =  self.nodes[0].listunspent(query_options={'assetGuid': self.asset, 'minAmountAsset':0.3,'maxAmountAsset':0.3})
        assert_equal(len(out), 1)
        out =  self.nodes[0].listunspent(query_options={'assetGuid': self.asset, 'minAmountAsset':0.6,'maxAmountAsset':0.6})
        assert_equal(len(out), 1)
        out =  self.nodes[0].listunspent(query_options={'assetGuid': self.asset, 'minAmountAsset':1.0,'maxAmountAsset':1.0})
        assert_equal(len(out), 1)

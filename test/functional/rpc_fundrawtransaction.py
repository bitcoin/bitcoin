...
class RawTransactionsTest(SyscoinTestFramework):
    ...
    def test_feerate_with_conf_target_and_estimate_mode(self):
        ...
        for unit, fee_rate in {"sys/kb": 0.1, "sat/b": 10000}.items():
        ...
        for mode in ["unset", "economical", "conservative", "sys/kb", "sat/b"]:
            self.log.debug("{}".format(mode))
            for k, v in {"string": "", "object": {"foo": "bar"}}.items():
                assert_raises_rpc_error(-3, "Expected type number for conf_target, got {}".format(k),
                    lambda: self.nodes[1].fundrawtransaction(rawtx, {"estimate_mode": mode, "conf_target": v}))
            if mode in ["sys/kb", "sat/b"]:
                assert_raises_rpc_error(-3, "Amount out of range",
                    lambda: self.nodes[1].fundrawtransaction(rawtx, {"estimate_mode": mode, "conf_target": -1}))
                assert_raises_rpc_error(-4, "Fee rate (0.00000000 SYS/kB) is lower than the minimum fee rate setting (0.00001000 SYS/kB)",
        ...
        for unit, fee_rate in {"sat/B": 0.99999999, "SYS/kB": 0.00000999}.items():
            self.log.info("- raises RPC error 'fee rate too low' if conf_target {} and estimate_mode {} are passed".format(fee_rate, unit))
            assert_raises_rpc_error(-4, "Fee rate (0.00000999 SYS/kB) is lower than the minimum fee rate setting (0.00001000 SYS/kB)",

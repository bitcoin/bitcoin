...
class WalletMuSigTest(BitcoinTestFramework):
    ...
    def test_success_case(self, comment, pattern, sighash_type=None, scriptpath=False, nosign_wallets=None, only_one_musig_wallet=False):
        ...
        # Participant maps are keyed by the aggregate pubkey, independent of participant order.
        expected_participant_maps = len({
            tuple(sorted(musig.split(",")))
            for musig in MUSIG_RE.findall(pat)
        })
    ...
    def test_same_musig_different_derivation_paths(self):
        self.log.info("Testing reused MuSig participants with different aggregate derivation paths")
        ...
        for psbt_map in [dec_psbt["inputs"][0], dec_psbt["outputs"][1]]:
            ...
            assert_equal(len(participant_pubkeys), len(keys))

            participant_paths = []
            for deriv_path in psbt_map["taproot_bip32_derivs"]:
                if deriv_path["pubkey"] in participant_pubkeys:
                    participant_paths.append(deriv_path["path"])

            assert_equal(len(participant_paths), len(keys))
    ...
    def run_test(self):
        ...
        self.test_success_case(
            "tr(H,and_v(v:pk(musig/*)),pk(same participants, different derivation/*)))",

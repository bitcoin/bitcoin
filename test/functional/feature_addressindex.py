...
class AddressIndexTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.stop_node(1)
        self.nodes[1].assert_start_raises_init_error(["-addressindex=0"], "You need to rebuild the database using -reindex to change -addressindex")
        self.start_node(1, ["-addressindex=0", "-reindex"])
        connect_nodes(self.nodes[0], 1)
        self.sync_all()
        self.stop_node(1)
        self.nodes[1].assert_start_raises_init_error(["-addressindex"], "You need to rebuild the database using -reindex to change -addressindex")

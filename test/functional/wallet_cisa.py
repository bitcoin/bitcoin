...
from test_framework.messages import CTxInWitness, tx_from_hex
from test_framework.psbt import PSBT, PSBT_IN_CISA_FULLAGG_PUB_NONCE, PSBT_IN_CISA_HALFAGG_SIG, PSBT_IN_CISA_MODE, PSBT_IN_TAP_KEY_SIG
...
class WalletCisaTest(BitcoinTestFramework):
    ...
    def test_raw_signing(self, wallet):
        self.log.info("Raw signing reports aggregated inputs as complete only when the transaction verifies")
        inputs = self.fund(wallet, 2)
        tx = tx_from_hex(self.nodes[0].createrawtransaction(inputs, [{self.funder.getnewaddress(): 3}]))
        tx.wit.vtxinwit = [CTxInWitness() for _ in inputs]
        for txinwit in tx.wit.vtxinwit:
            txinwit.scriptWitness.stack = [bytes(32)]
        res = wallet.signrawtransactionwithwallet(tx.serialize().hex())
        assert_equal(res["complete"], False)
        assert_equal([err["error"] for err in res["errors"]], ["Invalid CISA aggregation group structure"] * 2)
        assert_equal(res["hex"], tx.serialize().hex())

        aggregated = wallet.send(outputs=[{self.funder.getnewaddress(): 3}], options={"inputs": inputs, "add_inputs": False, "add_to_wallet": False, "cisa_mode": "halfagg"})["hex"]
        res = wallet.signrawtransactionwithwallet(aggregated)
        assert_equal(res["complete"], True)
        assert_equal(res["hex"], aggregated)
    ...
    def test_two_wallets(self, alice, bob):
        ...
        self.log.info("The Combiner keeps the first half-aggregation signature, the Finalizer rejects an invalid one")
        corrupted = PSBT.from_base64(bob_signed["psbt"])
        idx, bob_input = next((i, psbt_in) for i, psbt_in in enumerate(corrupted.i) if PSBT_IN_CISA_HALFAGG_SIG in psbt_in.map)
        bob_input.map[PSBT_IN_CISA_HALFAGG_SIG] = bytes(64)
        combined = node.combinepsbt([corrupted.to_base64(), bob_signed["psbt"]])
        assert_equal(node.decodepsbt(combined)["inputs"][idx]["cisa_halfagg_sig"], "00" * 64)
        assert_equal(alice.walletprocesspsbt(combined)["complete"], False)
    ...
    def test_finalizer(self, wallet):
        ...
        self.log.info("A finalized group is closed, a later input of its mode does not form a second group")
        node = self.nodes[0]
        psbt = wallet.walletcreatefundedpsbt(inputs=self.fund(wallet, 2), outputs=[{self.funder.getnewaddress(): 3}], options={"add_inputs": False, "cisa_mode": "halfagg"}, psbt_version=0)["psbt"]
        signed = PSBT.from_base64(wallet.walletprocesspsbt(psbt, finalize=False)["psbt"])
        mode, sig = signed.i[1].map.pop(PSBT_IN_CISA_MODE), signed.i[1].map.pop(PSBT_IN_CISA_HALFAGG_SIG)
        partial = node.finalizepsbt(signed.to_base64())
        assert_equal(partial["complete"], False)
        assert_equal(["final_scriptwitness" in psbt_in for psbt_in in node.decodepsbt(partial["psbt"])["inputs"]], [True, False])
        reopened = PSBT.from_base64(partial["psbt"])
        reopened.i[1].map[PSBT_IN_CISA_MODE] = mode
        reopened.i[1].map[PSBT_IN_CISA_HALFAGG_SIG] = sig
        res = node.finalizepsbt(reopened.to_base64())
        assert_equal(res["complete"], False)
        assert_equal(["final_scriptwitness" in psbt_in for psbt_in in node.decodepsbt(res["psbt"])["inputs"]], [True, False])

...
class BIP66Test(SyscoinTestFramework):
    ...
    def run_test(self):
        ...
        # Syscoin
        block = create_block(int(tip, 16), create_coinbase(DERSIG_HEIGHT - 1, dip4_activated=True), block_time)
        ...
        tip = block.sha256
        block_time += 1
        block = create_block(tip, create_coinbase(DERSIG_HEIGHT, dip4_activated=True), block_time)
        block.set_base_version(2)
        block.rehash()
        block.solve()

        with self.nodes[0].assert_debug_log(expected_msgs=['{}, bad-version(0x10000002)'.format(block.hash)]):
            self.nodes[0].p2p.send_and_ping(msg_block(block))

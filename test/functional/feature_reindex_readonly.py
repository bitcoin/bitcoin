...
- Start one node, generate blocks, then restart with -reindex after setting blk files to read-only
"""
...
class BlockstoreReindexTest(BitcoinTestFramework):
    ...
    def reindex_readonly(self):
        ...
        self.log.debug("Make the first block file immutable")
        ...
        # Undo permissions so block files can be cleaned up
        try:
            subprocess.run(['chattr', '-i', filename])
        except Exception as e:
            self.log.warning(f"Can not make file mutable again ({e})")

...
class RandomizeP2PPortTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(node0.getnetworkinfo()["localaddresses"][0]["port"], port0)
        ...
        with listening_socket(manual_port):
            node0.assert_start_raises_init_error(
                extra_args=["-randomizep2pport=1", f"-port={manual_port}"],
                expected_msg=rf"Error: Unable to bind to .*:{manual_port} on this computer",
                match=ErrorMatch.PARTIAL_REGEX,
            )

        self.log.info("Invalid local ports are rejected while remote port 8333 remains allowed")
        node0.assert_start_raises_init_error(
            extra_args=["-randomizep2pport=1", "-port=8333"],
            expected_msg="Error: -port cannot be set to reserved P2P port 8333 when -randomizep2pport is enabled.",
        )
        node0.assert_start_raises_init_error(
            extra_args=["-randomizep2pport=1", "-port=49151"],
            expected_msg="Error: -port must be in the randomized P2P port range 49152-65534 when -randomizep2pport is enabled.",
        )

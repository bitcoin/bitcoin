...
class EstimateFeeTest(BitcoinTestFramework):
    ...
    def test_estimatesmartfee_return_mempool_estimates(self):
        ...
        estimate_after_restart = node0.estimatesmartfee(1, block_policy_only=False)
        verify_estimate_response(estimate_after_restart, None, [BLOCK_POLICY_ESTIMATOR_ERROR])
        self.log.info("Test estimatesmartfee with sparse mempool caps stale block policy estimate")
        # Generate high-feerate transactions and mine them over 6 blocks to give block policy data.
        # After mining, the mempool is empty/sparse.
        high_feerate = Decimal("0.004")
        self.broadcast_and_maybe_mine(node0, high_feerate, TXS_COUNT, 6, miner)
        floor_feerate = max(Decimal(node0.getmempoolinfo()["minrelaytxfee"]), Decimal(node0.getmempoolinfo()["mempoolminfee"]))
        estimate_with_sparse_mempool = node0.estimatesmartfee(1, block_policy_only=False)

...
class AssumeutxoTest(BitcoinTestFramework):
    ...
    def test_invalid_snapshot_scenarios(self, valid_snapshot_path):
        ...
        cases = [
            ...
            # Here one byte is skipped from testing which is the coin size byte.
            # Its manipulation is tested below in "snapshot file with manipulated
            # coin size value" because manipulating this byte will always cause
            # parsing to fail which results in a different error than we are
            # testing for here (a mismatch of the expected hash).
            [b"\x01", 33, "9f4d897031ab8547665b4153317ae2fdbf0130c7840b66427ebc48b881cb80ad"],  # wrong outpoint index
            [b"\x81", 34, "3da966ba9826fb6d2604260e01607b55ba44e1a5de298606b08704bc62570ea8"],  # wrong coin code VARINT((coinbase ? 1 : 0) | (height << 1))
            [b"\x80", 34, "091e893b3ccb4334378709578025356c8bcb0a623f37c7c4e493133c988648e5"],  # another wrong coin code
        ]
        ...
        self.log.info("  - snapshot file with manipulated coin size value")
        with open(bad_snapshot_path, 'wb') as f:
            f.write(valid_snapshot_contents[:72])
            f.write((2).to_bytes(1, "little"))
            f.write(valid_snapshot_contents[73:])
        expected_error(log_msg=f"bad snapshot data after deserializing 1 coins")

...
class AssumeutxoTest(BitcoinTestFramework):
    ...
    def assert_disk_cleanup(self, node, assumeutxo_used):
        """
        Ensure an assumutxo node is cleaning up the background chainstate
        """
        msg = []
        if assumeutxo_used:
    ...
    def run_test(self):
        ...
        for i in (0, 1):
            ...
            with self.assert_disk_cleanup(n, True if i == 1 else False):
        ...
        for i in (0, 2):
            n = self.nodes[i]
            self.log.info(f"Restarting node {i} to ensure (Check|Load)BlockIndex passes")
            with self.assert_disk_cleanup(n, True if i == 2 else False):
                self.restart_node(i, extra_args=self.extra_args[i])

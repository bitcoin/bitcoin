...
class ReconciliationResponderTest(ReconciliationTest):
    ...
    def test_reconciliation_responder_protocol_violations(self):
        ...
        # Test disconnect on sending a RECONCILDIFF with a success field that is neither 0 nor 1
        self.log.info('Testing protocol violation: sending RECONCILDIFF with an invalid success field')
        peer = self.test_node.add_p2p_connection(TxReconTestP2PConn())
        peer.send_reqtxrcncl(0, wire_q())
        assert_equal(self.wait_for_sketch(peer).skdata, [])
        with self.test_node.assert_debug_log(['Peer is finalizing reconciliation with an invalid success field']):
            self.send_reconcildiff_from(peer, 2, [])
            peer.wait_for_disconnect()

...
class ReconciliationResponderTest(ReconciliationTest):
    ...
    def test_reconciliation_responder_flow_empty_sketch(self):
        ...
        peer.send_reqtxrcncl(0, int(math.ceil(RECON_Q * Q_PRECISION)))
        # We need to make sure the node has trickled for inbounds. Waiting for 20x the expected
    ...
    def test_reconciliation_responder_flow_interleaved_txs(self):
        ...
        # Bump well past the trickle interval so every peer has add all their
        # transaction to our reconciliation set before requesting reconciliation
        self.bump_mocktime_past_trickle(INBOUND_INVENTORY_BROADCAST_INTERVAL)
        fanout_inbound_peer.sync_with_ping()
        for peer in peers:
            peer.sync_with_ping()

        # Send a reconciliation request to all peers
        for peer in peers:
            peer.send_reqtxrcncl(n_local_txs, int(math.ceil(RECON_Q * Q_PRECISION)))
    ...
    def test_reconciliation_responder_flow_no_extension(self, n_mininode, n_node):
        ...
        # Send a reconciliation request to all peers
        for peer in peers:
            peer.send_reqtxrcncl(n_mininode, int(math.ceil(RECON_Q * Q_PRECISION)))

        # Pick a random reconciliation peer to proceed
        peer = pick_random_fanout_peer(peers)
        received_sketch = self.wait_for_sketch(peer)

...
class ReconciliationResponderTest(ReconciliationTest):
    ...
    def test_reconciliation_responder_flow_interleaved_txs(self):
        ...
        # Since Erlay inbounds are selected as a percentage of all inbounds, we only need
        # a single connection to ensure we'll be picked for reconciliation
        peer = self.test_node.add_p2p_connection(TxReconTestP2PConn())
        ...
        # Send a reconciliation request. And wait for them to trickle
        ...
        self.outbound_peers[0].send_txs_and_test(local_unique_txs[:1], self.test_node, success=True)

...
class PackageRelayTest(BitcoinTestFramework):
    ...
    def raise_network_minfee(self):
        ...
        for node in self.nodes:
            assert_equal(node.getmempoolinfo()['minrelaytxfee'], FEERATE_100SAT_KVB)
            assert_greater_than(node.getmempoolinfo()['mempoolminfee'], FEERATE_100SAT_KVB)

    def create_basic_1p1c(self, wallet):
        low_fee_parent = wallet.create_self_transfer(fee_rate=FEERATE_100SAT_KVB, confirmed_only=True)
        high_fee_child = wallet.create_self_transfer(utxo_to_spend=low_fee_parent["new_utxo"], fee_rate=999*FEERATE_100SAT_KVB)
    ...
    def create_package_2p1c(self, wallet):
        parent1 = wallet.create_self_transfer(fee_rate=FEERATE_100SAT_KVB*10, confirmed_only=True)
        parent2 = wallet.create_self_transfer(fee_rate=FEERATE_100SAT_KVB*20, confirmed_only=True)

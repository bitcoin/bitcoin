...
class MempoolAcceptanceTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': True, 'vsize': tx.get_vsize(), 'fees': {'base': fee_expected}}],
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'missing-inputs'}],
            rawtxs=[tx.serialize().hex()],
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': True, 'vsize': tx.get_vsize(), 'fees': { 'base': Decimal('0.1') - Decimal('0.05')}}],
            rawtxs=[tx.serialize().hex()],
            maxfeerate=0,
        )
        ...
        tx.vout = []
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'bad-txns-vout-empty'}],
            rawtxs=[tx.serialize().hex()],
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'bad-txns-oversize'}],
            rawtxs=[tx.serialize().hex()],
        )
        ...
        tx.vout[0].nValue *= -1
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'bad-txns-vout-negative'}],
            rawtxs=[tx.serialize().hex()],
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'bad-txns-vout-toolarge'}],
            rawtxs=[tx.serialize().hex()],
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'bad-txns-txouttotal-toolarge'}],
            rawtxs=[tx.serialize().hex()],
        )
        ...
        tx.vin = [tx.vin[0]] * 2
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'bad-txns-inputs-duplicate'}],
            rawtxs=[tx.serialize().hex()],
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'bad-txns-prevout-null'}],
            rawtxs=[tx.serialize().hex()],
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'coinbase'}],
            rawtxs=[tx.serialize().hex()],
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'version'}],
            rawtxs=[tx.serialize().hex()],
        )
        tx = tx_from_hex(raw_tx_reference)
        tx.vout[0].scriptPubKey = CScript([OP_0])  # Some non-standard script
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'scriptpubkey'}],
            rawtxs=[tx.serialize().hex()],
        )
        tx = tx_from_hex(raw_tx_reference)
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'bare-multisig'}],
            rawtxs=[tx.serialize().hex()],
        )
        tx = tx_from_hex(raw_tx_reference)
        tx.vin[0].scriptSig = CScript([OP_HASH160])  # Some not-pushonly scriptSig
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'scriptsig-not-pushonly'}],
            rawtxs=[tx.serialize().hex()],
        )
        tx = tx_from_hex(raw_tx_reference)
        tx.vin[0].scriptSig = CScript([b'a' * 1648]) # Some too large scriptSig (>1650 bytes)
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'scriptsig-size'}],
            rawtxs=[tx.serialize().hex()],
        )
        tx = tx_from_hex(raw_tx_reference)
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'tx-size'}],
            rawtxs=[tx.serialize().hex()],
        )
        tx = tx_from_hex(raw_tx_reference)
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'dust'}],
            rawtxs=[tx.serialize().hex()],
        )
        tx = tx_from_hex(raw_tx_reference)
        ...
        tx.vout = [tx.vout[0]] * 2
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'multi-op-return'}],
            rawtxs=[tx.serialize().hex()],
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'non-final'}],
            rawtxs=[tx.serialize().hex()],
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'non-BIP68-final'}],
            rawtxs=[tx.serialize().hex()],
            maxfeerate=0,
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': False, 'reject-reason': 'tx-size-small'}],
            rawtxs=[tx.serialize().hex()],
            maxfeerate=0,
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.hash, 'allowed': True, 'vsize': tx.get_vsize(), 'fees': { 'base': Decimal('0.00001000')}}],
            rawtxs=[tx.serialize().hex()],
            maxfeerate=0,
        )
        ...
        self.check_mempool_result(
            result_expected=[{'txid': anchor_nonempty_wit_spend.hash, 'allowed': False, 'reject-reason': 'bad-witness-nonstandard'}],
        ...
        assert_equal(anchor_spend.hash, anchor_spend.getwtxid())

        self.check_mempool_result(
            result_expected=[{'txid': anchor_spend.hash, 'allowed': True, 'vsize': anchor_spend.get_vsize(), 'fees': { 'base': Decimal('0.00000700')}}],
        ...
        self.check_mempool_result(
            result_expected=[{'txid': nested_anchor_spend.hash, 'allowed': False, 'reject-reason': 'non-mandatory-script-verify-flag (Witness version reserved for soft-fork upgrades)'}],
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx_spend.hash, 'allowed': True, 'vsize': tx_spend.get_vsize(), 'fees': { 'base': Decimal('0.00000700')}}],

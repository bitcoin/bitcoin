...
class CoinStatsIndexTest(BitcoinTestFramework):
    ...
    def _test_outdated_index_version_migration(self):
        ...
        # A coinstatsindex db v0 synced to the first 101 blocks.
        # This only works with the first 101 blocks because they are created
        # deterministically. Using the later blocks would trigger a different
        # failure because the best block would not match the chain on reruns
        # of the test.
    ...
    def _test_outdated_index_version_corrupted(self):
        ...
        index_db = index_node.chain_path / 'indexes' / 'coinstats' / 'db'
        shutil.rmtree(index_db)
        # This corrupt index db is a coinstatsindex db with v1 values that was
        # synced with the first 101 (deterministic) blocks but then only the
        # version field is reset back to v0. This causes the migration process
        # to start over but fail on unexpected values.
        v0_index_db_corrupt = Path(os.path.dirname(os.path.realpath(__file__))) / 'data' / 'coinstatsindex_v0_corrupt'
        shutil.copytree(v0_index_db_corrupt, index_db)

...
class OrphanHandlingTest(BitcoinTestFramework):
    ...
    def create_malleated_version(self, tx):
        ...
        assert_equal(tx["txid"], tx_bad_wit.hash)
    ...
    def test_orphan_rejected_parents_exceptions(self):
        ...
        peer2.wait_for_getdata([int(parent_low_fee["tx"].hash, 16)])
        ...
        assert_equal(parent_normal["txid"], parent1_witness_stripped.hash)
        assert parent1_witness_stripped.hash not in node.getrawmempool()
        ...
        self.nodes[0].bumpmocktime(NONPREF_PEER_TX_DELAY + TXID_RELAY_DELAY)
        peer2.wait_for_getdata([int(parent_normal["tx"].hash, 16)])
    ...
    def test_orphan_handling_prefer_outbound(self):
        ...
        self.nodes[0].bumpmocktime(TXID_RELAY_DELAY)
        peer_outbound.wait_for_parent_requests([int(parent_tx.hash, 16)])

        # There should be no request to the inbound peer
        peer_inbound.assert_never_requested(int(parent_tx.hash, 16))

        self.log.info("Test that, if the preferred peer doesn't respond, the node sends another request")
        self.nodes[0].bumpmocktime(GETDATA_TX_INTERVAL)
        peer_inbound.sync_with_ping()
        peer_inbound.wait_for_parent_requests([int(parent_tx.hash, 16)])
    ...
    def test_announcers_before_and_after(self):
        ...
        self.nodes[0].bumpmocktime(TXID_RELAY_DELAY)
        peer_early_disconnected.wait_for_parent_requests([int(parent_tx.hash, 16)])
        ...
        self.nodes[0].bumpmocktime(NONPREF_PEER_TX_DELAY)
        peer_early_unresponsive.wait_for_parent_requests([int(parent_tx.hash, 16)])
        ...
        self.nodes[0].bumpmocktime(GETDATA_TX_INTERVAL)
        peer_late_announcer.wait_for_parent_requests([int(parent_tx.hash, 16)])

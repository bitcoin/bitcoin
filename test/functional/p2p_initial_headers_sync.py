...
def calculate_headers_timeout(best_header_time, current_time):
    time_since_best_header = current_time - best_header_time
    variable_timeout_us = (
            HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER_US * time_since_best_header
            // POW_TARGET_SPACING_SEC
    )
    variable_timeout_sec = variable_timeout_us / 1_000_000
    return int(current_time + HEADERS_DOWNLOAD_TIMEOUT_BASE_SEC +
               variable_timeout_sec)
...
class HeadersSyncTest(BitcoinTestFramework):
    ...
    def test_initial_headers_sync(self):
        ...
        self.log.info(
            "Verify that peer2 and peer3 don't receive a getheaders "
            "after connecting")
        for p in all_peers:
            p.sync_with_ping()
        with p2p_lock:
        ...
        self.log.info(
            "Check that exactly 1 of {peer2, peer3} received a getheaders "
            "in response")
        peer_receiving_getheaders = self.assert_single_getheaders_recipient(
            [peer2, peer3])
        # Send empty response, see above
    ...
    def test_normal_peer_timeout(self):
        ...
        best_header_time = self.send_new_block(peer1)

        with self.nodes[0].assert_debug_log(
                ["Timeout downloading headers, disconnecting peer=0"]):
            self.trigger_headers_timeout(best_header_time)
    ...
    def test_noban_peer_timeout(self):
        ...
        self.log.info(
            "Check that exactly 1 of {peer1, peer2} receives a getheaders")
    ...
    def setup_timeout_test_peers(self):
        self.log.info(
            "Add peer1 and check it receives an initial getheaders request")
        node = self.nodes[0]
    ...
    def send_new_block(self, peer):
        self.log.info("Create and send new block to establish new best header")
        tip_hash = int(self.nodes[0].getbestblockhash(), 16)
        tip_height = self.nodes[0].getblockcount()
        tip_time = self.nodes[0].getblock(
            self.nodes[0].getbestblockhash())['time']
        new_block_time = tip_time + 1
        b = create_block(tip_hash, create_coinbase(tip_height + 1),
                         new_block_time)
        b.solve()
        peer.send_and_ping(msg_headers(headers=[CBlockHeader(b)]))
        # Also send the block to avoid block download timeout
        peer.send_and_ping(msg_block(b))

        new_tip_time = self.nodes[0].getblock(
            self.nodes[0].getbestblockhash())['time']
        assert_equal(new_tip_time, new_block_time)
        return new_block_time

    def trigger_headers_timeout(self, best_header_time):
        self.log.info(
            "Trigger the headers download timeout by advancing mock time")
        current_time = self.nodes[0].mocktime
        timeout = calculate_headers_timeout(best_header_time,
                                            current_time)

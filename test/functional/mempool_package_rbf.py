...
class PackageRBFTest(BitcoinTestFramework):
    ...
    def test_package_rbf_basic(self):
        ...
        assert_equal(set(submitres["replaced-transactions"]), set([tx.hash for tx in package_txns1]))
    ...
    def test_package_rbf_singleton(self):
        ...
        assert_equal(submitres["replaced-transactions"], [singleton_tx["tx"].hash])
    ...
    def test_package_rbf_additional_fees(self):
        ...
        assert_equal(f"package RBF failed: insufficient anti-DoS fees, rejecting replacement {package_txns2[1].hash}, less fees than conflicting txs; {PACKAGE_FEE_MINUS_ONE} < {PACKAGE_FEE}", pkg_results2["package_msg"])
        self.assert_mempool_contents(expected=package_txns1)
        ...
        assert_equal(f"package RBF failed: insufficient anti-DoS fees, rejecting replacement {failure_package_txns3[1].hash}, not enough additional fees to relay; {incremental_sats_short} < {incremental_sats_required}", pkg_results3["package_msg"])
        self.assert_mempool_contents(expected=package_txns1)
    ...
    def test_package_rbf_max_conflicts(self):
        ...
        assert_equal(f"package RBF failed: too many potential replacements, rejecting replacement {package_child['tx'].hash}; too many potential replacements (102 > 100)", pkg_results["package_msg"])
        self.assert_mempool_contents(expected=expected_txns)
        ...
        package_child = self.wallet.create_self_transfer(fee_rate=child_feerate, utxo_to_spend=package_parent["new_utxos"][0])
        pkg_results = node.submitpackage([package_parent["hex"], package_child["hex"]], maxfeerate=0)
        assert_equal(f"package RBF failed: too many potential replacements, rejecting replacement {package_child['tx'].hash}; too many potential replacements (101 > 100)", pkg_results["package_msg"])
        self.assert_mempool_contents(expected=expected_txns)
    ...
    def test_wrong_conflict_cluster_size_linear(self):
        ...
        assert_equal(f"package RBF failed: {parent_result['tx'].hash} has 2 descendants, max 1 allowed", package_result["package_msg"])
        ...
        assert_equal(f"package RBF failed: {child_result['tx'].hash} has both ancestor and descendant, exceeding cluster limit of 2", package_result["package_msg"])
        ...
        assert_equal(f"package RBF failed: {grandchild_result['tx'].hash} has 2 ancestors, max 1 allowed", package_result["package_msg"])
    ...
    def test_wrong_conflict_cluster_size_parents_child(self):
        ...
        package_hex1, _package_txns1 = self.create_simple_package(coin1, DEFAULT_FEE, DEFAULT_CHILD_FEE)
        package_result = node.submitpackage(package_hex1)
        assert_equal(f"package RBF failed: {child_result['tx'].hash} has 2 ancestors, max 1 allowed", package_result["package_msg"])
    ...
    def test_wrong_conflict_cluster_size_parent_children(self):
        ...
        package_hex1, _package_txns1 = self.create_simple_package(coin1, DEFAULT_FEE, DEFAULT_CHILD_FEE)
        package_result = node.submitpackage(package_hex1)
        assert_equal(f"package RBF failed: {child2_result['tx'].hash} is not the only child of parent {parent_result['tx'].hash}", package_result["package_msg"])

        package_hex2, _package_txns2 = self.create_simple_package(coin2, DEFAULT_FEE, DEFAULT_CHILD_FEE)
        package_result = node.submitpackage(package_hex2)
        assert_equal(f"package RBF failed: {child1_result['tx'].hash} is not the only child of parent {parent_result['tx'].hash}", package_result["package_msg"])

...
def assert_fee_amount(fee, tx_size, feerate_SNAIL_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_SNAIL_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s SNAIL too low! (Should be %s SNAIL)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_SNAIL_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s SNAIL too high! (Should be %s SNAIL)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting SNAIL values"""
...
class Binaries:
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke snailcoind"
        return self._argv("node", self.paths.snailcoind, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke snailcoin-cli"
        # Add -nonamed because "bitcoin rpc" enables -named by default, but snailcoin-cli doesn't
        return self._argv("rpc", self.paths.snailcoincli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke bitcoin-tx"
        return self._argv("tx", self.paths.snailcointx)

    def util_argv(self):
        "Return argv array that should be used to invoke bitcoin-util"
        return self._argv("util", self.paths.snailcoinutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke bitcoin-wallet"
        return self._argv("wallet", self.paths.snailcoinwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke bitcoin-chainstate"
        return self._argv("chainstate", self.paths.snailcoinchainstate)
...
def get_binary_paths(config):
    ...
    paths.snailcoin_cmd = shlex.split(os.getenv("BITCOIN_CMD", "")) or None
    return paths
...
def get_fee(tx_size, feerate_btc_kvb):
    """Calculate the fee in SNAIL given a feerate is SNAIL/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_btc_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  SNAIL
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `SnailcoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "snailcoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Snailcoin"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Snailcoin"
        else:
            datadir = temp_dir / ".snailcoin"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "snailcoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "snailcoin.conf")):
        with open(os.path.join(datadir, "snailcoin.conf"), 'r', encoding='utf8') as f:
            for line in f:

...
class TestNode():
    ...
    def kill_process(self):
        ...
        wallet_pid = None
        with open(self._debug_log_path(wallet=False), encoding="utf-8", errors="replace") as dl:
            for line in dl:
    ...
    def _debug_log_path(self, wallet: bool) -> Path:
    ...
    def debug_log_size(self, wallet: bool = False, **kwargs) -> int:
        with open(self._debug_log_path(wallet), **kwargs) as dl:
            dl.seek(0, 2)
            return dl.tell()
    ...
    def assert_debug_log(self, expected_msgs, unexpected_msgs=None, *, timeout=0, wallet=False):
        ...
        while True:
            with open(self._debug_log_path(wallet), encoding="utf-8", errors="replace") as dl:
                dl.seek(prev_size)
                log = dl.read()
    ...
    def busy_wait_for_debug_log(self, expected_msgs, timeout=60, wallet=False):
        ...
        while True:
            with open(self._debug_log_path(wallet), "rb") as dl:
                dl.seek(prev_size)
                log = dl.read()

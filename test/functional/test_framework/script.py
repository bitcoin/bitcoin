...
class CScript(bytes):
    ...
    def CountSigOps(self, fAccurate):
        """Get the SigOp count.
    ...
    def test_legacy_sigopcount(self):
        ...
        for n_ops in range(1, 100, 10):
            for singlesig_op in (OP_CHECKSIG, OP_CHECKSIGVERIFY):
                ...
                self.assertEqual(singlesigs_script.CountSigOps(fAccurate=False), n_ops)
                self.assertEqual(singlesigs_script.CountSigOps(fAccurate=True), n_ops)
        ...
        for n in range(1, 16+1):
            for multisig_op in (OP_CHECKMULTISIG, OP_CHECKMULTISIGVERIFY):
                ...
                self.assertEqual(multisig_script.CountSigOps(fAccurate=False), 20)
                self.assertEqual(multisig_script.CountSigOps(fAccurate=True), n)

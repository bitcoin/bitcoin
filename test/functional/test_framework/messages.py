...
COIN = 100000000  # 1 wcn in satoshis
...
# Objects that map to widecoind objects, which can be serialized/deserialized


class CAddress:
    __slots__ = ("net", "ip", "nServices", "port", "time")

    # see https://github.com/widecoin/bips/blob/master/bip-0155.mediawiki
    NET_IPV4 = 1
    NET_I2P = 5
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += struct.pack("<i", 0)  # Widecoin Core ignores version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in widecoind
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in widecoind
        try:
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in widecoind indicates these should be deserialized as blocks

...
TMPDIR_PREFIX = "revolt_func_test_"
...
class RevoltTestMetaClass(type):
    """Metaclass for RevoltTestFramework.

    Ensures that any attempt to register a subclass of `RevoltTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'RevoltTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("RevoltTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("RevoltTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class RevoltTestFramework(metaclass=RevoltTestMetaClass):
    """Base class for a revolt test script.

    Individual revolt test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave revoltds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop revoltds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use revolt-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_revoltd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "revoltd" + config["environment"]["EXEEXT"],
        )
        fname_revoltcli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "revolt-cli" + config["environment"]["EXEEXT"],
        )
        fname_revoltutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: revoltds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("RevoltRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        for i in range(num_nodes):
            ...
            if not test_node_i.version_is_at_least(170000):
                ...
                conf_file = test_node_i.revoltconf
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a revoltd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple revoltd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as revoltd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in revolt.conf
    ...
    def skip_if_no_revoltd_tracepoints(self):
        """Skip the running test if revoltd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("revoltd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_revoltd_zmq(self):
        """Skip the running test if revoltd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("revoltd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if revolt-wallet has not been compiled."""
    ...
    def skip_if_no_cli(self):
        """Skip the running test if revolt-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("revolt-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether revolt-cli was compiled."""

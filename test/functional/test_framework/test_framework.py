...
TMPDIR_PREFIX = "britanniacoin_func_test_"
...
class BritanniaCoinTestMetaClass(type):
    ...
    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BritanniaCoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BritanniaCoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BritanniaCoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BritanniaCoinTestFramework(metaclass=BritanniaCoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave britanniacoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop britanniacoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use britanniacoin-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_britanniacoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "britanniacoind" + config["environment"]["EXEEXT"],
        )
        fname_britanniacoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "britanniacoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_britanniacoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: britanniacoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("BritanniaCoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'britanniacoind', self.options.britanniacoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'britanniacoin-cli', self.options.britanniacoincli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                britanniacoind=binary[i],
                britanniacoin_cli=binary_cli[i],
                version=versions[i],
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    britanniacoind=self.options.britanniacoind,
                    britanniacoin_cli=self.options.britanniacoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
    ...
    def skip_if_no_britanniacoind_tracepoints(self):
        """Skip the running test if bitcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("britanniacoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_britanniacoind_zmq(self):
        """Skip the running test if bitcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("britanniacoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        ...
        if not self.is_wallet_tool_compiled():
            raise SkipTest("britanniacoin-wallet has not been compiled")

    def skip_if_no_britanniacoin_util(self):
        """Skip the running test if bitcoin-util has not been compiled."""
        if not self.is_britanniacoin_util_compiled():
            raise SkipTest("britanniacoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bitcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("britanniacoin-cli has not been compiled.")

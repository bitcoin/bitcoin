...
TMPDIR_PREFIX = "widecoin_func_test_"
...
class WidecoinTestMetaClass(type):
    """Metaclass for WidecoinTestFramework.

    Ensures that any attempt to register a subclass of `WidecoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'WidecoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("WidecoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("WidecoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class WidecoinTestFramework(metaclass=WidecoinTestMetaClass):
    """Base class for a widecoin test script.

    Individual widecoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use widecoin-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_widecoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "widecoind" + config["environment"]["EXEEXT"],
        )
        fname_widecoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "widecoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_widecoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "widecoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.widecoind = os.getenv("WIDECOIND", default=fname_widecoind)
        self.options.widecoincli = os.getenv("WIDECOINCLI", default=fname_widecoincli)
        self.options.widecoinutil = os.getenv("WIDECOINUTIL", default=fname_widecoinutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: widecoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("WidecoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'widecoin-cli', self.options.widecoincli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            if not test_node_i.version_is_at_least(170000):
                ...
                conf_file = test_node_i.widecoinconf
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a widecoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple widecoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as widecoind's debug.log with microprecision (so log files can be concatenated and sorted)
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    widecoind=self.options.widecoind,
                    widecoin_cli=self.options.widecoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
    ...
    def skip_if_no_widecoind_tracepoints(self):
        """Skip the running test if widecoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("widecoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_widecoind_zmq(self):
        """Skip the running test if widecoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("widecoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if widecoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("widecoin-wallet has not been compiled")

    def skip_if_no_widecoin_util(self):
        """Skip the running test if widecoin-util has not been compiled."""
        if not self.is_widecoin_util_compiled():
            raise SkipTest("widecoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if widecoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("widecoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether widecoin-cli was compiled."""
    ...
    def is_widecoin_util_compiled(self):
        """Checks whether widecoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_WIDECOIN_UTIL")

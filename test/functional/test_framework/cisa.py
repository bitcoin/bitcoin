...
def halfagg_aggregate(pms):
    ...
    for i, (pk, msg, sig) in enumerate(pms):
        ...
        if i == 0:
            ...
            z = int.from_bytes(TaggedHash("BIP0458/randomizer", randomizer_input), "big") % GE.ORDER
...
def fullagg_sign(seckeys, msgs):
    ...
    b = int.from_bytes(TaggedHash("BIP0459/noncecoef", nonce_data), "big") % GE.ORDER
    R = R1_agg + b * R2_agg
    ...
    s = 0
    for i in range(u):
        c_i = int.from_bytes(TaggedHash("BIP0459/sig", L + R.to_bytes_xonly() + pks[i] + msgs[i]), "big") % GE.ORDER

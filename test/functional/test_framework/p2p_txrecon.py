...
# Average inventory trickle (Poisson) intervals, taken from net_processing.cpp.
...
def wire_q():
    """The q value a BIP-330 sender puts on the wire, rounded up when narrowed to uint16."""
    return int(math.ceil(RECON_Q * Q_PRECISION))
...
def estimate_sketch_capacity(theirs, ours):
    ...
    # Sender side rounds q up when narrowing it to the wire format, as per BIP-330, so the receiver
    # reconstructs a slightly larger value than RECON_Q. Use the same value the node will.
    weighted_min_size = math.ceil(wire_q() / Q_PRECISION * min_size)
...
class ReconciliationTest(BitcoinTestFramework):
    ...
    def bump_mocktime_past_trickle(self, interval):
        # For transactions to have been added to the reconciliation sets of the node's peers, we need to
        # make sure that the inventory trickle (Poisson) timer has ticked for all of them. Each timer ticks
        # every `interval` seconds on average (OUTBOUND_INVENTORY_BROADCAST_INTERVAL for outbound peers,
        # INBOUND_INVENTORY_BROADCAST_INTERVAL for inbound ones). Given the nature of the test we have no way
        # of checking if the timer has ticked, but we can work around this by making sure we have waited long
        # enough. By bumping the time ~20 times the expected value, we have a 1/100000000 chances of any of
        # the timers not ticking (i.e. failing the test later on), which should be more than acceptable.
        self.test_node.bumpmocktime(interval * 20)

...
class WalletSendTest(SyscoinTestFramework):
    ...
    def run_test(self):
        ...
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, conf_target=0.00007, estimate_mode="sys/kb", add_to_wallet=False)
        ...
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, arg_conf_target=0.00004531, arg_estimate_mode="sys/kb", add_to_wallet=False)
        fee = self.nodes[1].decodepsbt(res["psbt"])["fee"]
        ...
        for mode in ["sys/kb", "sat/b"]:
            ...
            self.test_send(from_wallet=w0, to_wallet=w1, amount=1, conf_target=0, estimate_mode=mode,
                expect_error=(-4, "Fee rate (0.00000000 SYS/kB) is lower than the minimum fee rate setting (0.00001000 SYS/kB)"))
        ...
        # TODO: error should use sat/B instead of SYS/kB if sat/B is selected.
        ...
        for unit, fee_rate in {"sat/B": 0.99999999, "SYS/kB": 0.00000999}.items():
            ...
            self.test_send(from_wallet=w0, to_wallet=w1, amount=1, conf_target=fee_rate, estimate_mode=unit,
                expect_error=(-4, "Fee rate (0.00000999 SYS/kB) is lower than the minimum fee rate setting (0.00001000 SYS/kB)"))

        self.log.info("Explicit fee rate raises RPC error if estimate_mode is passed without a conf_target")
        for unit, fee_rate in {"sat/B": 100, "SYS/kB": 0.001}.items():
        ...
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1, change_address="not an address",
                       expect_error=(-5, "Change address must be a valid syscoin address"))

...
REGEX_DOC = r'(\w+Setting)::Register'
CMD_ROOT_DIR = '$(git rev-parse --show-toplevel)/{}'.format(FOLDER_GREP)
CMD_GREP_ARGS = r"git grep --perl-regexp '{}' -- {} ':(exclude){}'".format(REGEX_ARG, CMD_ROOT_DIR, FOLDER_TEST)
CMD_GREP_WALLET_ARGS = r"git grep --function-context 'void WalletInit::AddWalletOptions' -- {} | grep 'Setting::Register'".format(CMD_ROOT_DIR)
...
def lint_missing_hidden_wallet_args():
    ...
    wallet_hidden_args = set(re.findall(re.compile(r'([A-Za-z0-9]+Setting)Hidden::Register'), wallet_hidden_args))

...
Setup crown testnet server or update existing one
...
handle_arguments()
{
    ...
    if [ $install == true ]; then
        echo "About to install Crown testnet $blah"
    else
        echo "About to update to Crown testnet $blah"
    fi
    echo
    echo "==============================================================================="
}
...
download_package() {
    ...
    if [ -s "$dir/crown.zip" ]; then
        ...
        if [ -n "$bootstrap" ]; then
            ...
            wget "https://nextcloud.crownplatform.com/index.php/s/kaJ438j9CR4wbri/download" -O $dir/testnet.zip
	    if [ ! -e "$dir/testnet.zip" ]; then
	        echo "Failed to download snapshot. Continuing without it."
	        bootstrap=
            fi
        fi
    else
...
main() {
    ...
    echo
    grep "Crown version" ~/.crown/testnet3/debug.log | tail -1
    echo

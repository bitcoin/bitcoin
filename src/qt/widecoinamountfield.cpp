...
private:
    ...
    CAmount parse(const QString &text, bool *valid_out=nullptr) const
    {
        ...
        if(valid)
        {
            if(val < 0 || val > WidecoinUnits::maxMoney())
                valid = false;
        }
        if(valid_out)
            *valid_out = valid;
    ...
    amount(nullptr)
{
    ...
    unit->setModel(new WidecoinUnits(this));
    ...
    connect(unit, qOverload<int>(&QComboBox::currentIndexChanged), this, &WidecoinAmountField::unitChanged);

    // Set default based on configuration
    unitChanged(unit->currentIndex());
}

void WidecoinAmountField::clear()
{
    amount->clear();
    unit->setCurrentIndex(0);
}

void WidecoinAmountField::setEnabled(bool fEnabled)
{
    amount->setEnabled(fEnabled);
    unit->setEnabled(fEnabled);
}

bool WidecoinAmountField::validate()
{
    bool valid = false;
    value(&valid);
    setValid(valid);
...
void WidecoinAmountField::setValid(bool valid)
{
    if (valid)
...
bool WidecoinAmountField::eventFilter(QObject *object, QEvent *event)
{
...
QWidget *WidecoinAmountField::setupTabChain(QWidget *prev)
{
...
CAmount WidecoinAmountField::value(bool *valid_out) const
{
...
void WidecoinAmountField::unitChanged(int idx)
{

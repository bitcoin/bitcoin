    ...
    mapper(nullptr)
{
    ...
    ui->xbitAtStartup->setVisible(false);
    ui->verticalLayout_Main->removeWidget(ui->xbitAtStartup);
    ...
    ui->xbitAtStartup->setToolTip(ui->xbitAtStartup->toolTip().arg(PACKAGE_NAME));
    ui->xbitAtStartup->setText(ui->xbitAtStartup->text().arg(PACKAGE_NAME));

    ui->openXBitConfButton->setToolTip(ui->openXBitConfButton->toolTip().arg(PACKAGE_NAME));
    ...
    ui->unit->setModel(new XBitUnits(this));
...
{
    /* Main */
    mapper->addMapping(ui->xbitAtStartup, OptionsModel::StartAtStartup);
...
void OptionsDialog::on_openXBitConfButton_clicked()
{

    ...
    mapper(nullptr)
{
    ...
    ui->revoltAtStartup->setVisible(false);
    ui->verticalLayout_Main->removeWidget(ui->revoltAtStartup);
    ...
    ui->revoltAtStartup->setToolTip(ui->revoltAtStartup->toolTip().arg(PACKAGE_NAME));
    ui->revoltAtStartup->setText(ui->revoltAtStartup->text().arg(PACKAGE_NAME));

    ui->openRevoltConfButton->setToolTip(ui->openRevoltConfButton->toolTip().arg(PACKAGE_NAME));
    ...
    ui->unit->setModel(new RevoltUnits(this));
...
{
    /* Main */
    mapper->addMapping(ui->revoltAtStartup, OptionsModel::StartAtStartup);
...
void OptionsDialog::on_openRevoltConfButton_clicked()
{

...
Bitcredit_WalletModel::SendCoinsReturn Bitcredit_WalletModel::prepareDepositTransaction(Bitcredit_WalletModel *deposit_model, Bitcredit_WalletModelTransaction &transaction, const Credits_COutput& coin, Credits_CCoinsViewCache &credits_view, Bitcoin_CCoinsViewCache &bitcoin_view)
{
    QList<Bitcredit_SendCoinsRecipient> recipients = transaction.getRecipients();
...

    {
        ...
        bool fCreated = wallet->CreateDepositTransaction(deposit_model->wallet, send, *newTx, *keyDepositSignature, rcp.amount, strFailReason, coin, credits_view, bitcoin_view);
        //transaction.setTransactionFee(nFeeRequired);
...
Bitcredit_WalletModel::SendCoinsReturn Bitcredit_WalletModel::prepareClaimTransaction(Bitcoin_WalletModel *bitcoin_model, Bitcoin_CCoinsViewCache *claim_view, Bitcredit_WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    qint64 total = 0;
    QList<Bitcredit_SendCoinsRecipient> recipients = transaction.getRecipients();
    std::vector<std::pair<CScript, int64_t> > vecSend;

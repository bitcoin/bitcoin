This directory contains the source code for the BritanniaCoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### britanniacoingui.(h/cpp)

- Represents the main window of the BritanniaCoin UI.
...
- ClientModel is used by the main application `britanniacoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a britanniacoin:... link from a browser).
...
* UI elements like BritanniaCoinAmountField, which inherit from QWidget.
* `britanniacoinstrings.cpp`: automatically generated
* `britanniacoinunits.(h/cpp)`: BRT / mBRT / etc. handling
* `callback.h`
...
**Note:** Do not change `local/britanniacoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `britanniacoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `britanniacoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `britanniacoin-qt`
9. You're all set! Start developing, building, and debugging the BritanniaCoin Core GUI

This directory contains the source code for the Snailcoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Snailcoin UI.
...
* UI elements like SnailcoinAmountField, which inherit from QWidget.
* `bitcoinstrings.cpp`: automatically generated
* `bitcoinunits.(h/cpp)`: SNAIL / mSNAIL / etc. handling
* `callback.h`
...
4. Enter `snailcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `snailcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `snailcoin-qt`
9. You're all set! Start developing, building, and debugging the Snailcoin Core GUI

This directory contains the ButtcoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Buttcoin UI.
...
ClientModel is used by the main application `Buttcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a Buttcoin:... link from a browser).
...
* UI elements like ButtcoinAmountField, which inherit from QWidget.
* `Buttcoinstrings.cpp`: automatically generated
* `Buttcoinunits.(h/cpp)`: BTC / mBTC / etc handling
* `callback.h`
...
* don't change `local/Buttcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "Buttcoin-qt" under "Run", which is where you can also add command line arguments)

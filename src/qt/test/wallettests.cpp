                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<SnailcoinAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = SnailcoinUnits::formatWithUnit(unit, expected_balance, false, SnailcoinUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     build/bin/test_snailcoin-qt  # Linux
//     QT_QPA_PLATFORM=windows build/bin/test_snailcoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   build/bin/test_snailcoin-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    SnailcoinAmountField* amountInput = receiveCoinsDialog.findChild<SnailcoinAmountField*>("reqAmount");

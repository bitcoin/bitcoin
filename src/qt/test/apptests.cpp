...
//! Entry point for TortoisecoinApplication tests.
void AppTests::appTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping AppTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_tortoisecoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    ...
    connect(&m_app, &TortoisecoinApplication::windowShown, this, &AppTests::guiTests);
...
//! Entry point for TortoisecoinGUI tests.
void AppTests::guiTests(TortoisecoinGUI* window)
{
    HandleCallback callback{"guiTests", *this};
    connect(window, &TortoisecoinGUI::consoleShown, this, &AppTests::consoleTests);

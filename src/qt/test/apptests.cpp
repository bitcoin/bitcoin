...
//! Entry point for SnailcoinApplication tests.
void AppTests::appTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping AppTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_snailcoin-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    ...
    connect(&m_app, &SnailcoinApplication::windowShown, this, &AppTests::guiTests);
...
//! Entry point for SnailcoinGUI tests.
void AppTests::guiTests(SnailcoinGUI* window)
{
    HandleCallback callback{"guiTests", *this};
    connect(window, &SnailcoinGUI::consoleShown, this, &AppTests::consoleTests);

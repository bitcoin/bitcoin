...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<SnailcoinUnit>("SnailcoinUnit");
}
...
static const char* qt_argv = "snailcoin-qt";

SnailcoinApplication::SnailcoinApplication()
...
void SnailcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the SnailcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", SnailcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void SnailcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool SnailcoinApplication::createOptionsModel(bool resetSettings)
{
...
void SnailcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new SnailcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &SnailcoinGUI::quitRequested, this, &SnailcoinApplication::requestShutdown);
...
void SnailcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void SnailcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool SnailcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void SnailcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &SnailcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &SnailcoinApplication::handleRunawayException);
    connect(this, &SnailcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &SnailcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void SnailcoinApplication::parameterSetup()
{
...
void SnailcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void SnailcoinApplication::requestInitialize()
{
...
void SnailcoinApplication::requestShutdown()
{
...
void SnailcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SnailcoinGUI::handlePaymentRequest);
            connect(window, &SnailcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void SnailcoinApplication::handleRunawayException(const QString &message)
{
...
void SnailcoinApplication::handleNonFatalException(const QString& message)
{
...
WId SnailcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool SnailcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", SnailcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see snailcoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see snailcoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse snailcoin.conf, determine network, switch to network specific

...
const char* BIP71_MIMETYPE_PAYMENT = "application/syscoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/syscoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/syscoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the syscoin: URI contains a payment request, we are not able to detect the
        ...
        if (arg.startsWith(SYSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // syscoin: URI
        {
            ...
            {
                CSyscoinAddress address(r.address.toStdString());
...
//
// OSX-specific way of handling syscoin: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in syscoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(SYSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // syscoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            {
                CSyscoinAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CSyscoinAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom syscoin addresses are not supported
        ...
        // Syscoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (txOut.IsDust(dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(SyscoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);

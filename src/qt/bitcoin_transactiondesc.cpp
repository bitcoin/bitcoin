...
QString Bitcoin_TransactionDesc::FormatTxStatus(const Bitcoin_CWalletTx& wtx)
{
    ...
    if (!Bitcoin_IsFinalTx(wtx, bitcoin_chainActive.Height() + 1))
    {
        if (wtx.nLockTime < BITCOIN_LOCKTIME_THRESHOLD)
            return tr("Open for %n more block(s)", "", wtx.nLockTime - bitcoin_chainActive.Height());
        else
...
QString Bitcoin_TransactionDesc::toHTML(Bitcoin_CWallet *wallet, Bitcoin_CWalletTx &wtx, Bitcoin_TransactionRecord *rec, int unit)
{
    QString strHTML;
    ...
    int64_t nCredit = wtx.Bitcoin_GetCredit(mapEmptyTxInPoints);
    ...
    if (wtx.IsCoinBase() && nCredit == 0)
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            nUnmatured += wallet->GetCredit(txout, txout.nValue);
        ...
        else
        {
            ...
            BOOST_FOREACH(const Bitcoin_CTxIn& txin, wtx.vin)
                if (wallet->IsMine(txin))
                    strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(unit, -wallet->GetDebit(txin, NULL)) + "<br>";
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(unit, wallet->GetCredit(txout, txout.nValue)) + "<br>";
        }
    }
    ...
    strHTML += "<b>" + tr("Transaction ID") + ":</b> " + Bitcoin_TransactionRecord::formatSubTxId(wtx.GetHash(), rec->idx) + "<br>";
    ...
    if (wtx.IsCoinBase())
    {
        quint32 numBlocksToMaturity = BITCOIN_COINBASE_MATURITY +  1;

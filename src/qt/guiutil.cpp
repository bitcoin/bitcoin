...
bool parseWidecoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!WidecoinUnits::parse(WidecoinUnit::WCN, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatWidecoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("widecoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(WidecoinUnits::format(WidecoinUnit::WCN, info.amount, false, WidecoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetPathArg("-conf", WIDECOIN_CONF_FILENAME));
...
fs::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Widecoin (%s).lnk", chain));
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "widecoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("widecoin-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Widecoin\n";
        else
            optionFile << strprintf("Name=Widecoin (%s)\n", chain);

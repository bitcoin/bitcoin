...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Revolt address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new RevoltAddressEntryValidator(parent));
    widget->setCheckValidator(new RevoltAddressCheckValidator(parent));
}
...
{
    // return if URI is not valid or is no revolt: URI
    if(!uri.isValid() || uri.scheme() != QString("revolt"))
        return false;
...
bool parseRevoltURI(QString uri, SendCoinsRecipient *out)
{
    ...
    return parseRevoltURI(uriInstance, out);
}
...
QString formatRevoltURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("revolt:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openRevoltConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", REVOLT_CONF_FILENAME));
    ...
    /* Open revolt.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Revolt.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Revolt (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Revolt (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Revolt*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "revolt.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("revolt-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a revolt.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Revolt\n";
        else
            optionFile << strprintf("Name=Revolt (%s)\n", chain);

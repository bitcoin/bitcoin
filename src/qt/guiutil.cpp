...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new ButtcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new ButtcoinAddressCheckValidator(parent));
}

bool parseButtcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no Buttcoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!ButtcoinUnits::parse(ButtcoinUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseButtcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    return parseButtcoinURI(uriInstance, out);
}
...
QString formatButtcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("Buttcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ButtcoinUnits::format(ButtcoinUnits::BTC, info.amount, false, ButtcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", Buttcoin_CONF_FILENAME));
    ...
    /* Open Buttcoin.conf with the associated application */
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("Buttcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a Buttcoin.desktop file to the autostart directory:

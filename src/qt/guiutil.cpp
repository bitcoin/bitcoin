...
#endif
    LogPrintf(LogInstance(), "Qt %s (%s), plugin=%s (%s)\n", qVersion(), qt_link, QGuiApplication::platformName().toStdString(), plugin_link);
    const auto static_plugins = QPluginLoader::staticPlugins();
    if (static_plugins.empty()) {
        LogPrintf(LogInstance(), "No static plugins.\n");
    } else {
        LogPrintf(LogInstance(), "Static plugins:\n");
        for (const QStaticPlugin& p : static_plugins) {
            ...
            LogPrintf(LogInstance(), " %s, version %d\n", plugin_class, plugin_version);
        }
    }

    LogPrintf(LogInstance(), "Style: %s / %s\n", QApplication::style()->objectName().toStdString(), QApplication::style()->metaObject()->className());
    LogPrintf(LogInstance(), "System: %s, %s\n", QSysInfo::prettyProductName().toStdString(), QSysInfo::buildAbi().toStdString());
    for (const QScreen* s : QGuiApplication::screens()) {
        LogPrintf(LogInstance(), "Screen: %s %dx%d, pixel ratio=%.1f\n", s->name().toStdString(), s->size().width(), s->size().height(), s->devicePixelRatio());
    }
}

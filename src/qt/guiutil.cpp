...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Snailcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new SnailcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new SnailcoinAddressCheckValidator(parent));
}
...
bool parseSnailcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!SnailcoinUnits::parse(SnailcoinUnit::SNAIL, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseSnailcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    return parseSnailcoinURI(uriInstance, out);
}
...
QString formatSnailcoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(SnailcoinUnits::format(SnailcoinUnit::SNAIL, info.amount, false, SnailcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openSnailcoinConf()
{
    ...
    /* Open snailcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Snailcoin.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Snailcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Snailcoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Snailcoin*.lnk
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == ChainType::MAIN)
            optionFile << "Name=Snailcoin\n";
        else
            optionFile << strprintf("Name=Snailcoin (%s)\n", ChainTypeToString(chain));

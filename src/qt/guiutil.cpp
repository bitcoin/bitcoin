...
QString formatWidecoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(WidecoinUnits::format(WidecoinUnits::WCN, info.amount, false, WidecoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", WIDECOIN_CONF_FILENAME));
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("widecoin-%s.desktop", chain);
}

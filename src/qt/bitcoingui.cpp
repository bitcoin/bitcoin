...
const std::string SnailcoinGUI::DEFAULT_UIPLATFORM =
...
SnailcoinGUI::SnailcoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &SnailcoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &SnailcoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &SnailcoinGUI::showModalOverlay);
...
void SnailcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Snailcoin address"));
    ...
    connect(overviewAction, &QAction::triggered, this, &SnailcoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &SnailcoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &SnailcoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Snailcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Snailcoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Snailcoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Snailcoin Transaction from clipboard"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Snailcoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &SnailcoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &SnailcoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &SnailcoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &SnailcoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &SnailcoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &SnailcoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &SnailcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &SnailcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &SnailcoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &SnailcoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &SnailcoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &SnailcoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &SnailcoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &SnailcoinGUI::showDebugWindow);
}

void SnailcoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void SnailcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &SnailcoinGUI::setCurrentWalletBySelectorIndex);
...
void SnailcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &SnailcoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &SnailcoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &SnailcoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &SnailcoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void SnailcoinGUI::enableHistoryAction(bool privacy)
{
...
void SnailcoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &SnailcoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &SnailcoinGUI::removeWallet);
...
WalletController* SnailcoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void SnailcoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &SnailcoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &SnailcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &SnailcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &SnailcoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &SnailcoinGUI::incomingTransaction);
    connect(this, &SnailcoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void SnailcoinGUI::removeWallet(WalletModel* walletModel)
{
...
void SnailcoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void SnailcoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void SnailcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SnailcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void SnailcoinGUI::createTrayIcon()
{
...
void SnailcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &SnailcoinGUI::toggleHidden);
...
void SnailcoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void SnailcoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void SnailcoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void SnailcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void SnailcoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void SnailcoinGUI::openClicked()
{
...
void SnailcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SnailcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SnailcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SnailcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SnailcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SnailcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void SnailcoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void SnailcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Snailcoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void SnailcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SnailcoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void SnailcoinGUI::updateHeadersSyncProgressLabel()
{
...
void SnailcoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void SnailcoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &SnailcoinGUI::quitRequested);
...
void SnailcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void SnailcoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &SnailcoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void SnailcoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void SnailcoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &SnailcoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &SnailcoinGUI::show);
                e->ignore();
            }
        }
    }
...
void SnailcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SnailcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SnailcoinGUI::incomingTransaction(const QString& date, SnailcoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SnailcoinUnits::formatWithUnit(unit, amount, true));
...
void SnailcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SnailcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SnailcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SnailcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SnailcoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void SnailcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SnailcoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void SnailcoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void SnailcoinGUI::updateWindowTitle()
{
...
void SnailcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SnailcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SnailcoinGUI::detectShutdown()
{
...
void SnailcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void SnailcoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SnailcoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = SnailcoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void SnailcoinGUI::subscribeToCoreSignals()
{
...
void SnailcoinGUI::unsubscribeFromCoreSignals()
{
...
bool SnailcoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<SnailcoinUnit> units = SnailcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const SnailcoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, SnailcoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const SnailcoinUnit u : SnailcoinUnits::availableUnits()) {
        menu->addAction(SnailcoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(SnailcoinUnit newUnits)
{
    setText(SnailcoinUnits::longName(newUnits));
}

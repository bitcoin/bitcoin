...
qint64 Bitcoin_WalletModel::getBalance(Bitcoin_CCoinsViewCache* claim_view, map<uint256, set<int> >& mapFilterTxInPoints, const CCoinControl *coinControl) const
{
    wallet->MarkDirty();
...
qint64 Bitcoin_WalletModel::getUnconfirmedBalance(Bitcoin_CCoinsViewCache* claim_view, map<uint256, set<int> >& mapFilterTxInPoints) const
{
	wallet->MarkDirty();
    return wallet->GetUnconfirmedBalance(claim_view, mapFilterTxInPoints);
}

qint64 Bitcoin_WalletModel::getImmatureBalance(Bitcoin_CCoinsViewCache* claim_view, map<uint256, set<int> >& mapFilterTxInPoints) const
{
	wallet->MarkDirty();
...
void Bitcoin_WalletModel::getOutputs(const std::vector<COutPoint>& vOutpoints, std::vector<Bitcoin_COutput>& vOutputs, Bitcoin_CCoinsViewCache* claim_view)
{
    LOCK2(bitcoin_mainState.cs_main, wallet->cs_wallet);
    BOOST_FOREACH(const COutPoint& outpoint, vOutpoints)
    {
        ...
        if(claim_view == NULL) {
            ...
            if(claim_view->Claim_HaveCoins(hash)) {
    			const Claim_CCoins & claimCoins = claim_view->Claim_GetCoins(hash);
...
bool Bitcoin_WalletModel::isSpent(const COutPoint& outpoint, Bitcoin_CCoinsViewCache *claim_view) const
{
    LOCK2(bitcoin_mainState.cs_main, wallet->cs_wallet);
...
void Bitcoin_WalletModel::listCoins(std::map<QString, std::vector<Bitcoin_COutput> >& mapCoins, Bitcoin_CCoinsViewCache* claim_view, map<uint256, set<int> >& mapFilterTxInPoints) const
{
    wallet->MarkDirty();

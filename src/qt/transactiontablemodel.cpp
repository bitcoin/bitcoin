...
public:
    ...
    TransactionRecord *index(interfaces::Wallet& wallet, int idx)
    {
        if(idx >= 0 && idx < cachedWallet.size())
        {
            ...
            int64_t block_time;
            int64_t headers;
            if (wallet.tryGetTxStatus(rec->hash, wtx, numBlocks, block_time, headers))
				if (rec->statusUpdateNeeded(numBlocks,headers)) {
					rec->updateStatus(wtx, numBlocks, block_time, headers);
				}
            return rec;
        }
        return nullptr;

...
void CreateWalletActivity::createWallet()
{
    ...
    QTimer::singleShot(500, worker(), [this, name, flags] {
        std::unique_ptr<interfaces::Wallet> wallet = node().walletLoader().createWallet(*::coinJoinWalletManager, name, m_passphrase, flags, m_error_message, m_warning_message);
...
void OpenWalletActivity::open(const std::string& path)
{
    ...
    QTimer::singleShot(0, worker(), [this, path] {
        std::unique_ptr<interfaces::Wallet> wallet = node().walletLoader().loadWallet(*::coinJoinWalletManager, path, m_error_message, m_warning_message);

...
TalkcoinMobileGUI::TalkcoinMobileGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWindow *parent) :
    QQuickView(parent),
    m_node(node),
    ...
    m_networkStyle(networkStyle)
{
    ...
    Q_FOREACH(TalkcoinUnits::Unit unit, TalkcoinUnits::availableUnits()) {
        availableUnitNames.append(TalkcoinUnits::longName(unit));
    }
    ...
    if (m_walletPane) {
        connect(m_walletPane, SIGNAL(request()), this, SLOT(requestTalkcoin()));
    }

    m_sendPane = this->rootObject()->findChild<QObject*>("sendPane");

    if (m_sendPane) {
        connect(m_sendPane, SIGNAL(send(QString, int, int)), this, SLOT(sendTalkcoin(QString, int, int)));
    }
...
void TalkcoinMobileGUI::setClientModel(ClientModel *clientModel)
{
    this->m_clientModel = clientModel;
}

void TalkcoinMobileGUI::setWalletController(WalletController* wallet_controller)
{
...
void TalkcoinMobileGUI::addWallet(WalletModel* walletModel)
{
    const QString display_name = walletModel->getDisplayName();
    setWalletModel(walletModel);
}

static void InitMessage(TalkcoinMobileGUI *gui, const std::string &message)
{
...
static void ShowProgress(TalkcoinMobileGUI *gui, const std::string &title, int nProgress)
{
    InitMessage(gui, title + strprintf("%d", nProgress) + "%");
}

static bool ThreadSafeMessageBox(TalkcoinMobileGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void TalkcoinMobileGUI::subscribeToCoreSignals()
{
...
void TalkcoinMobileGUI::unsubscribeFromCoreSignals()
{
...
void TalkcoinMobileGUI::showInitMessage(const QString &message, int alignment, const QColor &color)
{
    Q_UNUSED(alignment)
    Q_UNUSED(color)
...
void TalkcoinMobileGUI::message(const QString& title, QString message, unsigned int style, bool* ret)
{
    Q_UNUSED(title)
    Q_UNUSED(message)
    Q_UNUSED(style)
...
void TalkcoinMobileGUI::splashFinished()
{
    QMetaObject::invokeMethod(this->rootObject(), "hideSplash");
}

void TalkcoinMobileGUI::requestTalkcoin()
{
...
void TalkcoinMobileGUI::sendTalkcoin(QString address, int amount, int confirmations)
{
...
void TalkcoinMobileGUI::setWalletModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        setBalance(balances);
        connect(model, &WalletModel::balanceChanged, this, &TalkcoinMobileGUI::setBalance);

        connect(model->getOptionsModel(), &OptionsModel::displayUnitChanged, this, &TalkcoinMobileGUI::updateDisplayUnit);
    }

    updateDisplayUnit();
}

void TalkcoinMobileGUI::setBalance(const interfaces::WalletBalances& balances)
{
    int unit = m_walletModel->getOptionsModel()->getDisplayUnit();
    m_balances = balances;

    QMetaObject::invokeMethod(m_walletPane, "updateBalance",
                              Q_ARG(QVariant, TalkcoinUnits::formatWithUnit(unit, balances.balance, false, TalkcoinUnits::separatorAlways)));
}

void TalkcoinMobileGUI::setClipboard(QString text)
{
    GUIUtil::setClipboard(text);
}

void TalkcoinMobileGUI::setDisplayUnit(int index)
{
...
void TalkcoinMobileGUI::executeRpcCommand(QString command)
{
    QString response;
...
void TalkcoinMobileGUI::updateDisplayUnit()
{
    if(m_walletModel && m_walletModel->getOptionsModel())
    {

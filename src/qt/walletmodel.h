...
public:
    ...
    SendCoinsReturn prepareDepositTransaction(Bitcredit_WalletModel *deposit_model, Bitcredit_WalletModelTransaction &transaction, const Credits_COutput& coin, Credits_CCoinsViewCache &credits_view, Bitcoin_CCoinsViewCache &bitcoin_view);

    // prepare claim transaction for getting txfee before sending coins
    SendCoinsReturn prepareClaimTransaction(Bitcoin_WalletModel *bitcoin_model, Bitcoin_CCoinsViewCache *claim_view, Bitcredit_WalletModelTransaction &transaction, const CCoinControl *coinControl = NULL);

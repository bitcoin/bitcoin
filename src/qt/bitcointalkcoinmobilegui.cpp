#include "bitcointalkcoinmobilegui.h"
...
BitcointalkcoinMobileGUI::BitcointalkcoinMobileGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWindow *parent) :
    QQuickView(parent),
    m_node(node),
    ...
    m_networkStyle(networkStyle)
{
    ...
    Q_FOREACH(BitcointalkcoinUnits::Unit unit, BitcointalkcoinUnits::availableUnits()) {
        availableUnitNames.append(BitcointalkcoinUnits::longName(unit));
    }
    ...
    if (m_walletPane) {
        connect(m_walletPane, SIGNAL(request()), this, SLOT(requestBitcointalkcoin()));
    }

    m_sendPane = this->rootObject()->findChild<QObject*>("sendPane");

    if (m_sendPane) {
        connect(m_sendPane, SIGNAL(send(QString, int, int)), this, SLOT(sendBitcointalkcoin(QString, int, int)));
    }
...
void BitcointalkcoinMobileGUI::setClientModel(ClientModel *clientModel)
{
    this->m_clientModel = clientModel;
}

void BitcointalkcoinMobileGUI::setWalletController(WalletController* wallet_controller)
{
...
void BitcointalkcoinMobileGUI::addWallet(WalletModel* walletModel)
{
    const QString display_name = walletModel->getDisplayName();
    setWalletModel(walletModel);
}

static void InitMessage(BitcointalkcoinMobileGUI *gui, const std::string &message)
{
...
static void ShowProgress(BitcointalkcoinMobileGUI *gui, const std::string &title, int nProgress)
{
    InitMessage(gui, title + strprintf("%d", nProgress) + "%");
}

static bool ThreadSafeMessageBox(BitcointalkcoinMobileGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void BitcointalkcoinMobileGUI::subscribeToCoreSignals()
{
...
void BitcointalkcoinMobileGUI::unsubscribeFromCoreSignals()
{
...
void BitcointalkcoinMobileGUI::showInitMessage(const QString &message, int alignment, const QColor &color)
{
    Q_UNUSED(alignment)
    Q_UNUSED(color)
...
void BitcointalkcoinMobileGUI::message(const QString& title, QString message, unsigned int style, bool* ret)
{
    Q_UNUSED(title)
    Q_UNUSED(message)
    Q_UNUSED(style)
...
void BitcointalkcoinMobileGUI::splashFinished()
{
    QMetaObject::invokeMethod(this->rootObject(), "hideSplash");
}

void BitcointalkcoinMobileGUI::requestBitcointalkcoin()
{
...
void BitcointalkcoinMobileGUI::sendBitcointalkcoin(QString address, int amount, int confirmations)
{
...
void BitcointalkcoinMobileGUI::setWalletModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        setBalance(balances);
        connect(model, &WalletModel::balanceChanged, this, &BitcointalkcoinMobileGUI::setBalance);

        connect(model->getOptionsModel(), &OptionsModel::displayUnitChanged, this, &BitcointalkcoinMobileGUI::updateDisplayUnit);
    }

    updateDisplayUnit();
}

void BitcointalkcoinMobileGUI::setBalance(const interfaces::WalletBalances& balances)
{
    int unit = m_walletModel->getOptionsModel()->getDisplayUnit();
    m_balances = balances;

    QMetaObject::invokeMethod(m_walletPane, "updateBalance",
                              Q_ARG(QVariant, BitcointalkcoinUnits::formatWithUnit(unit, balances.balance, false, BitcointalkcoinUnits::separatorAlways)));
}

void BitcointalkcoinMobileGUI::setClipboard(QString text)
{
    GUIUtil::setClipboard(text);
}

void BitcointalkcoinMobileGUI::setDisplayUnit(int index)
{
...
void BitcointalkcoinMobileGUI::executeRpcCommand(QString command)
{
    QString response;
...
void BitcointalkcoinMobileGUI::updateDisplayUnit()
{
    if(m_walletModel && m_walletModel->getOptionsModel())
    {

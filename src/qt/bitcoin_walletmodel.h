...
#ifndef BITCOIN_WALLETMODEL_H
#define BITCOIN_WALLETMODEL_H
...
{
public:
    explicit Bitcoin_SendCoinsRecipient() : amount(0), nVersion(Bitcoin_SendCoinsRecipient::CURRENT_VERSION) { }
    explicit Bitcoin_SendCoinsRecipient(const QString &addr, const QString &label, quint64 amount, const QString &message):
        address(addr), label(label), amount(amount), message(message), nVersion(Bitcoin_SendCoinsRecipient::CURRENT_VERSION) {}
    ...
    IMPLEMENT_SERIALIZE
    (
        Bitcoin_SendCoinsRecipient* pthis = const_cast<Bitcoin_SendCoinsRecipient*>(this);
...
class Bitcoin_WalletModel : public QObject
{
    Q_OBJECT
...
//    SendCoinsReturn sendCoins(Bitcoin_WalletModelTransaction &transaction);
    ...
    {
    public:
        UnlockContext(Bitcoin_WalletModel *wallet, bool valid, bool relock);
        ~UnlockContext();
...
signals:
    ...
    void coinsSent(Bitcoin_CWallet* wallet, Bitcoin_SendCoinsRecipient recipient, QByteArray transaction);

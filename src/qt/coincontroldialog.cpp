    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/flow/flow/issues/5716
...
{
    GUIUtil::setClipboard(FlowUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = FlowUnits::FLW;
    ...
    l2->setText(FlowUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(FlowUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(FlowUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(FlowUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            {
                sAddress = QString::fromStdString(CFlowAddress(outputAddress).ToString());

                // if listMode or change => show flow address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, FlowUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, FlowUnits::format(nDisplayUnit, nSum));

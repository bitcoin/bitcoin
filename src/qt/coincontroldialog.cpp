...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    CAmount nAmount             = 0*sats;
    CAmount nPayFee             = 0*sats;
    CAmount nAfterFee           = 0*sats;
    CAmount nChange             = 0*sats;
    ...
    if (nQuantity > 0)
    {
        ...
        if (nPayAmount > 0*sats)
        {
            ...
            if (nChange == 0*sats && !CoinControlDialog::fSubtractFeeFromAmount)
                nBytes -= 34;
        }

        // after fee
        nAfterFee = std::max(nAmount - nPayFee, 0*sats);
    }
    ...
    dialog->findChild<QLabel *>("labelCoinControlChangeText")   ->setEnabled(nPayAmount > 0*sats);
    dialog->findChild<QLabel *>("labelCoinControlChange")       ->setEnabled(nPayAmount > 0*sats);
    ...
    if (label)
        label->setVisible(nChange < 0*sats);
}

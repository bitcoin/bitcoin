    ...
    try {
    {
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            if (fFlushForPrune) {
                ...
                UnlinkPrunedFiles(m_blockman.m_blocks_dir, setFilesToPrune);
            }
            m_last_write = nNow;
        }
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (!ReadBlockFromDisk(chainstate.m_blockman.m_blocks_dir, block, pindex, consensus_params)) {
        ...
        if (nCheckLevel >= 2 && pindex) {
            ...
            if (!pindex->GetUndoPos().IsNull()) {
                if (!UndoReadFromDisk(chainstate.m_blockman.m_blocks_dir, undo, pindex)) {

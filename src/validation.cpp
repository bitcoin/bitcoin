...
BlockValidationState FatalError(Notifications& notifications, const bilingual_str& message)
{
    ...
    return BlockValidationState::MakeError(message.original);
}
...
[[nodiscard]] static BlockValidationState CheckBlockHeader(const CBlockHeader& block, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetHash(), block.nBits, consensusParams)) {
        return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_INVALID_HEADER, "high-hash", "proof of work failed");
    }
    return BlockValidationState{};
}
...
[[nodiscard]] static BlockValidationState CheckMerkleRoot(const CBlock& block)
{
    ...
    if (block.hashMerkleRoot != merkle_root) {
        return BlockValidationState::MakeInvalid(
...
BlockValidationState CheckBlock(const CBlock& block, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (consensusParams.signet_blocks && fCheckPOW && !CheckSignetBlockSolution(block, consensusParams)) {
        return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-signet-blksig", "signet block signature validation failure");
    }
    ...
    if (block.vtx.empty() || block.vtx.size() * WITNESS_SCALE_FACTOR > MAX_BLOCK_WEIGHT || ::GetSerializeSize(TX_NO_WITNESS(block)) * WITNESS_SCALE_FACTOR > MAX_BLOCK_WEIGHT) {
        return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-length", "size limits failed");
    }

    // First transaction must be coinbase, the rest must not be
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
        return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-missing", "first tx is not coinbase");
    }
    for (unsigned int i = 1; i < block.vtx.size(); i++) {
        if (block.vtx[i]->IsCoinBase()) {
            return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-multiple", "more than one coinbase");
        }
    }
    ...
    for (const auto& tx : block.vtx) {
        ...
        if (!CheckTransaction(*tx, tx_state)) {
            ...
            return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_CONSENSUS, tx_state.GetRejectReason(),
    ...
    if (nSigOps * WITNESS_SCALE_FACTOR > MAX_BLOCK_SIGOPS_COST) {
        return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-sigops", "out-of-bounds SigOpCount");
    }
...
[[nodiscard]] static BlockValidationState ContextualCheckBlockHeader(const CBlockHeader& block, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams)) {
        return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-diffbits", "incorrect proof of work");
    }

    // Check timestamp against prev
    if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast()) {
        return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_INVALID_HEADER, "time-too-old", "block's timestamp is too early");
    }
    ...
    if (consensusParams.enforce_BIP94) {
        ...
        if (nHeight % consensusParams.DifficultyAdjustmentInterval() == 0) {
            if (block.GetBlockTime() < pindexPrev->GetBlockTime() - MAX_TIMEWARP) {
                return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_INVALID_HEADER, "time-timewarp-attack", "block's timestamp is too early on diff adjustment block");
            }
        }
    }
    ...
    if (block.Time() > NodeClock::now() + std::chrono::seconds{MAX_FUTURE_BLOCK_TIME}) {
        return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_TIME_FUTURE, "time-too-new", "block timestamp too far in the future");
    }
    ...
    if ((block.nVersion < 2 && DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_HEIGHTINCB)) ||
        ...
        (block.nVersion < 4 && DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_CLTV))) {
            return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_INVALID_HEADER, strprintf("bad-version(0x%08x)", block.nVersion),
...
[[nodiscard]] static BlockValidationState ContextualCheckBlock(const CBlock& block, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    for (const auto& tx : block.vtx) {
        if (!IsFinalTx(*tx, nHeight, nLockTimeCutoff)) {
            return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-txns-nonfinal", "non-final transaction");
        }
    }
    ...
    if (DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_HEIGHTINCB))
    {
        ...
        if (block.vtx[0]->vin[0].scriptSig.size() < expect.size() ||
            ...
            return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-height", "block height mismatch in coinbase");
        }
    }
    ...
    if (const auto state = CheckWitnessMalleation(block, DeploymentActiveAfter(pindexPrev, chainman, Consensus::DEPLOYMENT_SEGWIT)); !state.IsValid()) {
        return state;
    }
    ...
    if (GetBlockWeight(block) > MAX_BLOCK_WEIGHT) {
        return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-weight", strprintf("%s : weight limit failed", __func__));
    }
...
BlockValidationState ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, CBlockIndex** ppindex, bool min_pow_checked)
{
    ...
    if (hash != GetConsensus().hashGenesisBlock) {
        if (miSelf != m_blockman.m_block_index.end()) {
            ...
            if (pindex->nStatus & BLOCK_FAILED_VALID) {
                ...
                return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_CACHED_INVALID, "duplicate-invalid",
        ...
        if (mi == m_blockman.m_block_index.end()) {
            ...
            return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_MISSING_PREV, "prev-blk-not-found");
        }
        pindexPrev = &((*mi).second);
        if (pindexPrev->nStatus & BLOCK_FAILED_VALID) {
            LogDebug(BCLog::VALIDATION, "header %s has prev block invalid: %s\n", hash.ToString(), block.hashPrevBlock.ToString());
            return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_INVALID_PREV, "bad-prevblk");
        }
    ...
    if (!min_pow_checked) {
        ...
        return BlockValidationState::MakeInvalid(BlockValidationResult::BLOCK_HEADER_LOW_WORK, "too-little-chainwork");
    }
...
BlockValidationState ChainstateManager::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    try {
        ...
        if (dbp) {
            ...
            if (blockPos.IsNull()) {
                return BlockValidationState::MakeError(strprintf("%s: Failed to find position to write new block to disk", __func__));
            }
        }
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        BlockValidationState state{CheckBlock(*block, GetConsensus())};
        if (state.IsValid()) {
            // Store to disk
    ...
    const bool check_merkle_root)
{
    ...
    if (block.hashPrevBlock != *Assert(tip->phashBlock)) {
        // Note: Here an Invalid BlockValidationState is used with BLOCK_RESULT_UNSET Result,
        // that's why MakeInvalid() is skipped.
        BlockValidationState state;
    ...
    if (const auto state = CheckBlock(block, chainstate.m_chainman.GetConsensus(), /*fCheckPow=*/check_pow, /*fCheckMerkleRoot=*/check_merkle_root); !state.IsValid()) {

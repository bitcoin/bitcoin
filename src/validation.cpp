...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, std::vector<CScriptCheck> *pvChecks)
{
	if (!tx.IsCoinBase())
	{
		...
		if (fScriptChecks) {
			...
			CSHA256().Write(scriptExecutionCacheNonce.begin(), 55 - sizeof(flags) - 32).Finalize(hashCacheEntry.begin());
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
	...
	int64_t nTime3 = GetTimeMicros(); nTimeConnect += nTime3 - nTime1;
	LogPrint("bench", "      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs]\n", (unsigned)block.vtx.size(), 0.001 * (nTime3 - nTime1), 0.001 * (nTime3 - nTime1) / block.vtx.size(), nInputs <= 1 ? 0 : 0.001 * (nTime3 - nTime1) / (nInputs - 1), nTimeConnect * 0.000001);
	...
	int64_t nTime4 = GetTimeMicros(); nTimeVerify += nTime4 - nTime1;
	LogPrint("bench", "    - Verify %u txins: %.2fms (%.3fms/txin) [%.2fs]\n", nInputs - 1, 0.001 * (nTime4 - nTime1), nInputs <= 1 ? 0 : 0.001 * (nTime4 - nTime1) / (nInputs - 1), nTimeVerify * 0.000001);

...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // already calculated.
    if (const auto err{SingleV3Checks(m_pool, ws.m_ptx, ws.m_ancestors, ws.m_conflicts, ws.m_vsize)}) {
...
bool MemPoolAccept::ReplacementChecks(Workspace& ws)
{
    ...
    if (const auto err_string{ImprovesFeerateDiagram(m_pool, ws.m_iters_conflicting, m_subpackage.m_all_conflicts, {{&(*ws.m_entry), ws.m_modified_fees}})}) {
                                         ...
                                         PackageValidationState& package_state)
{
    ...
    if (const auto err_tup{ImprovesFeerateDiagram(m_pool, direct_conflict_iters, m_subpackage.m_all_conflicts,
                {{&(*parent_ws.m_entry), parent_ws.m_modified_fees},
                 {&(*child_ws.m_entry), child_ws.m_modified_fees}})}) {
        return package_state.Invalid(PackageValidationResult::PCKG_POLICY,

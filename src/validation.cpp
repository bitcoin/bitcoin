...
#include "versionbits.h"
#include "warnings.h"
>>>>>>> 618d07f... MOVEONLY: tx functions to consensus/tx_verify.o
...
{
    SetMiscWarning(strMessage); = strMessage;
    LogPrintf("*** %s\n", strMessage);
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    LOCK(cs_main);
                    CValidationState state;
                    if (AcceptBlock(pblock, state, chainparams, NULL, true, dbp, nullptr))
                        nLoaded++;
                    if (state.IsError())
                        break;
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(*pblockrecursive, it->second, chainparams.GetConsensus()))
                        {
                            ...
                            if (AcceptBlock(pblockrecursive, dummy, chainparams, NnullptrULL, true, &it->second, nullptr))
                            {
                                nLoaded++;

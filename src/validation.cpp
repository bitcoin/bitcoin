...
bool MemPoolAccept::Finalize(const ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : ws.m_all_conflicting)
    {
        LogDebug(BCLog::MEMPOOL, "replacing tx %s (wtxid=%s) with %s (wtxid=%s) for %s additional fees, %d delta bytes\n",
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
        ...
        for (const auto& tx : block.vtx) {
            for (size_t o = 0; o < tx->vout.size(); o++) {
                if (view.HaveCoin(COutPoint(tx->GetHash(), o))) {
                    LogInfo("ERROR: ConnectBlock(): tried to overwrite transaction\n");
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            nFees += txfee;
            if (!MoneyRange(nFees)) {
                LogInfo("ERROR: %s: accumulated fee in the block out of range.\n", __func__);
            ...
            if (!SequenceLocks(tx, nLockTimeFlags, prevheights, *pindex)) {
                LogInfo("ERROR: %s: contains a non-BIP68-final transaction\n", __func__);
    ...
    if (!control.Wait()) {
        LogInfo("ERROR: %s: CheckQueue failed\n", __func__);
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (!chainstate.m_blockman.ReadBlockFromDisk(block, *pindex)) {
            LogInfo("Verification error: ReadBlockFromDisk failed at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
        ...
        bool in_memory)
{
    ...
    if (this->SnapshotBlockhash()) {
        LogInfo("[snapshot] can't activate a snapshot-based chainstate more than once\n");
        return false;
    }

    {
        ...
        if (Assert(m_active_chainstate->GetMempool())->size() > 0) {
            LogInfo("[snapshot] can't activate a snapshot when mempool not empty\n");
            return false;
        }
    }
    ...
    auto cleanup_bad_snapshot = [&](const char* reason) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        LogInfo("[snapshot] activation failed - %s\n", reason);
    ...
    const SnapshotMetadata& metadata)
{
    ...
    if (!snapshot_start_block) {
        ...
        LogInfo("[snapshot] Did not find snapshot start blockheader %s\n",
                  base_blockhash.ToString());
        return false;
    }
    ...
    if (!maybe_au_data) {
        LogInfo("[snapshot] assumeutxo height in snapshot metadata not recognized "
    ...
    if (WITH_LOCK(::cs_main, return !CBlockIndexWorkComparator()(ActiveTip(), snapshot_start_block))) {
        LogInfo("[snapshot] activation failed - work does not exceed active chainstate\n");
        return false;
    }
    ...
    LogInfo("[snapshot] loading coins from snapshot %s\n", base_blockhash.ToString());
    ...
    while (coins_left > 0) {
        try {
            ...
            LogInfo("[snapshot] bad snapshot format or truncated snapshot after deserializing %d coins\n",
        ...
        if (coin.nHeight > base_height ||
            ...
            LogInfo("[snapshot] bad snapshot data after deserializing %d coins\n",
                      coins_count - coins_left);
            return false;
        }
        if (!MoneyRange(coin.out.nValue)) {
            LogInfo("[snapshot] bad snapshot data after deserializing %d coins - bad tx out value\n",
                      coins_count - coins_left);
            return false;
        }
    ...
    if (!out_of_coins) {
        LogInfo("[snapshot] bad snapshot - coins left over after deserializing %d coins\n",
            coins_count);
        return false;
    }
    ...
    if (!maybe_stats.has_value()) {
        LogInfo("[snapshot] failed to generate coins stats\n");
        return false;
    }

    // Assert that the deserialized chainstate contents match the expected assumeutxo value.
    if (AssumeutxoHash{maybe_stats->hashSerialized} != au_data.hash_serialized) {
        LogInfo("[snapshot] bad snapshot content hash: expected %s, got %s\n",
...
SnapshotCompletionResult ChainstateManager::MaybeCompleteSnapshotValidation()
{
    ...
    auto handle_invalid_snapshot = [&]() EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        ...
        LogInfo("[snapshot] !!! %s\n", user_error.original);
        LogInfo("[snapshot] deleting snapshot, reverting to validated chain, and stopping node\n");
...
bool ChainstateManager::ValidatedSnapshotCleanup()
{
    ...
    auto rename_failed_abort = [this](
        ...
        LogInfo("Error renaming path (%s) -> (%s): %s\n",

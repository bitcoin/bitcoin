...
#if defined(NDEBUG)
# error "Qitcoin cannot be compiled without assertions."
#endif
...
bool fCheckWork = DEFAULT_CHECKWORK_ENABLED;
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (halvings >= 64) {
        ...
        nSubsidy = 75 * COIN;
        // Subsidy is cut in half every 700,000 blocks / 3minutes which will occur approximately every 4 years.
        nSubsidy >>= halvings;
    }
...
std::vector<CTxOut> GetBlockReward(const CBlockIndex* pindexPrev, const CAmount& nFees, const CAccountID& generatorID, const uint64_t& nPlotterId, const CCoinsViewCache& view, const Consensus::Params& consensusParams)
{
    ...
    {
        // all reward to fund
        for (const std::string &address : consensusParams.vFundAddressPool) {
...
static void CheckForkWarningConditions() EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // If our best fork is no longer within 24 blocks (+/- 3 hours if no one mines it)
    // of our head, drop it
    if (pindexBestForkTip && ::ChainActive().Height() - pindexBestForkTip->nHeight >= 24)
        pindexBestForkTip = nullptr;

    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWork > ::ChainActive().Tip()->nChainWork + (GetBlockProof(*::ChainActive().Tip(), Params().GetConsensus()) * 6)))
    {
...
static void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // with a tip within 24 blocks (+/- 3 hours if no one mines it) of ours
    ...
    if (pfork && (!pindexBestForkTip || pindexNewForkTip->nHeight > pindexBestForkTip->nHeight) &&
            pindexNewForkTip->nChainWork - pfork->nChainWork > (GetBlockProof(*pfork, Params().GetConsensus()) * 7) &&
                  ...
                  std::shared_ptr<std::map<COutPoint, Coin>> removedCoins)
{
    ...
    // Check generator
    const CAccountID generatorID = ExtractAccountID(pindex->minerRewardTxOut.scriptPubKey);
    if (generatorID.IsNull()) {
    ...
    const auto &vReward = GetBlockReward(pindex->pprev, nFees, generatorID, pindex->nPlotterId, view, chainparams.GetConsensus());
    const std::size_t cbOutSize = (!block.vtx[0]->vout.empty() && block.vtx[0]->vout.rbegin()->nValue == 0) ? (block.vtx[0]->vout.size() - 1) : block.vtx[0]->vout.size();
    if (cbOutSize != vReward.size()) {
        return state.Invalid(ValidationInvalidReason::CONSENSUS,
                        error("ConnectBlock(): coinbase output count missing match (actual=%d, require=%d)", cbOutSize, vReward.size()),
    ...
    for (std::size_t i = 0; i < vReward.size(); i++) {
        ...
        if (vReward[i].nValue != block.vtx[0]->vout[i].nValue)
            return state.Invalid(ValidationInvalidReason::CONSENSUS,
                        error("ConnectBlock(): coinbase output missing match (index=%d, actual=%d, require=%d)",
                            i, block.vtx[0]->vout[i].nValue, vReward[i].nValue),
        ...
        if (scriptPubKey != block.vtx[0]->vout[i].scriptPubKey)
            return state.Invalid(ValidationInvalidReason::CONSENSUS,
                        error("ConnectBlock(): coinbase script missing match (index=%d, actual=%s, require=%s)",
                            i, HexStr(scriptPubKey), HexStr(vReward[i].scriptPubKey)),
                        REJECT_INVALID, "bad-cb-script");

        // payload
        if (vReward[i].payload != block.vtx[0]->vout[i].payload)
            return state.Invalid(ValidationInvalidReason::CONSENSUS,
                        error("ConnectBlock(): coinbase payload missing match (index=%d, actual=%s, require=%s)",
                            i, HexStr(block.vtx[0]->vout[i].payload), HexStr(vReward[i].payload)),
                        REJECT_INVALID, "bad-cb-payload");
    }
...
CBlockIndex* BlockManager::AddToBlockIndex(const CBlockHeader& block)
{
    ...
    pindexNew->nChainWork = (pindexNew->pprev ? pindexNew->pprev->nChainWork : 0) + GetBlockProof(*pindexNew, Params().GetConsensus());
...
void CChainState::ReceivedBlockTransactions(const CBlock& block, CBlockIndex* pindexNew, const FlatFilePos& pos, const Consensus::Params& consensusParams)
{
    ...
    pindexNew->minerRewardTxOut = block.vtx[0]->vout[0];
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, bool fCheckWork = true)
{
    AssertLockHeld(cs_main);
    ...
    if (block.hashPrevBlock.IsNull()) {
        if (hashBlock != chainparams.GetConsensus().hashGenesisBlock)
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_PREV, false, REJECT_INVALID, "block-hash", "invalid genesis block");
    ...
    if (pindexPrev->nHeight == 1 &&
            chainparams.GetConsensus().nBeginMiningTime != 0 && chainparams.GetConsensus().nBeginMiningTime != block.GetBlockTime())
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-blk-time", "incorrect mining time");
    ...
    if (pindexPrev->nHeight >= 1) {
        if (block.vchPubKey.size() != CPubKey::COMPRESSED_PUBLIC_KEY_SIZE || block.vchSignature.size() > CPubKey::SIGNATURE_SIZE)
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-blk-sign", "require block signature");

        CPubKey pubkey(block.vchPubKey);
        if (!pubkey.Verify(block.GetUnsignaturedHash(), block.vchSignature))
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-blk-sign", "incorrect block signature");
    } else {
        if (!block.vchPubKey.empty() || !block.vchSignature.empty())
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-blk-sign", "not allow block signature");
    }
    ...
    if (!chainparams.Checkpoints().mapCheckpoints.empty()) {
        ...
        if (itCheckpoint != mapCheckpoints.end() && itCheckpoint->second != hashBlock)
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-blk", "diff checkpoint");

        // Skip when reindexing
        int nLastCheckpointHeight = mapCheckpoints.rbegin()->first;
        if (!fCheckWork && fReindex && pindexPrev->nHeight + 1 <= nLastCheckpointHeight)
            return true;
    }

    // Skip check work for already valid block
    const CBlockIndex* pindex = LookupBlockIndex(hashBlock);
    if (pindex != nullptr && pindex->IsValid(BLOCK_VALID_TREE))
        return true;
    ...
    if (!poc::CheckProofOfCapacity(*pindexPrev, block, chainparams.GetConsensus()))
        return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_HEADER, false, REJECT_INVALID, "bad-work", "check work failed");
...
bool CheckBlock(const CBlock& block, CValidationState& state, const CChainParams& chainparams, bool fCheckWork, bool fCheckMerkleRoot)
{
    ...
    if (!block.vchPubKey.empty() && GetScriptForPubKey(CPubKey(block.vchPubKey)) != block.vtx[0]->vout[0].scriptPubKey) {
        return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-blk-sign", "incorrect signatory");
    }
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    int nLockTimeFlags = 0;
    if (nHeight != 0 && nHeight >= consensusParams.CSVHeight) {
        assert(pindexPrev != nullptr);
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fCheckWork)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
    uint256 hash = block.GetHash();
...
bool ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, CValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex, CBlockHeader *first_invalid)
{
    ...
    {
        LOCK(cs_main);
        const CBlockIndex* pindexPrev = LookupBlockIndex(headers[0].hashPrevBlock);
        if (pindexPrev != nullptr) {
            ...
            if (pindexLast->nHeight - pindexPrev->nHeight != (int) headers.size() && !::ChainActive().Contains(pindexLast)) {
                ...
                if (::ChainActive().Height() > pindexFork->nHeight) {
                    if (::ChainActive().Height() >= pindexFork->nHeight + 6) {
                        ...
                        if (nChainWork <= ::ChainActive().Tip()->nChainWork)
                            return state.Invalid(ValidationInvalidReason::BLOCK_SHORT_FORK, error("%s: Recv same or small chain work fork blocks", __func__), 0, "dos-suspicion");
                    }
    ...
    if (!fCheckWork && headers.size() >= MAX_BLOCKS_TO_ANNOUNCE && !chainparams.Checkpoints().mapCheckpoints.empty()) {
        ...
        LogPrint(BCLog::POC, "%s: %s-%s, Verify work [%d,%d)\n", __func__, headers.begin()->GetHash().ToString(), headers.rbegin()->GetHash().ToString(), nLastKnownBlockIndex + 1, (int) headers.size());
    }

    // Connect block
    {
        // Don't hold cs_main too long time
        std::size_t beginCheckWorkIndex = (std::size_t) (nLastKnownBlockIndex + 1);
        for (std::size_t index = 0; index < headers.size();) {
            if (index > 0) { // Let's other thread hold cs_main
                NotifyHeaderTip();
                if (ShutdownRequested())
                    break;
                MilliSleep(10);
            ...
            { // Hold cs_main
                LOCK(cs_main);
                for (int processed = 0; index < headers.size(); index++, processed++) {
                    const CBlockHeader& header = headers[index];
                    CBlockIndex *pindex = nullptr;
                    bool accepted = g_blockman.AcceptBlockHeader(header, state, chainparams, &pindex, (index >= beginCheckWorkIndex));
                    ...
                    if (index >= beginCheckWorkIndex && processed >= 20)
                        break;
                }
            }
        }
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckWork, bool fCheckMerkleRoot)
{
    ...
    indexDummy.minerRewardTxOut = block.vtx[0]->vout[0];
...
bool CChainState::RewindBlockIndex(const CChainParams& params)
{
    ...
    int nHeight = 1;
    {
        ...
        while (nHeight <= m_chain.Height()) {
            ...
            if (nHeight == 2 && params.GetConsensus().nBeginMiningTime != 0 && params.GetConsensus().nBeginMiningTime != m_chain[nHeight]->GetBlockTime()) {
                break;
            }
            nHeight++;
        }

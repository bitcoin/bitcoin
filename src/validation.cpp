...
static bool CheckInputInvariants(const CBlock& block, CValidationState& state)
{
     ...
     * the prevout's hash xor'd with the index. Although each hash is not
     * dependent on the entire hash, a single 64-bit collision would be expected
     * with 4-billion UTXOS, and even then that would not trigger a collision in
     * this algorithm directly as there are 3 other 64-bit hashes to collide.
     *
     *
    ...
    auto hasher = [k1, k2, k3, k4, k5, k6, k7, k8, pcg](const COutPoint& out) {
        return pos{
            pcg(out.hash.GetUint64(0) ^ k1 ^ out.n, 1 | k1),
            pcg(out.hash.GetUint64(0) ^ k2 ^ out.n, 1 | k2),
            pcg(out.hash.GetUint64(1) ^ k3 ^ out.n, 1 | k3),
            false,
            pcg(out.hash.GetUint64(1) ^ k4 ^ out.n, 1 | k4),
            pcg(out.hash.GetUint64(2) ^ k5 ^ out.n, 1 | k5),
            pcg(out.hash.GetUint64(2) ^ k6 ^ out.n, 1 | k6),
            false,
            pcg(out.hash.GetUint64(3) ^ k7 ^ out.n, 1 | k7),
            pcg(out.hash.GetUint64(3) ^ k8 ^ out.n, 1 | k8),
            0,
        };
    };

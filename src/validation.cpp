...
util::Expected<bool, kernel::FatalError> Chainstate::InvalidateBlock(CBlockIndex* const pindex)
{
    ...
    while (true) {
        ...
        MaybeUpdateMempoolForReorg(disconnectpool, /* fAddToMempool = */ (++disconnected <= 10) && ret && *ret);
...
util::Expected<bool, kernel::FatalError> ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
    ...
    if (auto res{ActiveChainstate().ActivateBestChain(block)}; !res || !*res) {
        if (!res) {
            LogError("%s: ActivateBestChain failed (%s)\n", __func__, res.error().message());
            return util::Unexpected(std::move(res).error());
        }
        LogError("%s: ActivateBestChain failed\n", __func__);
        return false;
    }

    Chainstate* bg_chain{WITH_LOCK(cs_main, return HistoricalChainstate())};
    if (bg_chain) {
        if (auto res{bg_chain->ActivateBestChain(block)}; !res || !*res) {
            if (!res) {
                LogError("%s: [background] ActivateBestChain failed (%s)\n", __func__, res.error().message());
                return util::Unexpected(std::move(res).error());
            }
            LogError("%s: [background] ActivateBestChain failed\n", __func__);
            return false;
        }
    }
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (hash == params.GetConsensus().hashGenesisBlock && WITH_LOCK(::cs_main, return ActiveHeight()) == -1) {
                    if (auto res{ActiveChainstate().ActivateBestChain(nullptr)}; !res || !*res) {
                        break;
                    }
                }
...
util::Result<void> ChainstateManager::ActivateBestChains()
{
    ...
    for (Chainstate* chainstate : chainstates) {
        if (auto res{chainstate->ActivateBestChain(nullptr)}; !res || !*res) {
            LOCK(GetMutex());

...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!bypass_limits && !CheckFeeRate(ws.m_vsize, ws.m_modified_fees, state, isZDAGNoRBF)) return false;
    ...
    if (!hashAssumeValid.IsNull()) {
        ...
        if (it != m_blockman.m_block_index.end()) {
            if (it->second->GetAncestor(pindex->nHeight) == pindex &&
                ...
                if(fRegTest)
                    timeForWork /= 10;
                fScriptChecks = (GetBlockProofEquivalentTime(*pindexBestHeader, *pindex, *pindexBestHeader, m_params.GetConsensus()) <= timeForWork);
            }
        }
    }
 ...
 */
// SYSCOIN
static bool ContextualCheckBlockHeader(const bool ibd, const CBlockHeader& block, BlockValidationState& state, node::BlockManager& blockman, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...
// SYSCOIN
bool ChainstateManager::AcceptBlockHeader(const bool ibd, const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!ContextualCheckBlockHeader(ibd, block, state, m_blockman, chainparams, pindexPrev, GetAdjustedTime())) {
        ...
        if (llmq::chainLocksHandler->HasConflictingChainLock(pindexPrev->nHeight + 1, hash)) {
            m_blockman.AddToBlockIndex(block, BLOCK_CONFLICT_CHAINLOCK);
            return state.Invalid(BlockValidationResult::BLOCK_MISSING_PREV, "bad-chainlock");
        }
    }
...
bool ChainstateManager::ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, BlockValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex)
{
    ...
    const bool ibd = ActiveChainstate().IsInitialBlockDownload();
    {
        ...
        for (const CBlockHeader& header : headers) {
            ...
            // SYSCOIN
            bool accepted{AcceptBlockHeader(ibd, header, state, chainparams, &pindex)};
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    bool accepted_header{m_chainman.AcceptBlockHeader(ibd || !fNewBlock, block, state, m_params, &pindex)};
    CheckBlockIndex();

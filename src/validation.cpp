...
private:
    ...
    struct Workspace {
        ...
        CFeeRate m_package_feerate{0*sats};
    ...
    bool CheckFeeRate(size_t package_size, CAmount package_fee, TxValidationState& state) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, m_pool.cs)
    {
        ...
        if (mempoolRejectFee > 0*sats && package_fee < mempoolRejectFee) {
...
PackageMempoolAcceptResult MemPoolAccept::AcceptMultipleTransactionsInternal(const std::vector<CTransactionRef>& txns, ATMPArgs& args)
{
    ...
    m_subpackage.m_total_modified_fees = std::accumulate(workspaces.cbegin(), workspaces.cend(), 0*sats,

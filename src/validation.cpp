                        ...
                        bool bypass_limits, const CAmount nAbsurdFee, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // SYSCOIN
    bool bDuplicate = false;
    MemPoolAccept::ATMPArgs args { chainparams, state, pfMissingInputs, nAcceptTime, plTxnReplaced, bypass_limits, nAbsurdFee, coins_to_uncache, test_accept, bDuplicate };
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, std::unordered_set<std::string> &actorSet, bool fJustCheck)
{
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, nFees, nTotalRewardWithMasternodes)) {
        ...
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, error("%s: couldn't find masternode or superblock payments", __func__), "bad-cb-payee");
    }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    const Consensus::Params& consensusParams = params.GetConsensus();
     if (!consensusParams.AllowLegacyBlocks(nHeight) && block.IsLegacy())
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, error("%s : legacy block after auxpow start",
                                    __func__),
                          "late-legacy-block");
            ...
            if(!mintSyscoin.IsNull()){
                ...
                if(ethTxRootShouldExist){
                    ...
                    if(nTime < txRootDB.nTimestamp) {
                        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, false, "invalid-timestamp", FormatStateMessage(tx_state));
                    }
                    else if((nTime - txRootDB.nTimestamp) > 604800) {
                        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, false, "mint-blockheight-too-old", FormatStateMessage(tx_state));
                    }
                    ...
                    // also ensure sanity test that the current height that our node thinks Eth is on isn't less than the requested block for spv proof
                        ...
                        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, false, "mint-insufficient-confirmations", FormatStateMessage(tx_state));
                    }
                } 
            }
        }

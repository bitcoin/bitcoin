...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (block.Time() <= NodeSeconds{}) {
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "time-invalid", "block timestamp out of range");
    }

    // Check proof-of-work and difficulty target. For proof-of-stake blocks,
    // header-specific stake validation is handled elsewhere, but the encoded
    // difficulty must still be sane.
    if (fCheckPOW) {

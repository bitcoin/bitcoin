...
bool CheckAssetAllocationInputs(const CTransaction &tx, const uint256& txHash, const CAssetAllocation &theAssetAllocation, CValidationState &state, const CCoinsViewCache &inputs, const bool &fJustCheck, const int &nHeight, const uint256& blockhash, AssetSupplyStatsMap &mapAssetSupplyStats, AssetAllocationMap &mapAssetAllocations, AssetPrevTxMap &mapAssetPrevTxs, std::vector<COutPoint> &vecLockedOutpoints,  const bool &bSanityCheck = false, const bool &bMiner = false);

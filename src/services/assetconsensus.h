...
public:
    ...
    bool ReadAssetUndo(const uint256& txHash, CAssetUndo& assetUndo) {
        return Read(txHash, assetUndo);
    }  	
    bool Flush(const AssetMap &mapAssets, const AssetUndoMap &mapAssetsUndo);
};
...
bool DisconnectAssetUpdate(const CTransaction &tx, const uint256& txHash, AssetMap &mapAssets, AssetUndoMap &mapAssetsUndo);
...
bool CheckAssetInputs(const CTransaction &tx, const uint256& txHash, TxValidationState &tstate, const bool &fJustCheck, const int &nHeight, const uint256& blockhash, AssetMap &mapAssets, AssetUndoMap &mapAssetsUndo, const bool &bSanityCheck=false);
...
bool CheckSyscoinInputs(const bool &ibd, const CTransaction& tx,  const uint256& txHash, TxValidationState &tstate, const bool &fJustCheck, const int &nHeight, const int64_t& nTime, const uint256 & blockHash, const bool &bSanityCheck, AssetMap &mapAssets, AssetUndoMap &mapAssetsUndo, EthereumMintTxMap &mapMintKeys);

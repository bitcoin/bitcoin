...
std::vector<bool> DecodeAsmap(fs::path path)
{
    ...
    if (file.IsNull()) {
        LogPrintf(LogInstance(), "Failed to open asmap file from disk\n");
        return bits;
    }
    fseek(filestr, 0, SEEK_END);
    int length = ftell(filestr);
    LogPrintf(LogInstance(), "Opened asmap file %s (%d bytes) from disk\n", fs::quoted(fs::PathToString(path)), length);
    ...
    if (!SanityCheckASMap(bits, 128)) {
        LogPrintf(LogInstance(), "Sanity check of asmap file %s failed\n", fs::quoted(fs::PathToString(path)));
        return {};
    }
    return bits;
}

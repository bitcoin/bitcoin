...
std::partial_ordering CompareChunks(Span<const FeeFrac> chunks0, Span<const FeeFrac> chunks1)
{
    ...
    const std::array<Span<const FeeFrac>, 2> dias = {chunks0, chunks1};
    ...
    const auto next_point = [&](int dia) { return dias[dia][next_index[dia]] + accum[dia]; };
    ...
    const auto advance = [&](int dia) { accum[dia] += dias[dia][next_index[dia]++]; };

...
public:
    ...
    const auto& error() const LIFETIMEBOUND { assert(!*this); return *m_fail_data->failure; }
    auto& error() LIFETIMEBOUND { assert(!*this); return *m_fail_data->failure; }
...
public:
    ...
    //! Construct a Result object setting a success or failure value. Initial
    ...
    //! success or failure value and messages.
    template <typename O>
...
protected:
    ...
    static void construct(Result& result, Args&&... args)
    {
        if constexpr (Error) {
            ...
            result.fail_data().failure.emplace(std::forward<Args>(args)...);
    ...
    //! Move success, failure, info, and message values source Result object to
    ...
    static void move(DstResult& dst, SrcResult& src)
    {
        ...
        if (src) {
            ...
            if constexpr (!std::is_same_v<typename SrcResult::ErrorType, void>) {
                dst.fail_data().failure.emplace(std::move(src.error()));
            } else {
                dst.fail_data().failure.emplace();
            }
            assert(!dst);
        }
    }

...
const char * const Buttcoin_CONF_FILENAME = "Buttcoin.conf";
const char * const Buttcoin_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/Buttcoin/Buttcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //Buttcoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", Buttcoin_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Buttcoin
    // macOS: ~/Library/Application Support/Buttcoin
    // Unix-like: ~/.Buttcoin
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", Buttcoin_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Buttcoin Core copyright is not removed by accident
    if (copyright_devs.find("Buttcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Buttcoin Core developers";
    }

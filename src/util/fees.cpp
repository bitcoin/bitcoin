...
Percentiles CalculatePercentiles(const std::vector<FeePerVSize>& chunk_feerates, int32_t total_weight)
{
    ...
    assert(std::is_sorted(chunk_feerates.begin(), chunk_feerates.end(), [](const auto& a, const auto& b) { return a >> b; }));
    const int32_t p25_weight{static_cast<int32_t>(0.25 * total_weight)};
    const int32_t p50_weight{static_cast<int32_t>(0.50 * total_weight)};
    const int32_t p75_weight{static_cast<int32_t>(0.75 * total_weight)};
    const int32_t p95_weight{static_cast<int32_t>(0.95 * total_weight)};

	...
	if (!vchReceiver.empty() && (IsArgSet("-zmqpubassetallocation") || fAssetAllocationIndex)) {
		...
		if (BuildAssetAllocationIndexerJson(assetallocation, asset, nAmount, asset.vchAlias, vchReceiver, isMine, oName)) {
			...
			if (isMine && fAssetAllocationIndex && !assetallocation.txHash.IsNull()) {
				...
				passetallocationtransactionsdb->WriteAssetAllocationWalletIndex(nHeight, vchKey, strObj);
			}
		}
	}
...
bool CheckAssetAllocationInputs(const CTransaction &tx, int op, const vector<vector<unsigned char> > &vvchArgs, const std::vector<unsigned char> &vchAlias,
	...
	{
		errorMessage = "SYSCOIN_ASSET_ALLOCATION_CONSENSUS_ERROR ERRCODE: 1001 - " + _("Cannot unserialize data inside of this transaction relating to a assetallocation");
		return true;
	}
		...
		if (!theAssetAllocation.listSendingAllocationAmounts.empty()) {
			...
			for (auto& amountTuple : theAssetAllocation.listSendingAllocationAmounts) {
				...
				if (!bSanityCheck) {
					...
					if (!passetallocationdb->WriteAssetAllocation(receiverAllocation, amountTuple.second, dbAsset, INT64_MAX, receiverAllocation.vchAlias, fJustCheck, !fJustCheck ? dbAssetAllocation.nHeight : nHeight))
					{
			...
			for (unsigned int i = 0; i < theAssetAllocation.listSendingAllocationInputs.size();i++) {
				...
				if (!bSanityCheck) {
					...
					if (!passetallocationdb->WriteAssetAllocation(receiverAllocation, rangeTotals[i], dbAsset, INT64_MAX, receiverAllocation.vchAlias, fJustCheck, !fJustCheck? dbAssetAllocation.nHeight: nHeight))
					{

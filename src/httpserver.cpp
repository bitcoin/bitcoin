...
void HTTPRemoteClient::ReadRequest(HTTPRequest& req)
{
    ...
    // TryReadRequest() keeps a complete request back while the client's send
    // buffer is full. Bytes left in the receive buffer belong to the next
    // request and wait until it reaches a worker.
    if (req.GetState() == HTTPRequest::State::Complete) return;

...
MAIN_FUNCTION
{
    ...
    block_template->submitSolution(block.nVersion, block.nTime, block.nNonce, block.vtx.at(0), reason, debug);

    if (tip->hash != mining->getTip()->hash) {
        tfm::format(std::cout, "New block mined or received; chain tip is now %s.\n", mining->getTip()->hash.ToString());
    } else {

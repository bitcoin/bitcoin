...
public:
     ...
     * | -nofoo=1 |   X    |  X  |  X   |      X       |     X     |
     *
     * Once validated, settings can be retrieved by calling GetSetting(),
...
private:
    ...
    bool CheckArgFlags(const std::string& name, uint32_t require, uint32_t forbid, const char* context) const EXCLUSIVE_LOCKS_REQUIRED(!cs_args);

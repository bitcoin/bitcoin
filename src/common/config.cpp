...
bool IsConfSupported(KeyInfo& key, std::string& error) {
    ...
    if (key.name == "reindex") {
        ...
        LogPrintf(LogInstance(), "Warning: reindex=1 is set in the configuration file, which will significantly slow down startup. Consider removing or commenting out this option for better performance, unless there is currently a condition which makes rebuilding the indexes necessary\n");
        return true;
    }
    return true;
}
    ...
    for (const std::pair<std::string, std::string>& option : options) {
        ...
        if (flags) {
            ...
            if (ignore_invalid_keys) {
                LogPrintf(LogInstance(), "Ignoring unknown configuration value %s\n", option.first);
            } else {
        ...
        if (use_conf_file) {
            ...
            for (const std::string& conf_file_name : conf_file_names) {
                ...
                if (conf_file_stream.good()) {
                    ...
                    LogPrintf(LogInstance(), "Included configuration file %s\n", conf_file_name);
                } else {

...
const char * const BITCOIN_CONF_FILENAME = "snailcoin.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Snailcoin
    //   new: C:\Users\Username\AppData\Local\Snailcoin
    // macOS: ~/Library/Application Support/Snailcoin
    // Unix-like: ~/.snailcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Snailcoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Snailcoin";
#else
    fs::path pathRet;

...
private:
    ...
    std::unique_ptr<secp256k1_silentpayments_recipient_public_data> m_public_data;

public:
    PublicDataImpl() : m_public_data{std::make_unique<secp256k1_silentpayments_recipient_public_data>()} {}

    // Delete copy constructors
    PublicDataImpl(const PublicDataImpl&) = delete;
    PublicDataImpl& operator=(const PublicDataImpl&) = delete;

    secp256k1_silentpayments_recipient_public_data* Get() const { return m_public_data.get(); }
};

PublicData::PublicData() : m_impl{std::make_unique<PublicDataImpl>()} {}

PublicData::PublicData(PublicData&&) noexcept = default;
PublicData& PublicData::operator=(PublicData&&) noexcept = default;

PublicData::~PublicData() = default;

secp256k1_silentpayments_recipient_public_data* PublicData::Get() const
{
    return m_impl->Get();
}
...
std::optional<PublicData> CreateInputPubkeysTweak(

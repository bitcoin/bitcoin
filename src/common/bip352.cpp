...
SilentPaymentLabel::SilentPaymentLabel(secp256k1_silentpayments_label&& label)
    : m_label{std::make_shared<secp256k1_silentpayments_label>(label)} {}

bool SilentPaymentLabel::operator==(const SilentPaymentLabel& other) const
{
    return memcmp(this->m_label->data, other.m_label->data, SIZE) == 0;
}
...
const unsigned char* LabelLookupCallback(const unsigned char* key, const void* context) {
    auto label_context = static_cast<const std::map<SilentPaymentLabel, uint256>*>(context);
    secp256k1_silentpayments_label label_obj;
    bool ret = secp256k1_silentpayments_recipient_label_parse(
        secp256k1_context_static, &label_obj, key);
    assert(ret);
...
std::pair<SilentPaymentLabel, uint256> CreateLabelTweak(const CKey& scan_key, const int m) {
    secp256k1_silentpayments_label label_obj;
    unsigned char label_tweak[32];
    bool ret = secp256k1_silentpayments_recipient_label_create(secp256k1_context_sign, &label_obj, label_tweak, UCharCast(scan_key.data()), m);
    assert(ret);
    return {SilentPaymentLabel{std::move(label_obj)}, uint256{label_tweak}};
}

CPubKey CreateLabeledSpendPubKey(const CPubKey& spend_pubkey, const secp256k1_silentpayments_label& label) {
    ...
    assert(ret);
    ret = secp256k1_silentpayments_recipient_create_labeled_spend_pubkey(secp256k1_context_static, &labeled_spend_obj, &spend_obj, &label);
    assert(ret);
...
V0SilentPaymentDestination SilentPaymentLabel::GenerateLabeledAddress(const V0SilentPaymentDestination& recipient) const
{
    CPubKey labeled_spend_pubkey = CreateLabeledSpendPubKey(recipient.m_spend_pubkey, this->Get());
...
std::optional<std::vector<SilentPaymentOutput>> ScanForSilentPaymentOutputs(
    ...
    for (size_t i = 0; i < n_found_outputs; i++) {
        ...
        if (found_output_objs[i].found_with_label) {
            sp_output.label = SilentPaymentLabel(std::move(found_output_objs[i].label));
        }
        outputs.push_back(std::move(sp_output));
    }
    return outputs;
}
}; // namespace bip352

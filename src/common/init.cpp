...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        const fs::path orig_config_path{AbsPathForConfigVal(args, args.GetPathArg("-conf", SYSCOIN_CONF_FILENAME), /*net_specific=*/false)};
        ...
        const fs::path base_config_path = base_path / SYSCOIN_CONF_FILENAME;
        if (fs::exists(base_config_path) && !fs::equivalent(orig_config_path, base_config_path)) {
            ...
            const std::string error = strprintf(
                ...
                fs::quoted(SYSCOIN_CONF_FILENAME),

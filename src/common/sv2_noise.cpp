...
Sv2CipherState::Sv2CipherState(std::array<uint8_t, Sv2CipherState::HASHLEN> key) : m_key(key) {
    LogTrace(BCLog::SV2, "Sv2CipherState %s\n", HexStr(m_key));
};
...
{
    std::array<uint8_t, Sv2CipherState::HASHLEN> out0, out1;
...
void Sv2SymmetricState::HKDF2(const Span<const std::byte> input_key_material, std::array<uint8_t, Sv2CipherState::HASHLEN>& out0, std::array<uint8_t, Sv2CipherState::HASHLEN>& out1)
{
    ...
    std::vector<uint8_t> in1(Sv2CipherState::HASHLEN + 1, 0);
...
{
    std::array<uint8_t, Sv2CipherState::HASHLEN> send_key, recv_key;
    HKDF2({}, send_key, recv_key);
    return {Sv2CipherState{send_key}, Sv2CipherState{recv_key}};
}

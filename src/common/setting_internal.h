...
template<typename T>
constexpr auto GetPtr(std::optional<T>(*ptr)(const SettingsValue&)) { return ptr; }
...
{
    static bool Get(const SettingsValue& setting, setting_t& out) { out = setting; return true; }
};
...
T ConstructDefault(const DefaultFn& default_fn)
{
...
template<StringLiteral summary, typename T, SettingOptions options, StringLiteral help, auto help_fn, auto default_fn, auto get_fn, OptionsCategory category>
struct SettingImpl
{
    template<typename... Args>
    static void Register(ArgsManager& manager, Args&&... args)
    {
        manager.AddArg(summary.value, Help(std::forward<Args>(args)...), SettingFlags<options>(), category);
    }

    template<typename GetType = T>
    static GetType Get(const ArgsManager& manager)
    {
        return Get(manager, default_fn);
    }

    template<typename GetType = T>
    static GetType Get(const ArgsManager& manager, const auto& get_default)
    {
        using Traits = SettingTraits<GetType, options>;
        const std::string name{SettingName(summary.value)};
        GetType out{ConstructDefault<GetType>(get_default)};
    ...
    struct HelpArg {
        ...
        {
            return ConstructDefault<T>(default_fn);
        }

        static std::string Format(const auto&... args)
        {
            constexpr util::ConstevalFormatString<sizeof...(args)> fmt{help.value};
            return strprintf(fmt, args...);
        }
    };

    template<typename... Args>
    static std::string Help(Args&&... args)
    {
        std::string help_str;
        if constexpr (!std::is_same_v<decltype(help_fn), std::nullptr_t>) {
            help_str = help_fn(HelpArg{}, std::forward<Args>(args)...);

        ...
        if (!fanout) {
            ...
            if  (!result.m_succeeded) {
                ...
                if (const auto collision = result.m_collision; collision.has_value()) {
                    ...
                    tx_relay->m_tx_inventory_to_send.insert(collision.value());
                }
            }
        }
        if (fanout) {
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::SKETCH) {
        ...
        if (valid_sketch) {
            if (recon_result.has_value()) {
                // Handles both successful and failed reconciliation (but not the case per which
                // we want to request extension).
                MakeAndPushMessage(pfrom, NetMsgType::RECONCILDIFF, recon_result.value(), txs_to_request);
    ...
    if (msg_type == NetMsgType::RECONCILDIFF) {
        ...
        bool valid_finalization = m_txreconciliation->HandleReconcilDiff(pfrom.GetId(), recon_result, ask_shortids, remote_missing);
        ...
        if (auto tx_relay = peer->GetTxRelay(); tx_relay != nullptr) {
            ...
            for (auto& [inv, out_fanout_count] : to_be_announced) {
                ...
                if (!should_fanout) {
                    ...
                    Assume(m_txreconciliation);
                    const auto result = m_txreconciliation->AddToSet(pto->GetId(), Wtxid::FromUint256(inv.hash));
                    if (!result.m_succeeded) {
                        should_fanout = true;
                        if (const auto collision = result.m_collision; collision.has_value()) {

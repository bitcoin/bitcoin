...
struct Peer {
    ...
    bool m_wtxid_relay GUARDED_BY(m_mutex_message_handling) {false};
     ...
     *  If this is set to nullptr, addr relay is disabled.
     *  We disabled for outbound block-relay-only peers to
...
public:
    ...
    bool GetPeerStats(NodeId nodeid, PeerStats& stats) const override;
...
void PeerManagerImpl::MaybeSendFeefilter(CNode& pto, Peer& peer, std::chrono::microseconds current_time)
{
    ...
    if (m_chainman.ActiveChainstate().IsInitialBlockDownload()) {
        ...
        if (peer.m_tx_relay->m_fee_filter_ours == MAX_FILTER) {
    ...
    if (current_time > peer.m_tx_relay->m_next_send_feefilter) {
        ...
        if (filterToSend != peer.m_tx_relay->m_fee_filter_ours) {
            m_connman.PushMessage(&pto, CNetMsgMaker(pto.GetCommonVersion()).Make(NetMsgType::FEEFILTER, filterToSend));
            peer.m_tx_relay->m_fee_filter_ours = filterToSend;
        }
    ...
    // until scheduled broadcast, then move the broadcast to within MAX_FEEFILTER_CHANGE_DELAY.
                ...
                (currentFilter < 3 * peer.m_tx_relay->m_fee_filter_ours / 4 || currentFilter > 4 * peer.m_tx_relay->m_fee_filter_ours / 3)) {

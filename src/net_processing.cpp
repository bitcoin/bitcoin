...
void PeerManagerImpl::ProcessCompactBlockTxns(CNode& pfrom, Peer& peer, const BlockTransactions& block_transactions)
{
    ...
    bool fBlockRead{false};
    {
        ...
        if (status == READ_STATUS_INVALID) {
            ...
            // out to be invalid.
            m_blockdownloadman.SetBlockSource(block_transactions.blockhash, pfrom.GetId(), false);
        }
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (fBlockReconstructed) {
            ...
            {
                LOCK(cs_main);
                m_blockdownloadman.SetBlockSource(pblock->GetHash(), pfrom.GetId(), false);
            }
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        bool min_pow_checked = false;
        {
            ...
            m_blockdownloadman.SetBlockSource(hash, pfrom.GetId(), true);
    ...
    if (!state.m_chain_sync.m_protect && pto.IsOutboundOrBlockRelayConn() && m_blockdownloadman.GetSyncStarted(pto.GetId())) {
        ...
        if (peer_best_known != nullptr && peer_best_known->nChainWork >= m_chainman.ActiveChain().Tip()->nChainWork) {
            ...
            if (state.m_chain_sync.m_sent_getheaders) {
                ...
                LogInfo("Outbound peer has old chain, best known block = %s, %s\n", peer_best_known != nullptr ? peer_best_known->GetBlockHash().ToString() : "<none>", pto.DisconnectMsg(fLogIPs));
    ...
    if (m_connman.GetExtraBlockRelayCount() > 0) {
        ...
        m_connman.ForNode(to_disconnect, [&](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
            ...
            if (node_state == nullptr ||
                (now - pnode->m_connected >= MINIMUM_CONNECT_TIME && peer_blocks_in_flight.empty())) {
                pnode->fDisconnect = true;
                ...
                LogDebug(BCLog::NET, "keeping block-relay-only peer=%d chosen for eviction (connect time: %d, blocks_in_flight: %d)\n",
                         pnode->GetId(), count_seconds(pnode->m_connected), peer_blocks_in_flight.size());
            }
            return false;
        });
    }

...
struct Peer {
    ...
    /** Greatest common protocol version that both we and the peer support. Set
...
public:
    ...
    void InitializeNode(const ConnectionContext& conn_ctx, ServiceFlags our_services) override
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, !m_version_nonces_mutex);
    void FinalizeNode(const ConnectionContext& conn_ctx, bool was_successfully_connected) override
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, !m_headers_presync_mutex, !m_version_nonces_mutex);
    bool ProcessMessages(NodeId id, std::atomic<bool>& interrupt) override
    ...
    bool SendMessages(NodeId id) override
    ...
    std::optional<std::string> FetchBlock(NodeId peer_id, const CBlockIndex& block_index) override
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
    bool GetPeerStats(NodeId nodeid, PeerStats& stats) const override EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
    bool IgnoresIncomingTxs() override { return m_opts.ignore_incoming_txs; }
    void SendPings() override EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
    void RelayTransaction(const uint256& txid, const uint256& wtxid) override EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
    void SetBestHeight(int height) override { m_best_height = height; };
    void UnitTestMisbehaving(NodeId peer_id, int howmuch) override EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex) { Misbehaving(*Assert(GetPeerRef(peer_id)), howmuch, ""); };
    void ProcessMessage(NodeId id, const std::string& msg_type, CDataStream& vRecv,
...
private:
    ...
    */
    ...
    void HandleFewUnconnectingHeaders(Peer& peer, const std::vector<CBlockHeader>& headers) EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex);
     ...
     */
    bool IsContinuationOfLowWorkHeadersSync(Peer& peer, std::vector<CBlockHeader>& headers)
     ...
     */
    bool TryLowWorkHeadersSync(Peer& peer, const CBlockIndex* chain_start_header,
                               std::vector<CBlockHeader>& headers)
    ...
    bool RejectIncomingTxs(const ConnectionContext& conn_ctx) const;
     ...
     */
    void ProcessGetCFilters(Peer& peer, CDataStream& vRecv);
     ...
     * @param[in]   vRecv           The raw message received
     */
    void ProcessGetCFHeaders(Peer& peer, CDataStream& vRecv);
     ...
     * @param[in]   vRecv           The raw message received
     */
    void ProcessGetCFCheckPt(Peer& peer, CDataStream& vRecv);
...
void PeerManagerImpl::PushNodeVersion(const Peer& peer)
{
    ...
    uint64_t nonce{m_connman.GetDeterministicRandomizer(RANDOMIZER_ID_LOCALHOSTNONCE).Write(peer.m_id).Finalize()};
    // If this is an outbound connection, store the version nonce so we can check that
    // we haven't connected to ourselves.
    if (!peer.m_conn_ctx.IsInboundConn()) {
        LOCK(m_version_nonces_mutex);
        m_version_nonces.emplace_hint(m_version_nonces.end(), peer.m_id, nonce);
    }
    const int nNodeStartingHeight{m_best_height};
    CAddress addr = peer.m_conn_ctx.addr;

    CService addr_you = addr.IsRoutable() && !IsProxy(addr) && addr.IsAddrV1Compatible() ? addr : CService();
    uint64_t your_services{addr.nServices};

    const bool tx_relay{!RejectIncomingTxs(peer.m_conn_ctx)};
    m_connman.PushMessage(peer.m_id, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::VERSION, PROTOCOL_VERSION, my_services, nTime,
            your_services, addr_you, // Together the pre-version-31402 serialization of CAddress "addrYou" (without nTime)
            my_services, CService(), // Together the pre-version-31402 serialization of CAddress "addrMe" (without nTime)
            nonce, strSubVersion, nNodeStartingHeight, tx_relay));

    if (fLogIPs) {
        LogPrint(BCLog::NET, "send version message: version %d, blocks=%d, them=%s, txrelay=%d, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight, addr_you.ToStringAddrPort(), tx_relay, peer.m_id);
    } else {
        LogPrint(BCLog::NET, "send version message: version %d, blocks=%d, txrelay=%d, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight, tx_relay, peer.m_id);
    }
}
...
void PeerManagerImpl::InitializeNode(const ConnectionContext& conn_ctx, ServiceFlags our_services)
{
    {
        LOCK(cs_main);
        m_node_states.emplace_hint(m_node_states.end(), std::piecewise_construct, std::forward_as_tuple(conn_ctx.id), std::forward_as_tuple(conn_ctx.IsInboundConn()));
        assert(m_txrequest.Count(conn_ctx.id) == 0);
    }
    PeerRef peer = std::make_shared<Peer>(conn_ctx, our_services);
    {
        LOCK(m_peer_mutex);
        m_peer_map.emplace_hint(m_peer_map.end(), conn_ctx.id, peer);
    }
...
void PeerManagerImpl::FinalizeNode(const ConnectionContext& conn_ctx, bool was_successfully_connected)
{
    WITH_LOCK(m_version_nonces_mutex, m_version_nonces.erase(conn_ctx.id));
    int misbehavior{0};
    {
    LOCK(cs_main);
    {
        ...
        // destructed.
        PeerRef peer = RemovePeer(conn_ctx.id);
        assert(peer != nullptr);
        misbehavior = WITH_LOCK(peer->m_misbehavior_mutex, return peer->m_misbehavior_score);
        m_wtxid_relay_peers -= peer->m_wtxid_relay;
        assert(m_wtxid_relay_peers >= 0);
    }
    CNodeState *state = State(conn_ctx.id);
    assert(state != nullptr);
    ...
    m_orphanage.EraseForPeer(conn_ctx.id);
    m_txrequest.DisconnectedPeer(conn_ctx.id);
    if (m_txreconciliation) m_txreconciliation->ForgetPeer(conn_ctx.id);
    ...
    m_node_states.erase(conn_ctx.id);
    ...
    if (was_successfully_connected && misbehavior == 0 &&
        !conn_ctx.IsBlockOnlyConn() && !conn_ctx.IsInboundConn()) {
        // Only change visible addrman state for full outbound peers.  We don't
        // call Connected() for feeler connections since they don't have
        // fSuccessfullyConnected set.
        m_addrman.Connected(conn_ctx.addr);
    }
    {
        LOCK(m_headers_presync_mutex);
        m_headers_presync_stats.erase(conn_ctx.id);
    }
...
bool PeerManagerImpl::GetPeerStats(NodeId nodeid, PeerStats& stats) const
{
    {
        LOCK(cs_main);
...
std::unique_ptr<PeerManager> PeerManager::make(ConnectionsInterface& connman, AddrMan& addrman, EvictionManager& evictionman,
                                               ...
                                               CTxMemPool& pool, Options opts)
{
    return std::make_unique<PeerManagerImpl>(connman, addrman, evictionman, banman, chainman, pool, opts);
}
...
void PeerManagerImpl::ProcessGetBlockData(Peer& peer, const CInv& inv)
{
    ...
    if (m_connman.OutboundTargetReached(true) &&
        ...
        !peer.m_conn_ctx.HasPermission(NetPermissionFlags::Download) // nodes with the download permission may exceed target
    ) {
    ...
    if (!peer.m_conn_ctx.HasPermission(NetPermissionFlags::NoBan) && (
    ...
    if (a_recent_block && a_recent_block->GetHash() == pindex->GetBlockHash()) {
        ...
        m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::BLOCK, Span{block_data}));
    ...
    if (pblock) {
        if (inv.IsMsgBlk()) {
            m_connman.PushMessage(peer.m_id, msgMaker.Make(SERIALIZE_TRANSACTION_NO_WITNESS, NetMsgType::BLOCK, *pblock));
        } else if (inv.IsMsgWitnessBlk()) {
            m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::BLOCK, *pblock));
            ...
            if (sendMerkleBlock) {
                m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::MERKLEBLOCK, merkleBlock));
                ...
                for (PairType& pair : merkleBlock.vMatchedTxn)
                    m_connman.PushMessage(peer.m_id, msgMaker.Make(SERIALIZE_TRANSACTION_NO_WITNESS, NetMsgType::TX, *pblock->vtx[pair.first]));
            }
            // else
            // no response
            ...
            if (CanDirectFetch() && pindex->nHeight >= m_chainman.ActiveChain().Height() - MAX_CMPCTBLOCK_DEPTH) {
                if (a_recent_compact_block && a_recent_compact_block->header.GetHash() == pindex->GetBlockHash()) {
                    m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::CMPCTBLOCK, *a_recent_compact_block));
                } else {
                    CBlockHeaderAndShortTxIDs cmpctblock{*pblock};
                    m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::CMPCTBLOCK, cmpctblock));
                }
            } else {
                m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::BLOCK, *pblock));
            }
...

    {
        ...
        if (inv.hash == peer.m_continuation_block) {
            ...
            m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::INV, vInv));
...
void PeerManagerImpl::ProcessGetData(Peer& peer, const std::atomic<bool>& interruptMsgProc)
{
    ...
    while (it != peer.m_getdata_requests.end() && it->IsGenTxMsg()) {
        ...
        if (m_connman.IsSendingPaused(peer.m_id)) break;
        ...
        if (tx) {
            ...
            m_connman.PushMessage(peer.m_id, msgMaker.Make(nSendFlags, NetMsgType::TX, *tx));
    ...
    if (it != peer.m_getdata_requests.end() && !m_connman.IsSendingPaused(peer.m_id)) {
        const CInv &inv = *it++;
        if (inv.IsGenBlkMsg()) {
    ...
    if (!vNotFound.empty()) {
        ...
        m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::NOTFOUND, vNotFound));
    }
}
...
void PeerManagerImpl::SendBlockTransactions(Peer& peer, const CBlock& block, const BlockTransactionsRequest& req)
{
    ...
    const CNetMsgMaker msgMaker(peer.m_greatest_common_version);
    m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::BLOCKTXN, resp));
}
 ...
 */
void PeerManagerImpl::HandleFewUnconnectingHeaders(Peer& peer, const std::vector<CBlockHeader>& headers)
{
    ...
    if (MaybeSendGetHeaders(GetLocator(best_header), peer)) {
        LogPrint(BCLog::NET, "received header %s: missing prev block %s, sending getheaders (%d) to end (peer=%d, m_num_unconnecting_headers_msgs=%d)\n",
            ...
            best_header->nHeight,
            peer.m_id, peer.m_num_unconnecting_headers_msgs);
    }

    // Set hashLastUnknownBlock for this peer, so that if we
    // eventually get the headers - even from a different peer -
    // we can use this peer to download.
    WITH_LOCK(cs_main, UpdateBlockAvailability(peer.m_id, headers.back().GetHash()));
...
bool PeerManagerImpl::IsContinuationOfLowWorkHeadersSync(Peer& peer, std::vector<CBlockHeader>& headers)
{
    if (peer.m_headers_sync) {
            ...
            if (!locator.vHave.empty()) {
                ...
                if (sent_getheaders) {
                    LogPrint(BCLog::NET, "more getheaders (from %s) to peer=%d\n",
                            locator.vHave.front().ToString(), peer.m_id);
                } else {
        ...
        if (peer.m_headers_sync->GetState() == HeadersSyncState::State::FINAL) {
            ...
            LOCK(m_headers_presync_mutex);
            m_headers_presync_stats[peer.m_id] = stats;
            ...
            if (best_it == m_headers_presync_stats.end()) {
                ...
                best_updated = (peer_best == peer.m_id);
            } else if (best_it->first == peer.m_id || stats > best_it->second) {
                // This peer was and remains the best peer, or it just became best.
                m_headers_presync_bestpeer = peer.m_id;
                best_updated = true;
            }
...
bool PeerManagerImpl::MaybeSendGetHeaders(const CBlockLocator& locator, Peer& peer)
{
    ...
    if (current_time - peer.m_last_getheaders_timestamp > HEADERS_RESPONSE_TIME) {
        m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::GETHEADERS, locator, uint256()));
...
void PeerManagerImpl::HeadersDirectFetchBlocks(const Peer& peer, const CBlockIndex& last_header)
{
    ...
    LOCK(cs_main);
    CNodeState *nodestate = State(peer.m_id);
    ...
    if (CanDirectFetch() && last_header.IsValid(BLOCK_VALID_TREE) && m_chainman.ActiveChain().Tip()->nChainWork <= last_header.nChainWork) {
        ...
        if (!m_chainman.ActiveChain().Contains(pindexWalk)) {
            ...
            for (const CBlockIndex *pindex : reverse_iterate(vToFetch)) {
                ...
                BlockRequested(peer.m_id, *pindex);
                LogPrint(BCLog::NET, "Requesting block %s from  peer=%d\n",
            ...
            if (vGetData.size() > 0) {
                ...
                m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::GETDATA, vGetData));
            }
        }
    }
}
        ...
        const CBlockIndex& last_header, bool received_new_header, bool may_have_more_headers)
{
    ...
    UpdateBlockAvailability(peer.m_id, last_header.GetBlockHash());
    ...
    if (received_new_header && last_header.nChainWork > m_chainman.ActiveChain().Tip()->nChainWork) {
        m_evictionman.UpdateLastBlockAnnounceTime(peer.m_id, GetTime<std::chrono::seconds>());
    }
    ...
    if (m_chainman.ActiveChainstate().IsInitialBlockDownload() && !may_have_more_headers) {
        ...
        if (nodestate->pindexBestKnownBlock && nodestate->pindexBestKnownBlock->nChainWork < m_chainman.MinimumChainWork()) {
            ...
            if (peer.m_conn_ctx.IsOutboundOrBlockRelayConn()) {
    ...
    if (!m_connman.IsDisconnected(peer.m_id) && peer.m_conn_ctx.IsFullOutboundConn() && nodestate->pindexBestKnownBlock != nullptr) {
        if (m_outbound_peers_with_protect_from_disconnect < MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCONNECT && nodestate->pindexBestKnownBlock->nChainWork >= m_chainman.ActiveChain().Tip()->nChainWork && !nodestate->m_chain_sync.m_protect) {
            LogPrint(BCLog::NET, "Protecting outbound peer=%d from eviction\n", peer.m_id);
            nodestate->m_chain_sync.m_protect = true;
            m_evictionman.UpdateSlowChainProtected(peer.m_id);
                                            ...
                                            bool via_compact_block)
{
    ...
    if (!headers_connect_blockindex) {
        if (nCount <= MAX_BLOCKS_TO_ANNOUNCE) {
            ...
            // could be benign.
            HandleFewUnconnectingHeaders(peer, headers);
        } else {
    ...
    if (!m_chainman.ProcessNewBlockHeaders(headers, /*min_pow_checked=*/true, state, &pindexLast)) {
        if (state.IsInvalid()) {
            MaybePunishNodeForBlock(peer.m_id, state, via_compact_block, "invalid header received");
            return;
        }
    }
    assert(pindexLast);
    ...
    if (nCount == MAX_HEADERS_RESULTS && !have_headers_sync) {
        ...
        if (MaybeSendGetHeaders(GetLocator(pindexLast), peer)) {
            LogPrint(BCLog::NET, "more getheaders (%d) to end to peer=%d (startheight:%d)\n",
                    pindexLast->nHeight, peer.m_id, peer.m_starting_height);
        }
    }

    UpdatePeerStateForReceivedHeaders(peer, *pindexLast, received_new_header, nCount == MAX_HEADERS_RESULTS);
                                                ...
                                                BlockFilterIndex*& filter_index)
{
    ...
    if (!supported_filter_type) {
        LogPrint(BCLog::NET, "peer %d requested unsupported block filter type: %d\n",
                 peer.m_id, static_cast<uint8_t>(filter_type));
        m_connman.DisconnectNode(peer.m_id);
        return false;
    }
    ...
    if (start_height > stop_height) {
        LogPrint(BCLog::NET, "peer %d sent invalid getcfilters/getcfheaders with " /* Continued */
                 ...
                 peer.m_id, start_height, stop_height);
        m_connman.DisconnectNode(peer.m_id);
        return false;
    }
    if (stop_height - start_height >= max_height_diff) {
        LogPrint(BCLog::NET, "peer %d requested too many cfilters/cfheaders: %d / %d\n",
                 peer.m_id, stop_height - start_height + 1, max_height_diff);
        m_connman.DisconnectNode(peer.m_id);
        return false;
    }
...
void PeerManagerImpl::ProcessGetCFilters(Peer& peer, CDataStream& vRecv)
{
    ...
    for (const auto& filter : filters) {
        CSerializedNetMsg msg = CNetMsgMaker(peer.m_greatest_common_version)
            .Make(NetMsgType::CFILTER, filter);
        m_connman.PushMessage(peer.m_id, std::move(msg));
    }
}

void PeerManagerImpl::ProcessGetCFHeaders(Peer& peer, CDataStream& vRecv)
{
    uint8_t filter_type_ser;
    uint32_t start_height;
    uint256 stop_hash;
...
void PeerManagerImpl::ProcessGetCFCheckPt(Peer& peer, CDataStream& vRecv)
{
    uint8_t filter_type_ser;
    uint256 stop_hash;
...
void PeerManagerImpl::ProcessCompactBlockTxns(Peer& peer, const BlockTransactions& block_transactions)
{
    ...
    const CNetMsgMaker msgMaker(peer.m_greatest_common_version);
    {
        ...
        bool first_in_flight = already_in_flight == 0 || (range_flight.first->second.first == peer.m_id);

        while (range_flight.first != range_flight.second) {
            auto [node_id, block_it] = range_flight.first->second;
            if (node_id == peer.m_id && block_it->partialBlock) {
        ...
        if (status == READ_STATUS_INVALID) {
            RemoveBlockRequest(block_transactions.blockhash, peer.m_id); // Reset in-flight state in case Misbehaving does not result in a disconnect
            ...
            if (first_in_flight) {
                ...
                RemoveBlockRequest(block_transactions.blockhash, peer.m_id);
            ...
            // updated, etc.
            RemoveBlockRequest(block_transactions.blockhash, peer.m_id); // it is now an empty pointer
            fBlockRead = true;
            ...
            // out to be invalid.
            mapBlockSource.emplace(block_transactions.blockhash, std::make_pair(peer.m_id, false));
        }
    ...
    if (fBlockRead) {
        ...
        ProcessBlock(peer.m_id, pblock, /*force_processing=*/true, /*min_pow_checked=*/true);
    }
    return;
}

void PeerManagerImpl::ProcessMessage(NodeId id, const std::string& msg_type, CDataStream& vRecv,
                                     const std::chrono::microseconds time_received,
                                     const std::atomic<bool>& interruptMsgProc)
{
    AssertLockHeld(g_msgproc_mutex);

    LogPrint(BCLog::NET, "received: %s (%u bytes) peer=%d\n", SanitizeString(msg_type), vRecv.size(), id);
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        if (!peer->m_conn_ctx.IsInboundConn())
        {
            m_addrman.SetServices(peer->m_conn_ctx.addr, nServices);
        }
        if (peer->m_conn_ctx.ExpectServicesFromConn() && !HasAllDesirableServiceFlags(nServices))
        {
            LogPrint(BCLog::NET, "peer=%d does not offer the expected services (%08x offered, %08x expected); disconnecting\n", peer->m_id, nServices, GetDesirableServiceFlags(nServices));
            m_connman.DisconnectNode(peer->m_id);
            return;
        }
        ...
        if (peer->m_conn_ctx.IsInboundConn() && !CheckIncomingNonce(nNonce))
        {
            LogPrintf("connected to self at %s, disconnecting\n", peer->m_conn_ctx.addr.ToStringAddrPort());
            m_connman.DisconnectNode(peer->m_id);
            return;
        }

        if (peer->m_conn_ctx.IsInboundConn() && addrMe.IsRoutable())
        {
            SeenLocal(addrMe);
        }
        ...
        peer->m_greatest_common_version = greatest_common_version;
        peer->m_version = nVersion;

        const CNetMsgMaker msg_maker(greatest_common_version);

        if (greatest_common_version >= WTXID_RELAY_VERSION) {
            m_connman.PushMessage(peer->m_id, msg_maker.Make(NetMsgType::WTXIDRELAY));
        }
        ...
        if (greatest_common_version >= 70016) {
            ...
            m_connman.PushMessage(peer->m_id, msg_maker.Make(NetMsgType::SENDADDRV2));
        }

        m_evictionman.UpdateRelevantServices(peer->m_id, HasAllDesirableServiceFlags(nServices));
        ...
        if (!peer->m_conn_ctx.IsBlockOnlyConn() &&
            !peer->m_conn_ctx.IsFeelerConn() &&
            ...
            if (fRelay) {
                m_evictionman.UpdateRelayTxs(peer->m_id);
            }
        }
        ...
        if (greatest_common_version >= WTXID_RELAY_VERSION && m_txreconciliation) {
            ...
            if (tx_relay && WITH_LOCK(tx_relay->m_bloom_filter_mutex, return tx_relay->m_relay_txs) &&
                !peer->m_conn_ctx.IsAddrFetchConn() && !m_opts.ignore_incoming_txs) {
                const uint64_t recon_salt = m_txreconciliation->PreRegisterPeer(peer->m_id);
                m_connman.PushMessage(peer->m_id, msg_maker.Make(NetMsgType::SENDTXRCNCL,
                                                             TXRECONCILIATION_VERSION, recon_salt));
            }
        }

        m_connman.PushMessage(peer->m_id, msg_maker.Make(NetMsgType::VERACK));

        // Potentially mark this peer as a preferred download peer.
        {
            LOCK(cs_main);
            CNodeState* state = State(peer->m_id);
            state->fPreferredDownload = (!peer->m_conn_ctx.IsInboundConn() || peer->m_conn_ctx.HasPermission(NetPermissionFlags::NoBan)) && !peer->m_conn_ctx.IsAddrFetchConn() && CanServeBlocks(*peer);
            m_num_preferred_download_peers += state->fPreferredDownload;
        }
        ...
        if (send_getaddr) {
            ...
            m_connman.PushMessage(peer->m_id, CNetMsgMaker(greatest_common_version).Make(NetMsgType::GETADDR));
        ...
        if (!peer->m_conn_ctx.IsInboundConn()) {
            ...
            m_addrman.Good(peer->m_conn_ctx.addr);
        }

        std::string remoteAddr;
        if (fLogIPs)
            remoteAddr = ", peeraddr=" + peer->m_conn_ctx.addr.ToStringAddrPort();

        const auto mapped_as{m_connman.GetMappedAS(peer->m_conn_ctx.addr)};
        LogPrint(BCLog::NET, "receive version message: %s: version %d, blocks=%d, us=%s, txrelay=%d, peer=%d%s%s\n",
                  cleanSubVer, peer->m_version,
                  peer->m_starting_height, addrMe.ToStringAddrPort(), fRelay, peer->m_id,
        ...
        if (!peer->m_conn_ctx.IsInboundConn()) {
            ...
            AddTimeData(peer->m_conn_ctx.addr, nTimeOffset);
        }
        ...
        if (greatest_common_version <= 70012) {
            ...
            m_connman.PushMessage(peer->m_id, CNetMsgMaker(greatest_common_version).Make("alert", Span{finalAlert}));
        }

        // Feeler connections exist only to verify if address is online.
        if (peer->m_conn_ctx.IsFeelerConn()) {
    ...
    if (msg_type == NetMsgType::VERACK) {
        if (m_connman.IsSuccessfullyConnected(peer->m_id)) {
        ...
        if (!peer->m_conn_ctx.IsInboundConn()) {
            ...
            LogPrintf("New outbound peer connected: version: %d, blocks=%d, peer=%d%s%s (%s)\n",
                      ...
                      peer->m_id, (fLogIPs ? strprintf(", peeraddr=%s", peer->m_conn_ctx.addr.ToStringAddrPort()) : ""),
                      (mapped_as ? strprintf(", mapped_as=%d", mapped_as) : ""),
                      peer->m_conn_ctx.ConnectionTypeAsString());
        }

        if (peer->m_greatest_common_version >= SHORT_IDS_BLOCKS_VERSION) {
            // Tell our peer we are willing to provide version 2 cmpctblocks.
            // However, we do not request new block announcements using
            // cmpctblock messages.
            // We send this to non-NODE NETWORK peers as well, because
            // they may wish to request compact blocks from us
            m_connman.PushMessage(peer->m_id, msgMaker.Make(NetMsgType::SENDCMPCT, /*high_bandwidth=*/false, /*version=*/CMPCTBLOCKS_VERSION));
        }

        if (m_txreconciliation) {
            if (!peer->m_wtxid_relay || !m_txreconciliation->IsPeerRegistered(peer->m_id)) {
                ...
                m_txreconciliation->ForgetPeer(peer->m_id);
            }
        }
        ...
        WITH_LOCK(m_version_nonces_mutex, m_version_nonces.erase(peer->m_id));
        m_connman.SetSuccessfullyConnected(peer->m_id);
        m_evictionman.UpdateSuccessfullyConnected(peer->m_id);
        return;
    }
    ...
    if (msg_type == NetMsgType::WTXIDRELAY) {
        ...
        if (peer->m_greatest_common_version >= WTXID_RELAY_VERSION) {
    ...
    if (msg_type == NetMsgType::SENDTXRCNCL) {
        ...
        if (RejectIncomingTxs(peer->m_conn_ctx)) {
        ...
        const ReconciliationRegisterResult result = m_txreconciliation->RegisterPeer(peer->m_id, peer->m_conn_ctx.IsInboundConn(),
    ...
    if (msg_type == NetMsgType::ADDR || msg_type == NetMsgType::ADDRV2) {
        ...
        if (!SetupAddressRelay(*peer)) {
            LogPrint(BCLog::NET, "ignoring %s message from %s peer=%d\n", msg_type, peer->m_conn_ctx.ConnectionTypeAsString(), peer->m_id);
            return;
        }
        ...
        const bool rate_limited = !peer->m_conn_ctx.HasPermission(NetPermissionFlags::Addr);
        uint64_t num_proc = 0;
        ...
        for (CAddress& addr : vAddr)
        {
            ...
            if (addr.nTime > current_a_time - 10min && !peer->m_getaddr_sent && vAddr.size() <= 10 && addr.IsRoutable()) {
                // Relay to a limited number of other nodes
                RelayAddress(peer->m_id, addr, fReachable);
            }
        ...
        LogPrint(BCLog::NET, "Received addr: %u addresses (%u processed, %u rate-limited) from peer=%d\n",
                 vAddr.size(), num_proc, num_rate_limit, peer->m_id);

        m_addrman.Add(vAddrOk, peer->m_conn_ctx.addr, 2h);
        if (vAddr.size() < 1000) peer->m_getaddr_sent = false;

        // AddrFetch: Require multiple addresses to avoid disconnecting on self-announcements
        if (peer->m_conn_ctx.IsAddrFetchConn() && vAddr.size() > 1) {
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        const bool reject_tx_invs{RejectIncomingTxs(peer->m_conn_ctx)};
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.IsMsgBlk()) {
                ...
                LogPrint(BCLog::NET, "got inv: %s  %s peer=%d\n", inv.ToString(), fAlreadyHave ? "have" : "new", peer->m_id);

                UpdateBlockAvailability(peer->m_id, inv.hash);
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        if (locator.vHave.size() > MAX_LOCATOR_SZ) {
            LogPrint(BCLog::NET, "getblocks locator size %lld > %d, disconnect peer=%d\n", locator.vHave.size(), MAX_LOCATOR_SZ, peer->m_id);
            m_connman.DisconnectNode(peer->m_id);
            return;
        }
        ...
        int nLimit = 500;
        LogPrint(BCLog::NET, "getblocks %d to %s limit %d from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), nLimit, peer->m_id);
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        if (m_chainman.ActiveTip() == nullptr ||
                (m_chainman.ActiveTip()->nChainWork < m_chainman.MinimumChainWork() && !peer->m_conn_ctx.HasPermission(NetPermissionFlags::Download))) {
            LogPrint(BCLog::NET, "Ignoring getheaders from peer=%d because active chain has too little work; sending empty response\n", peer->m_id);
            // Just respond with an empty headers message, to tell the peer to
            // go away but not treat us as unresponsive.
            m_connman.PushMessage(peer->m_id, msgMaker.Make(NetMsgType::HEADERS, std::vector<CBlock>()));
            return;
        }
        ...
        LogPrint(BCLog::NET, "getheaders %d to %s from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), peer->m_id);
        for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex))
        {
        ...
        m_connman.PushMessage(peer->m_id, msgMaker.Make(NetMsgType::HEADERS, vHeaders));
        return;
    }
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        m_txrequest.ReceivedResponse(peer->m_id, txid);
        if (tx.HasWitness()) m_txrequest.ReceivedResponse(peer->m_id, wtxid);
        ...
        if (AlreadyHaveTx(GenTxid::Wtxid(wtxid))) {
            if (peer->m_conn_ctx.HasPermission(NetPermissionFlags::ForceRelay)) {
                // Always relay transactions received from peers with forcerelay
                // permission, even if they were already in the mempool, allowing
                // the node to function as a gateway for nodes hidden behind it.
                if (!m_mempool.exists(GenTxid::Txid(tx.GetHash()))) {
                    LogPrintf("Not relaying non-mempool transaction %s from forcerelay peer=%d\n", tx.GetHash().ToString(), peer->m_id);
                } else {
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            m_evictionman.UpdateLastTxTime(peer->m_id, GetTime<std::chrono::seconds>());
        ...
        else if (state.GetResult() == TxValidationResult::TX_MISSING_INPUTS)
        {
            ...
            if (!fRejectedParents) {
                ...
                if (m_orphanage.AddTx(ptx, peer->m_id)) {
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (!m_chainman.ProcessNewBlockHeaders({cmpctblock.header}, /*min_pow_checked=*/true, state, &pindex)) {
            if (state.IsInvalid()) {
                MaybePunishNodeForBlock(peer->m_id, state, /*via_compact_block=*/true, "invalid header via cmpctblock");
                return;
            }
        }
        ...
        assert(pindex);
        UpdateBlockAvailability(peer->m_id, pindex->GetBlockHash());
        ...
        while (range_flight.first != range_flight.second) {
            if (range_flight.first->second.first == peer->m_id) {
                requested_block_from_this_peer = true;
                break;
            }
            range_flight.first++;
        ...
        if (pindex->nChainWork <= m_chainman.ActiveChain().Tip()->nChainWork || // We know something better
            ...
            if (requested_block_from_this_peer) {
                ...
                m_connman.PushMessage(peer->m_id, msgMaker.Make(NetMsgType::GETDATA, vInv));
            }
            return;
        }
        ...
        if (pindex->nHeight <= m_chainman.ActiveChain().Height() + 2) {
            if ((already_in_flight < MAX_CMPCTBLOCKS_INFLIGHT_PER_BLOCK && nodestate->vBlocksInFlight.size() < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (!BlockRequested(peer->m_id, *pindex, &queuedBlockIt)) {
                ...
                if (status == READ_STATUS_INVALID) {
                    RemoveBlockRequest(pindex->GetBlockHash(), peer->m_id); // Reset in-flight state in case Misbehaving does not result in a disconnect
                    ...
                    if (first_in_flight)  {
                        ...
                        RemoveBlockRequest(pindex->GetBlockHash(), peer->m_id);
                    }
                    return;
                }
                ...
                if (req.indexes.empty()) {
                    ...
                    m_connman.PushMessage(peer->m_id, msgMaker.Make(NetMsgType::GETBLOCKTXN, req));
                } else if (peer->m_bip152_highbandwidth_to &&
                    (!peer->m_conn_ctx.IsInboundConn() ||
        ...
        if (fBlockReconstructed) {
            ...
            {
                LOCK(cs_main);
                mapBlockSource.emplace(pblock->GetHash(), std::make_pair(peer->m_id, false));
            }
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        bool min_pow_checked = false;
        {
            ...
            RemoveBlockRequest(hash, peer->m_id);
            // mapBlockSource is only used for punishing peers and setting
            // which peers send us compact blocks, so the race between here and
            // cs_main in ProcessNewBlock is fine.
            mapBlockSource.emplace(hash, std::make_pair(peer->m_id, true));
        ...
        ProcessBlock(peer->m_id, pblock, forceProcessing, min_pow_checked);
        return;
    }
    ...
    if (msg_type == NetMsgType::GETADDR) {
        ...
        std::vector<CAddress> vAddr;
        if (peer->m_conn_ctx.HasPermission(NetPermissionFlags::Addr)) {
            vAddr = m_connman.GetAddresses(MAX_ADDR_TO_SEND, MAX_PCT_ADDR_TO_SEND, /*network=*/std::nullopt);
        } else {
            vAddr = m_connman.GetAddresses(peer->m_conn_ctx, MAX_ADDR_TO_SEND, MAX_PCT_ADDR_TO_SEND);
        }
        FastRandomContext insecure_rand;
    ...
    if (msg_type == NetMsgType::MEMPOOL) {
        ...
        if (!(peer->m_our_services & NODE_BLOOM) && !peer->m_conn_ctx.HasPermission(NetPermissionFlags::Mempool))
        {
            if (!peer->m_conn_ctx.HasPermission(NetPermissionFlags::NoBan))
            {
        ...
        if (m_connman.OutboundTargetReached(false) && !peer->m_conn_ctx.HasPermission(NetPermissionFlags::Mempool))
        {
            if (!peer->m_conn_ctx.HasPermission(NetPermissionFlags::NoBan))
            {
    ...
    if (msg_type == NetMsgType::PING) {
        if (peer->m_greatest_common_version > BIP0031_VERSION) {
            uint64_t nonce = 0;
            vRecv >> nonce;
            ...
            // return very quickly.
            m_connman.PushMessage(peer->m_id, msgMaker.Make(NetMsgType::PONG, nonce));
        }
        return;
    }
            ...
            if (peer->m_ping_nonce_sent != 0) {
                if (nonce == peer->m_ping_nonce_sent) {
                    ...
                    if (ping_time.count() >= 0) {
                        ...
                        m_evictionman.UpdateMinPingTime(peer->m_id, ping_time);
                    } else {
    ...
    if (msg_type == NetMsgType::FILTERLOAD) {
        ...
        if (!filter.IsWithinSizeConstraints())
        {
            ...
            m_evictionman.UpdateLoadedBloomFilter(peer->m_id, true);
            m_evictionman.UpdateRelayTxs(peer->m_id);
        }
        return;
    }
    ...
    if (msg_type == NetMsgType::FILTERCLEAR) {
        ...
        m_evictionman.UpdateLoadedBloomFilter(peer->m_id, false);
        m_evictionman.UpdateRelayTxs(peer->m_id);
        return;
    }
    ...
    if (msg_type == NetMsgType::FEEFILTER) {
        ...
        if (MoneyRange(newFeeFilter)) {
            ...
            LogPrint(BCLog::NET, "received: feefilter of %s from peer=%d\n", CFeeRate(newFeeFilter).ToString(), peer->m_id);
        }
        return;
    }
        ...
        if (vInv.size() <= MAX_PEER_TX_ANNOUNCEMENTS + MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            for (CInv &inv : vInv) {
                if (inv.IsGenTxMsg()) {
                    ...
                    m_txrequest.ReceivedResponse(peer->m_id, inv.hash);
                }
            }
        }
        return;
...
bool PeerManagerImpl::MaybeDiscourageAndDisconnect(Peer& peer)
{
    ...
    if (peer.m_conn_ctx.IsManualConn()) {
        // We never disconnect or discourage manual peers for bad behavior
        LogPrintf("Warning: not punishing manually connected peer %d!\n", peer.m_id);
        return false;
    }

    if (peer.m_conn_ctx.addr.IsLocal()) {
        // We disconnect local peers for bad behavior but don't discourage (since that would discourage
        // all peers on the same local address)
        LogPrint(BCLog::NET, "Warning: disconnecting but not discouraging %s peer %d!\n",
                 peer.m_conn_ctx.is_inbound_onion ? "inbound onion" : "local", peer.m_id);
        m_connman.DisconnectNode(peer.m_id);
        return true;
    }
    ...
    if (m_banman) m_banman->Discourage(peer.m_conn_ctx.addr);
    m_connman.DisconnectNode(peer.m_conn_ctx.addr);
    return true;
}

bool PeerManagerImpl::ProcessMessages(NodeId id, std::atomic<bool>& interruptMsgProc)
{
    AssertLockHeld(g_msgproc_mutex);
    ...
    if (m_connman.IsDisconnected(peer->m_id))
        return false;
    ...
    if (m_connman.IsSendingPaused(peer->m_id)) return false;

    auto poll_result{m_connman.PollMessage(peer->m_id)};
    if (!poll_result) {
    ...
    try {
        ProcessMessage(peer->m_id, msg.m_type, msg.m_recv, msg.m_time, interruptMsgProc);
...
void PeerManagerImpl::ConsiderEviction(Peer& peer, std::chrono::seconds time_in_seconds)
{
    ...
    if (!state.m_chain_sync.m_protect && peer.m_conn_ctx.IsOutboundOrBlockRelayConn() && state.fSyncStarted) {
        ...
        if (state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->nChainWork >= m_chainman.ActiveChain().Tip()->nChainWork) {
            ...
            if (state.m_chain_sync.m_sent_getheaders) {
                ...
                LogPrintf("Disconnecting outbound peer %d for old chain, best known block = %s\n", peer.m_id, state.pindexBestKnownBlock != nullptr ? state.pindexBestKnownBlock->GetBlockHash().ToString() : "<none>");
                m_connman.DisconnectNode(peer.m_id);
            } else {
                ...
                MaybeSendGetHeaders(GetLocator(state.m_chain_sync.m_work_header->pprev),
                                    peer);
                LogPrint(BCLog::NET, "sending getheaders to outbound peer=%d to verify chain work (current best known block:%s, benchmark blockhash: %s)\n", peer.m_id, state.pindexBestKnownBlock != nullptr ? state.pindexBestKnownBlock->GetBlockHash().ToString() : "<none>", state.m_chain_sync.m_work_header->GetBlockHash().ToString());
...
void PeerManagerImpl::CheckForStaleTipAndEvictPeers()
{
    ...
    const auto& [block_relay_peer_to_evict, full_relay_peer_to_evict] = m_evictionman.SelectOutboundNodesToEvict(now);
    if (block_relay_peer_to_evict) {
        m_connman.DisconnectNode(*block_relay_peer_to_evict);
    }

    if (full_relay_peer_to_evict) {
        m_connman.DisconnectNode(*full_relay_peer_to_evict);
...
{
    if (m_connman.ShouldRunInactivityChecks(peer.m_conn_ctx, std::chrono::duration_cast<std::chrono::seconds>(now)) &&
        peer.m_ping_nonce_sent &&
    ...
    if (pingSend) {
        ...
        if (peer.m_greatest_common_version > BIP0031_VERSION) {
            ...
            m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::PING, nonce));
        } else {
            // Peer is too old to support ping command with nonce, pong will never arrive.
            peer.m_ping_nonce_sent = 0;
            m_connman.PushMessage(peer.m_id, msgMaker.Make(NetMsgType::PING));
        }
    }
}
...
void PeerManagerImpl::MaybeSendAddr(Peer& peer, std::chrono::microseconds current_time)
{
    ...
    if (fListen && !m_chainman.ActiveChainstate().IsInitialBlockDownload() &&
        ...
        if (std::optional<CService> local_service = GetLocalAddrForPeer(peer.m_conn_ctx, peer.GetAddrLocal())) {
    ...
    m_connman.PushMessage(peer.m_id, CNetMsgMaker(peer.m_greatest_common_version).Make(make_flags, msg_type, peer.m_addrs_to_send));
    peer.m_addrs_to_send.clear();
...
void PeerManagerImpl::MaybeSendSendHeaders(Peer& peer)
{
    ...
    if (!peer.m_sent_sendheaders && peer.m_greatest_common_version >= SENDHEADERS_VERSION) {
        LOCK(cs_main);
        CNodeState &state = *State(peer.m_id);
        if (state.pindexBestKnownBlock != nullptr &&
            ...
            // nodes)
            m_connman.PushMessage(peer.m_id, CNetMsgMaker(peer.m_greatest_common_version).Make(NetMsgType::SENDHEADERS));
...
{
    if (m_opts.ignore_incoming_txs) return;
    if (peer.m_greatest_common_version < FEEFILTER_VERSION) return;
    // peers with the forcerelay permission should not filter txs to us
    if (peer.m_conn_ctx.HasPermission(NetPermissionFlags::ForceRelay)) return;
    // Don't send feefilter messages to outbound block-relay-only peers since they should never announce
    // transactions to us, regardless of feefilter state.
    if (peer.m_conn_ctx.IsBlockOnlyConn()) return;
    ...
    if (current_time > peer.m_next_send_feefilter) {
        ...
        if (filterToSend != peer.m_fee_filter_sent) {
            m_connman.PushMessage(peer.m_id, CNetMsgMaker(peer.m_greatest_common_version).Make(NetMsgType::FEEFILTER, filterToSend));
...
bool PeerManagerImpl::RejectIncomingTxs(const ConnectionContext& conn_ctx) const
{
    // block-relay-only peers may never send txs to us
    if (conn_ctx.IsBlockOnlyConn()) return true;
    if (conn_ctx.IsFeelerConn()) return true;
    // In -blocksonly mode, peers need the 'relay' permission to send txs to us
    if (m_opts.ignore_incoming_txs && !conn_ctx.HasPermission(NetPermissionFlags::Relay)) return true;
    return false;
}
...
bool PeerManagerImpl::SendMessages(NodeId id)
{
    AssertLockHeld(g_msgproc_mutex);
    ...
    if (!m_connman.IsSuccessfullyConnected(peer->m_id) ||
        m_connman.IsDisconnected(peer->m_id))
        return true;
    ...
    if (peer->m_conn_ctx.IsAddrFetchConn() && current_time - peer->m_conn_ctx.connected > 10 * AVG_ADDRESS_BROADCAST_INTERVAL) {
    ...
    if (m_connman.IsDisconnected(peer->m_id)) return true;
...

    {
        ...
        } else if (CanServeBlocks(*peer) && !peer->m_conn_ctx.IsAddrFetchConn()) {
        ...
        if (!state.fSyncStarted && CanServeBlocks(*peer) && !m_chainman.m_blockman.LoadingBlocks()) {
            ...
            if ((nSyncStarted == 0 && sync_blocks_and_headers_from_peer) || m_chainman.m_best_header->Time() > GetAdjustedTime() - 24h) {
                ...
                if (MaybeSendGetHeaders(GetLocator(pindexStart), *peer)) {
                    LogPrint(BCLog::NET, "initial getheaders (%d) to peer=%d (startheight:%d)\n", pindexStart->nHeight, peer->m_id, peer->m_starting_height);
        ...
        //
        {
            ...
            ProcessBlockAvailability(peer->m_id); // ensure pindexBestKnownBlock is up-to-date
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.m_requested_hb_cmpctblocks) {
                    ...
                    LogPrint(BCLog::NET, "%s sending header-and-ids %s to peer=%d\n", __func__,
                            vHeaders.front().GetHash().ToString(), peer->m_id);
                    ...
                    if (cached_cmpctblock_msg.has_value()) {
                        m_connman.PushMessage(peer->m_id, std::move(cached_cmpctblock_msg.value()));
                    } else {
                        CBlock block;
                    ...
                    if (vHeaders.size() > 1) {
                        LogPrint(BCLog::NET, "%s: %u headers, range (%s, %s), to peer=%d\n", __func__,
                                ...
                                vHeaders.back().GetHash().ToString(), peer->m_id);
                    } else {
                ...
                if (!peer->m_blocks_for_headers_relay.empty()) {
                    ...
                    if (!PeerHasHeader(&state, pindex)) {
                        ...
                        LogPrint(BCLog::NET, "%s: sending inv peer=%d hash=%s\n", __func__,
                            peer->m_id, hashToAnnounce.ToString());
                    }
                }
            }
        ...
        if (auto tx_relay = peer->GetTxRelay(); tx_relay != nullptr) {
                ...
                bool fSendTrickle = peer->m_conn_ctx.HasPermission(NetPermissionFlags::NoBan);
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (current_time > state.m_downloading_since + std::chrono::seconds{consensusParams.nPowTargetSpacing} * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
                LogPrintf("Timeout downloading block %s from peer=%d%s, disconnecting\n", queuedBlock.pindex->GetBlockHash().ToString(), peer->m_id, fLogIPs ? strprintf(" peeraddr=%s", peer->m_conn_ctx.addr.ToStringAddrPort()) : "");
                m_connman.DisconnectNode(peer->m_id);
                return true;
            }
        }
        ...
        auto requestable = m_txrequest.GetRequestable(peer->m_id, current_time, &expired);
        ...
        for (const GenTxid& gtxid : requestable) {
            if (!AlreadyHaveTx(gtxid)) {
                LogPrint(BCLog::NET, "Requesting %s %s peer=%d\n", gtxid.IsWtxid() ? "wtx" : "tx",
                    gtxid.GetHash().ToString(), peer->m_id);
                vGetData.emplace_back(gtxid.IsWtxid() ? MSG_WTX : (MSG_TX | GetFetchFlags(*peer)), gtxid.GetHash());
                if (vGetData.size() >= MAX_GETDATA_SZ) {
                    m_connman.PushMessage(peer->m_id, msgMaker.Make(NetMsgType::GETDATA, vGetData));
                    vGetData.clear();
                }
                m_txrequest.RequestedTx(peer->m_id, gtxid.GetHash(), current_time + GETDATA_TX_INTERVAL);
            } else {

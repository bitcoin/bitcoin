...
struct Peer {
    ...
    /** At most one outstanding deferred cfilter request at a time. */
    std::optional<PendingCFilterRequest> m_pending_cfilter_request GUARDED_BY(NetEventsInterface::g_msgproc_mutex);
                                                 ...
                                                 BlockFilterType filter_type, const uint256& stop_hash)
{
    ...
    if (filter_index.IsRacing(&stop_index)) {
        return false;
    }
...
void PeerManagerImpl::ProcessGetCFilters(CNode& node, Peer& peer, DataStream& vRecv)
{
    ...
    if (!TryRespondToCFilterRequest(node, *filter_index, *stop_index, start_height, filter_type, stop_hash)) {
        ...
        // filter should be ready. If a request is already pending, keep the
        // older one rather than overwriting it.
        if (!peer.m_pending_cfilter_request) {
            ...
            peer.m_pending_cfilter_request = Peer::PendingCFilterRequest{filter_type, start_height, stop_hash};
        }
    }
}
...
bool PeerManagerImpl::SendMessages(CNode& node)
{
    ...
    //
    // Retry any cfilter request that raced the filter index earlier.
    //
    if (peer.m_pending_cfilter_request) {
        const Peer::PendingCFilterRequest& req = *peer.m_pending_cfilter_request;

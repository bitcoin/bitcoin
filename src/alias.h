...
bool CheckAliasInputs(const CTransaction &tx, CCoinsViewCache& view, int op, const std::vector<std::vector<unsigned char> > &vvchArgs, bool fJustCheck, int nHeight, std::string &errorMessage, bool & bDestCheckFailed,bool bSanityCheck=false);
...
bool FindAliasInTx(const CTransaction& tx, CCoinsViewCache& view, std::vector<std::vector<unsigned char> >& vvch);

        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    {
        CAddress addr;
        connman.OpenNetworkConnection(addr, /*fCountFailure=*/false, /*grant_outbound=*/{}, node_arg.data(), ConnectionType::MANUAL, use_v2transport);
        return UniValue::VNULL;
    }
        ...
        [&](const RPCHelpMan& help, const JSONRPCRequest& request) -> UniValue
{
    ...
    CSubNet subNet;
    CNetAddr netAddr;
    auto subnet_arg{request.params[0].get_str()};

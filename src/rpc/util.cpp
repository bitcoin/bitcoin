    ...
    void Push(const RPCArg& arg, const size_t current_indent = 5, const OuterType outer_type = OuterType::NONE)
    {
        ...
        case RPCArg::Type::OBJ_NAMED_PARAMS: {
            ...
            if (arg.m_opts.type_str.size() != 0 && push_name) {
                left += "\"" + arg.m_name + "\": " + arg.m_opts.type_str.at(0);
            } else {
    ...
    for (const auto& arg : m_args) {
        ...
        if (arg.m_type == RPCArg::Type::OBJ_NAMED_PARAMS) {
            for (const auto& inner : arg.m_inner) {
                auto& param_type = param_names[inner.m_name];
...
{
    auto it{std::find_if(
        m_args.begin(), m_args.end(), [&key](const auto& arg) { return arg.m_name == key;}
    )};
...
std::string RPCHelpMan::ToString() const
{
    ...
    for (size_t i{0}; i < m_args.size(); ++i) {
        ...
        sections.m_sections.emplace_back(util::ToString(i + 1) + ". " + arg.m_name, arg.ToDescriptionString(/*is_named_arg=*/true));
        ...
        if (arg.m_type == RPCArg::Type::OBJ_NAMED_PARAMS) {
            for (const auto& arg_inner : arg.m_inner) {
                named_only_sections.PushSection({arg_inner.m_name, arg_inner.ToDescriptionString(/*is_named_arg=*/true)});
...
UniValue RPCHelpMan::GetArgMap() const
{
    ...
    for (int i{0}; i < int(m_args.size()); ++i) {
        const auto& arg = m_args.at(i);
        std::vector<std::string> arg_names = SplitString(arg.m_name, '|');

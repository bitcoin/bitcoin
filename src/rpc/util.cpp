...
RPCHelpMan::RPCHelpMan(std::string name, std::string description, std::vector<RPCArg> args, RPCResults results, RPCExamples examples, RPCLegacyMethodImpl fun)
    : RPCHelpMan(std::move(name), std::move(description), std::move(args), std::move(results), std::move(examples), [fun](const RPCContext& ctx) -> UniValue {
        return fun(ctx.helpman, ctx.request);
    })
{}
...
template <> UniValue RPCParam<UniValue>::value(const UniValue& value) const
{
    return value;
}

template <> bool RPCParam<bool>::value(const UniValue& value) const
{
    return value.get_bool();
}

template <> int RPCParam<int>::value(const UniValue& value) const
{
    return value.get_int();
}

template <> uint64_t RPCParam<uint64_t>::value(const UniValue& value) const
{
    return value.get_int64();
}

template <> int64_t RPCParam<int64_t>::value(const UniValue& value) const
{
    return value.get_int64();
}

template <> double RPCParam<double>::value(const UniValue& value) const
{
    return value.get_real();
}

template <> std::string RPCParam<std::string>::value(const UniValue& value) const
{
    return value.get_str();
}

template <> uint256 RPCParam<uint256>::value(const UniValue& value) const
{
    return ParseHashV(value, m_arg.GetName());
}

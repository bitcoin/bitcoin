        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const bool background_sync{chainman.BackgroundSyncInProgress()};
    obj.pushKV("backgroundvalidation", background_sync);
    if(background_sync) {
        UniValue bValidation(UniValue::VOBJ);
        const CBlockIndex& btip{*CHECK_NONFATAL(chainman.GetBackgroundSyncTip())};
        const std::optional<int> bHeight{chainman.GetSnapshotBaseHeight()};

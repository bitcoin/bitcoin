...
UniValue CRPCTable::buildOpenRPCDoc(bool include_hidden) const
{
    ...
    for (const auto& method_name : method_names) {
        ...
        if (discriminator.has_value()) {
            ...
            CHECK_NONFATAL(result_schema.find_value("oneOf").size() == discriminator->m_values.size());
            UniValue disc_meta{UniValue::VOBJ};
            disc_meta.pushKV("parameter", discriminator->m_param_name);
            disc_meta.pushKV("parameterIndex", static_cast<int>(discriminator->m_param_index));
            UniValue vals{UniValue::VARR};
            vals.push_backV(discriminator->m_values);
            disc_meta.pushKV("values", std::move(vals));
            result_schema.pushKV("x-bitcoin-discriminated-result", std::move(disc_meta));
        }

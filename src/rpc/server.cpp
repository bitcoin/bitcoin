...
void StartRPC()
{
    LogPrint(LogInstance(), BCLog::RPC, "Starting RPC\n");
    g_rpc_running = true;
...
void InterruptRPC()
{
    ...
    std::call_once(g_rpc_interrupt_flag, []() {
        LogPrint(LogInstance(), BCLog::RPC, "Interrupting RPC\n");
...
void StopRPC()
{
    ...
    std::call_once(g_rpc_stop_flag, []() {
        LogPrint(LogInstance(), BCLog::RPC, "Stopping RPC\n");
...
void RPCRunLater(const std::string& name, std::function<void()> func, int64_t nSeconds)
{
    ...
    LogPrint(LogInstance(), BCLog::RPC, "queue run of timer %s in %i seconds (using %s)\n", name, nSeconds, timerInterface->Name());

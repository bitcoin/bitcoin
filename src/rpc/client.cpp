 ...
 * named parameter for this method, it falls back to checking the rule for the
...
UniValue RPCConvertNamedValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    for (std::string_view s: strParams) {
        ...
        if (pos == std::string_view::npos) {
            positional_args.push_back(ParseParam(rpc_convert::FromPosition(strMethod, static_cast<int>(positional_args.size())), s));
            continue;
        }
        ...
        if (!named_param) {
            const CRPCConvertParam* positional_param = rpc_convert::FromPosition(strMethod, static_cast<int>(positional_args.size()));

    ...
    return RPCMethod{
                        ...
                        {RPCResult::Type::OBJ, "", "",
                            {
                                ...
                                {RPCResult::Type::OBJ, "received", /*optional=*/true, "Information about how this transaction was received back from the network",
                                    {
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            for (const auto& tx_info : txs) {
                ...
                if (tx_info.received.has_value()) {
                    UniValue r(UniValue::VOBJ);
                    r.pushKV("address", tx_info.received->from.ToStringAddrPort());
                    r.pushKV("time", TicksSinceEpoch<std::chrono::seconds>(tx_info.received->when));
                    o.pushKV("received", std::move(r));
                }

...
static RPCMethod sendrawtransaction()
{
    return RPCMethod{
        ...
        {
            const CAmount max_burn_amount = request.params[2].isNull() ? 0*sats : AmountFromValue(request.params[2]);
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            for (const auto& tx : txns) {
                ...
                if (tx_result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
                    ...
                    if (max_raw_tx_fee != 0*sats && fee > max_raw_tx_fee) {
...
static RPCMethod submitpackage()
{
    return RPCMethod{"submitpackage",
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            if (max_raw_tx_fee_rate == CFeeRate(0*sats)) {

...
static std::map<Txid, CTransactionRef> ParsePrevTxs(const UniValue& prev_txs)
{
    ...
    for (unsigned int i = 0; i < prev_txs.size(); ++i) {
        ...
        result.emplace(tx->GetHash(), std::move(tx));
    }
    return result;
}
        ...
        "Updates all segwit inputs and outputs in a PSBT with data from output descriptors, the UTXO set, txindex, the mempool, or raw previous transactions.\n",
            {
                ...
                {"prev_txs", RPCArg::Type::ARR, RPCArg::Optional::OMITTED, "An array of raw transaction hex strings for previous transactions whose outputs are being spent in this PSBT but may not be available in the UTXO set, txindex, or the mempool", {

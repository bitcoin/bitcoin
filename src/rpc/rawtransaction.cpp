...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the dobbscoin address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
    ...
    for (const std::string& name_ : addrList) {
        ...
        if (name_ == "data") {
            ...
            if (!IsValidDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Dobbscoin address: ") + name_);
            }

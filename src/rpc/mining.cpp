        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (size_t i = 0; i < raw_txs_or_txids.size(); i++) {
        ...
        if (auto txid{Txid::FromHex(str)}) {
            ...
            throw JSONRPCError(RPC_DESERIALIZATION_ERROR, strprintf("TX decode failed for tx %d. Make sure the transaction is complete, correctly serialized, hex-encoded, and has at least one input.", i));
        }
    }

...
static UniValue generateBlocks(ChainstateManager& chainman, CCreditPoolManager& cpoolman, CEvoDB& evodb, CChainstateHelper& chain_helper, CMNHFManager& mnhfman,
                               LLMQContext& llmq_ctx, const CTxMemPool& mempool, const CScript& coinbase_script, int nGenerate, uint64_t nMaxTries)
{
    int nHeightEnd = 0;
    int nHeight = 0;
    ...
    {
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(chainman.ActiveChainstate(), cpoolman, evodb, chain_helper, mnhfman, llmq_ctx, mempool, Params()).CreateNewBlock(coinbase_script));
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    CBlock block;
    {
        ...
        std::unique_ptr<CBlockTemplate> blocktemplate(BlockAssembler(active_chainstate, *node.cpoolman, *node.evodb, *node.chain_helper, *node.mnhf_manager, llmq_ctx, empty_mempool, chainparams).CreateNewBlock(coinbase_script));
        if (!blocktemplate) {
            throw JSONRPCError(RPC_INTERNAL_ERROR, "Couldn't create new block");
        }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        pblocktemplate = BlockAssembler(active_chainstate, *node.cpoolman, *node.evodb, *node.chain_helper, *node.mnhf_manager, llmq_ctx, mempool, Params()).CreateNewBlock(scriptDummy);
        if (!pblocktemplate)

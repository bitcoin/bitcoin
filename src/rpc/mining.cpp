...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    GenerateSyscoins(fGenerate, nGenProcLimit, Params(), *g_connman);
...
UniValue getauxblock(const UniValue& params, bool fHelp)
{
	...
	if (params.size() == 0)
	{
		...
		// Update block
		{
					...
					&& GetTime() - nStart > 60))
			{
				...
				std::unique_ptr<CBlockTemplate> newBlock(CreateNewBlock(Params(), coinbaseScript->reserveScript));
				if (!newBlock)
					throw JSONRPCError(RPC_OUT_OF_MEMORY, "out of memory");
	...
	submitblock_StateCatcher sc(block.GetHash());
	RegisterValidationInterface(&sc);
	bool fAccepted = ProcessNewBlock(Params(), &block,
		true, nullptr, NULL);
	UnregisterValidationInterface(&sc);

...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total liberta balance of the wallet\n"
            "  \"money supply\": xxxxxx,     (numeric) the current number of blocks processed in the server\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"libertaaddress\"\n"
            "\nReturn information about the given liberta address.\n"
            "\nArguments:\n"
            "1. \"libertaaddress\"     (string, required) The liberta address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"libertaaddress\", (string) The liberta address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Libertaaddress and we have full public key:
        CLibertaAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are liberta addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) liberta address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CLibertaAddress address(innerID);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"libertaaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"libertaaddress\"  (string, required) The liberta address to use for the signature.\n"

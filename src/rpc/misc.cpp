...
UniValue createhtlc(const UniValue& params, bool fHelp)
{
    ...
    CPubKey seller_key;
    CPubKey refund_key;
    _publickey_from_string(params[0].get_str(), seller_key);
    _publickey_from_string(params[1].get_str(), refund_key);

    std::string hs = params[2].get_str();
    ...
    uint32_t blocks;
    // This will make params[3] forward compatible with '10h'/'3d' style timeout
    // values if it is extended in the future.
    {
        ...
        UniValue timeout;
        if (!timeout.read(std::string("[")+params[3].get_str()+std::string("]")) ||
            !timeout.isArray() || timeout.size()!=1)
            throw runtime_error(string("Error parsing JSON:")+params[3].get_str());

    ...
    const btck_BlockHeader* header)
{
    try {
        auto& chainman = btck_ChainstateManager::get(chainstate_manager).m_chainman;
        auto state{std::make_unique<BlockValidationState>()};
        auto header_accepted = chainman->ProcessNewBlockHeaders({&btck_BlockHeader::get(header), 1}, /*min_pow_checked=*/true, *state, /*ppindex=*/nullptr);
        assert(header_accepted == state->IsValid());
        return btck_BlockValidationState::ref(state.release());
    } catch (const std::exception& e) {

 ...
 * user_data freed through the destroy callback if one was provided. After this
 * call, @ref btck_logging_set_callback must not be called anymore.
 */
 ...
 * is not thread-safe and must not be called concurrently with itself or with
 * @ref btck_logging_disable. Log messages produced before this function is first
 * called are buffered and on calling this function are logged immediately.
 * Calling this function again replaces the previous callback. Any previously set
 * user_data will be freed through its destroy callback if one was provided,
 * also when setting the new callback fails.
 *
 ...
 *                                       to the logger, also when this function fails.

...
public:
     ...
     */
    util::Expected<void, kernel::FatalError> FlushStateToDisk(
        FlushStateMode mode,
     ...
     *
     * @returns a kernel::FatalError if a system error occurred
     */
    util::Expected<void, kernel::FatalError> ActivateBestChain(
    ...
    DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
        EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    util::Expected<BlockValidationState, kernel::FatalError> ConnectBlock(const CBlock& block, CBlockIndex* pindex,
     ...
     */
    util::Expected<void, kernel::FatalError> PreciousBlock(CBlockIndex* pindex)
        EXCLUSIVE_LOCKS_REQUIRED(!m_chainstate_mutex)
        LOCKS_EXCLUDED(::cs_main);
...
protected:
    util::Expected<void, kernel::FatalError> ActivateBestChainStep(CBlockIndex& index_most_work, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, std::vector<ConnectedBlock>& connected_blocks) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool->cs);
    util::Expected<BlockValidationState, kernel::FatalError> ConnectTip(
...
public:
    ...
    * even though that may not actually be the case. */
     ...
     */
    util::Expected<BlockValidationState, kernel::FatalError> ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block) LOCKS_EXCLUDED(cs_main);
     ...
     *
     * @returns util::Unexpected<kernel::FatalError> on fatal error; otherwise a BlockValidationState,
     *          valid if the block was accepted, invalid if it failed validation
     */
    util::Expected<BlockValidationState, kernel::FatalError> AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked) EXCLUSIVE_LOCKS_REQUIRED(cs_main);

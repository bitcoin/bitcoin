...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, const CAmount &nFee, CAmount &blockReward, CTxOut& txoutMasternodeRet)
{
	...
	if (!mnpayments.GetBlockPayee(nBlockHeight, payee)) {
		...
		payee = GetScriptForDestination(mnInfo.pubKeyCollateralAddress.Get());
	}
	else
...
bool CMasternodePayments::IsScheduled(CMasternode& mn, int nNotBlockHeight)
{
    ...
    CScript mnpayee;
    mnpayee = GetScriptForDestination(mn.pubKeyCollateralAddress.Get());
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight, CConnman& connman)
{
    ...
    CScript payee = GetScriptForDestination(mnInfo.pubKeyCollateralAddress.Get());

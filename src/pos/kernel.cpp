...
bool CheckStakeKernelHash(const CBlockIndex *pindexPrev,
    ...
    const CTransaction &txPrev, const COutPoint &prevout, unsigned int nTime,
    uint256 &hashProofOfStake, uint256 &targetProofOfStake,
    bool fPrintProofOfStake)
{
    // CheckStakeKernelHashV2
    
    if (nTime < blockFrom.nTime)  // Transaction timestamp violation
    ...
    CDataStream ss(SER_GETHASH, 0);
    ss << bnStakeModifier;
    ss << blockFrom.nTime << prevout.hash << prevout.n << nTime;
    ...
    if (fPrintProofOfStake)
    {
        ...
        LogPrintf("%s: check modifier=%s nTimeKernel=%u nPrevout=%u nTime=%u hashProof=%s\n",
            ...
            blockFrom.nTime, prevout.n, nTime,
...
bool CheckProofOfStake(const CBlockIndex *pindexPrev, const CTransaction &tx, int64_t nTime, unsigned int nBits, uint256 &hashProofOfStake, uint256 &targetProofOfStake)
{
    ...
    if (!CheckStakeKernelHash(pindexPrev, nBits, blockKernel,
        *txKernel.get(), txin.prevout, nTime, hashProofOfStake, targetProofOfStake, fDebug))
...
bool CheckKernel(const CBlockIndex *pindexPrev, unsigned int nBits, int64_t nTime, const COutPoint &prevout, int64_t *pBlockTime)
{
    ...
    // pindexPrev is the previous block in the chain
    // block is the block containing prevout
    CBlock block;
    CTransactionRef tx;
    ...
    return CheckStakeKernelHash(pindexPrev, nBits, block,
        *tx.get(), prevout, nTime, hashProofOfStake, targetProofOfStake);
}

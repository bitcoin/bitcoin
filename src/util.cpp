...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bitzeny.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bitzenyd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BitZeny will not work properly.");

...
#else
    const char* pszModule = "cosbycoin";
#endif
    if (pex)
...
#ifdef GUI
    if (wxTheApp && !fDaemon)
        MyMessageBox(pszMessage, "cosbycoin", wxOK | wxICON_ERROR);
#endif
    throw;
}
...
{
    // Windows: C:\Documents and Settings\username\Application Data\cosbycoin
    // Mac: ~/Library/Application Support/cosbycoin
    // Unix: ~/.cosbycoin
#ifdef __WXMSW__
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) + "\\cosbycoin";
#else
...
string GetConfigFile()
{
    ...
    fs::path pathConfig(GetArg("-conf", "cosbycoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override cosbycoin.conf
...
{
    namespace fs = boost::filesystem;
    fs::path pathConfig(GetArg("-pid", "cosbycoind.pid"));
    if (!pathConfig.is_complete())
        pathConfig = fs::path(GetDataDir()) / pathConfig;
    return pathConfig.string();
}
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong cosbycoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    boost::thread(boost::bind(ThreadSafeMessageBox, strMessage+" ", string("cosbycoin"), wxOK | wxICON_EXCLAMATION, (wxWindow*)NULL, -1, -1));
                }
            }
        }

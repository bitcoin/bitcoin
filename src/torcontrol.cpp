...
void TorControlConnection::readcb(struct bufferevent *bev, void *ctx)
{
        ...
        if (ch == ' ') {
            ...
            if (self->message.code >= 600) {
                ...
                if (!self->reply_handlers.empty()) {
                    ...
                    LogPrint(LogInstance(), BCLog::TOR, "Received unexpected sync reply %i\n", self->message.code);
                }
            }
            self->message.Clear();
        }
    ...
    if (evbuffer_get_length(input) > MAX_LINE_LENGTH) {
        LogPrintf(LogInstance(), "tor: Disconnecting because MAX_LINE_LENGTH exceeded\n");
        self->Disconnect();
    }
}
...
void TorControlConnection::eventcb(struct bufferevent *bev, short what, void *ctx)
{
    ...
    if (what & BEV_EVENT_CONNECTED) {
        LogPrint(LogInstance(), BCLog::TOR, "Successfully connected!\n");
        self->connected(*self);
    } else if (what & (BEV_EVENT_EOF|BEV_EVENT_ERROR)) {
        if (what & BEV_EVENT_ERROR) {
            LogPrint(LogInstance(), BCLog::TOR, "Error connecting to Tor control socket\n");
        } else {
            LogPrint(LogInstance(), BCLog::TOR, "End of stream\n");
        }
        self->Disconnect();
...
bool TorControlConnection::Connect(const std::string& tor_control_center, const ConnectionCB& _connected, const ConnectionCB& _disconnected)
{
    ...
    if (!control_service.has_value()) {
        LogPrintf(LogInstance(), "tor: Failed to look up control center %s\n", tor_control_center);
        return false;
    }
    ...
    if (!control_service.value().GetSockAddr(reinterpret_cast<struct sockaddr*>(&control_address), &control_address_len)) {
        LogPrintf(LogInstance(), "tor: Error parsing socket address %s\n", tor_control_center);
        return false;
    }
    ...
    if (bufferevent_socket_connect(b_conn, reinterpret_cast<struct sockaddr*>(&control_address), control_address_len) < 0) {
        LogPrintf(LogInstance(), "tor: Error connecting to address %s\n", tor_control_center);
        return false;
    }
    return true;
}
    ...
    m_target(target)
{
    ...
    if (!reconnect_ev)
        LogPrintf(LogInstance(), "tor: Failed to create event for reconnection: out of memory?\n");
    // Start connection attempts immediately
    if (!conn.Connect(m_tor_control_center, std::bind(&TorController::connected_cb, this, std::placeholders::_1),
         std::bind(&TorController::disconnected_cb, this, std::placeholders::_1) )) {
        LogPrintf(LogInstance(), "tor: Initiating connection to Tor control port %s failed\n", m_tor_control_center);
    }
    // Read service private key if cached
    std::pair<bool,std::string> pkf = ReadBinaryFile(GetPrivateKeyFile());
    if (pkf.first) {
        LogPrint(LogInstance(), BCLog::TOR, "Reading cached private key from %s\n", fs::PathToString(GetPrivateKeyFile()));
...
void TorController::get_socks_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    ...
    if (reply.code == 250) {
        ...
        if (!socks_location.empty()) {
            LogPrint(LogInstance(), BCLog::TOR, "Get SOCKS port command yielded %s\n", socks_location);
        } else {
            LogPrintf(LogInstance(), "tor: Get SOCKS port command returned nothing\n");
        }
    } else if (reply.code == 510) {  // 510 Unrecognized command
        LogPrintf(LogInstance(), "tor: Get SOCKS port command failed with unrecognized command (You probably should upgrade Tor)\n");
    } else {
        LogPrintf(LogInstance(), "tor: Get SOCKS port command failed; error code %d\n", reply.code);
    }
    ...
    Assume(resolved.IsValid());
    LogPrint(LogInstance(), BCLog::TOR, "Configuring onion proxy for %s\n", resolved.ToStringAddrPort());
...
{
    if (reply.code == 250) {
        LogPrint(LogInstance(), BCLog::TOR, "ADD_ONION successful\n");
        ...
        if (service_id.empty()) {
            LogPrintf(LogInstance(), "tor: Error parsing ADD_ONION parameters:\n");
            for (const std::string &s : reply.lines) {
        ...
        LogInfo(LogInstance(), "Got tor service ID %s, advertising service %s\n", service_id, service.ToStringAddrPort());
        if (WriteBinaryFile(GetPrivateKeyFile(), private_key)) {
            LogPrint(LogInstance(), BCLog::TOR, "Cached service private key to %s\n", fs::PathToString(GetPrivateKeyFile()));
        } else {
            LogPrintf(LogInstance(), "tor: Error writing service private key to %s\n", fs::PathToString(GetPrivateKeyFile()));
        }
        AddLocal(service, LOCAL_MANUAL);
        // ... onion requested - keep connection open
    } else if (reply.code == 510) { // 510 Unrecognized command
        LogPrintf(LogInstance(), "tor: Add onion failed with unrecognized command (You probably need to upgrade Tor)\n");
    } else {
        LogPrintf(LogInstance(), "tor: Add onion failed; error code %d\n", reply.code);
    }
}
...
{
    if (reply.code == 250) {
        LogPrint(LogInstance(), BCLog::TOR, "Authentication successful\n");
        ...
        LogPrintf(LogInstance(), "tor: Authentication failed\n");
    }
}
...
{
    if (reply.code == 250) {
        LogPrint(LogInstance(), BCLog::TOR, "SAFECOOKIE authentication challenge successful\n");
        std::pair<std::string,std::string> l = SplitTorReplyLine(reply.lines[0]);
        if (l.first == "AUTHCHALLENGE") {
            std::map<std::string,std::string> m = ParseTorReplyMapping(l.second);
            if (m.empty()) {
                LogPrintf(LogInstance(), "tor: Error parsing AUTHCHALLENGE parameters: %s\n", SanitizeString(l.second));
                return;
            }
            std::vector<uint8_t> serverHash = ParseHex(m["SERVERHASH"]);
            std::vector<uint8_t> serverNonce = ParseHex(m["SERVERNONCE"]);
            LogPrint(LogInstance(), BCLog::TOR, "AUTHCHALLENGE ServerHash %s ServerNonce %s\n", HexStr(serverHash), HexStr(serverNonce));
            if (serverNonce.size() != 32) {
                LogPrintf(LogInstance(), "tor: ServerNonce is not 32 bytes, as required by spec\n");
                return;
            }

            std::vector<uint8_t> computedServerHash = ComputeResponse(TOR_SAFE_SERVERKEY, cookie, clientNonce, serverNonce);
            if (computedServerHash != serverHash) {
                LogPrintf(LogInstance(), "tor: ServerHash %s does not match expected ServerHash %s\n", HexStr(serverHash), HexStr(computedServerHash));
                return;
            }
            ...
            LogPrintf(LogInstance(), "tor: Invalid reply to AUTHCHALLENGE\n");
        }
    } else {
        LogPrintf(LogInstance(), "tor: SAFECOOKIE authentication challenge failed\n");
    }
}
    ...
    if (reply.code == 250) {
        ...
        for (const std::string &s : reply.lines) {
            ...
            if (l.first == "AUTH") {
                ...
                if ((i = m.find("Tor")) != m.end()) {
                    LogPrint(LogInstance(), BCLog::TOR, "Connected to Tor version %s\n", i->second);
                }
            }
        }
        for (const std::string &s : methods) {
            LogPrint(LogInstance(), BCLog::TOR, "Supported authentication method: %s\n", s);
        }
        ...
        if (!torpassword.empty()) {
            if (methods.count("HASHEDPASSWORD")) {
                LogPrint(LogInstance(), BCLog::TOR, "Using HASHEDPASSWORD authentication\n");
                ReplaceAll(torpassword, "\"", "\\\"");
                _conn.Command("AUTHENTICATE \"" + torpassword + "\"", std::bind(&TorController::auth_cb, this, std::placeholders::_1, std::placeholders::_2));
            } else {
                LogPrintf(LogInstance(), "tor: Password provided with -torpassword, but HASHEDPASSWORD authentication is not available\n");
            }
        } else if (methods.count("NULL")) {
            LogPrint(LogInstance(), BCLog::TOR, "Using NULL authentication\n");
            _conn.Command("AUTHENTICATE", std::bind(&TorController::auth_cb, this, std::placeholders::_1, std::placeholders::_2));
        } else if (methods.count("SAFECOOKIE")) {
            // Cookie: hexdump -e '32/1 "%02x""\n"'  ~/.tor/control_auth_cookie
            LogPrint(LogInstance(), BCLog::TOR, "Using SAFECOOKIE authentication, reading cookie authentication from %s\n", cookiefile);
            ...
            if (status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE) {
                ...
                if (status_cookie.first) {
                    LogPrintf(LogInstance(), "tor: Authentication cookie %s is not exactly %i bytes, as is required by the spec\n", cookiefile, TOR_COOKIE_SIZE);
                } else {
                    LogPrintf(LogInstance(), "tor: Authentication cookie %s could not be opened (check permissions)\n", cookiefile);
                }
            }
        } else if (methods.count("HASHEDPASSWORD")) {
            LogPrintf(LogInstance(), "tor: The only supported authentication mechanism left is password, but no password provided with -torpassword\n");
        } else {
            LogPrintf(LogInstance(), "tor: No supported authentication method\n");
        }
    } else {
        LogPrintf(LogInstance(), "tor: Requesting protocol info failed\n");
    }
}
...
void TorController::connected_cb(TorControlConnection& _conn)
{
    ...
    if (!_conn.Command("PROTOCOLINFO 1", std::bind(&TorController::protocolinfo_cb, this, std::placeholders::_1, std::placeholders::_2)))
        LogPrintf(LogInstance(), "tor: Error sending initial protocolinfo command\n");
}
...
void TorController::disconnected_cb(TorControlConnection& _conn)
{
    ...
    LogPrint(LogInstance(), BCLog::TOR, "Not connected to Tor control port %s, trying to reconnect\n", m_tor_control_center);
...
void TorController::Reconnect()
{
    ...
    if (!conn.Connect(m_tor_control_center, std::bind(&TorController::connected_cb, this, std::placeholders::_1),
         std::bind(&TorController::disconnected_cb, this, std::placeholders::_1) )) {
        LogPrintf(LogInstance(), "tor: Re-initiating connection to Tor control port %s failed\n", m_tor_control_center);
    }
}
...
void StartTorControl(CService onion_service_target)
{
    ...
    if (!gBase) {
        LogPrintf(LogInstance(), "tor: Unable to create event_base\n");
        return;
    }
...
{
    if (gBase) {
        LogPrintf(LogInstance(), "tor: Thread interrupt\n");

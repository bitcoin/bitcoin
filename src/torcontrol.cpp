    ...
    m_target(target)
{
    ...
    if (!m_conn.Connect(m_tor_control_center, std::bind(&TorController::connected_cb, this, std::placeholders::_1),
...
void TorController::authchallenge_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == TOR_REPLY_OK) {
        ...
        if (l.first == "AUTHCHALLENGE") {
            ...
            LogDebug(BCLog::TOR, "AUTHCHALLENGE ServerHash %s ServerNonce %s", HexStr(serverHash), HexStr(serverNonce));
            if (serverNonce.size() != 32) {
                LogWarning("tor: ServerNonce is not 32 bytes, as required by spec");
                return;
            }

            std::vector<uint8_t> computedServerHash = ComputeResponse(TOR_SAFE_SERVERKEY, m_cookie, m_client_nonce, serverNonce);
            if (computedServerHash != serverHash) {
                LogWarning("tor: ServerHash %s does not match expected ServerHash %s", HexStr(serverHash), HexStr(computedServerHash));
                return;
            }

            std::vector<uint8_t> computedClientHash = ComputeResponse(TOR_SAFE_CLIENTKEY, m_cookie, m_client_nonce, serverNonce);
    ...
    if (reply.code == TOR_REPLY_OK) {
        ...
        if (!torpassword.empty()) {
            ...
            if (status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE) {
                ...
                _conn.Command("AUTHCHALLENGE SAFECOOKIE " + HexStr(m_client_nonce), std::bind(&TorController::authchallenge_cb, this, std::placeholders::_1, std::placeholders::_2));
            } else {

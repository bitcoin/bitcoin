...
=======
            "importprivkey \"crowncoinprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"crowncoinprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
Value importwallet(const Array& params, bool fHelp)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CCrowncoinAddress(keyid).ToString());
            continue;
        }
...
>>>>>>> origin/dirty-merge-dash-0.11.0
    ...
    return CCrowncoinSecret(vchSecret).ToString();
}
...
=======
    file << strprintf("# Wallet dump created by Crowncoin %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
>>>>>>> origin/dirty-merge-dash-0.11.0
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CCrowncoinAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            if (pwalletMain->mapAddressBook.count(keyid)) {
                file << strprintf("%s %s label=%s # addr=%s\n", CCrowncoinSecret(key).ToString(), strTime, EncodeDumpString(pwalletMain->mapAddressBook[keyid].name), strAddr);
            } else if (setKeyPool.count(keyid)) {
                file << strprintf("%s %s reserve=1 # addr=%s\n", CCrowncoinSecret(key).ToString(), strTime, strAddr);
            } else {

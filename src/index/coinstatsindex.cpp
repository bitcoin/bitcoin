...
bool CoinStatsIndex::CustomAppend(const interfaces::BlockInfo& block)
{
    ...
    if (block.height > 0) {
        ...
        if (read_out.first != expected_block_hash) {
            LogPrintf(LogInstance(), "WARNING: previous block header belongs to unexpected block %s; expected %s\n",
                      read_out.first.ToString(), expected_block_hash.ToString());

            if (!m_db->Read(DBHashKey(expected_block_hash), read_out)) {
                LogError(LogInstance(), "%s: previous block header not found; expected %s\n",
...

    {
        ...
        do {
            ...
            if (!m_chainstate->m_blockman.ReadBlockFromDisk(block, *iter_tip)) {
                LogError(LogInstance(), "%s: Failed to read block %s from disk\n",

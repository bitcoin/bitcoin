...
void BaseIndex::Sync()
{
    ...
    if (!m_synced) {
        ...
        while (true) {
            if (m_interrupt) {
                LogPrintf(LogInstance(), "%s: m_interrupt set; exiting ThreadSync\n", GetName());
            ...
            if (last_log_time + SYNC_LOG_INTERVAL < current_time) {
                LogPrintf(LogInstance(), "Syncing %s with block chain from height %d\n",
    ...
    if (pindex) {
        LogPrintf(LogInstance(), "%s is enabled at height %d\n", GetName(), pindex->nHeight);
    } else {
        LogPrintf(LogInstance(), "%s is enabled\n", GetName());
    }
}
...
bool BaseIndex::Commit()
{
    ...
    if (!ok) {
        LogError(LogInstance(), "%s: Failed to commit latest %s state\n", __func__, GetName());
        return false;
    }
    return true;
}
...
void BaseIndex::BlockConnected(ChainstateRole role, const std::shared_ptr<const CBlock>& block, const CBlockIndex* pindex)
{
    ...
    if (!best_block_index) {
        ...
        if (best_block_index->GetAncestor(pindex->nHeight - 1) != pindex->pprev) {
            LogPrintf(LogInstance(), "%s: WARNING: Block %s does not connect to an ancestor of "
...
void BaseIndex::ChainStateFlushed(ChainstateRole role, const CBlockLocator& locator)
{
    ...
    if (best_block_index->GetAncestor(locator_tip_index->nHeight) != locator_tip_index) {
        LogPrintf(LogInstance(), "%s: WARNING: Locator contains block (hash=%s) not on known best "
...
bool BaseIndex::BlockUntilSyncedToCurrentChain() const
{
    ...
    LogPrintf(LogInstance(), "%s: %s is catching up on block notifications\n", __func__, GetName());

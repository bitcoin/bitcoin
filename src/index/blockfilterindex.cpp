...
bool BlockFilterIndex::CustomInit(const std::optional<interfaces::BlockKey>& block)
{
    if (!m_db->Read(DB_FILTER_POS, m_next_filter_pos)) {
        ...
        if (m_db->Exists(DB_FILTER_POS)) {
            LogError(LogInstance(), "%s: Cannot read current %s state; index may be corrupted\n",
                         __func__, GetName());
            return false;
        }
    ...
    if (block) {
        ...
        if (!op_last_header) {
            LogError(LogInstance(), "Cannot read last block filter header; index may be corrupted\n");
            return false;
        }
...
bool BlockFilterIndex::CustomCommit(CDBBatch& batch)
{
    ...
    if (file.IsNull()) {
        LogError(LogInstance(), "%s: Failed to open filter file %d\n", __func__, pos.nFile);
        return false;
    }
    if (!FileCommit(file.Get())) {
        LogError(LogInstance(), "%s: Failed to commit filter file %d\n", __func__, pos.nFile);
        return false;
    }
...
bool BlockFilterIndex::ReadFilterFromDisk(const FlatFilePos& pos, const uint256& hash, BlockFilter& filter) const
{
    ...
    try {
        ...
        if (Hash(encoded_filter) != hash) {
            LogError(LogInstance(), "Checksum mismatch in filter decode.\n");
            return false;
        }
        filter = BlockFilter(GetFilterType(), block_hash, std::move(encoded_filter), /*skip_decode_check=*/true);
    }
    catch (const std::exception& e) {
        LogError(LogInstance(), "%s: Failed to deserialize block filter from disk: %s\n", __func__, e.what());
        return false;
    }
...
size_t BlockFilterIndex::WriteFilterToDisk(FlatFilePos& pos, const BlockFilter& filter)
{
    ...
    if (pos.nPos + data_size > MAX_FLTR_FILE_SIZE) {
        ...
        if (last_file.IsNull()) {
            LogPrintf(LogInstance(), "%s: Failed to open filter file %d\n", __func__, pos.nFile);
            return 0;
        }
        if (!TruncateFile(last_file.Get(), pos.nPos)) {
            LogPrintf(LogInstance(), "%s: Failed to truncate filter file %d\n", __func__, pos.nFile);
            return 0;
        }
        if (!FileCommit(last_file.Get())) {
            LogPrintf(LogInstance(), "%s: Failed to commit filter file %d\n", __func__, pos.nFile);
            return 0;
        }
    ...
    if (out_of_space) {
        LogPrintf(LogInstance(), "%s: out of disk space\n", __func__);
        return 0;
    }
...
std::optional<uint256> BlockFilterIndex::ReadFilterHeader(int height, const uint256& expected_block_hash)
{
    ...
    if (read_out.first != expected_block_hash) {
        LogError(LogInstance(), "%s: previous block header belongs to unexpected block %s; expected %s\n",
                                       ...
                                       int start_height, int stop_height)
{
    ...
    for (int height = start_height; height <= stop_height; ++height) {
        if (!db_it.GetKey(key) || key.height != height) {
            LogError(LogInstance(), "%s: unexpected key in %s: expected (%c, %d)\n",
        ...
        if (!db_it.GetValue(value)) {
            LogError(LogInstance(), "%s: unable to read value in %s at key (%c, %d)\n",
                         __func__, index_name, DB_BLOCK_HEIGHT, height);
            return false;
        }
...
{
    if (start_height < 0) {
        LogError(LogInstance(), "%s: start height (%d) is negative\n", __func__, start_height);
        return false;
    }
    if (start_height > stop_index->nHeight) {
        LogError(LogInstance(), "%s: start height (%d) is greater than stop height (%d)\n",
    ...
    for (const CBlockIndex* block_index = stop_index;
        ...
        if (!db.Read(DBHashKey(block_hash), results[i])) {
            LogError(LogInstance(), "%s: unable to read value in %s at key (%c, %s)\n",

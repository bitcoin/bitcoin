...
public:
	...
	inline bool IsNotNull() const { return (!vchEscrow.empty() || !txHash.IsNull() || nHeight != 0 || nPaymentOption != 0 || nQty != 0 || !buyerAliasTuple.first.empty() || !arbiterAliasTuple.first.empty() || !sellerAliasTuple.first.empty() || !offerTuple.first.empty() || !scriptSigs.empty() || !vchRedeemScript.empty()|| role != 0 || nAmountOrBidPerUnit != 0 || !vchWitness.empty() || fBidPerUnit != 0 || nDeposit != 0 || nArbiterFee != 0 || nShipping != 0 || nCommission != 0 || nNetworkFee != 0 || nWitnessFee != 0 || !extTxId.IsNull() || bBuyNow || bPaymentAck || !redeemTxId.IsNull() || !linkSellerAliasTuple.first.empty() || !linkAliasTuple.first.empty() || feedback.IsNotNull() || op != 0); }

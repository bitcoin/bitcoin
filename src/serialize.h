...
template <typename Stream, typename E> void Serialize(Stream& s, Span<E> span) { s.write(AsBytes(span)); } // template on E to require the call site to explicitly pass a Span{} type
...
template <typename Stream, typename E> void Unserialize(Stream& s, Span<E> span) { s.read(AsWritableBytes(span)); } // template on E to require the call site to explicitly pass a Span{} type

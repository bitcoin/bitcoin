...
template<typename Stream, typename T, typename A> void Unserialize_impl(Stream& is, std::vector<T, A>& v, const unsigned char&, size_t maxSize = 0);
template<typename Stream, typename T, typename A, typename V> void Unserialize_impl(Stream& is, std::vector<T, A>& v, const V&, size_t maxSize = 0);
template<typename Stream, typename T, typename A> inline void Unserialize(Stream& is, std::vector<T, A>& v, size_t maxSize = 0);
...
LimitedVector<T, A, N> WrapLimitedVector(std::vector<T, A>& v) { return LimitedVector<T, A, N>(v); }
...
template<typename Stream, typename T, typename A>
void Unserialize_impl(Stream& is, std::vector<T, A>& v, const unsigned char&, size_t maxSize)
{
    //// Limit size per read so bogus size value won't cause out of memory
    v.clear();
    unsigned int nSize = ReadCompactSize(is);
...
template<typename Stream, typename T, typename A, typename V>
void Unserialize_impl(Stream& is, std::vector<T, A>& v, const V&, size_t maxSize)
{
    v.clear();
    unsigned int nSize = ReadCompactSize(is);
    unsigned int i = 0;
...
template<typename Stream, typename T, typename A>
inline void Unserialize(Stream& is, std::vector<T, A>& v, size_t maxSize)
{
    Unserialize_impl(is, v, T());
}
...
// GetSerializeType for data stream. Bitcoin Core delete GetType() on da74db0940
template <typename Stream>

    ...
    if(strCommand == NetMsgType::DSACCEPT) {
        ...
        //check it like a transaction
        {
...

            {
                ...
                if(!AcceptToMemoryPool(mempool, false, validationState, CTransaction(tx), false, NULL, false, true, true)) {
...
void CPrivateSendServer::CommitFinalTransaction(CConnman& connman)
{
...

    {
        ...
        if(!lockMain || !AcceptToMemoryPool(mempool, false, validationState, finalTransaction, false, NULL, false, true, true))
        {
...
void CPrivateSendServer::ChargeFees(CConnman& connman)
{
    ...
    if(nState == POOL_STATE_ACCEPTING_ENTRIES || nState == POOL_STATE_SIGNING) {
        ...
        bool fMissingInputs;
        if(!AcceptToMemoryPool(mempool, false, state, vecOffendersCollaterals[0], false, &fMissingInputs, false, true)) {
...
void CPrivateSendServer::ChargeRandomFees(CConnman& connman)
{
    ...
    BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollaterals) {
        ...
        CValidationState state;
        bool fMissingInputs;
        if(!AcceptToMemoryPool(mempool, false, state, txCollateral, false, &fMissingInputs, false, true)) {
            // should never really happen

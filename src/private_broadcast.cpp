    ...
    EXCLUSIVE_LOCKS_REQUIRED(!m_mutex)
{
    ...
    auto with_attempts_remaining{m_transactions | std::views::filter([this](const auto& entry) { return IsPending(entry.second); })};
    const auto it{std::ranges::max_element(
            with_attempts_remaining,
            [](const auto& a, const auto& b) { return a < b; },
            [](const auto& el) { return DerivePriority(el.second.send_statuses); })};

    if (it != with_attempts_remaining.end()) {
        auto& [tx, state]{*it};

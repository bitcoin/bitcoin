...
private:
    ecgroup_order() : pgroup(EC_GROUP_new_by_curve_name(NID_secp256k1)) {}
    ~ecgroup_order() { EC_GROUP_free(pgroup); }
    EC_GROUP *pgroup;
};
 ...
 */
int ECDSA_SIG_recover_key_GFp(EC_KEY *eckey,
    ECDSA_SIG *ecsig,
...
CECKey::~CECKey() { EC_KEY_free(pkey); }

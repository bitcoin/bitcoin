...
void ValidationSignals::MempoolTransactionsRemovedForBlock(const std::vector<CTransactionRef>& block_txs, const std::vector<RemovedMempoolTransactionInfo>& txs_removed_for_block, unsigned int block_height, const uint256& block_hash)
{
    auto log_msg = LOG_MSG("%s: block hash=%s block height=%s txs removed=%s block txs=%s", __func__,
                           ...
                           block_txs.size());
    auto event = [block_txs, txs_removed_for_block, block_height, block_hash, this] {
        m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.MempoolTransactionsRemovedForBlock(block_txs, txs_removed_for_block, block_height, block_hash); });
    };
    ENQUEUE_AND_LOG_EVENT(std::move(event), std::move(log_msg));
}

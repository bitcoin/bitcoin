...
void CMainSignals::NotifyCheckNEVMBlob(const CNEVMData &nevmData, BlockValidationState &state) {
    m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.NotifyCheckNEVMBlob(nevmData, state);});
}

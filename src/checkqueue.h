...
{
private:
    //! Whether T's checks defer their Schnorr signatures into a batch that has
    //! to be verified before the queue's result is final.
    static constexpr bool is_batched{std::is_same_v<decltype(std::declval<T>()()), BatchableResult>};
    ...
    std::optional<R> Loop(bool fMaster) EXCLUSIVE_LOCKS_REQUIRED(!m_mutex)
    {
        ...
        std::optional<BatchSchnorrVerifier> batch;
        if constexpr (is_batched) batch.emplace();
        do {
            {

...
using BlockNotifySetting = common::Setting<
    ...
    "Execute command when the best block changes (%s in cmd is replaced by block hash). Not run for blocks connected during initial block download or reindexing.">;
...
using DbCacheSetting = common::Setting<
    ...
    ::HelpFn<[](const auto& help) { return help.Format(MIN_DBCACHE_BYTES / 1_MiB, node::GetDefaultDBCache() / 1_MiB); }>;
...
using LoadBlockSetting = common::Setting<
    ...
    "Imports blocks from an external file on startup. Obfuscated blocks are not supported.">;
...
using PrevoutfetchthreadsSetting = common::Setting<
    "-prevoutfetchthreads=<n>", common::Unset, common::SettingOptions{.legacy = true},
    "Set the number of threads used to prefetch block input prevouts from the chainstate database (0 disables, up to %d, default: %d). Negative values are rejected.">
    ::HelpArgs<MAX_PREVOUTFETCH_THREADS, DEFAULT_PREVOUTFETCH_THREADS>;
...
using PruneSetting = common::Setting<
    ...
    ::HelpArgs<MIN_DISK_SPACE_FOR_BLOCK_FILES / 1_MiB>;
...
using TxospenderindexSetting = common::Setting<
    "-txospenderindex", bool, common::SettingOptions{.legacy = true},
    "Maintain a transaction output spender index, used by the gettxspendingprevout rpc call (default: %u)">
    ::Default<DEFAULT_TXOSPENDERINDEX>;
...
using AsMapSetting = common::Setting<
    ...
    ::HelpFn<[](const auto& help, const auto& embedded_asmap_doc) { return help.Format(embedded_asmap_doc); }>
    ::Category<OptionsCategory::CONNECTION>;

using AsMapSettingBool = common::Setting<
    "-asmap=<file>", bool, common::SettingOptions{.legacy = true}>;
...
using MaxConnectionsSetting = common::Setting<
    ...
    ::HelpArgs<DEFAULT_MAX_PEER_CONNECTIONS, MAX_OUTBOUND_FULL_RELAY_CONNECTIONS + MAX_BLOCK_RELAY_ONLY_CONNECTIONS + MAX_FEELER_CONNECTIONS, MAX_ADDNODE_CONNECTIONS, MAX_PRIVATE_BROADCAST_CONNECTIONS>
    ::Category<OptionsCategory::CONNECTION>;

using InboundrelaypercentSetting = common::Setting<
    "-inboundrelaypercent=<n>", int64_t, common::SettingOptions{.legacy = true},
    "Permit a maximum percent of inbound connections to relay transactions, to limit memory utilization (0 to 100, default: %u).">
    ::Default<DEFAULT_FULL_RELAY_INBOUND_PCT>
    ::Category<OptionsCategory::CONNECTION>;
...
using PrintPrioritySetting = common::Setting<
    "-printpriority", std::optional<bool>, common::SettingOptions{.legacy = true, .debug_only = true},
...
using TxsendrateSetting = common::Setting<
    "-txsendrate=<n>", std::optional<int64_t>, common::SettingOptions{.legacy = true, .debug_only = true},
    "Set the maximum ongoing rate for sending transactions to (inbound) peers (default: %u tx/s)">
    ::HelpArgs<DEFAULT_TX_SEND_RATE>
    ::Category<OptionsCategory::NODE_RELAY>;
...
using BlockMaxWeightSetting = common::Setting<
    "-blockmaxweight=<n>", common::Unset, common::SettingOptions{.legacy = true, .debug_only = true},
...
using BlockreservedweightSetting = common::Setting<
    "-blockreservedweight=<n>", common::Unset, common::SettingOptions{.legacy = true},
    "Reserve space for the fixed-size block header plus the largest coinbase transaction the mining software may add to the block. Only affects mining RPC clients, not IPC clients. (default: %d).">
...
using RpcmaxconnectionsSetting = common::Setting<
    "-rpcmaxconnections=<n>", common::Unset, common::SettingOptions{.legacy = true},
    "The maximum number of connected HTTP clients (default: %d)">
    ::HelpArgs<DEFAULT_MAX_HTTP_CONNECTIONS>
    ::Category<OptionsCategory::RPC>;

using RpcThreadsSetting = common::Setting<
    "-rpcthreads=<n>", common::Unset, common::SettingOptions{.legacy = true},
    "Set the number of threads to service RPC calls (default: %d)">
    ::HelpArgs<DEFAULT_HTTP_THREADS>
    ::Category<OptionsCategory::RPC>;

using RpcWorkQueueSetting = common::Setting<
    "-rpcworkqueue=<n>", common::Unset, common::SettingOptions{.legacy = true, .debug_only = true},
    "Set the maximum depth of the work queue to service RPC calls (default: %d)">
    ::HelpArgs<DEFAULT_HTTP_WORKQUEUE>
    ::Category<OptionsCategory::RPC>;

...
public:
    ...
    {
        return operator()(ann.m_txhash.ToUint256(), ann.m_peer, ann.m_preferred);
    }
};
...
struct ByPeerViewExtractor
{
    ...
    {
        return ByPeerView{ann.m_peer, ann.GetState() == State::CANDIDATE_BEST, ann.m_txhash.ToUint256()};
    }
};
...
public:
    ...
    result_type operator()(const Announcement& ann) const
    {
        ...
        return ByTxHashView{ann.m_txhash.ToUint256(), ann.GetState(), prio};
    }
};
...
std::map<uint256, TxHashInfo> ComputeTxHashInfo(const Index& index, const PriorityComputer& computer)
{
    ...
    for (const Announcement& ann : index) {
        TxHashInfo& info = ret[ann.m_txhash.ToUint256()];
...
private:
    ...
    void PromoteCandidateReady(Iter<ByTxHash> it)
    {
        ...
        if (it_next == m_index.get<ByTxHash>().end() || it_next->m_txhash.ToUint256() != it->m_txhash.ToUint256() ||
    ...
    bool IsOnlyNonCompleted(Iter<ByTxHash> it)
    {
        ...
        if (it != m_index.get<ByTxHash>().begin() && std::prev(it)->m_txhash.ToUint256() == it->m_txhash.ToUint256()) return false;

        // This announcement has a successor that belongs to the same txhash, and is not COMPLETED.
        if (std::next(it) != m_index.get<ByTxHash>().end() && std::next(it)->m_txhash.ToUint256() == it->m_txhash.ToUint256() &&
    ...
    bool MakeCompleted(Iter<ByTxHash> it)
    {
        ...
        if (IsOnlyNonCompleted(it)) {
            ...
            uint256 txhash = it->m_txhash.ToUint256();
            do {
                it = Erase<ByTxHash>(it);
            } while (it != m_index.get<ByTxHash>().end() && it->m_txhash.ToUint256() == txhash);
            return false;
        }
...
public:
    ...
    {
        auto it = m_index.get<ByTxHash>().lower_bound(ByTxHashView{txhash.ToUint256(), State::CANDIDATE_DELAYED, 0});
        while (it != m_index.get<ByTxHash>().end() && it->m_txhash.ToUint256() == txhash.ToUint256()) {
            it = Erase<ByTxHash>(it);
        }
    }
    ...
    {
        auto it = m_index.get<ByTxHash>().lower_bound(ByTxHashView{txhash.ToUint256(), State::CANDIDATE_DELAYED, 0});
        while (it != m_index.get<ByTxHash>().end() && it->m_txhash.ToUint256() == txhash.ToUint256() && it->GetState() != State::COMPLETED) {
    ...
    {
        auto it = m_index.get<ByPeer>().find(ByPeerView{peer, true, txhash.ToUint256()});
        if (it == m_index.get<ByPeer>().end()) {
            ...
            it = m_index.get<ByPeer>().find(ByPeerView{peer, false, txhash.ToUint256()});
            ...
            auto it_old = m_index.get<ByTxHash>().lower_bound(ByTxHashView{txhash.ToUint256(), State::CANDIDATE_BEST, 0});
            if (it_old != m_index.get<ByTxHash>().end() && it_old->m_txhash.ToUint256() == txhash.ToUint256()) {
    ...
    void ReceivedResponse(NodeId peer, const GenTxidVariant& txhash)
    {
        ...
        auto it = m_index.get<ByPeer>().find(ByPeerView{peer, false, txhash.ToUint256()});
        if (it == m_index.get<ByPeer>().end()) {
            it = m_index.get<ByPeer>().find(ByPeerView{peer, true, txhash.ToUint256()});
        }
    ...
    uint64_t ComputePriority(const GenTxidVariant& txhash, NodeId peer, bool preferred) const
    {
        ...
        return uint64_t{m_computer(txhash.ToUint256(), peer, preferred)};
    }

...
bool SerializeDB(Stream& stream, const Data& data)
{
    ...
    try {
        ...
        LogError(LogInstance(), "%s: Serialize or I/O error - %s\n", __func__, e.what());
        return false;
    }
...
bool SerializeFileDB(const std::string& prefix, const fs::path& path, const Data& data)
{
    ...
    if (fileout.IsNull()) {
        fileout.fclose();
        remove(pathTmp);
        LogError(LogInstance(), "%s: Failed to open file %s\n", __func__, fs::PathToString(pathTmp));
        return false;
    }
    ...
    if (!FileCommit(fileout.Get())) {
        fileout.fclose();
        remove(pathTmp);
        LogError(LogInstance(), "%s: Failed to flush file %s\n", __func__, fs::PathToString(pathTmp));
        return false;
    }
    fileout.fclose();
    ...
    if (!RenameOver(pathTmp, path)) {
        remove(pathTmp);
        LogError(LogInstance(), "%s: Rename-into-place failed\n", __func__);
        return false;
    }
...
bool CBanDB::Read(banmap_t& banSet)
{
    if (fs::exists(m_banlist_dat)) {
        LogPrintf(LogInstance(), "banlist.dat ignored because it can only be read by " PACKAGE_NAME " version 22.x. Remove %s to silence this warning.\n", fs::quoted(fs::PathToString(m_banlist_dat)));
    }
    ...
    if (!common::ReadSettings(m_banlist_json, settings, errors)) {
        for (const auto& err : errors) {
            LogPrintf(LogInstance(), "Cannot load banlist %s: %s\n", fs::PathToString(m_banlist_json), err);
        }
        return false;
    }
    ...
    try {
        ...
        LogPrintf(LogInstance(), "Cannot parse banlist %s: %s\n", fs::PathToString(m_banlist_json), e.what());
        return false;
    }
...
util::Result<std::unique_ptr<AddrMan>> LoadAddrman(const NetGroupManager& netgroupman, const ArgsManager& args)
{
    ...
    try {
        ...
        LogPrintf(LogInstance(), "Loaded %i addresses from peers.dat  %dms\n", addrman->Size(), Ticks<std::chrono::milliseconds>(SteadyClock::now() - start));
    } catch (const DbNotFoundError&) {
        // Addrman can be in an inconsistent state after failure, reset it
        addrman = std::make_unique<AddrMan>(netgroupman, deterministic, /*consistency_check_ratio=*/check_addrman);
        LogPrintf(LogInstance(), "Creating peers.dat because the file was not found (%s)\n", fs::quoted(fs::PathToString(path_addr)));
        ...
        // Addrman can be in an inconsistent state after failure, reset it
        addrman = std::make_unique<AddrMan>(netgroupman, deterministic, /*consistency_check_ratio=*/check_addrman);
        LogPrintf(LogInstance(), "Creating new peers.dat because the file version was not compatible (%s). Original backed up to peers.dat.bak\n", fs::quoted(fs::PathToString(path_addr)));
        DumpPeerAddresses(args, *addrman);
    } catch (const std::exception& e) {
...
std::vector<CAddress> ReadAnchors(const fs::path& anchors_db_path)
{
    ...
    try {
        ...
        LogPrintf(LogInstance(), "Loaded %i addresses from %s\n", anchors.size(), fs::quoted(fs::PathToString(anchors_db_path.filename())));

...
std::optional<uint8_t> AggregatedCISAMarker(const CTxOut& utxo, const CScriptWitness& witness)
{
    if (!utxo.scriptPubKey.IsPayToCisa()) return std::nullopt;
    std::span<const std::vector<unsigned char>> stack{witness.stack};
    if (stack.size() >= 2 && !stack.back().empty() && stack.back()[0] == ANNEX_TAG) stack = stack.first(stack.size() - 1);

...
namespace BCLog {
    static constexpr bool IsSuspicious(const char ch) noexcept
    {
    ...
    {
        const auto suspicious_count{std::ranges::count_if(str, IsSuspicious)};
        if (!suspicious_count) return str;

        std::string ret;
        ret.reserve(str.size() + 3 * suspicious_count);
        for (auto ch : str) {
            if (IsSuspicious(ch)) {

...
CDKGLogger::CDKGLogger(const CDKGSession& _quorumDkg, const std::string& _func) :
    CDKGLogger(_quorumDkg.params.name, _quorumDkg.quorumHash, _quorumDkg.quorumHeight, _quorumDkg.AreWeMember(), _func)
{
}
...
bool CDKGSession::Init(const CBlockIndex* pindexQuorum, const std::vector<CDeterministicMNCPtr>& mns, const uint256& _myProTxHash)
{
    ...
    if (!myProTxHash.IsNull()) {
        quorumDKGDebugManager->InitLocalSessionStatus(params.type, quorumHash, quorumHeight);
...
void CDKGSession::ReceiveMessage(const uint256& hash, const CDKGContribution& qc, bool& retBan)
{
    ...
    dkgManager.WriteVerifiedVvecContribution(params.type, quorumHash, qc.proTxHash, qc.vvec);
...
void CDKGSession::SendCommitment(CDKGPendingMessages& pendingMessages)
{
    ...
    std::vector<BLSVerificationVectorPtr> vvecs;
    BLSSecretKeyVector skContributions;
    if (!dkgManager.GetVerifiedContributions(params.type, quorumHeight, quorumHash, qc.validMembers, memberIndexes, vvecs, skContributions)) {

    ...
    m_current_height(chain_start->nHeight)
{
    ...
    LogPrint(LogInstance(), BCLog::NET, "Initial headers sync started with peer=%d: height=%i, max_commitments=%i, min_work=%s\n", m_id, m_current_height, m_max_commitments, m_minimum_required_work.ToString());
}
    ...
    if (m_download_state == State::PRESYNC) {
        ...
        if (ret.success) {
            if (full_headers_message || m_download_state == State::REDOWNLOAD) {
                ...
                LogPrint(LogInstance(), BCLog::NET, "Initial headers sync aborted with peer=%d: incomplete headers message at height=%i (presync phase)\n", m_id, m_current_height);
            }
        }
        ...
        if (ret.success) {
            ...
            if (m_redownloaded_headers.empty() && m_process_all_remaining_headers) {
                LogPrint(LogInstance(), BCLog::NET, "Initial headers sync complete with peer=%d: releasing all at height=%i (redownload phase)\n", m_id, m_redownload_buffer_last_height);
                ...
                LogPrint(LogInstance(), BCLog::NET, "Initial headers sync aborted with peer=%d: incomplete headers message at height=%i (redownload phase)\n", m_id, m_redownload_buffer_last_height);
            }
        }
    }
...
bool HeadersSyncState::ValidateAndStoreHeadersCommitments(const std::vector<CBlockHeader>& headers)
{
    ...
    if (headers[0].hashPrevBlock != m_last_header_received.GetHash()) {
        ...
        LogPrint(LogInstance(), BCLog::NET, "Initial headers sync aborted with peer=%d: non-continuous headers at height=%i (presync phase)\n", m_id, m_current_height);
        return false;
    }
    ...
    if (m_current_chain_work >= m_minimum_required_work) {
        ...
        LogPrint(LogInstance(), BCLog::NET, "Initial headers sync transition with peer=%d: reached sufficient work at height=%i, redownloading from height=%i\n", m_id, m_current_height, m_redownload_buffer_last_height);
    }
    return true;
}
...
bool HeadersSyncState::ValidateAndProcessSingleHeader(const CBlockHeader& current)
{
    ...
    if (!PermittedDifficultyTransition(m_consensus_params, next_height,
        ...
        LogPrint(LogInstance(), BCLog::NET, "Initial headers sync aborted with peer=%d: invalid difficulty transition at height=%i (presync phase)\n", m_id, next_height);
        return false;
    }
    ...
    if (next_height % HEADER_COMMITMENT_PERIOD == m_commit_offset) {
        ...
        if (m_header_commitments.size() > m_max_commitments) {
            ...
            // try again later.
            LogPrint(LogInstance(), BCLog::NET, "Initial headers sync aborted with peer=%d: exceeded max commitments at height=%i (presync phase)\n", m_id, next_height);
            return false;
        }
    }
...
bool HeadersSyncState::ValidateAndStoreRedownloadedHeader(const CBlockHeader& header)
{
    ...
    if (header.hashPrevBlock != m_redownload_buffer_last_hash) {
        LogPrint(LogInstance(), BCLog::NET, "Initial headers sync aborted with peer=%d: non-continuous headers at height=%i (redownload phase)\n", m_id, next_height);
        return false;
    }
    ...
    if (!PermittedDifficultyTransition(m_consensus_params, next_height,
        ...
        LogPrint(LogInstance(), BCLog::NET, "Initial headers sync aborted with peer=%d: invalid difficulty transition at height=%i (redownload phase)\n", m_id, next_height);
        return false;
    }
    ...
    if (!m_process_all_remaining_headers && next_height % HEADER_COMMITMENT_PERIOD == m_commit_offset) {
        if (m_header_commitments.size() == 0) {
            LogPrint(LogInstance(), BCLog::NET, "Initial headers sync aborted with peer=%d: commitment overrun at height=%i (redownload phase)\n", m_id, next_height);
        ...
        if (commitment != expected_commitment) {
            LogPrint(LogInstance(), BCLog::NET, "Initial headers sync aborted with peer=%d: commitment mismatch at height=%i (redownload phase)\n", m_id, next_height);
            return false;
        }
    }

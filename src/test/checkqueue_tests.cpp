...
static void Correct_Queue_range(std::vector<size_t> range)
{
    ...
    for (const size_t i : range) {
        ...
        while (total) {
            vChecks.clear();
            vChecks.resize(std::min<size_t>(total, g_rng.randrange(10)));
            total -= vChecks.size();
...
BOOST_AUTO_TEST_CASE(test_CheckQueue_Correct_Random)
{
    ...
    for (size_t i = 2; i < 100000; i += std::max((size_t)1, (size_t)g_rng.randrange(std::min((size_t)1000, ((size_t)100000) - i))))
        range.push_back(i);
    Correct_Queue_range(range);
}

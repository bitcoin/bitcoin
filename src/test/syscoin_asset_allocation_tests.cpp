...
BOOST_AUTO_TEST_CASE(generate_asset_allocation_lock)
{
	...
	BOOST_CHECK_NO_THROW(r = CallRPC(node, "signrawtransactionwithwallet " + r.get_str()));
	...
	BOOST_CHECK_THROW(r = CallRPC(node, "sendrawtransaction " + hex_str, true, false), runtime_error);

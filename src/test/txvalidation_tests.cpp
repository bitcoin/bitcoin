...
BOOST_FIXTURE_TEST_CASE(version3_tests, RegTestingSetup)
{
    ...
    script_multisig << OP_2 << OP_CHECKMULTISIG;
    {
        ...
        const int64_t total_sigops{static_cast<int64_t>(tx_many_sigops->GetInputs().size()) * static_cast<int64_t>(script_multisig.GetSigOpCount(/*fAccurate=*/false))};
        BOOST_CHECK_EQUAL(total_sigops, tx_many_sigops->GetInputs().size() * MAX_PUBKEYS_PER_MULTISIG);

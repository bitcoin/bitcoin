...
BOOST_FIXTURE_TEST_CASE(version3_tests, RegTestingSetup)
{
    ...
    // Cannot spend from an unconfirmed TRUC transaction unless this tx is also TRUC.
    {
        ...
        auto result_v2_from_v3{SingleTRUCChecks(pool, tx_v2_from_v3, empty_conflicts_set, GetVirtualTransactionSize(*tx_v2_from_v3))};
        ...
        const auto expected_error_str_2{strprintf("non-version=3 tx %s (wtxid=%s) cannot spend from version=3 tx %s (wtxid=%s)",
            ...
            mempool_tx_v3->GetHash().ToString(), mempool_tx_v3->GetWitnessHash().ToString())};
        auto result_v2_from_both{SingleTRUCChecks(pool, tx_v2_from_v2_and_v3, empty_conflicts_set, GetVirtualTransactionSize(*tx_v2_from_v2_and_v3))};
    ...
    // TRUC cannot spend from an unconfirmed non-TRUC transaction.
    {
        ...
        auto result_v3_from_v2{SingleTRUCChecks(pool, tx_v3_from_v2, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_from_v2))};
        ...
        const auto expected_error_str_2{strprintf("version=3 tx %s (wtxid=%s) cannot spend from non-version=3 tx %s (wtxid=%s)",
            ...
            mempool_tx_v2->GetHash().ToString(), mempool_tx_v2->GetWitnessHash().ToString())};
        auto result_v3_from_both{SingleTRUCChecks(pool, tx_v3_from_v2_and_v3, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_from_v2_and_v3))};
    ...
    // V3 from V3 is ok, and non-V3 from non-V3 is ok.
    {
        ...
        BOOST_CHECK(SingleTRUCChecks(pool, tx_v3_from_v3, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_from_v3))
                    == std::nullopt);
        ...
        BOOST_CHECK(SingleTRUCChecks(pool, tx_v2_from_v2, empty_conflicts_set, GetVirtualTransactionSize(*tx_v2_from_v2))
                    == std::nullopt);
    ...
    // Configuration where the tx has multiple direct parents.
    {
        ...
        auto result{SingleTRUCChecks(pool, tx_v3_multi_parent, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_multi_parent))};
    ...
    // Configuration where the tx is in a multi-generation chain.
    {
        ...
        auto result{SingleTRUCChecks(pool, tx_v3_multi_gen, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_multi_gen))};
        BOOST_CHECK_EQUAL(result->first, expected_error_str);
        BOOST_CHECK_EQUAL(result->second, nullptr);
    ...
    many_inputs.emplace_back(mempool_tx_v3->GetHash(), 0);
    {
        ...
        auto result{SingleTRUCChecks(pool, tx_v3_child_big, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_child_big))};
        BOOST_CHECK_EQUAL(result->first, expected_error_str);
        BOOST_CHECK_EQUAL(result->second, nullptr);
    ...
    script_multisig << OP_2 << OP_CHECKMULTISIG;
    {
        ...
        BOOST_CHECK(SingleTRUCChecks(pool, tx_many_sigops, empty_conflicts_set, bip141_vsize) == std::nullopt);
        // ...but sigop limit is.
        const auto expected_error_str{strprintf("version=3 child tx %s (wtxid=%s) is too big: %u > %u virtual bytes",
        ...
        auto result{SingleTRUCChecks(pool, tx_many_sigops, empty_conflicts_set,
    ...
    auto tx_mempool_v3_child = make_tx({COutPoint{mempool_tx_v3->GetHash(), 0}}, /*version=*/3);
    {
        ...
        BOOST_CHECK(SingleTRUCChecks(pool, tx_mempool_v3_child, empty_conflicts_set, GetVirtualTransactionSize(*tx_mempool_v3_child)) == std::nullopt);
    ...
    // A TRUC transaction cannot have more than 1 descendant. Sibling is returned when exactly 1 exists.
    {
        ...
        const auto expected_error_str{strprintf("tx %s (wtxid=%s) would exceed descendant count limit",
            mempool_tx_v3->GetHash().ToString(), mempool_tx_v3->GetWitnessHash().ToString())};
        auto result_with_sibling_eviction{SingleTRUCChecks(pool, tx_v3_child2, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_child2))};
        ...
        BOOST_CHECK(SingleTRUCChecks(pool, tx_v3_child2, {tx_mempool_v3_child->GetHash()}, GetVirtualTransactionSize(*tx_v3_child2))
                    == std::nullopt);
        ...
        auto result_2children{SingleTRUCChecks(pool, tx_v3_child3, empty_conflicts_set, GetVirtualTransactionSize(*tx_v3_child3))};
    ...
    // Sibling eviction: parent already has 1 other child, which also has its own child (no sibling eviction allowed). This may happen as the result of a reorg.
    {
        ...
        auto result_3gen{SingleTRUCChecks(pool, tx_to_submit, empty_conflicts_set, GetVirtualTransactionSize(*tx_to_submit))};

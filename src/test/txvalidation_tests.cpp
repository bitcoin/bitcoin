...
BOOST_FIXTURE_TEST_CASE(ephemeral_tests, RegTestingSetup)
{
    ...
    // Dust checks pass
    BOOST_CHECK(CheckEphemeralSpends({grandparent_tx_1, dust_spend}, CFeeRate(0*sats), pool, child_state, child_wtxid));
    BOOST_CHECK(child_state.IsValid());
    BOOST_CHECK_EQUAL(child_wtxid, Wtxid());
...
BOOST_FIXTURE_TEST_CASE(version3_tests, RegTestingSetup)
{
    ...
    script_multisig << OP_2 << OP_CHECKMULTISIG;
    {
        ...
        mtx_many_sigops.vout.back().nValue = 10000*sats;

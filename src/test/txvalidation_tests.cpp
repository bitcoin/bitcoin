...
BOOST_FIXTURE_TEST_CASE(ephemeral_tests, RegTestingSetup)
{
    ...
    // Spend dust from one but not another
    ...
    BOOST_CHECK(!CheckEphemeralSpends({grandparent_tx_1, grandparent_tx_2, dust_spend_all_outpoints}, minrelay).has_value());
    ...
    BOOST_CHECK(CheckEphemeralSpends({grandparent_tx_1, grandparent_tx_2, parent_with_dust}, minrelay).has_value());
    auto child_no_dust = make_tx({COutPoint{parent_with_dust->GetHash(), dust_index}}, /*version=*/2);
    BOOST_CHECK(!CheckEphemeralSpends({grandparent_tx_1, grandparent_tx_2, parent_with_dust, child_no_dust}, minrelay).has_value());
    ...
    BOOST_CHECK(CheckEphemeralSpends({grandparent_tx_1, grandparent_tx_2, parent_with_dust, child_with_dust}, minrelay).has_value());

    // Tests for PreChecks-based checks

    // Empty ancestors means this should never fail since it only checks parents' dust
    BOOST_CHECK(!CheckEphemeralSpends(grandparent_tx_1, empty_ancestors, minrelay).has_value());

    // Add first grandparent to mempool and fetch entry
    pool.addUnchecked(entry.FromTx(grandparent_tx_1));
    const auto grandparent_entry_1 = pool.GetIter(grandparent_tx_1->GetHash().ToUint256()).value();

    // Ignores ancestors that aren't direct parents
    BOOST_CHECK(!CheckEphemeralSpends(child_no_dust, {grandparent_entry_1}, minrelay).has_value());

    // Valid spend of dust with grandparent in mempool
    BOOST_CHECK(!CheckEphemeralSpends(parent_with_dust, {grandparent_entry_1}, minrelay).has_value());

    // Add second grandparent
    pool.addUnchecked(entry.FromTx(grandparent_tx_2));

    const auto grandparent_entry_2 = pool.GetIter(grandparent_tx_2->GetHash().ToUint256()).value();
    CTxMemPool::setEntries grandparent_entries{grandparent_entry_1, grandparent_entry_2};

    // Only spends single dust out of two direct parents
    BOOST_CHECK(CheckEphemeralSpends(dust_non_spend_both_parents, grandparent_entries, minrelay).has_value());

    // Spends both parents' dust
    BOOST_CHECK(!CheckEphemeralSpends(parent_with_dust, grandparent_entries, minrelay).has_value());

    // Now add dusty parent to mempool
    pool.addUnchecked(entry.FromTx(parent_with_dust));
    const auto parent_entry = pool.GetIter(parent_with_dust->GetHash().ToUint256()).value();
    CTxMemPool::setEntries all_entries{grandparent_entry_1, grandparent_entry_2, parent_entry};

    // Passes dust checks even with non-parent ancestors
    BOOST_CHECK(!CheckEphemeralSpends(child_no_dust, all_entries, minrelay).has_value());
}

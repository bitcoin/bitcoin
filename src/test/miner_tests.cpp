...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
...

    {
        ...
        BOOST_CHECK_EXCEPTION(miner->createNewBlock(options), std::runtime_error, HasReason("bad-blk-sigops"));
    }

    {
        ...
        BOOST_CHECK(miner->createNewBlock(options));
    }
...

    {
        ...
        BOOST_CHECK_EXCEPTION(miner->createNewBlock(options), std::runtime_error, HasReason("bad-txns-inputs-missingorspent"));
    }
...

    {
        ...
        BOOST_CHECK_EXCEPTION(miner->createNewBlock(options), std::runtime_error, HasReason("bad-cb-multiple"));
    }
...

    {
        ...
        hash = tx.GetHash();
        AddToMempool(tx_mempool, entry.Fee(LOWFEE).Time(Now<NodeSeconds>()).SpendsCoinbase(false).FromTx(tx));
        BOOST_CHECK_EXCEPTION(miner->createNewBlock(options), std::runtime_error, HasReason("mandatory-script-verify-flag-failed"));
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    BOOST_REQUIRE(block_template);
    {
        ...
        {
            std::string reason;
            BOOST_REQUIRE(!miner->checkBlock(block, {.check_pow = false}, reason));
...

        {
            ...
            std::string reason;
            BOOST_REQUIRE(!miner->checkBlock(block, {.check_pow = true}, reason));

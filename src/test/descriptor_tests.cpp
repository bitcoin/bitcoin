...
BOOST_AUTO_TEST_CASE(parse_multipath_string_test)
{
    ...
    const auto check{[](const std::string& desc_str, const std::string& expected_multipath, size_t num_descs) {
        ...
        // Expanded descriptors also use h in public and private output.
        for (const auto& desc : descs) {
            BOOST_CHECK_EQUAL(desc->ToString(), desc->ToCanonicalString());
            std::string priv;
            BOOST_REQUIRE(desc->ToPrivateString(keys, priv));
            BOOST_CHECK_EQUAL(priv, UseHInsteadOfApostrophe(priv));
        }
    }};
    ...
    // Hardened key origins within miniscript are normalized for either spelling.
    check("wsh(pk([deadbeef/49']" + xprv + "/<0;1>/*))", "wsh(pk([deadbeef/49h]" + xpub + "/<0;1>/*))", 2);
    check("wsh(pk([deadbeef/49h]" + xprv + "/<0;1>/*))", "wsh(pk([deadbeef/49h]" + xpub + "/<0;1>/*))", 2);

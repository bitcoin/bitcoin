...
BOOST_AUTO_TEST_CASE(musig_duplicate_keys)
{
...

    {
        ...
        BOOST_REQUIRE_MESSAGE(!descs.empty(),
...
BOOST_AUTO_TEST_CASE(musig_origin_paths)
{
    const std::string internal_key =
        "xpub661MyMwAqRbcFW31YEwpkMuc5THy2PSt5bDMsktWQcFF8syAmRUapSCGu8ED9W6oDMSgv6Zz8idoc4a6mr8BDzTJY47LJhkJ8UB7WEGuduB";
    const std::string xpub_a =
    ...
    BOOST_CHECK(scripts == scripts_cached);
    ...
    const std::set<std::string> expected_fingerprints{"03c88353", "857c9a7c", "9f4a3b9c"};
    const auto check_participant_origins = [&](const FlatSigningProvider& signing_provider) {
        ...
        for (const auto& [_, participants] : signing_provider.aggregate_pubkeys) {
            ...
            for (const auto& participant : participants) {
                ...
                fingerprints.insert(HexStr(info.fingerprint));
            }
        }
        BOOST_CHECK_EQUAL(participant_ids.size(), 3U);
        BOOST_CHECK_EQUAL_COLLECTIONS(fingerprints.begin(), fingerprints.end(), expected_fingerprints.begin(), expected_fingerprints.end());
    };

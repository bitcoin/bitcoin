...
void StartNode(const string &dataDir, bool regTest, const string& extraArgs)
{
	...
	if(regTest)
		nodePath += string(" -regtest -assetallocationindex -addressindex -tpstest");
	if(!extraArgs.empty())
...
void GenerateBlocks(int nBlocks, const string& node)
{
  ...
  BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "getinfo"));
  height = find_value(r.get_obj(), "blocks").get_int();
  ...
  while(!otherNode2.empty() &&height < newHeight)
  {
	  try
	  {
		r = CallExtRPC(otherNode2, "getinfo");
	  }
	  catch(const runtime_error &e)
	  {
		r = NullUniValue;
...
string GetNewFundedAddress(const string &node) {
	...
	if (node == "node1")
		...
		sendnode = "node1";
	BOOST_CHECK_THROW(CallExtRPC(sendnode, "sendtoaddress", "\"" + newaddress + "\",10"), runtime_error);
...
string AliasNew(const string& node, const string& aliasname, const string& pubdata, string witness, string newaddress)
{
	...
	BOOST_CHECK_NO_THROW(CallExtRPC(node, "importprivkey", "\"" + CSyscoinSecret(privKey).ToString() + "\",\"\",false"));
	BOOST_CHECK_NO_THROW(CallExtRPC(node, "importprivkey", "\"" + CSyscoinSecret(privEncryptionKey).ToString() + "\",\"\",false"));
	...
	UniValue r;
	// registration
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "aliasnew", "\"" + aliasname + "\",\"" + pubdata + "\"," + acceptTransfers +  "," + expireTime + ",\"" + strAddress + "\",\"" + strEncryptionPrivateKeyHex + "\",\"" + HexStr(vchPubEncryptionKey) + "\",\"" + witness+ "\""));
	UniValue varray = r.get_array();
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "syscointxfund", "\""+ varray[0].get_str() + "\""));
	varray = r.get_array();
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "signrawtransaction", "\"" + varray[0].get_str() +"\""));
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "syscoinsendrawtransaction", "\"" + find_value(r.get_obj(), "hex").get_str() + "\""));
	GenerateBlocks(5, node);
	// activation
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "aliasnew", "\"" + aliasname + "\",\"" + pubdata + "\"," + acceptTransfers + "," + expireTime + ",\"" + strAddress + "\",\"" + strEncryptionPrivateKeyHex + "\",\"" + HexStr(vchPubEncryptionKey) + "\",\"" + witness + "\""));
	UniValue varray1 = r.get_array();
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "syscointxfund", "\"" + varray1[0].get_str() + "\""));
	varray1 = r.get_array();
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "signrawtransaction", "\"" + varray1[0].get_str() + "\""));
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "syscoinsendrawtransaction", "\"" + find_value(r.get_obj(), "hex").get_str() + "\""));

	GenerateBlocks(5, node);
	GenerateBlocks(5, node);
	BOOST_CHECK_NO_THROW(CallExtRPC(node, "sendtoaddress", "\"" + aliasname + "\",10"));
	GenerateBlocks(5, node);
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "aliasbalance", "\"" + aliasname + "\""));
	CAmount balanceAfter = AmountFromValue(find_value(r.get_obj(), "balance"));
	BOOST_CHECK(balanceAfter >= 10*COIN);
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "aliasinfo", "\"" + aliasname + "\""));

	if(!otherNode1.empty())
	{
		BOOST_CHECK_NO_THROW(r = CallExtRPC(otherNode1, "aliasinfo", "\"" + aliasname + "\""));
	...
	if(!otherNode2.empty())
	{
		BOOST_CHECK_NO_THROW(r = CallExtRPC(otherNode2, "aliasinfo", "\"" + aliasname + "\""));
		BOOST_CHECK(find_value(r.get_obj(), "_id").get_str() == aliasname);
		BOOST_CHECK_EQUAL(find_value(r.get_obj(), "publicvalue").get_str(), pubdata);
		BOOST_CHECK_EQUAL(find_value(r.get_obj(), "expired").get_bool(), false);
		BOOST_CHECK_EQUAL(find_value(r.get_obj(), "address").get_str() , strAddress);
...
string AssetNew(const string& node, const string& name, const string& alias, const string& pubdata, const string& precision, const string& useinputranges, const string& supply, const string& maxsupply, const string& interestrate, const string& canadjustinterest, const string& witness)
{
	...
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "assetnew", "\"" + name + "\",\"" + alias + "\",\"" + pubdata + "\",\"assets\"," + precision + "," + useinputranges + "," + supply + "," + maxsupply + "," + interestrate + "," + canadjustinterest + ",\"" + witness + "\""));
	UniValue arr = r.get_array();
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "signrawtransaction", "\"" + arr[0].get_str() + "\""));
	string hex_str = find_value(r.get_obj(), "hex").get_str();
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "syscoinsendrawtransaction", "\"" + hex_str + "\""));
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "decoderawtransaction", "\"" + hex_str + "\""));
	string guid = arr[1].get_str();
	GenerateBlocks(5, node);
	BOOST_CHECK_NO_THROW(r = CallExtRPC(node, "assetinfo", "\"" + guid + "\",false"));
	...
	if (!otherNode1.empty())
	{
		BOOST_CHECK_NO_THROW(r = CallExtRPC(otherNode1, "assetinfo", "\"" + guid + "\",false"));
		BOOST_CHECK(find_value(r.get_obj(), "_id").get_str() == guid);
		BOOST_CHECK(find_value(r.get_obj(), "symbol").get_str() == nameupper);
		BOOST_CHECK(find_value(r.get_obj(), "publicvalue").get_str() == pubdata);
		UniValue balance = find_value(r.get_obj(), "balance");
	...
	if (!otherNode2.empty())
	{
		BOOST_CHECK_NO_THROW(r = CallExtRPC(otherNode2, "assetinfo", "\"" + guid + "\",false"));
		BOOST_CHECK(find_value(r.get_obj(), "_id").get_str() == guid);
		BOOST_CHECK(find_value(r.get_obj(), "symbol").get_str() == nameupper);
		BOOST_CHECK(find_value(r.get_obj(), "publicvalue").get_str() == pubdata);
		UniValue balance = find_value(r.get_obj(), "balance");

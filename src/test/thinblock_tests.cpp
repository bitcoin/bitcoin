...
BOOST_AUTO_TEST_CASE(thinblock_test) {
    ...
    /* Message Consistency checking for p2p thinblock messages.  If the messages are not formatted correctly then a
       a ban will result.
     */
    fImporting = false;
    fReindex = false;
    ...
    CAddress addr1a(ipaddress(0xa0b0c01a));
    CNode dummyNode1a(INVALID_SOCKET, addr1a, "", true);
    ...
    BOOST_CHECK(CNode::IsBanned(addr1a));

    // test invalid block header
    CAddress addr1b(ipaddress(0xa0b0c01b));
    CNode dummyNode1b(INVALID_SOCKET, addr1b, "", true);
    ...
    BOOST_CHECK(CNode::IsBanned(addr1b));

    /** THINBLOCK message consistency  checks */

    // test empty missingtx vector
    CAddress addr2(ipaddress(0xa0b0c02a));
    ...
    // test invalid or missing coinbase
    CAddress addr2b(ipaddress(0xa0b0c02b));
    CNode dummyNode2b(INVALID_SOCKET, addr2b, "", true);
    ...
    BOOST_CHECK(CNode::IsBanned(addr2b));

    // create invalid block header
    CAddress addr2c(ipaddress(0xa0b0c02c));
    CNode dummyNode2c(INVALID_SOCKET, addr2c, "", true);
    dummyNode2c.nVersion = 1;
    ...
    thin.header.nBits = 1;
    vRecv2 << thin;
    ProcessMessage(&dummyNode2c, NetMsgType::THINBLOCK, vRecv2, GetTime());
    SendMessages(&dummyNode2c);
    BOOST_CHECK(!CheckBlockHeader(thin.header, state, true));
    BOOST_CHECK(CNode::IsBanned(addr2c));


    /** XBLOCKTX message consistency checks */

    // test null hash
    CBlock block3 = TestBlock();
    CAddress addr3(ipaddress(0xa0b0c003));
    ...
    CAddress addr3a(ipaddress(0xa0b0c03a));
    CNode dummyNode3a(INVALID_SOCKET, addr3a, "", true);
    ...
    BOOST_CHECK(CNode::IsBanned(addr3a));

    // test txns hashes in xblocktx not matching num in pfrom->xThinBlockHashes
    CAddress addr3b(ipaddress(0xa0b0c03b));
    CNode dummyNode3b(INVALID_SOCKET, addr3b, "", true);
    ...
    BOOST_CHECK(CNode::IsBanned(addr3b));


    /** GET_XBLOCKTX message consistency checks */

    // test null hash
    CAddress addr4a(ipaddress(0xa0b0c04a));
    CNode dummyNode4a(INVALID_SOCKET, addr4a, "", true);
    ...
    BOOST_CHECK(nullhash.IsNull());
    BOOST_CHECK(CNode::IsBanned(addr4a));

    // test empty setHashesToRequest
    CAddress addr4b(ipaddress(0xa0b0c04b));
    CNode dummyNode4b(INVALID_SOCKET, addr4b, "", true);
    dummyNode4b.nVersion = 1;
    ...
    ProcessMessage(&dummyNode4b, NetMsgType::GET_XBLOCKTX, vRecv4, GetTime());
    SendMessages(&dummyNode4b);
    BOOST_CHECK(setHashesToRequest.empty());
    BOOST_CHECK(CNode::IsBanned(addr4b));


    /** GET_XTHIN message consistency checks */

    // test get_xthin with null hash
    CAddress addr5a(ipaddress(0xa0b0c05a));
    CNode dummyNode5a(INVALID_SOCKET, addr5a, "", true);
    ...
    BOOST_CHECK(nullhash.IsNull());
    BOOST_CHECK(CNode::IsBanned(addr5a));

    // test get_xthin with invalid message type
    CAddress addr5b(ipaddress(0xa0b0c05b));
    CNode dummyNode5b(INVALID_SOCKET, addr5b, "", true);
    dummyNode5b.nVersion = 1;
    ...
    ProcessMessage(&dummyNode5b, NetMsgType::GET_XTHIN, vRecv5, GetTime());
    SendMessages(&dummyNode5b);
    BOOST_CHECK(inv2.type != MSG_THINBLOCK && inv2.type != MSG_XTHINBLOCK);
    BOOST_CHECK(CNode::IsBanned(addr5b));
}

...
BOOST_AUTO_TEST_CASE(MempoolEstimator)
{
    ...
    LOCK2(cs_main, m_node.mempool->cs);
    {
        ...
        BOOST_CHECK(fee_estimate.error_message.value() == strprintf("Confirmation target %s is above maximum limit of %s, mempool conditions might change and forecasts above %s block may be unreliable",
    ...
    // Test when there are not enough mempool transactions to get an accurate estimate
    {
        ...
        BOOST_CHECK(fee_estimate.empty() == true);
        BOOST_CHECK(fee_estimate.error_message.value() == "No enough transactions in the mempool to provide a feerate forecast");
    }
    ...
    // Mempool transactions are enough to provide feerate estimate
    {
        ...
        BOOST_CHECK(fee_estimate.opt.low_priority_estimate == CFeeRate(med_fee, tx_vsize));
        BOOST_CHECK(fee_estimate.opt.high_priority_estimate == CFeeRate(high_fee, tx_vsize));
    }
}

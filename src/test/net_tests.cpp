...
{
    CKey key;
    uint256 key_data = g_rng.rand256();
...
public:
    ...
    explicit V2TransportTester(bool test_initiator)
        : m_transport{0, test_initiator},
          m_cipher{GenerateRandomTestKey(), MakeByteSpan(g_rng.rand256())},
    ...
    InteractResult Interact()
    {
        ...
        while (true) {
            ...
            if (!m_to_send.empty()) {
                Span<const uint8_t> to_send = Span{m_to_send}.first(1 + g_rng.randrange(m_to_send.size()));
            ...
            if (m_transport.ReceivedMessageComplete() && (!progress || g_rng.randbool())) {
                bool reject{false};
            ...
            if (!m_msg_to_send.empty() && (!progress || g_rng.randbool())) {
            ...
            if (!recv_bytes.empty() && (!progress || g_rng.randbool())) {
                size_t to_receive = 1 + g_rng.randrange(recv_bytes.size());
    ...
    {
        CMessageHeader hdr(magic, "version", 126 + g_rng.randrange(11));
        DataStream ser{};
        ser << hdr;
    ...
    void SendGarbage(size_t garbage_len)
    {
        ...
        SendGarbage(g_rng.randbytes<uint8_t>(garbage_len));
    }

    /** Schedule garbage (with valid random length) to be sent to the transport. */
    void SendGarbage()
    {
         SendGarbage(g_rng.randrange(V2Transport::MAX_GARBAGE_LEN + 1));
    }
    ...
    void Damage()
    {
        m_to_send[g_rng.randrange(m_to_send.size())] ^= (uint8_t{1} << g_rng.randrange(8));
    }
};
...
BOOST_AUTO_TEST_CASE(v2transport_test)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        auto msg_data_1 = g_rng.randbytes<uint8_t>(g_rng.randrange(100000));
        auto msg_data_2 = g_rng.randbytes<uint8_t>(g_rng.randrange(1000));
        ...
        while (true) {
            ...
            auto msg_data_3 = g_rng.randbytes<uint8_t>(g_rng.randrange(10000));
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        auto msg_data_3 = g_rng.randbytes<uint8_t>(4005000);
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        for (unsigned v = 0; v < num_ignore_version; ++v) {
            size_t ver_ign_data_len = g_rng.randbool() ? 0 : g_rng.randrange(1000);
            auto ver_ign_data = g_rng.randbytes<uint8_t>(ver_ign_data_len);
        ...
        for (unsigned d = 0; d < num_decoys_1; ++d) {
            auto decoy_data = g_rng.randbytes<uint8_t>(g_rng.randrange(1000));
            tester.SendPacket(/*content=*/decoy_data, /*aad=*/{}, /*ignore=*/true);
        }
        auto msg_data_1 = g_rng.randbytes<uint8_t>(g_rng.randrange(4000000));
    ...
    // Send garbage that includes the first 15 garbage terminator bytes somewhere.
    {
        ...
        size_t len_before = g_rng.randrange(V2Transport::MAX_GARBAGE_LEN - 16 + 1);
        /** The number of random garbage bytes after it. */
        size_t len_after = g_rng.randrange(V2Transport::MAX_GARBAGE_LEN - 16 - len_before + 1);
        // Construct len_before + 16 + len_after random bytes.
        auto garbage = g_rng.randbytes<uint8_t>(len_before + 16 + len_after);
        ...
        garbage[len_before + 15] ^= (uint8_t(1) << g_rng.randrange(8));
        ...
        tester.ReceiveVersion();
        tester.CompareSessionIDs();
        auto msg_data_1 = g_rng.randbytes<uint8_t>(4000000); // test that receiving 4M payload works
        auto msg_data_2 = g_rng.randbytes<uint8_t>(4000000); // test that sending 4M payload works
        tester.SendMessage(uint8_t(g_rng.randrange(223) + 33), {}); // unknown short id

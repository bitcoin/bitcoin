...
BOOST_AUTO_TEST_CASE(ipv4_peer_with_ipv6_addrMe_test)
{
    ...
    GetLocalAddrForPeer(pnode->GetContext(), addrLocal);
...
BOOST_AUTO_TEST_CASE(get_local_addr_for_peer_port)
{
    ...
    auto chosen_local_addr = GetLocalAddrForPeer(peer_out.GetContext(), peer_us);
    ...
    chosen_local_addr = GetLocalAddrForPeer(peer_in.GetContext(), peer_us);
    BOOST_REQUIRE(chosen_local_addr);
...
BOOST_AUTO_TEST_CASE(advertise_local_address)
{
    ...
    AddLocal(addr_ipv4);
    BOOST_CHECK(GetLocalAddress(peer_ipv4->GetContext()) == addr_ipv4);
    BOOST_CHECK(GetLocalAddress(peer_ipv6->GetContext()) == addr_ipv4);
    BOOST_CHECK(GetLocalAddress(peer_ipv6_tunnel->GetContext()) == addr_ipv4);
    BOOST_CHECK(GetLocalAddress(peer_teredo->GetContext()) == addr_ipv4);
    BOOST_CHECK(GetLocalAddress(peer_cjdns->GetContext()) == addr_ipv4);
    BOOST_CHECK(!GetLocalAddress(peer_onion->GetContext()).IsValid());
    BOOST_CHECK(!GetLocalAddress(peer_i2p->GetContext()).IsValid());
    RemoveLocal(addr_ipv4);

    // local privacy addresses - don't advertise to clearnet peers
    AddLocal(addr_onion);
    AddLocal(addr_i2p);
    BOOST_CHECK(!GetLocalAddress(peer_ipv4->GetContext()).IsValid());
    BOOST_CHECK(!GetLocalAddress(peer_ipv6->GetContext()).IsValid());
    BOOST_CHECK(!GetLocalAddress(peer_ipv6_tunnel->GetContext()).IsValid());
    BOOST_CHECK(!GetLocalAddress(peer_teredo->GetContext()).IsValid());
    BOOST_CHECK(!GetLocalAddress(peer_cjdns->GetContext()).IsValid());
    BOOST_CHECK(GetLocalAddress(peer_onion->GetContext()) == addr_onion);
    BOOST_CHECK(GetLocalAddress(peer_i2p->GetContext()) == addr_i2p);
    ...
    AddLocal(addr_cjdns);
    BOOST_CHECK(GetLocalAddress(peer_ipv4->GetContext()) == addr_ipv4);
    BOOST_CHECK(GetLocalAddress(peer_ipv6->GetContext()) == addr_ipv6);
    BOOST_CHECK(GetLocalAddress(peer_ipv6_tunnel->GetContext()) == addr_ipv6);
    BOOST_CHECK(GetLocalAddress(peer_teredo->GetContext()) == addr_ipv4);
    BOOST_CHECK(GetLocalAddress(peer_onion->GetContext()) == addr_onion);
    BOOST_CHECK(GetLocalAddress(peer_i2p->GetContext()) == addr_i2p);
    BOOST_CHECK(GetLocalAddress(peer_cjdns->GetContext()) == addr_cjdns);
    RemoveLocal(addr_ipv4);

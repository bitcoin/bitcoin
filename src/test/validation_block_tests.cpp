...
std::shared_ptr<CBlock> MinerTestingSetup::Block(const uint256& prev_hash)
{
    ...
    auto ptemplate = BlockAssembler(::ChainstateActive(), *m_node.evodb, *m_node.chain_helper, *m_node.mnhf_manager, *m_node.llmq_ctx, *m_node.mempool, Params()).CreateNewBlock(pubKey);

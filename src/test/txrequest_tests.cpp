...
std::chrono::microseconds RandomTime8s() { return std::chrono::microseconds{1 + g_rng.randbits(23)}; }
std::chrono::microseconds RandomTime1y() { return std::chrono::microseconds{1 + g_rng.randbits(45)}; }
...
public:
    ...
    {
        return g_rng.randbool() ? GenTxid::Wtxid(NewTxHash(orders)) : GenTxid::Txid(NewTxHash(orders));
    }
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    if (config >> 3) { // We'll request the transaction
        ...
        if ((config >> 3) == 1) { // The request will time out
            ...
            scenario.AdvanceTime(std::chrono::microseconds{g_rng.randrange(expiry.count())});
...
void BuildPriorityTest(Scenario& scenario, int config)
{
    ...
    if (g_rng.randbool()) scenario.AdvanceTime(RandomTime8s());
...
void BuildBigPriorityTest(Scenario& scenario, int peers)
{
    ...
    int num_pref = g_rng.randrange(peers + 1) ; // Some preferred, ...
    ...
    std::shuffle(announce_order.begin(), announce_order.end(), g_rng);
    ...
    for (int i = 0; i < peers; ++i) {
        if (g_rng.randbool()) scenario.AdvanceTime(RandomTime8s());
        const int pos = g_rng.randrange(request_order.size());
...
void BuildWtxidTest(Scenario& scenario, int config)
{
    ...
    auto reqtimeT = g_rng.randbool() ? MIN_TIME : scenario.Now() + RandomTime8s();
    auto reqtimeW = g_rng.randbool() ? MIN_TIME : scenario.Now() + RandomTime8s();
...
void TestInterleavedScenarios()
{
    ...
    std::shuffle(builders.begin(), builders.end(), g_rng);

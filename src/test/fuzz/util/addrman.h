#ifndef BITCOIN_TEST_FUZZ_UTIL_ADDRMAN_H
#define BITCOIN_TEST_FUZZ_UTIL_ADDRMAN_H
...
static int32_t GetCheckRatio(const ArgsManager& args)
{
    return std::clamp<int32_t>(args.GetIntArg("-checkaddrman", 0), 0, 1000000);
}

[[nodiscard]] inline static NetGroupManager ConsumeNetGroupManager(FuzzedDataProvider& fuzzed_data_provider) noexcept
{
...
public:
    explicit AddrManDeterministic(const NetGroupManager& netgroupman, FuzzedDataProvider& fuzzed_data_provider,
        ...
        : AddrMan(netgroupman, /*deterministic=*/true, consistency_check_ratio)
    {

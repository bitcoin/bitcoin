    ...
    LIMITED_WHILE (outpoints.size() < 200'000 && fuzzed_data_provider.ConsumeBool(), 1000) {
        ...
        const CTransactionRef tx = [&] {
            ...
            for (uint32_t i = 0; i < num_out; i++) {
                tx_mut.vout.emplace_back(0*sats, CScript{});
            }
    ...
    LIMITED_WHILE (outpoints.size() < 400 && fuzzed_data_provider.ConsumeBool(), 1000) {
        ...
        const CTransactionRef tx = [&] {
            ...
            for (uint32_t i = 0; i < num_out; i++) {
                ...
                if (payload_size) {
                    tx_mut.vout.emplace_back(0*sats, CScript() << OP_RETURN << std::vector<unsigned char>(payload_size));
                } else {
                    tx_mut.vout.emplace_back(0*sats, CScript{});
                }
            }
...
FUZZ_TARGET(txorphanage_sim)
{
    ...
    for (unsigned t = 0; t < NUM_TX; ++t) {
        ...
        if (t > 0 && rng.randrange(4) == 0) {
            ...
            for (unsigned output = 0; output < num_outputs; ++output) {
                ...
                tx.vout.emplace_back(0*sats, std::move(scriptpubkey));
            }
    ...
    LIMITED_WHILE (provider.remaining_bytes() > 0, 200) {
        ...
        while (true) {
            ...
            if (!oversized) break;
            // Find worst peer.
            FeeFrac worst_dos_score{0*sats, 1};
            ...
            assert(ByRatio{worst_dos_score} > ByRatio{FeeFrac(1*sats, 1)});

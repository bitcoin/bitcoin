...
FUZZ_TARGET(txorphanage_sim)
{
    ...
    LIMITED_WHILE(provider.remaining_bytes() > 0, 200) {
        ...
        while (true) {
            if (sim_announcements.size() < MAX_ANN && command-- == 0) {
                ...
                while (true) {
                    bool oversized = sim_announcements.size() > real->MaxGlobalAnnouncements();
                    if (!oversized) {
                ...
                real->LimitOrphans();
                // After invoking LimitOrphans, NeedsTrim cannot be true anymore (otherwise
                // LimitOrphans should have continued further).
                assert(!real->NeedsTrim());
                break;
            }
        }
    }
    ...
    // MaxGlobalAnnouncements
    assert(max_global_ann == real->MaxGlobalAnnouncements());
    // ReservedPeerUsage
    assert(reserved_peer_usage == real->ReservedPeerUsage());
    // MaxPeerAnnouncements
    auto present_peers = count_peers_fn();
    assert(max_global_ann / std::max<unsigned>(1, present_peers) == real->MaxPeerAnnouncements());
    // MaxGlobalUsage
    assert(reserved_peer_usage * std::max<unsigned>(1, present_peers) == real->MaxGlobalUsage());
    // NeedsTrim
    assert(real->NeedsTrim() == (sim_announcements.size() > max_global_ann || orphan_usage > reserved_peer_usage * present_peers));
}

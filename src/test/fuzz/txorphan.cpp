    ...
    LIMITED_WHILE (outpoints.size() < 400 && fuzzed_data_provider.ConsumeBool(), 1000) {
        ...
        const CTransactionRef tx = [&] {
            ...
            for (uint32_t i = 0; i < num_out; i++) {
                ...
                if (payload_size) {
                    tx_mut.vout.emplace_back(CAmount{0}, CScript() << OP_RETURN << std::vector<unsigned char>(payload_size));
                } else {
                    tx_mut.vout.emplace_back(CAmount{0}, CScript{});
                }
            }
...
FUZZ_TARGET(txorphanage_sim)
{
    ...
    LIMITED_WHILE (provider.remaining_bytes() > 0, 200) {
        ...
        while (true) {
            ...
            if (!oversized) break;
            // Find worst peer.
            FeeFrac worst_dos_score{CAmount{0}, 1};
            ...
            assert(ByRatio{worst_dos_score} > ByRatio{FeeFrac(CAmount{1}, 1)});

...
FUZZ_TARGET(txgraph)
{
    ...
    auto get_diagram_fn = [&](bool main_only) -> std::vector<FeePerWeight> {
        ...
        for (const auto& cluster : clusters) {
            ...
            for (const FeeFrac& chunk_feerate : ChunkLinearization(sim.graph, linearization)) {
                ret.push_back(FeePerWeight::FromFeeFrac(chunk_feerate));
            }
        }
    ...
    if (!sims[0].IsOversized()) {
        ...
        // Convert from FeePerWeight to a vector of FeeFracs, so it can be compared with the output
        // of ChunkLinearization.
        std::vector<FeeFrac> main_diagram_feefracs;
        main_diagram_feefracs.reserve(main_diagram.size());
        for (const auto& feerate : main_diagram) main_diagram_feefracs.push_back(feerate);
        auto expected_main_diagram = ChunkLinearization(sims[0].graph, vec1);
        assert(CompareChunks(main_diagram_feefracs, expected_main_diagram) == 0);
        ...
        if (sims.size() >= 2 && !sims[1].IsOversized()) {
            ...
            // tie breaker among equal-feerate FeePerWeights does not matter, but it has to be
            ...
            // by staging.
            std::vector<FeePerWeight> missing_main_cmp;
            ...
            std::vector<FeePerWeight> missing_stage_cmp;

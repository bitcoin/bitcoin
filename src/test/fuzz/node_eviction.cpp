...
FUZZ_TARGET(node_eviction)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 10000) {
        eviction_candidates.push_back({
            ...
            /*m_slow_chain_protected=*/fuzzed_data_provider.ConsumeBool(),
            /*m_successfully_connected=*/fuzzed_data_provider.ConsumeBool(),
        });
    }
    // Make a copy since eviction_candidates may be in some valid but otherwise
    // indeterminate state after the SelectInboundNodeToEvict(&&) call.
    const std::vector<NodeEvictionCandidate> eviction_candidates_copy = eviction_candidates;
    const std::optional<NodeId> node_to_evict = SelectInboundNodeToEvict(std::move(eviction_candidates));
    if (node_to_evict) {

...
FUZZ_TARGET(asmap_direct)
{
    ...
    // Checks on asmap
    auto asmap = BitsToBytes(std::span{buffer}.first(sep_pos));
    if (SanityCheckASMap(asmap, ip_len)) {
        ...
        for (size_t prefix_len = sep_pos - 1; prefix_len > 0; --prefix_len) {
            auto prefix = BitsToBytes(std::span{buffer}.first(prefix_len));
        ...
        auto addr = BitsToBytes(std::span{buffer}.subspan(sep_pos + 1));

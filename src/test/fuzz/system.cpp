...
FUZZ_TARGET(system, .init = initialize_system)
{
    ...
    // Add the -help-debug arg at the beginning of this test, because if it is
    // not added, later AddArg calls may add it with incorrect flags, causing
    // HelpRequested() to raise a logic_error when retrieving the arg.
    args_manager.AddArg("-help-debug", "", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 3000)
    {
        CallOneOf(
            ...
            [&] {
                ...
                // Avoid Can't call ForceSetArg on arg registered with flags[...]
                try {
            ...
            [&] {
                auto str_arg = fuzzed_data_provider.ConsumeRandomLengthString(16);
                auto str_value = fuzzed_data_provider.ConsumeRandomLengthString(16);
                // Can't call ForceSetArg on arg registered with flags[...]
                try {
            ...
            [&] {
                ...
                // Avoid Can't call SoftSetBoolArg on arg registered with flags[...]
                try {

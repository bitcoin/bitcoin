...
BOOST_AUTO_TEST_CASE(calculate_percentile_test)
{
    ...
    BOOST_CHECK(CalculatePercentiles({}, DEFAULT_BLOCK_MAX_WEIGHT).IsEmpty());
    ...
    for (int i = 0; i < chunk_size; ++i) {
        if (i < 3) {
            ...
            BOOST_CHECK(CalculatePercentiles(chunk_feerates, DEFAULT_BLOCK_MAX_WEIGHT).IsEmpty()); // CalculatePercentiles should return empty until reaching the 95th percentile
        } else {

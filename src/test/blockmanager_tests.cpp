...
BOOST_AUTO_TEST_CASE(blockmanager_find_block_pos)
{
    ...
    LOCK(::cs_main);
    BOOST_CHECK_EQUAL(blockman.WriteBlock(params->GenesisBlock(), 0).value().nPos, STORAGE_HEADER_BYTES);
    ...
    FlatFilePos actual{blockman.WriteBlock(params->GenesisBlock(), 1).value()};
...
BOOST_AUTO_TEST_CASE(blockmanager_flush_block_file)
{
    ...
    FlatFilePos pos1{blockman.WriteBlock(block1, /*nHeight=*/1).value()};

    // Write second block
    FlatFilePos pos2{blockman.WriteBlock(block2, /*nHeight=*/2).value()};

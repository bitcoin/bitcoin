...
BOOST_AUTO_TEST_CASE(dbwrapper)
{
    ...
    for (const bool obfuscate : {false, true}) {
        ...
        CDBWrapper dbw(m_logger, {.path = ph, .cache_bytes = 1 << 20, .memory_only = true, .wipe_data = false, .obfuscate = obfuscate});
        uint8_t key{'k'};
...
BOOST_AUTO_TEST_CASE(dbwrapper_basic_data)
{
    ...
    for (bool obfuscate : {false, true}) {
        ...
        CDBWrapper dbw(m_logger, {.path = ph, .cache_bytes = 1 << 20, .memory_only = false, .wipe_data = true, .obfuscate = obfuscate});
...
BOOST_AUTO_TEST_CASE(iterator_ordering)
{
    ...
    CDBWrapper dbw(m_logger, {.path = ph, .cache_bytes = 1 << 20, .memory_only = true, .wipe_data = false, .obfuscate = false});
...
BOOST_AUTO_TEST_CASE(unicodepath)
{
    ...
    CDBWrapper dbw(m_logger, {.path = ph, .cache_bytes = 1 << 20});

...
BOOST_AUTO_TEST_CASE(is_racing_true_within_window)
{
    ...
    // Racing check is a cheap, synchronous read of GetSummary() -- no thread or timeout needed.
    IndexSummary summary{filter_index.GetSummary()};
    int ahead = new_tip->nHeight - summary.best_block_height;
    BOOST_CHECK(summary.synced && ahead >= 0 && ahead <= CF_MAX_BLOCKS_AHEAD_RACE_WAIT);
...
BOOST_AUTO_TEST_CASE(is_racing_false_beyond_window)
{
    ...
    IndexSummary summary{filter_index.GetSummary()};
    for (size_t i = 0; i < new_tips.size(); ++i) {
        int ahead = new_tips[i]->nHeight - summary.best_block_height;
        bool racing = summary.synced && ahead >= 0 && ahead <= CF_MAX_BLOCKS_AHEAD_RACE_WAIT;
        if (i < 2) {
            BOOST_CHECK(racing);
        } else {
            BOOST_CHECK(!racing);
...
BOOST_AUTO_TEST_CASE(is_racing_false_for_stale_miss)
{
    ...
    IndexSummary summary{filter_index.GetSummary()};
    int ahead = old_block->nHeight - summary.best_block_height;
    BOOST_CHECK(!(summary.synced && ahead >= 0 && ahead <= CF_MAX_BLOCKS_AHEAD_RACE_WAIT));
...
BOOST_AUTO_TEST_CASE(is_racing_false_when_not_synced)
{
    ...
    BOOST_REQUIRE(tip);
    IndexSummary summary{filter_index.GetSummary()};
    int ahead = tip->nHeight - summary.best_block_height;
    BOOST_CHECK(!(summary.synced && ahead >= 0 && ahead <= CF_MAX_BLOCKS_AHEAD_RACE_WAIT));
...
// is queued but not yet processed, the racing check for B must still say yes.
// This exercises the `ahead == 0` case.
BOOST_AUTO_TEST_CASE(is_racing_true_for_same_height_reorg)
{
    ...
    IndexSummary summary{filter_index.GetSummary()};
    int ahead = b_index->nHeight - summary.best_block_height;
    BOOST_CHECK(summary.synced && ahead >= 0 && ahead <= CF_MAX_BLOCKS_AHEAD_RACE_WAIT);
...
// for the same deterministic control over the race window as the racing-check
// tests above.
BOOST_AUTO_TEST_CASE(deferred_cfilter_response_reaches_peer)
{
    ...
    BOOST_REQUIRE(new_tip);
    // sanity: we are actually in the window
    IndexSummary summary{filter_index->GetSummary()};
    int ahead = new_tip->nHeight - summary.best_block_height;
    BOOST_REQUIRE(summary.synced && ahead >= 0 && ahead <= CF_MAX_BLOCKS_AHEAD_RACE_WAIT);
...
// A peer that races the filter index with more getcfilters requests than the
// deferred-request queue can hold must have the excess silently dropped --
// not deferred, and not served -- once the race resolves.
BOOST_AUTO_TEST_CASE(deferred_cfilter_queue_drops_excess_requests)
{
    LOCK(NetEventsInterface::g_msgproc_mutex);

    constexpr int NUM_REQUESTS = MAX_PENDING_CFILTER_REQUESTS + 1;
    ...
    int cfilter_sent_count = 0;
    const auto CaptureMessageOrig = CaptureMessage;
    CaptureMessage = [&](const CAddress&, const std::string& msg_type, std::span<const unsigned char>, bool is_incoming) {
        if (!is_incoming && msg_type == NetMsgType::CFILTER) ++cfilter_sent_count;
    };

    // Send one more racing getcfilters request than the queue can hold, all
    // for the same block whose filter hasn't been written yet.
    ...
    BOOST_CHECK_EQUAL(cfilter_sent_count, 0);

    // Release the scheduler; CustomAppend runs, the filter lands on disk.
    blocker->Release();
    m_node.validation_signals->SyncWithValidationInterfaceQueue();

    // Only the first MAX_PENDING_CFILTER_REQUESTS made it into the queue; the
    // last one was dropped on arrival and must never be answered.
    m_node.peerman->SendMessages(peer);
    BOOST_CHECK_EQUAL(cfilter_sent_count, MAX_PENDING_CFILTER_REQUESTS);

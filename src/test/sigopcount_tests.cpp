...
{
    CScript s1;
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(/*fAccurate=*/false), 0U);
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(/*fAccurate=*/true), 0U);

    uint160 dummy;
    s1 << OP_1 << ToByteVector(dummy) << ToByteVector(dummy) << OP_2 << OP_CHECKMULTISIG;
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(/*fAccurate=*/true), 2U);
    s1 << OP_IF << OP_CHECKSIG << OP_ENDIF;
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(/*fAccurate=*/true), 3U);
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(/*fAccurate=*/false), 21U);
    ...
    BOOST_CHECK_EQUAL(s2.GetSigOpCount(/*fAccurate=*/true), 3U);
    BOOST_CHECK_EQUAL(s2.GetSigOpCount(/*fAccurate=*/false), 20U);

    p2sh = GetScriptForDestination(ScriptHash(s2));
    BOOST_CHECK_EQUAL(p2sh.GetSigOpCount(/*fAccurate=*/true), 0U);
    BOOST_CHECK_EQUAL(p2sh.GetSigOpCount(/*fAccurate=*/false), 0U);
    CScript scriptSig2;

...
public:
    ...
    // Pretend bytes moved in either direction at `when`, as
    // HTTPRemoteClient::Receive() and MaybeSendBytesFromBuffer() do when they
    // reset the idle timer.
    void NotIdleAt(SteadySeconds when)
    {
        SetIdleSince(when);
    }
    ...
    // cannot move.
    void RequestStartedAt(SteadySeconds when)
    {
...
BOOST_AUTO_TEST_CASE(http_request_deadline_tests)
{
...

    {
        ...
        // That first read started the deadline, so once it has passed the client
        // is disconnected even with a fresh idle timer. This relies on the read
        // above happening within a few seconds of `start`.
        client->NotIdleAt(expired);

...
BOOST_AUTO_TEST_CASE (generate_aliasmultiupdate)
{
	...
	for (unsigned int i = 0; i < MAX_ALIAS_UPDATES_PER_BLOCK; i++)
	{
		BOOST_CHECK_NO_THROW(r = CallRPC("node2", "aliasupdate jagmultiupdate changedata3 " + oldAddressStr + " false " + " 0 " + encryptionprivkey + " " + encryptionkey + " ''"));
		UniValue varray = r.get_array();
		BOOST_CHECK_NO_THROW(r = CallRPC("node2", "signrawtransaction " + varray[0].get_str()));
		BOOST_CHECK_NO_THROW(CallRPC("node2", "syscoinsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
	}

	BOOST_CHECK_THROW(r = CallRPC("node2", "aliasupdate jagmultiupdate changedata3 " + oldAddressStr + " false " + " 0 " + encryptionprivkey + " " + encryptionkey + " ''"), runtime_error);

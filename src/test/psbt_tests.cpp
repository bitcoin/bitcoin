...
BOOST_AUTO_TEST_CASE(psbt_merge_sighash_type)
{
    // PSBTInput::Merge must (a) copy sighash_type when only one side has it,
    // and (b) reject when both sides have conflicting values (BIP 174 combiner rules).
    CMutableTransaction mtx;
    mtx.vin.emplace_back(COutPoint{});
    mtx.vout.emplace_back(1, CScript());

    // Helper to build a minimal PSBTv0 with a given sighash_type on input 0.
    auto make_psbt = [&](std::optional<int> sighash) {
        PartiallySignedTransaction psbt(mtx, /*version=*/0);
        psbt.inputs[0].sighash_type = sighash;
        return psbt;
    };

    // Case 1: neither side has sighash_type — merge succeeds, field stays absent.
    {
        auto a = make_psbt(std::nullopt);
        auto b = make_psbt(std::nullopt);
        BOOST_CHECK(a.Merge(b));
        BOOST_CHECK(!a.inputs[0].sighash_type.has_value());
    }

    // Case 2: only the other PSBT has sighash_type — merge copies it over.
    {
        auto a = make_psbt(std::nullopt);
        auto b = make_psbt(SIGHASH_ALL);
        BOOST_CHECK(a.Merge(b));
        BOOST_CHECK(a.inputs[0].sighash_type.has_value());
        BOOST_CHECK_EQUAL(*a.inputs[0].sighash_type, SIGHASH_ALL);
    }

    // Case 3: only this PSBT has sighash_type — merge preserves it.
    {
        auto a = make_psbt(SIGHASH_NONE);
        auto b = make_psbt(std::nullopt);
        BOOST_CHECK(a.Merge(b));
        BOOST_CHECK(a.inputs[0].sighash_type.has_value());
        BOOST_CHECK_EQUAL(*a.inputs[0].sighash_type, SIGHASH_NONE);
    }

    // Case 4: both sides have the same sighash_type — merge succeeds.
    {
        auto a = make_psbt(SIGHASH_ALL);
        auto b = make_psbt(SIGHASH_ALL);
        BOOST_CHECK(a.Merge(b));
        BOOST_CHECK_EQUAL(*a.inputs[0].sighash_type, SIGHASH_ALL);
    }

    // Case 5: conflicting sighash types — merge must fail (BIP 174).
    {
        auto a = make_psbt(SIGHASH_ALL);
        auto b = make_psbt(SIGHASH_SINGLE);
        BOOST_CHECK(!a.Merge(b));
    }

    // Case 6: conflict with ANYONECANPAY variant.
    {
        auto a = make_psbt(SIGHASH_ALL);
        auto b = make_psbt(SIGHASH_ALL | SIGHASH_ANYONECANPAY);
        BOOST_CHECK(!a.Merge(b));
    }
}

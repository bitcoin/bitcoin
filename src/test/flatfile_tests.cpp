...
BOOST_AUTO_TEST_CASE(flatfile_open)
{
    ...
    {
        CAutoFile file(seq.Open(FlatFilePos(0, pos1, 1)), SER_DISK, CLIENT_VERSION);
        file << LIMITED_STRING(line1, 256);
    }

    // Attempt to append to file opened in read-only mode.
    {
        CAutoFile file(seq.Open(FlatFilePos(0, pos2, 1), true), SER_DISK, CLIENT_VERSION);
        BOOST_CHECK_THROW(file << LIMITED_STRING(line2, 256), std::ios_base::failure);
    }

    // Append second line to file.
    {
        CAutoFile file(seq.Open(FlatFilePos(0, pos2, 1)), SER_DISK, CLIENT_VERSION);
    ...
    {
        std::string text;
        CAutoFile file(seq.Open(FlatFilePos(0, pos1, 1), true), SER_DISK, CLIENT_VERSION);
    ...
    {
        std::string text;
        CAutoFile file(seq.Open(FlatFilePos(1, pos2, 1)), SER_DISK, CLIENT_VERSION);
...
BOOST_AUTO_TEST_CASE(flatfile_allocate)
{
    ...
    BOOST_CHECK_EQUAL(seq.Allocate(FlatFilePos(0, 0, 1), 1, out_of_space), 100);
    BOOST_CHECK_EQUAL(fs::file_size(seq.FileName(FlatFilePos(0, 0, 1))), 100);
    BOOST_CHECK(!out_of_space);

    BOOST_CHECK_EQUAL(seq.Allocate(FlatFilePos(0, 99, 1), 1, out_of_space), 0);
    BOOST_CHECK_EQUAL(fs::file_size(seq.FileName(FlatFilePos(0, 99, 1))), 100);
    BOOST_CHECK(!out_of_space);

    BOOST_CHECK_EQUAL(seq.Allocate(FlatFilePos(0, 99, 1), 2, out_of_space), 101);
    BOOST_CHECK_EQUAL(fs::file_size(seq.FileName(FlatFilePos(0, 99, 1))), 200);
    BOOST_CHECK(!out_of_space);
}

...
/** At namespace scope we can only compile declarations and no bare expressions.
 * This allows macros to place function pointers in the registry.
 */
struct Registrar {
...
/** Total run summary, reported separately from the log level */
struct RunSummary {
    int passed = 0;
    int failed = 0;
    int skipped = 0;
...
inline std::string to_hex(std::span<const char> s) { return to_hex(MakeUCharSpan(s)); }
inline std::string to_hex(std::span<const std::byte> s) { return to_hex(MakeUCharSpan(s)); }

/** These concepts determine the string representation of failing cases. */
template <typename T>
...
template <typename T>
concept has_hex_repr = requires(const T& v) { { to_hex(v) } -> std::same_as<std::string>; };

template <typename T>
concept is_pair_like = requires(const T& v) { v.first; v.second; };

template <typename T>
concept is_variant = requires(const T& v) {
    []<typename... Ts>(const std::variant<Ts...>&) {}(v);
};
...
template <typename T>
concept is_range_check = std::ranges::range<T> && !streamable<T> && !is_cstring<T> && !HasToString<T>;

template <typename T>
concept is_non_bool_char_int = std::is_integral_v<T> &&
                               !std::same_as<std::remove_cv_t<T>, bool> &&
                               !std::same_as<std::remove_cv_t<T>, char> &&
                               !std::same_as<std::remove_cv_t<T>, wchar_t> &&
                               !std::same_as<std::remove_cv_t<T>, char8_t>;
template <typename T>
constexpr bool is_null_cstring(const T& v)
{
...
/** The main debugging output for most types on test failure. */
template <typename T>
std::string stringify(const T& value)
{
    if constexpr (is_non_char_pointer<T>) {
        ...
        os << value;
        return os.str();
    } else if constexpr (has_hex_repr<T>) {
        return std::string{typeid(T).name()} + ": " + to_hex(value);
    } else if constexpr (is_pair_like<T>) {
...
inline std::string stringify(std::nullopt_t) { return "nullopt"; }
...
inline std::string stringify(const std::u8string& v)
{
...
template <typename T>
std::string stringify(const util::Result<T>& v)
{
    return v ? "OK" : "error: " + util::ErrorString(v).original;
}

template <typename T, typename E>
std::string stringify(const util::Expected<T, E>& v)
{
    return v ? "OK" : "error: " + stringify(v.error());
}
...
private:
    ...
    mutable std::mutex m_scoped_info_mutex;
    std::unordered_map<std::thread::id, std::vector<std::string>> m_scoped_info_map;
...
public:
    ...
    void push_scoped_info(std::string info)
    {
        std::scoped_lock lock{m_scoped_info_mutex};
        m_scoped_info_map[std::this_thread::get_id()].emplace_back(info);
    }

    void pop_scoped_info()
    {
        std::scoped_lock lock{m_scoped_info_mutex};
    ...
    std::optional<std::string> render_scoped_info() const
    {
        std::scoped_lock lock{m_scoped_info_mutex};
        auto it = m_scoped_info_map.find(std::this_thread::get_id());
...
/** Pushes contextual info on construction, pops it on destruction. The info is
 * appended to any check failure recorded while this guard is in scope. */
class InfoScopeGuard
{
    TestContext& m_ctx;

public:
    explicit InfoScopeGuard(std::string info) : m_ctx{test_context()}
    {
        m_ctx.push_scoped_info(std::move(info));
    }
    ~InfoScopeGuard() noexcept { m_ctx.pop_scoped_info(); }
    InfoScopeGuard(const InfoScopeGuard&) = delete;
    InfoScopeGuard& operator=(const InfoScopeGuard&) = delete;
    InfoScopeGuard(InfoScopeGuard&&) = delete;
    InfoScopeGuard& operator=(InfoScopeGuard&&) = delete;
};
...
Result check_equal_ranges(const R1& r1, const R2& r2)
{
    ...
    auto fmt = [](const E& v) -> std::string {
        ...
        } else if constexpr (std::same_as<E, unsigned char> || std::same_as<E, signed char> || std::same_as<E, char8_t>) {
...
struct CapturedExpression {
    ...
    /** The captured expression cannot preserve operator semantics, so this operator must be removed.
     * If you have made it to this line of source because your test is not compiling, try wrapping
     * the conditions in `( ... )`. */
    template <typename U>
...
#define DECOMPOSE_OP(op, cmp)                                                                                      \
    ...
    {                                                                                                              \
        if constexpr (std::string_view{#op} == "==" && is_range_check<T> && is_range_check<U>) {                   \
            return check_equal_ranges(lhs, rhs);                                                                   \
        } else {                                                                                                   \
            bool btc_test_result;                                                                                  \
            if constexpr (is_cstring<T> && is_cstring<U>) {                                                        \
                if (is_null_cstring(lhs) || is_null_cstring(rhs)) {                                                \
                    btc_test_result = (lhs op rhs);                                                                \
                } else {                                                                                           \
                    btc_test_result = (std::strcmp(lhs, rhs) op 0);                                                \
                }                                                                                                  \
            } else if constexpr (is_non_bool_char_int<T> && is_non_bool_char_int<U> &&                             \
...
inline ReportLevel to_report_level(std::string_view s)
{
...
inline CatchSystemError to_catch_system_error(std::string_view s)
{
...
struct RuntimeOptions {
    ...
    ReportLevel show_run_report{ReportLevel::Show};
...
inline std::optional<unsigned int> parse_shuffle_seed(const char* raw)
{
...
/** Register a test case using the enclosing suite's fixture, or no fixture if none is set.
 *
 *  e.g: `TEST_CASE(my_test) { ... }`
 * */
#define TEST_CASE(name) FIXTURE_TEST_CASE(name, btc_suite_fixture)

// In reference to the macro below:
...
// the test case. The function pointer is then forward declared, and
// is implement with the `runner()` function. That runner is registered
// with the test suite so it may be called later. Finally the function
// body is defined by the user.

/** Register a test case using a specified fixture.
 *
 * e.g: `FIXTURE_TEST_CASE(my_chain_test, SomeChainInit) { ... }`
 * */
...
/** Same as CHECK, however in the case of failure, no meaningful
 * expression will be output. This macro should generally be avoided,
 * unless the type does not having a meaningful output. */
...
/** Same as REQUIRE, however in the case of failure, no meaningful
 * expression will be output. This macro should generally be avoided,
 * unless the type does not having a meaningful output. */
#define REQUIRE_NO_DISPLAY(expr, ...)                                                                      \
    do {                                                                                                   \
        auto& btc_test_ctx_{::framework::test_context()};                                                  \
        if (!btc_test_ctx_.on_owner_thread()) {                                                            \
            ::framework::Result btc_test_res_{::framework::Result::failed(                                 \
...
/** Pushes contextual info that is appended to any CHECK/REQUIRE failure until
 * the enclosing scope exits. Nests. Accepts stream-style expressions.
 *
 * Example: TEST_INFO_SCOPE("i=" << i << " name=" << name); */
#define TEST_INFO_SCOPE(expr)                                              \
    std::ostringstream BITCOIN_TEST_CAT(btc_test_info_os_, __LINE__);      \
    BITCOIN_TEST_CAT(btc_test_info_os_, __LINE__) << expr;                 \
    ::framework::InfoScopeGuard BITCOIN_TEST_CAT(btc_test_info_, __LINE__) \
    {                                                                      \
        BITCOIN_TEST_CAT(btc_test_info_os_, __LINE__).str()                \
    }
...
#define BOOST_TEST_INFO_SCOPE(expr) TEST_INFO_SCOPE(expr)

#ifdef BITCOIN_TEST_MAIN
// This lint is to avoid ODR violations, but this function is behind an `ifdef`,
// so it will only be defined in a TU that invoke `define`. This should only occur
// once per test binary, as there is a single main function for each test binary.
// This lint can be safely ignored.

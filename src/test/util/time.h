...
class ElapseTime
{
    std::chrono::seconds m_t{std::chrono::seconds::max()};

public:
    /** Initialize with initial time */
    ElapseTime(std::chrono::seconds init_time) { set(init_time); }
    ElapseTime(NodeSeconds init_time) { set(init_time.time_since_epoch()); }
    ...
    /** Set mocktime */
    void set(std::chrono::seconds t) { SetMockTime(m_t = t); }
    void set(NodeSeconds t) { SetMockTime(m_t = t.time_since_epoch()); }

...
BOOST_AUTO_TEST_CASE(TxGraphClusterMergeTest)
{
    ...
    for (int i=0; i < 30; ++i) {
        all_entries.emplace_back(rng.randrange<unsigned int>(1000)+1, rng.randrange<unsigned int>(1000)+1);
        if (i < 10) {
...
BOOST_AUTO_TEST_CASE(TxGraphClusterSortTest)
{
    ...
    for (int i=0; i < 30; ++i) {
        all_entries.emplace_back(rng.randrange<unsigned>(1000)+1, rng.randrange<unsigned>(1000)+1);
        for (int j=0; j<i; ++j) {
            if (rng.randrange<unsigned>(4) == 0) {
...
BOOST_AUTO_TEST_CASE(TxGraphTest)
{
    ...
    for (int i=0; i<1000; ++i) {
        ...
        int rand_val = rng.randrange<int>(100);
        if (rand_val < 85) {
            // Add a random transaction, 85% of the time.
            TxEntry *entry = new TxEntry(rng.randrange<unsigned>(1000)+1, rng.randrange<unsigned>(1000)+1);
            all_entries.emplace_back(entry);
            std::vector<TxEntry::TxEntryRef> parents;
            for (size_t j=0; j<all_entries.size(); ++j) {
                if (rng.randrange<unsigned>(100) < 1 && in_mempool.count(all_entries[j]->unique_id)) {
            ...
            int64_t random_index = rng.randrange<unsigned>(all_entries.size());
            if (in_mempool.count(all_entries[random_index]->unique_id)) {
                ...
                TxEntry *new_entry = new TxEntry(rng.randrange<unsigned>(1000)+1, rng.randrange<unsigned>(1000)+1);
                ...
                if (changeset.AddTx(*new_entry, parents)) {
                    ...
                    if (rng.randrange<unsigned>(100) < 50) {
            ...
            uint64_t num_to_remove = rng.randrange<unsigned>(in_mempool.size()+1) / 20;
            ...
            int64_t num_invocations = rng.randrange<unsigned>(in_mempool.size()+1) / 20;
            ...
            int64_t num_to_add = rng.randrange<unsigned>(20)+1;
            ...
            for (int k=0; k<num_to_add; ++k) {
                all_entries.emplace_back(new TxEntry(rng.randrange<unsigned>(1000)+1, rng.randrange<unsigned>(1000)+1));
                ...
                for (auto &id : in_mempool) {
                    if (rng.randrange<unsigned>(100) < 10) {
                ...
                std::vector<TxEntry::TxEntryRef> parents;
                for (int m=0; m<k; ++m) {
                    if (rng.randrange<unsigned>(100) < 30) {

...
BOOST_AUTO_TEST_CASE(key_io_valid_gen)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        if (isPrivkey) {
        ...
        } else if (isSilentPayments && silentPaymentsVersion == 0) {
            ...
            CTxDestination dest = UnknownSilentPaymentsDestination{static_cast<unsigned int>(silentPaymentsVersion), scan_pubkey, spend_pubkey, std::move(extra_data)};
            BOOST_CHECK_EQUAL(EncodeDestination(dest), exp_base58string);
        } else {
            CTxDestination dest;

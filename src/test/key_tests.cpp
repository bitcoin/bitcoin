...
{
    // When entropy is specified without grind, we should see at least one high R signature within 20 signatures
    ...
    bool found = false;
    for (int i = 1; i <=20; ++i) {
        arith_uint256 extra_entropy(i);
        sig.clear();
        BOOST_CHECK(key.Sign(msg_hash, sig, false, &extra_entropy));

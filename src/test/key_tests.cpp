...
BOOST_AUTO_TEST_CASE(batch_schnorr_verifier_schnorr_test_vectors)
{
    std::vector<SchnorrSignatureToVerify> valids;
    std::vector<SchnorrSignatureToVerify> invalids;

    auto make_stv = [](const std::array<std::string,3>& triple) -> SchnorrSignatureToVerify {
        SchnorrSignatureToVerify stv{
            /* sig    */ {},
            /* pubkey */ XOnlyPubKey(ParseHex(triple[0])),
            /* hash   */ uint256(ParseHex(triple[1]))
        };
        const std::vector<unsigned char> sig{ParseHex(triple[2])};
        Assert(sig.size() == stv.sig.size());
        std::copy(sig.begin(), sig.end(), stv.sig.begin());
        return stv;
    };

    for (const auto& test : VECTORS) {
        if (test.second) {
            valids.push_back(make_stv(test.first));
        } else {
            XOnlyPubKey pubkey(ParseHex(test.first[0]));
            if (!pubkey.IsFullyValid()) {
                // malformed pubkey, secp would reject it at parse time
                continue;
            }
            invalids.push_back(make_stv(test.first));
        }
    }

    // Empty batch object should be valid
    BatchSchnorrVerifier batch;
    BOOST_CHECK(batch.Verify());

    // Add all valid signatures and check validity of batch object at each step
    for (const auto& valid : valids) {
        BOOST_CHECK(batch.Add(valid.sig, valid.pubkey, valid.sighash));
        BOOST_CHECK(batch.Verify());
    }

    // Clean up and ensure reset batch object is valid
    batch.Reset();
    BOOST_CHECK(batch.Verify());

    for (const auto& invalid : invalids) {
        // Make a randomized valid batch object
        for (const auto& valid : valids) {
            if (m_rng.randbool()) {
                BOOST_CHECK(batch.Add(valid.sig, valid.pubkey, valid.sighash));
            }
        ...
        // Each invalid test vector should either be rejected to add or cause
        // the batch to fail verification
        bool accepted = batch.Add(invalid.sig, invalid.pubkey, invalid.sighash);
        if (accepted) {
            BOOST_CHECK(!batch.Verify());
        }
        batch.Reset();

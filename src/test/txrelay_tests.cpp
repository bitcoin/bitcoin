...
BOOST_AUTO_TEST_CASE(txrelay_inventory_gated_until_send_scheduled)
{
    ...
    BOOST_CHECK(!tx_relay.PushInventory(std::array{wtxid}));
    BOOST_CHECK(!tx_relay.PushInventory({}));
...
BOOST_AUTO_TEST_CASE(txrelay_inventory_queues_known_and_duplicate_transactions)
{
    node::TxRelay tx_relay;
    ...
    tx_relay.PushInventory(std::array{Wtxid::FromUint256(first_hash)});
    BOOST_CHECK_EQUAL(tx_relay.GetInventoryStats().m_inv_to_send, 1U);

    const uint256 known_hash{uint256{2}};
    tx_relay.AddKnownTx(known_hash);
    BOOST_CHECK(tx_relay.TxInventoryKnownContains(known_hash));
    tx_relay.PushInventory(std::array{Wtxid::FromUint256(known_hash), Wtxid::FromUint256(known_hash)});
    BOOST_CHECK_EQUAL(tx_relay.GetInventoryStats().m_inv_to_send, 3U);

    // Known inventory and duplicates are filtered when selecting announcements.
    auto batch{tx_relay.StartTxInventoryBatch(/*send_trickle=*/true)};
    const std::vector<Wtxid> expected{Wtxid::FromUint256(first_hash), Wtxid::FromUint256(known_hash), Wtxid::FromUint256(known_hash)};
    BOOST_CHECK(batch.QueuedCandidates() == expected);
}
...
BOOST_AUTO_TEST_CASE(txrelay_inventory_batch_moves_and_returns_queued_inventory)
{
    ...
    // Inventory queued during batch processing must survive its return, even
    // when the new and returned entries contain the same wtxid.
    tx_relay.PushInventory(std::array{Wtxid::FromUint256(hash)});
    tx_relay.ReturnTxInventory(std::move(batch));
    BOOST_CHECK_EQUAL(tx_relay.GetInventoryStats().m_inv_to_send, 2U);

    auto processed_batch{tx_relay.StartTxInventoryBatch(/*send_trickle=*/true)};
    const auto candidates{processed_batch.TakeQueuedCandidates()};
    BOOST_CHECK_EQUAL(candidates.size(), 2U);
    BOOST_CHECK(processed_batch.QueuedCandidates().empty());
    tx_relay.ReturnTxInventory(std::move(processed_batch));
    BOOST_CHECK_EQUAL(tx_relay.GetInventoryStats().m_inv_to_send, 0U);

    tx_relay.PushInventory(std::array{Wtxid::FromUint256(hash)});
    auto mempool_batch{tx_relay.StartTxInventoryBatch(/*send_trickle=*/true)};
    tx_relay.PushInventory(std::array{Wtxid::FromUint256(hash)});
    mempool_batch.ClearQueued();
    tx_relay.ReturnTxInventory(std::move(mempool_batch));
    BOOST_CHECK_EQUAL(tx_relay.GetInventoryStats().m_inv_to_send, 1U);
}
...
BOOST_AUTO_TEST_CASE(txrelay_inventory_batch_conserves_under_concurrent_push)
{
    ...
    std::thread producer{[&] {
        for (const auto& wtxid : universe) {
            tx_relay.PushInventory(std::array{wtxid});
        }

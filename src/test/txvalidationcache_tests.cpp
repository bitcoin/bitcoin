...
// Both CheckInputScripts and PrepareInputScriptChecks use the same caching
...
static void ValidateCheckInputsForAllFlags(const CTransaction &tx, uint32_t failing_flags, bool add_to_cache, CCoinsViewCache& active_coins_tip, ValidationCache& validation_cache) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    for (int count = 0; count < 10000; ++count) {
        ...
        if (ret && add_to_cache) {
            ...
            std::vector<CScriptCheck> scriptchecks = PrepareInputScriptChecks(tx, &active_coins_tip, test_flags, true, add_to_cache, txdata, validation_cache);
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, Dersig100Setup)
{
    ...
    // spend_tx is invalid according to DERSIG
    {
        ...
        // If we call PrepareInputScriptChecks to get scriptchecks instead (as happens in
        // ConnectBlock), we should add a script check object for this -- we're
        // not caching invalidity (if that changes, delete this test case).
        std::vector<CScriptCheck> scriptchecks = PrepareInputScriptChecks(CTransaction(spend_tx), &m_node.chainman->ActiveChainstate().CoinsTip(), SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG, true, true, ptd_spend_tx, m_node.chainman->m_validation_cache);
...

    {
        ...
        // input was valid)
        std::vector<CScriptCheck> scriptchecks = PrepareInputScriptChecks(CTransaction(tx), &m_node.chainman->ActiveChainstate().CoinsTip(), SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, true, true, txdata, m_node.chainman->m_validation_cache);

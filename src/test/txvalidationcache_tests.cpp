...
static void ValidateCheckInputsForAllFlags(const CTransaction &tx, uint32_t failing_flags, bool add_to_cache, CCoinsViewCache& active_coins_tip, ValidationCache& validation_cache) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    for (const auto& txin : tx.vin) {
        ...
        const Coin& coin = active_coins_tip.AccessCoin(prevout);
        assert(!coin.IsSpent());
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, Dersig100Setup)
{
    ...
    // spend_tx is invalid according to DERSIG
    {
        ...
        std::vector<CTxOut> spent_outputs;
        spent_outputs.reserve(spend_tx.vin.size());
        for (const auto& txin : spend_tx.vin) {
            const COutPoint& prevout = txin.prevout;
            const Coin& coin = m_node.chainman->ActiveChainstate().CoinsTip().AccessCoin(prevout);
            assert(!coin.IsSpent());
            spent_outputs.emplace_back(coin.out);
        }
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        std::vector<CTxOut> spent_outputs;
        spent_outputs.reserve(invalid_with_cltv_tx.vin.size());
        for (const auto& txin : invalid_with_cltv_tx.vin) {
            const COutPoint& prevout = txin.prevout;
            const Coin& coin = m_node.chainman->ActiveChainstate().CoinsTip().AccessCoin(prevout);
            assert(!coin.IsSpent());
            spent_outputs.emplace_back(coin.out);
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        std::vector<CTxOut> spent_outputs;
        spent_outputs.reserve(invalid_with_csv_tx.vin.size());
        for (const auto& txin : invalid_with_csv_tx.vin) {
            const COutPoint& prevout = txin.prevout;
            const Coin& coin = m_node.chainman->ActiveChainstate().CoinsTip().AccessCoin(prevout);
            assert(!coin.IsSpent());
            spent_outputs.emplace_back(coin.out);

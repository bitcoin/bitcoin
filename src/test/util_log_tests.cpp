...
BOOST_AUTO_TEST_CASE(dispatcher_no_filter)
{
    // Without a filter predicate, log callbacks should always be dispatched.
    Dispatcher dispatcher{};
    int callback_count{0};
    auto handle = dispatcher.RegisterCallback([&](const Entry&) { ++callback_count; });

    dispatcher.Log(Level::Trace, /*category=*/0, __func__, /*should_ratelimit=*/false, "trace");
    dispatcher.Log(Level::Info, /*category=*/0, __func__, /*should_ratelimit=*/false, "info");
    dispatcher.Log(Level::Error, /*category=*/0, __func__, /*should_ratelimit=*/false, "error");
    BOOST_CHECK_EQUAL(callback_count, 3);

    dispatcher.UnregisterCallback(handle);
}

BOOST_AUTO_TEST_CASE(dispatcher_filter)
{
    ...
    // Level is set to Debug, so Trace is skipped
    min_level = Level::Debug;
    dispatcher.Log(Level::Trace, /*category=*/0, __func__, /*should_ratelimit=*/false, "skipped");
    dispatcher.Log(Level::Debug, /*category=*/0, __func__, /*should_ratelimit=*/false, "logged");
    BOOST_CHECK_EQUAL(callback_count, 1);

    // Raise min level to Warning
    callback_count = 0;
    min_level = Level::Warning;
    dispatcher.Log(Level::Info, /*category=*/0, __func__, /*should_ratelimit=*/false, "skipped");
    dispatcher.Log(Level::Warning, /*category=*/0, __func__, /*should_ratelimit=*/false, "logged");
    BOOST_CHECK_EQUAL(callback_count, 1);

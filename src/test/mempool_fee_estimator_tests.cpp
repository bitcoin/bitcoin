...
{
    auto mempool_estimator = MemPoolFeeRateEstimator(MempoolPolicyFeeestPath(*m_node.args), m_node.mempool.get(), &(m_node.chainman->ActiveChainstate()));
    ...
    {
        // Test when targetBlocks > MEMPOOL_FEE_ESTIMATOR_MAX_TARGET
        const auto result = mempool_estimator.EstimateFeeRate(conf_target, /*conservative=*/true);
        BOOST_CHECK(result.feerate.IsEmpty());
        BOOST_CHECK(result.errors.back() == data_err);
    }
    {
    ...
    // Mempool transactions are enough to provide feerate estimate
    {
        ...
        BOOST_CHECK(!result_conservative.feerate.IsEmpty() &&
                    !result_economical.feerate.IsEmpty());

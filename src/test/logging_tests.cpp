...
BOOST_FIXTURE_TEST_CASE(logging_LogPrintStr, LogSetup)
{
    ...
    std::vector<std::string> expected = {
        "[" + file_name + ":" + util::ToString(source_locs[0].line()) + "] [fn1] [net] foo1: bar1",
        "[" + file_name + ":" + util::ToString(source_locs[1].line()) + "] [fn2] [net:info] foo2: bar2",
        "[" + file_name + ":" + util::ToString(source_locs[2].line()) + "] [fn3] [debug] foo3: bar3",
        "[" + file_name + ":" + util::ToString(source_locs[3].line()) + "] [fn4] foo4: bar4",
        "[" + file_name + ":" + util::ToString(source_locs[4].line()) + "] [fn5] [debug] foo5: bar5",
        "[" + file_name + ":" + util::ToString(source_locs[5].line()) + "] [fn6] foo6: bar6",
    };
...
BOOST_AUTO_TEST_CASE(rate_limiting)
{
    ...
    for (int i = 0; i < 2048; ++i) {
        ...
        BOOST_CHECK_MESSAGE(log_file_size < fs::file_size(LogInstance().m_file_path), "location 0 should be able to log to disk, when rate limiting is disabled");
    }

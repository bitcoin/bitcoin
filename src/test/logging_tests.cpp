...
struct LogSetup : public BasicTestingSetup {
    ...
    ~LogSetup()
    {
        ...
        LogPrintf(LogInstance(), "Sentinel log to reopen log file\n");
...
BOOST_FIXTURE_TEST_CASE(logging_LogPrintf_, LogSetup)
{
    ...
    LogInstance().LogPrintf_("fn1", "src1", 1, BCLog::LogFlags::NET, BCLog::Level::Debug, "foo1: %s\n", "bar1");
    LogInstance().LogPrintf_("fn2", "src2", 2, BCLog::LogFlags::NET, BCLog::Level::Info, "foo2: %s\n", "bar2");
    LogInstance().LogPrintf_("fn3", "src3", 3, BCLog::LogFlags::ALL, BCLog::Level::Debug, "foo3: %s\n", "bar3");
    LogInstance().LogPrintf_("fn4", "src4", 4, BCLog::LogFlags::ALL, BCLog::Level::Info, "foo4: %s\n", "bar4");
    LogInstance().LogPrintf_("fn5", "src5", 5, BCLog::LogFlags::NONE, BCLog::Level::Debug, "foo5: %s\n", "bar5");
    LogInstance().LogPrintf_("fn6", "src6", 6, BCLog::LogFlags::NONE, BCLog::Level::Info, "foo6: %s\n", "bar6");
...
{
    LogPrintf(LogInstance(), "foo5: %s\n", "bar5");
    LogPrint(LogInstance(), BCLog::NET, "foo6: %s\n", "bar6");
    LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Trace, "foo4: %s\n", "bar4"); // not logged
    LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Debug, "foo7: %s\n", "bar7");
    LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Info, "foo8: %s\n", "bar8");
    LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Warning, "foo9: %s\n", "bar9");
    LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Error, "foo10: %s\n", "bar10");
    LogPrintfCategory(LogInstance(), BCLog::VALIDATION, "foo11: %s\n", "bar11");
    std::ifstream file{tmp_log_path};
    std::vector<std::string> log_lines;
    for (std::string log; std::getline(file, log);) {
        log_lines.push_back(log);
...
{
    LogTrace(LogInstance(), BCLog::NET, "foo6: %s\n", "bar6"); // not logged
    LogDebug(LogInstance(), BCLog::NET, "foo7: %s\n", "bar7");
    LogInfo(LogInstance(), "foo8: %s\n", "bar8");
    LogWarning(LogInstance(), "foo9: %s\n", "bar9");
    LogError(LogInstance(), "foo10: %s\n", "bar10");
    std::ifstream file{tmp_log_path};
    std::vector<std::string> log_lines;
    for (std::string log; std::getline(file, log);) {
        log_lines.push_back(log);
...
BOOST_FIXTURE_TEST_CASE(logging_LogPrintMacros_CategoryName, LogSetup)
{
    ...
    for (const auto& [category, name] : expected_category_names) {
        LogPrint(LogInstance(), category, "foo: %s\n", "bar");
...
BOOST_FIXTURE_TEST_CASE(logging_SeverityLevels, LogSetup)
{
    ...
    // Global log level
    LogPrintLevel(LogInstance(), BCLog::HTTP, BCLog::Level::Info, "foo1: %s\n", "bar1");
    LogPrintLevel(LogInstance(), BCLog::MEMPOOL, BCLog::Level::Trace, "foo2: %s. This log level is lower than the global one.\n", "bar2");
    LogPrintLevel(LogInstance(), BCLog::VALIDATION, BCLog::Level::Warning, "foo3: %s\n", "bar3");
    LogPrintLevel(LogInstance(), BCLog::RPC, BCLog::Level::Error, "foo4: %s\n", "bar4");

    // Category-specific log level
    LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Warning, "foo5: %s\n", "bar5");
    LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Debug, "foo6: %s. This log level is the same as the global one but lower than the category-specific one, which takes precedence. \n", "bar6");
    LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Error, "foo7: %s\n", "bar7");

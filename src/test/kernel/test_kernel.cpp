...
BOOST_AUTO_TEST_CASE(btck_transaction_input)
{
    ...
    // P2PKH: DER sig + compressed pubkey push = 106 bytes.
    BOOST_CHECK_EQUAL(input_0.GetScriptSig().size(), 106);
    BOOST_CHECK_EQUAL(input_1.GetScriptSig().size(), 106);

    // Native SegWit inputs have no scriptSig, witness carries the data.
    ...
    BOOST_CHECK(segwit_tx.GetInput(0).GetScriptSig().empty());
}

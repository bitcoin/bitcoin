...
BOOST_FIXTURE_TEST_CASE(rbf_conflicts_calculator, TestChain100Setup)
{
    ...
    TryAddToMempool(pool, entry.Fee(normal_fee).FromTx(parent_tx));
...
BOOST_FIXTURE_TEST_CASE(calc_feerate_diagram_rbf, TestChain100Setup)
{
    ...
    const auto low_size = entry_low->GetTxWeight();
    ...
    // Replacement of size 1
    {
        ...
        std::vector<FeeFrac> expected_new_chunks{{0, entry_replacement.GetTxWeight()}};
    ...
    // Non-zero replacement fee/size
    {
        ...
        std::vector<FeeFrac> expected_new_diagram{{high_fee, entry_replacement.GetTxWeight()}};
    ...
    const auto high_size = entry_high->GetTxWeight();

    {
        ...
        std::vector<FeeFrac> expected_new_chunks{{high_fee, entry_replacement.GetTxWeight()}};
    ...
    // Conflict with the 2nd tx, resulting in new diagram with three entries
    {
        ...
        std::vector<FeeFrac> expected_new_chunks{{high_fee, entry_replacement.GetTxWeight()}, {low_fee, low_size}};
    ...
    const auto high_size_2 = entry_high_2->GetTxWeight();
    ...
    const auto low_size_2 = entry_low_2->GetTxWeight();

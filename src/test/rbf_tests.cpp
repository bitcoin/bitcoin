...
BOOST_FIXTURE_TEST_CASE(rbf_helper_functions, TestChain100Setup)
{
    ...
    BOOST_CHECK(PaysForRBF(/*original_fees=*/high_fee,
                           ...
                           /*replacement_vsize=*/1,
                           /*relay_fee=*/CFeeRate(0*sats),
                           /*txid=*/unused_txid)
                           == std::nullopt);
    BOOST_CHECK(PaysForRBF(high_fee, high_fee - 1*sats, 1, CFeeRate(0*sats), unused_txid).has_value());
    BOOST_CHECK(PaysForRBF(high_fee + 1*sats, high_fee, 1, CFeeRate(0*sats), unused_txid).has_value());
    // Additional fees must cover the replacement's vsize at incremental relay fee
    BOOST_CHECK(PaysForRBF(high_fee, high_fee + 1*sats, 11, incremental_relay_feerate, unused_txid).has_value());
    BOOST_CHECK(PaysForRBF(high_fee, high_fee + 1*sats, 10, incremental_relay_feerate, unused_txid) == std::nullopt);
    BOOST_CHECK(PaysForRBF(high_fee, high_fee + 2*sats, 11, higher_relay_feerate, unused_txid).has_value());
    BOOST_CHECK(PaysForRBF(high_fee, high_fee + 4*sats, 20, higher_relay_feerate, unused_txid) == std::nullopt);
    BOOST_CHECK(PaysForRBF(low_fee, high_fee, 99999999, incremental_relay_feerate, unused_txid).has_value());
    BOOST_CHECK(PaysForRBF(low_fee, high_fee + 99999999*sats, 99999999, incremental_relay_feerate, unused_txid) == std::nullopt);
}
...
BOOST_FIXTURE_TEST_CASE(improves_feerate, TestChain100Setup)
{
    ...
    changeset->StageRemoval(entry1);
    changeset->StageRemoval(entry2);
    changeset->StageAddition(tx1_conflict, tx1_fee+1*sats, 0, 1, 0, false, 4, LockPoints());
    changeset->StageAddition(tx3, tx2_fee, 0, 1, 0, false, 4, LockPoints());
    ...
    pool.PrioritiseTransaction(entry1->GetSharedTx()->GetHash(), /*nFeeDelta=*/1*sats);
    changeset = pool.GetChangeSet();
    changeset->StageRemoval(entry1);
    changeset->StageRemoval(entry2);
    changeset->StageAddition(tx1_conflict, tx1_fee+1*sats, 0, 1, 0, false, 4, LockPoints());
    ...
    changeset->StageAddition(tx1_conflict, tx1_fee, 0, 1, 0, false, 4, LockPoints());
    changeset->StageAddition(entry4.GetSharedTx(), tx2_fee + entry5->GetModifiedFee() + 1*sats, 0, 1, 0, false, 4, LockPoints());
...
BOOST_FIXTURE_TEST_CASE(calc_feerate_diagram_rbf, TestChain100Setup)
{
    ...
    // Replacement of size 1
    {
        ...
        changeset->StageAddition(replacement_tx, 0*sats, 0, 1, 0, false, 4, LockPoints());
        ...
        std::vector<FeeFrac> expected_new_chunks{{0*sats, entry_replacement.GetAdjustedWeight()}};
...
BOOST_AUTO_TEST_CASE(feerate_chunks_utilities)
{
    ...
    std::vector<FeeFrac> old_chunks{{{950*sats, 300}, {100*sats, 100}}};
    std::vector<FeeFrac> new_chunks{{{1000*sats, 300}, {50*sats, 100}}};
    ...
    old_chunks = {{950*sats, 300}, {100*sats, 100}};
    new_chunks = {{1000*sats, 300}, {0*sats, 100}};
    ...
    old_chunks = {{950*sats, 300}, {100*sats, 100}};
    new_chunks = {{1100*sats, 100}, {0*sats, 100}};
    ...
    old_chunks = {{950*sats, 300}, {100*sats, 100}};
    new_chunks = {{750*sats, 100}, {249*sats, 250}, {151*sats, 650}};
    ...
    old_chunks = {{950*sats, 300}, {100*sats, 100}};
    new_chunks = {{750*sats, 100}, {250*sats, 250}, {150*sats, 150}};
    ...
    old_chunks = {{950*sats, 300}, {100*sats, 100}};
    new_chunks = {{950*sats, 300}, {100*sats, 100}};
    ...
    old_chunks = {{950*sats, 300}, {100*sats, 99}};
    new_chunks = {{950*sats, 300}, {100*sats, 100}};
    ...
    old_chunks = {{950*sats, 300}, {100*sats, 99}};
    new_chunks = {{950*sats, 300}, {100*sats, 100}, {0*sats, 1}, {0*sats, 1}};

    BOOST_CHECK(std::is_gt(CompareChunks(old_chunks, new_chunks)));
    BOOST_CHECK(std::is_lt(CompareChunks(new_chunks, old_chunks)));

    // Multiple tail fee check steps, unordered result
    new_chunks = {{950*sats, 300}, {100*sats, 100}, {0*sats, 1}, {0*sats, 1}, {1*sats, 1}};
    BOOST_CHECK(CompareChunks(old_chunks, new_chunks) == std::partial_ordering::unordered);
    BOOST_CHECK(CompareChunks(new_chunks, old_chunks) == std::partial_ordering::unordered);
}

...
BOOST_AUTO_TEST_CASE(package_submission_tests)
{
    ...
    mineBlocks(3);
    MockMempoolMinFee(CFeeRate(CAmount{5000}), *m_node.mempool);
    LOCK(cs_main);
    ...
    // In-mempool parent and child with missing parent.
    {
        ...
        auto tx_parent_2 = MakeTransactionRef(CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[2], /*input_vout=*/0,
                                                                            /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                                            /*output_destination=*/parent_locking_script,
                                                                            /*output_amount=*/CAmount(50 * COIN - CAmount{800}), /*submit=*/false));
...
BOOST_AUTO_TEST_CASE(package_single_tx)
{
    ...
    auto mtx_single_low_fee = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                    /*output_destination=*/single_locking_script,
                                                    /*output_amount=*/CAmount(49 * COIN - CAmount{1}), /*submit=*/false);
...
BOOST_AUTO_TEST_CASE(package_cpfp_tests)
{
    ...
    // child pays enough for the package feerate to meet the threshold.
    {
        ...
        if (auto err_cpfp{CheckPackageMempoolAcceptResult(package_cpfp, submit_cpfp, /*expect_valid=*/true, m_node.mempool.get())}) {
            ...
            BOOST_CHECK(it_child->second.m_wtxids_fee_calculations.value() == expected_wtxids);
            BOOST_CHECK(expected_feerate.GetFeePerK() > CAmount{1000});
        }
        expected_pool_size += 2;
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
    }
...
BOOST_AUTO_TEST_CASE(package_rbf_tests)
{
    ...
    // Test that de-duplication works. This is not actually package rbf.
    {
        ...
        CTransactionRef tx_child_1 = MakeTransactionRef(CreateValidMempoolTransaction(tx_parent, 0, 101, child_key, child_spk, coinbase_value - low_fee_amt - CAmount{300}, false));
        package1.push_back(tx_child_1);
        CTransactionRef tx_child_2 = MakeTransactionRef(CreateValidMempoolTransaction(tx_parent, 0, 101, child_key, child_spk, coinbase_value - low_fee_amt - CAmount{500}, false));
    ...
    // Test package rbf.
    {
        ...
        BOOST_CHECK_EQUAL(it_parent_3->second.m_effective_feerate.value().GetFee(package3_total_vsize), CAmount{199 + 1300});
        BOOST_CHECK_EQUAL(it_child_3->second.m_effective_feerate.value().GetFee(package3_total_vsize), CAmount{199 + 1300});
        ...
        m_node.mempool->PrioritiseTransaction(tx_child_1->GetHash(), CAmount{1363});

...
inline CTransactionRef create_placeholder_tx(size_t num_inputs, size_t num_outputs)
{
    ...
    auto random_script = CScript() << ToByteVector(g_rng.rand256()) << ToByteVector(g_rng.rand256());
    for (size_t i{0}; i < num_inputs; ++i) {
        mtx.vin[i].prevout.hash = Txid::FromUint256(g_rng.rand256());
...
BOOST_FIXTURE_TEST_CASE(package_sanitization_tests, TestChain100Setup)
{
    ...
    COutPoint same_prevout{Txid::FromUint256(g_rng.rand256()), 0};
    ...
    const COutPoint rand_prevout{Txid::FromUint256(g_rng.rand256()), 0};

...
BOOST_AUTO_TEST_CASE(RegisterPeerTest)
{
    ...
    BOOST_REQUIRE(!tracker.RegisterPeer(0, true, 1, salt).has_value());
    BOOST_REQUIRE(tracker.IsPeerRegistered(0));
    BOOST_REQUIRE(!tracker.IsPeerRegistered(1));
    tracker.PreRegisterPeer(1);
    BOOST_REQUIRE(!tracker.RegisterPeer(1, false, 1, salt).has_value());
    BOOST_REQUIRE(tracker.IsPeerRegistered(1));

    // Reconciliation version is higher than ours, should be able to register.
    BOOST_REQUIRE(!tracker.IsPeerRegistered(2));
    tracker.PreRegisterPeer(2);
    BOOST_REQUIRE(!tracker.RegisterPeer(2, true, 2, salt).has_value());
    BOOST_REQUIRE(tracker.IsPeerRegistered(2));

    // Try registering for the second time.
    BOOST_REQUIRE_EQUAL(tracker.RegisterPeer(1, false, 1, salt).value(), ReconciliationError::ALREADY_REGISTERED);

    // Do not register if there were no pre-registration for the peer.
    BOOST_REQUIRE_EQUAL(tracker.RegisterPeer(100, true, 1, salt).value(), ReconciliationError::NOT_FOUND);
...
BOOST_AUTO_TEST_CASE(IsPeerNextToReconcileWithTest)
{
    ...
    // right away
    BOOST_REQUIRE(!tracker.RegisterPeer(peer_id0, false, TXRECONCILIATION_VERSION, 1).has_value());
    ...
    BOOST_REQUIRE(!tracker.RegisterPeer(peer_id1, false, TXRECONCILIATION_VERSION, 1).has_value());
    tracker.PreRegisterPeer(peer_id2);
    BOOST_REQUIRE(!tracker.RegisterPeer(peer_id2, false, TXRECONCILIATION_VERSION, 1).has_value());
...
BOOST_AUTO_TEST_CASE(HandleSketchTest)
{
    ...
    for (size_t i=0; i < added_txs.size(); i++) {
        if (i % 2 == 0) {
            auto short_id = ComputeShortIDHelper(Wtxid::FromUint256(added_txs[i]), full_salt);
    ...
    BOOST_REQUIRE(std::holds_alternative<ReconCoefficients>(tracker.InitiateReconciliationRequest(peer_id0))); // Re set the peer's phase
    add_txs_to_reconset(peer_id0, added_txs, n_txs_to_add); // Sets are cleared after reconciliation, so add more txs

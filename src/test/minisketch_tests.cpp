...
{
    for (int i = 0; i < 100; ++i) {
        uint32_t errors = 0 + g_rng.randrange(11);
        uint32_t start_a = 1 + g_rng.randrange(1000000000);
        uint32_t a_not_b = g_rng.randrange(errors + 1);
        uint32_t b_not_a = errors - a_not_b;
        uint32_t both = g_rng.randrange(10000);

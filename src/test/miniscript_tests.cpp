...
void Test(const std::string& ms, const std::string& hexscript, int mode, const KeyConverter& converter, int opslimit = -1, int stacklimit = -1, std::optional<uint32_t> max_wit_size = std::nullopt)
{
...
void Test(const std::string& ms, const std::string& hexscript, const std::string& hextapscript, int mode, int opslimit = -1, int stacklimit = -1, std::optional<uint32_t> max_wit_size = std::nullopt)
{
    CONVERTER.SetContext(miniscript::MiniscriptContext::P2WSH);
    Test(ms, hexscript, mode, CONVERTER, opslimit, stacklimit, max_wit_size);
    CONVERTER.SetContext(miniscript::MiniscriptContext::TAPSCRIPT);
    Test(ms, hextapscript == "=" ? hexscript : hextapscript, mode, CONVERTER, opslimit, stacklimit, max_wit_size);
}
} // namespace

...
BOOST_FIXTURE_TEST_CASE(tx_rejection_types, TestChain100Setup)
{
    ...
    for (const auto& [result, expected_behavior] : expected_behaviors) {
        ...
        Behaviors actual_behavior{
            /*txid_rejects=*/txdownload_impl.RecentRejectsFilter().contains(txid.ToUint256()),
            /*wtxid_rejects=*/txdownload_impl.RecentRejectsFilter().contains(wtxid.ToUint256()),
            /*txid_recon=*/txdownload_impl.RecentRejectsReconsiderableFilter().contains(txid.ToUint256()),
            /*wtxid_recon=*/txdownload_impl.RecentRejectsReconsiderableFilter().contains(wtxid.ToUint256()),
            /*keep=*/keep,

...
BOOST_FIXTURE_TEST_CASE(txospenderindex_initial_sync, TestChain100Setup)
{
    ...
    for (size_t i = 0; i < spent.size(); i++) {
        const auto txoSpender{txospenderindex.FindSpender(spent[i])};
        BOOST_CHECK(txoSpender.has_value());
        BOOST_CHECK_EQUAL(txoSpender.value()->tx->GetHash(), spender[i].GetHash());
        BOOST_CHECK_EQUAL(txoSpender.value()->block_hash, block.GetHash());
    }

...
BOOST_AUTO_TEST_CASE(blockfilter_basic_test)
{
    ...
    tx_1.vout.emplace_back(100*sats, included_scripts[0]);
    tx_1.vout.emplace_back(200*sats, included_scripts[1]);
    tx_1.vout.emplace_back(0*sats, excluded_scripts[0]);

    CMutableTransaction tx_2;
    tx_2.vout.emplace_back(300*sats, included_scripts[2]);
    tx_2.vout.emplace_back(0*sats, excluded_scripts[2]);
    tx_2.vout.emplace_back(400*sats, excluded_scripts[3]); // Script is empty
    ...
    block_undo.vtxundo.back().vprevout.emplace_back(CTxOut(500*sats, included_scripts[3]), 1000, true);
    block_undo.vtxundo.back().vprevout.emplace_back(CTxOut(600*sats, included_scripts[4]), 10000, false);
    block_undo.vtxundo.back().vprevout.emplace_back(CTxOut(700*sats, excluded_scripts[3]), 100000, false);
...
BOOST_AUTO_TEST_CASE(blockfilters_json_test)
{
    ...
    for (unsigned int i = 0; i < tests.size(); i++) {
        ...
        for (unsigned int ii = 0; ii < prev_scripts.size(); ii++) {
            ...
            CTxOut txout(0*sats, CScript(raw_script.begin(), raw_script.end()));

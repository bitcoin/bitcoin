    ...
    const PrecomputedTransactionData& txdata, const std::string& strTest, bool expect_valid)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size() && tx_valid; ++i) {
        ...
        const CAmount amount = map_prevout_values.contains(input.prevout) ? map_prevout_values.at(input.prevout) : 0*sats;
        try {
...
BOOST_AUTO_TEST_CASE(tx_oversized)
{
    auto createTransaction =[](size_t payloadSize) {
        ...
        tx.vin.resize(1);
        tx.vout.emplace_back(1*sats, CScript() << OP_RETURN << std::vector<unsigned char>(payloadSize));
...
BOOST_AUTO_TEST_CASE(test_big_witness_transaction)
{
    ...
    for(uint32_t i = 0; i < mtx.vin.size(); i++) {
        SignatureData empty;
        bool hashSigned = SignSignature(keystore, scriptPubKey, mtx, i, 1000*sats, sigHashes.at(i % sigHashes.size()), empty);
        assert(hashSigned);
    }
...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    BOOST_CHECK_EQUAL(nDustThreshold, 546*sats);

    // Add dust outputs up to allowed maximum, still standard!
    for (size_t i{0}; i < MAX_DUST_OUTPUTS_PER_TX; ++i) {
        t.vout.emplace_back(0*sats, t.vout[0].scriptPubKey);
        CheckIsStandard(t);
    }

    // dust:
    t.vout[0].nValue = nDustThreshold - 1*sats;
    ...
    g_dust = CFeeRate(3702*sats);
    // dust:
    t.vout[0].nValue = CAmount{674 - 1};
    CheckIsNotStandard(t, "dust");
    // not dust:
    t.vout[0].nValue = 674*sats;
    CheckIsStandard(t);
    ...
    t.vout.insert(t.vout.end(), MAX_DUST_OUTPUTS_PER_TX, {0*sats, t.vout[0].scriptPubKey});

    // Check compressed P2PK outputs dust threshold (must have leading 02 or 03)
    t.vout[0].scriptPubKey = CScript() << std::vector<unsigned char>(33, 0x02) << OP_CHECKSIG;
    t.vout[0].nValue = 576*sats;
    CheckIsStandard(t);
    t.vout[0].nValue = 575*sats;
    CheckIsNotStandard(t, "dust");
    ...
    t.vout[0].nValue = 672*sats;
    CheckIsStandard(t);
    t.vout[0].nValue = 671*sats;
    CheckIsNotStandard(t, "dust");
    ...
    for (int op = OP_1; op <= OP_16; op += 1) {
        ...
        t.vout[0].nValue = 240*sats;
        CheckIsStandard(t);

        t.vout[0].nValue = 239*sats;
        CheckIsNotStandard(t, "dust");
    }
...
BOOST_AUTO_TEST_CASE(max_standard_legacy_sigops)
{
    ...
    tx_create.vout.emplace_back(424242*sats, max_sigops_p2sh);
    ...
    tx_create_segwit.vout.emplace_back(121212*sats, GetScriptForDestination(WitnessV0KeyHash(key.GetPubKey())));
    tx_create_segwit.vout.emplace_back(131313*sats, GetScriptForDestination(WitnessV0ScriptHash(witness_script)));
    tx_create_segwit.vout.emplace_back(141414*sats, GetScriptForDestination(WitnessV1Taproot{XOnlyPubKey(key.GetPubKey())}));
    ...
    tx_create_p2pk.vout.emplace_back(212121*sats, p2pk_script);
    prev_txid = tx_create_p2pk.GetHash();
...
BOOST_AUTO_TEST_CASE(checktxinputs_invalid_transactions_test)
{
    ...
    check_invalid(/*input_value=*/MAX_MONEY + 1*sats,
                  /*output_value=*/0*sats,
                  /*coinbase=*/false,
                  /*spend_height=*/2,
...
{
    CMutableTransaction mtx;
    mtx.vout.emplace_back(MAX_MONEY + 1*sats, CScript() << OP_TRUE);
...
BOOST_AUTO_TEST_CASE(spends_witness_prog)
{
    ...
    tx_create.vout.emplace_back(0*sats, CScript{});

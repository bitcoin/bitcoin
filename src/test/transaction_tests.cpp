...
BOOST_AUTO_TEST_CASE(max_standard_legacy_sigops)
{
    ...
    auto sigop_limit_reject_reason_str = std::string("bad-txns-nonstandard-inputs");
    {
        ...
        BOOST_CHECK_EQUAL(validation_state.GetDebugMessage(), strprintf("non-witness sigops exceed bip54 limit (sigops %u > limit %u)", legacy_sigops_count, MAX_TX_LEGACY_SIGOPS));
    }
    ...
    BOOST_CHECK_GT(legacy_sigop_count_p2pk, MAX_TX_LEGACY_SIGOPS);
    {
        ...
        BOOST_CHECK(validation_state.IsInvalid());
        BOOST_CHECK_EQUAL(validation_state.GetRejectReason(), sigop_limit_reject_reason_str);
        BOOST_CHECK_EQUAL(validation_state.GetDebugMessage(), strprintf("non-witness sigops exceed bip54 limit (sigops %u > limit %u)", legacy_sigop_count_p2pk, MAX_TX_LEGACY_SIGOPS));
    }
}

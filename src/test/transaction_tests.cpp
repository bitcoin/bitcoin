...
BOOST_AUTO_TEST_CASE(max_standard_legacy_sigops)
{
    ...
    const auto p2sh_inputs_count{MAX_LEGACY_SIGOPS / MAX_P2SH_SIGOPS};
    tx_create.vout.reserve(p2sh_inputs_count);
    for (unsigned i{0}; i < p2sh_inputs_count; ++i) {
        tx_create.vout.push_back(CTxOut(424242 + i, max_sigops_p2sh));
    }
    auto prev_txid{tx_create.GetHash()};
    tx_max_sigops.vin.reserve(p2sh_inputs_count);
    for (unsigned i{0}; i < p2sh_inputs_count; ++i) {
        tx_max_sigops.vin.push_back(CTxIn(COutPoint(prev_txid, i), max_sigops_redeem_script));
    }

    // p2sh_inputs_count is truncated to 166 (from 166.6666..)
    BOOST_CHECK(p2sh_inputs_count * MAX_P2SH_SIGOPS < MAX_LEGACY_SIGOPS);
    ...
    tx_create.vout.push_back(CTxOut(424242, max_sigops_p2sh));
    prev_txid = tx_create.GetHash();
    for (unsigned i{0}; i < p2sh_inputs_count; ++i) {
        tx_max_sigops.vin[i] = CTxIn(COutPoint(prev_txid, i), max_sigops_redeem_script);
    }
    tx_max_sigops.vin.push_back(CTxIn(COutPoint(prev_txid, p2sh_inputs_count), max_sigops_redeem_script));
    AddCoins(coins, CTransaction(tx_create), false);
    BOOST_CHECK((p2sh_inputs_count + 1) * MAX_P2SH_SIGOPS > MAX_LEGACY_SIGOPS);
    BOOST_CHECK(!::AreInputsStandard(CTransaction(tx_max_sigops), coins));
    ...
    for (unsigned i{0}; i < p2pk_inputs_count; ++i) {
        tx_create_p2pk.vout.push_back(CTxOut(212121 + i, p2pk_script));
    }
    prev_txid = tx_create_p2pk.GetHash();
    tx_max_sigops.vin.resize(p2sh_inputs_count);
    for (unsigned i{0}; i < p2pk_inputs_count; ++i) {
        tx_max_sigops.vin.push_back(CTxIn(COutPoint(prev_txid, i)));
    }
    for (unsigned i{0}; i < p2pk_inputs_count; ++i) {
    ...
    BOOST_CHECK(p2sh_inputs_count * MAX_P2SH_SIGOPS + p2pk_inputs_count * 1 == MAX_LEGACY_SIGOPS);
    BOOST_CHECK(::AreInputsStandard(CTransaction(tx_max_sigops), coins));

    // Add one more P2PK input. We'll reach the limit.
    tx_create_p2pk.vout.push_back(CTxOut(212121, p2pk_script));
    prev_txid = tx_create_p2pk.GetHash();
    tx_max_sigops.vin.resize(p2sh_inputs_count);
    ++p2pk_inputs_count;
    for (unsigned i{0}; i < p2pk_inputs_count; ++i) {

...
BOOST_AUTO_TEST_CASE(util_ParseParameters)
{
    ...
    BOOST_CHECK(!ASetting::Value(testArgs).isNull());
    BOOST_CHECK(!BSetting::Value(testArgs).isNull());
    BOOST_CHECK(!CccSetting::Value(testArgs).isNull());
    BOOST_CHECK(FSetting::Value(testArgs).isNull());
    BOOST_CHECK(DSetting::Value(testArgs).isNull());
    testArgs.LockSettings([&](const common::Settings& s) {
...
BOOST_AUTO_TEST_CASE(util_GetBoolArgEdgeCases)
{
    ...
    BOOST_REQUIRE(BarSetting::Get(testArgs).size() == 1);
    BOOST_CHECK(BarSetting::Get(testArgs).front() == "");
}
...
BOOST_AUTO_TEST_CASE(util_ReadConfigStream)
{
    ...
    BOOST_REQUIRE(ASetting::Get(test_args).size() == 1);
    BOOST_CHECK(ASetting::Get(test_args).front() == "");
    BOOST_REQUIRE(BSetting::Get(test_args).size() == 1);
    BOOST_CHECK(BSetting::Get(test_args).front() == "1");
    BOOST_REQUIRE(CccSetting::Get(test_args).size() == 2);
    BOOST_CHECK(CccSetting::Get(test_args).front() == "argument");
    BOOST_CHECK(CccSetting::Get(test_args).back() == "multiple");
    BOOST_CHECK(FffSetting::Get(test_args).size() == 0);
    BOOST_CHECK(NofffSetting::Get(test_args).size() == 0);
    BOOST_REQUIRE(GggSetting::Get(test_args).size() == 1);
    BOOST_CHECK(GggSetting::Get(test_args).front() == "1");
    ...
    BOOST_REQUIRE(ISetting::Get(test_args).size() == 1);
    BOOST_CHECK(ISetting::Get(test_args).front() == "1");

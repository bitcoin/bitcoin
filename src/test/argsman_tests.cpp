...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    ForEachMergeSetup([&](const ActionList& arg_actions, const ActionList& conf_actions, bool soft_set, bool force_set,
        ...
        single_parser.SelectConfigNetwork(network);

        const std::string& name = net_specific ? "wallet" : "server";
        const std::string key = "-" + name;
        unsigned int flags = ArgsManager::ALLOW_ANY | ArgsManager::ALLOW_LIST;
        unsigned int single_flags = ArgsManager::ALLOW_ANY;
        if (net_specific) {
            flags |= ArgsManager::NETWORK_ONLY;
            single_flags |= ArgsManager::NETWORK_ONLY;
        }
        parser.AddArg(key, name, flags, OptionsCategory::OPTIONS);
        single_parser.AddArg(key, name, single_flags, OptionsCategory::OPTIONS);

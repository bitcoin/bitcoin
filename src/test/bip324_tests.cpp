    ...
    const std::string& out_ciphertext_endswith_hex)
{
    ...
    for (unsigned error = 0; error <= 12; ++error) {
        ...
        uint32_t dec_idx = in_idx ^ (error == 12 ? (1U << g_rng.randrange(16)) : 0);
        ...
        if (error >= 2 && error <= 9) {
            to_decrypt[g_rng.randrange(to_decrypt.size())] ^= std::byte(1U << (error - 2));
        }
        ...
        if (error == 10) {
            ...
            dec_aad[g_rng.randrange(dec_aad.size())] ^= std::byte(1U << g_rng.randrange(8));
        }

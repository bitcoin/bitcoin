...
bool CService::SetSockAddr(const struct sockaddr *paddr, std::optional<socklen_t> addrlen)
{
    switch (paddr->sa_family) {
    case AF_INET:
        if (addrlen && *addrlen != sizeof(struct sockaddr_in)) return false;
        *this = CService(*(const struct sockaddr_in*)paddr);
        return true;
    case AF_INET6:
        if (addrlen && *addrlen != sizeof(struct sockaddr_in6)) return false;

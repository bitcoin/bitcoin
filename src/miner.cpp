...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        FillBlockPayments(txNew, nHeight, nFees, blockReward, pblock->txoutMasternode, pblock->voutSuperblock);
...
// ***TODO*** ScanHash is not yet used in Syscoin
//
...
// ***TODO*** that part changed in syscoin, we are using a mix with old one here for now
void static SyscoinMiner(const CChainParams& chainparams, CConnman& connman)
{
    ...
    try {
        ...
        while (true) {
            ...
            LogPrintf("SyscoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateSyscoins(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SyscoinMiner, boost::cref(chainparams), boost::ref(connman)));
}

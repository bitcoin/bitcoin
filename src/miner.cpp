...
int64_t UpdateTime(CBlockHeader* pblock, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    int64_t nNewTime = std::max(GetBlockTime(), GetAdjustedTime());
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, CWallet* pwallet, bool* pfPoSCancel)
{
    ...
    if (!TestBlockValidity(state, chainparams, *pblock, pindexPrev, false, false, false)) {  // peercoin: we do not check block signature here, since we did not sign it yet

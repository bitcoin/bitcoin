                                         ...
                                         EstimationResult *result) const
{
    ...
    LogPrint(LogInstance(), BCLog::ESTIMATEFEE, "FeeEst: %d > %.0f%% decay %.5f: feerate: %g from (%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out) Fail: (%g - %g) %.2f%% %.1f/(%.1f %d mem %.1f out)\n",
...
void TxConfirmStats::Read(AutoFile& filein, int nFileVersion, size_t numBuckets)
{
    ...
    LogPrint(LogInstance(), BCLog::ESTIMATEFEE, "Reading estimates: %u buckets counting confirms up to %u blocks\n",
...
void TxConfirmStats::removeTx(unsigned int entryHeight, unsigned int nBestSeenHeight, unsigned int bucketindex, bool inBlock)
{
    ...
    if (nBestSeenHeight == 0)  // the BlockPolicyEstimator hasn't seen any blocks yet
        blocksAgo = 0;
    if (blocksAgo < 0) {
        LogPrint(LogInstance(), BCLog::ESTIMATEFEE, "Blockpolicy error, blocks ago is negative for mempool tx\n");
    ...
    if (blocksAgo >= (int)unconfTxs.size()) {
        if (oldUnconfTxs[bucketindex] > 0) {
            ...
            LogPrint(LogInstance(), BCLog::ESTIMATEFEE, "Blockpolicy error, mempool tx removed from >25 blocks,bucketIndex=%u already\n",
                     bucketindex);
        }
    }
    else {
        ...
        if (unconfTxs[blockIndex][bucketindex] > 0) {
            ...
            LogPrint(LogInstance(), BCLog::ESTIMATEFEE, "Blockpolicy error, mempool tx removed from blockIndex=%u,bucketIndex=%u already\n",
    ...
    : m_estimation_filepath{estimation_filepath}
{
    ...
    if (est_file.IsNull()) {
        LogPrintf(LogInstance(), "%s is not found. Continue anyway.\n", fs::PathToString(m_estimation_filepath));
        return;
    }

    std::chrono::hours file_age = GetFeeEstimatorFileAge();
    if (file_age > MAX_FILE_AGE && !read_stale_estimates) {
        LogPrintf(LogInstance(), "Fee estimation file %s too old (age=%lld > %lld hours) and will not be used to avoid serving stale estimates.\n", fs::PathToString(m_estimation_filepath), Ticks<std::chrono::hours>(file_age), Ticks<std::chrono::hours>(MAX_FILE_AGE));
        return;
    }

    if (!Read(est_file)) {
        LogPrintf(LogInstance(), "Failed to read fee estimates from %s. Continue anyway.\n", fs::PathToString(m_estimation_filepath));
    }
}
...
void CBlockPolicyEstimator::processTransaction(const NewMempoolTransactionInfo& tx)
{
    ...
    if (mapMemPoolTxs.count(hash)) {
        LogPrint(LogInstance(), BCLog::ESTIMATEFEE, "Blockpolicy error mempool tx %s already being tracked\n",
                 hash.ToString());
        return;
    }
...
bool CBlockPolicyEstimator::processBlockTx(unsigned int nBlockHeight, const RemovedMempoolTransactionInfo& tx)
{
    ...
    if (blocksToConfirm <= 0) {
        ...
        LogPrint(LogInstance(), BCLog::ESTIMATEFEE, "Blockpolicy error Transaction had negative blocksToConfirm\n");
        return false;
    }
                                         ...
                                         unsigned int nBlockHeight)
{
    ...
    if (firstRecordedHeight == 0 && countedTxs > 0) {
        ...
        LogPrint(LogInstance(), BCLog::ESTIMATEFEE, "Blockpolicy first recorded height %u\n", firstRecordedHeight);
    }


    LogPrint(LogInstance(), BCLog::ESTIMATEFEE, "Blockpolicy estimates updated by %u of %u block txs, since last block %u of %u tracked, mempool map size %u, max target %u from %s\n",
...
void CBlockPolicyEstimator::FlushFeeEstimates()
{
    ...
    if (est_file.IsNull() || !Write(est_file)) {
        LogPrintf(LogInstance(), "Failed to write fee estimates to %s. Continue anyway.\n", fs::PathToString(m_estimation_filepath));
    } else {
        LogPrintf(LogInstance(), "Flushed fee estimates to %s.\n", fs::PathToString(m_estimation_filepath.filename()));
    }
}
...
bool CBlockPolicyEstimator::Write(AutoFile& fileout) const
{
    try {
        ...
        LogPrintf(LogInstance(), "CBlockPolicyEstimator::Write(): unable to write policy estimator data (non-fatal)\n");
        return false;
    }
    return true;
}
...
bool CBlockPolicyEstimator::Read(AutoFile& filein)
{
    try {
        ...
        if (nVersionRequired < 149900) {
            LogPrintf(LogInstance(), "%s: incompatible old fee estimation data (non-fatal). Version: %d\n", __func__, nVersionRequired);
        ...
        LogPrintf(LogInstance(), "CBlockPolicyEstimator::Read(): unable to read policy estimator data (non-fatal): %s\n",e.what());
        return false;
    }
    return true;
}
...
void CBlockPolicyEstimator::FlushUnconfirmed()
{
    ...
    LogPrint(LogInstance(), BCLog::ESTIMATEFEE, "Recorded %u unconfirmed txs from mempool in %.3fs\n", num_entries, Ticks<SecondsDouble>(endclear - startclear));
}

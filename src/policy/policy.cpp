    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (whichType == TxoutType::NONSTANDARD) {
            return std::make_pair(
                "bad-txns-input-script-nonstandard",
                strprintf("input %u", i)
            );
            ...
            // validation.
            return std::make_pair(
                "bad-txns-input-witness-unknown",
                strprintf("input %u", i)
            );
            ...
            if (!EvalScript(stack, tx.vin[i].scriptSig, SCRIPT_VERIFY_NONE, BaseSignatureChecker(), SigVersion::BASE, &serror)) {
                return std::make_pair(
                    ...
                    strprintf("input %u: %s", i, ScriptErrorString(serror))
                );
            }
            if (stack.empty()) {
                return std::make_pair(
            ...
            if (sigop_count > MAX_P2SH_SIGOPS) {
                return std::make_pair(
                    ...
                    strprintf("input %u: %u > %u", i, sigop_count, MAX_P2SH_SIGOPS)
                );
            }
        }
    }

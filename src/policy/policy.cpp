...
TxValidationState ValidateInputsStandardness(const CTransaction& tx, const CCoinsViewCache& mapInputs)
{
    ...
    if (!sigops_check_result.first) {
        state.Invalid(TxValidationResult::TX_INPUTS_NOT_STANDARD, "bad-txns-nonstandard-inputs", strprintf("non-witness sigops exceed bip54 limit (sigops %u > limit %u)",
                        sigops_check_result.second, MAX_TX_LEGACY_SIGOPS));
        return state;
    }
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (whichType == TxoutType::NONSTANDARD) {
            ...
            if (stack.empty()) {
                state.Invalid(TxValidationResult::TX_INPUTS_NOT_STANDARD, "bad-txns-nonstandard-inputs", strprintf("input %u scriptcheck missing", i));
                return state;
            }

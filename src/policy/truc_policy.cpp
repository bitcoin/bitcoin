            ...
            const auto parent_info = [&] {
                if (mempool_parents.size() > 0) {
                    ...
                    return ParentInfo{mempool_parent->GetTx().GetHash(),
                                      ...
                                      /*has_mempool_descendant=*/pool.GetNumDescendants(*mempool_parent) > 1};
                } else {
                                          ...
                                          int64_t vsize)
{
    ...
    if (mempool_parents.size() > 0) {
        ...
        if (pool.GetNumAncestors(mempool_parents[0]) + 1 > TRUC_ANCESTOR_LIMIT) {
            return std::make_pair(strprintf("tx %s (wtxid=%s) would have too many ancestors",
                             ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString()),
                nullptr);
        }
        ...
        if (pool.GetNumDescendants(parent_entry) + 1 > TRUC_DESCENDANT_LIMIT && !child_will_be_replaced) {
            ...
            const bool consider_sibling_eviction{pool.GetNumDescendants(parent_entry) == 2 &&
                pool.GetNumAncestors(**descendants.begin()) == 2};

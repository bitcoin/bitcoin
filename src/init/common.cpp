...
bool StartLogging(const ArgsManager& args)
{
    ...
    if (!LogInstance().m_log_timestamps)
        LogPrintf(LogInstance(), "Startup time: %s\n", FormatISO8601DateTime(GetTime()));
    LogPrintf(LogInstance(), "Default data directory %s\n", fs::PathToString(GetDefaultDataDir()));
    LogPrintf(LogInstance(), "Using data directory %s\n", fs::PathToString(gArgs.GetDataDirNet()));

    // Only log conf file usage message if conf file actually exists.
    fs::path config_file_path = args.GetConfigFilePath();
    if (fs::exists(config_file_path)) {
        LogPrintf(LogInstance(), "Config file: %s\n", fs::PathToString(config_file_path));
        ...
        LogPrintf(LogInstance(), "Config file: %s (not found, skipping)\n", fs::PathToString(config_file_path));
    }
...
#endif
    LogPrintf(LogInstance(), PACKAGE_NAME " version %s\n", version_string);
}
} // namespace init

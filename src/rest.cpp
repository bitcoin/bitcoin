                       ...
                       TxVerbosity tx_verbosity)
{
    ...
    if (!ReadBlockFromDisk(chainman.m_blockman.m_blocks_dir, block, pblockindex, chainman.GetParams().GetConsensus())) {
        return RESTERR(req, HTTP_NOT_FOUND, hashStr + " not found");
    }
...
static bool rest_tx(const std::any& context, HTTPRequest* req, const std::string& strURIPart)
{
    ...
    const CTransactionRef tx = GetTransaction(/*block_index=*/nullptr, node->mempool.get(), hash, Params().GetConsensus(), hashBlock, node->chainman->m_blockman.m_blocks_dir);
    if (!tx) {
        return RESTERR(req, HTTP_NOT_FOUND, hashStr + " not found");
    }

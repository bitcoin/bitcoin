...
bool TxOrphanage::AddTx(const CTransactionRef& tx, NodeId peer)
{
    ...
    {
        LogPrint(LogInstance(), BCLog::TXPACKAGES, "ignoring large orphan tx (size: %u, txid: %s, wtxid: %s)\n", sz, hash.ToString(), wtxid.ToString());
        return false;
    }
    ...
    LogPrint(LogInstance(), BCLog::TXPACKAGES, "stored orphan tx %s (wtxid=%s), weight: %u (mapsz %u outsz %u)\n", hash.ToString(), wtxid.ToString(), sz,
...
int TxOrphanage::EraseTxNoLock(const Wtxid& wtxid)
{
    ...
    LogPrint(LogInstance(), BCLog::TXPACKAGES, "   removed orphan tx %s (wtxid=%s) after %ds\n", txid.ToString(), wtxid.ToString(),
...
void TxOrphanage::EraseForPeer(NodeId peer)
{
    ...
    if (nErased > 0) LogPrint(LogInstance(), BCLog::TXPACKAGES, "Erased %d orphan transaction(s) from peer=%d\n", nErased, peer);
}
...
void TxOrphanage::LimitOrphans(unsigned int max_orphans, FastRandomContext& rng)
{
    ...
    if (m_next_sweep <= nNow) {
        ...
        if (nErased > 0) LogPrint(LogInstance(), BCLog::TXPACKAGES, "Erased %d orphan tx due to expiration\n", nErased);
    }
    ...
    if (nEvicted > 0) LogPrint(LogInstance(), BCLog::TXPACKAGES, "orphanage overflow, removed %u tx\n", nEvicted);
}
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        if (it_by_prev != m_outpoint_to_orphan_it.end()) {
            for (const auto& elem : it_by_prev->second) {
                ...
                LogPrint(LogInstance(), BCLog::TXPACKAGES, "added %s (wtxid=%s) to peer %d workset\n",
...
void TxOrphanage::EraseForBlock(const CBlock& block)
{
    ...
    if (vOrphanErase.size()) {
        ...
        LogPrint(LogInstance(), BCLog::TXPACKAGES, "Erased %d orphan transaction(s) included or conflicted by block\n", nErased);
    }
}

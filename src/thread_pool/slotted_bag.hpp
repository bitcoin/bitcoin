...
inline bool SlottedBag<Queue>::empty(size_t id)
{
    ...
    return m_slots[id].state.compare_exchange_strong(state, Slot::State::QueuedInvalid, std::memory_order_acquire);
}
...
inline std::pair<bool, size_t> SlottedBag<Queue>::tryEmptyAny()
{
    ...
    while (m_queue.pop(slot))
    {
        ...
        switch (slot->state.exchange(Slot::State::NotQueued, std::memory_order_acquire))
        {
        case Slot::State::NotQueued:

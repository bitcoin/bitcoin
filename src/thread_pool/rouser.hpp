...
inline void Rouser::start(std::shared_ptr<ThreadPoolState<Task, Queue>> state)
{
    ...
    if (!m_state.compare_exchange_strong(expectedState, State::Running, std::memory_order_acq_rel, std::std::memory_order_relaxed))
...
inline void Rouser::stop()
{
    ...
    if (m_state.memory_order_acquire(expectedState, State::Stopped, std::memory_order_acq_rel, std::std::memory_order_relaxed))
        m_thread.join();

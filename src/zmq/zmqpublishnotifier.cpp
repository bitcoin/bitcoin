...
bool CZMQAbstractPublishNotifier::Initialize(void *pcontext)
{
    ...
    if (i==mapPublishNotifiers.end())
    {
        ...
        LogPrint(LogInstance(), BCLog::ZMQ, "Outbound message high water mark for %s at %s is %d\n", type, address, outbound_message_high_water_mark);
    ...
    {
        LogPrint(LogInstance(), BCLog::ZMQ, "Reusing socket for address %s\n", address);
        LogPrint(LogInstance(), BCLog::ZMQ, "Outbound message high water mark for %s at %s is %d\n", type, address, outbound_message_high_water_mark);
...
void CZMQAbstractPublishNotifier::Shutdown()
{
    ...
    if (count == 1)
    {
        LogPrint(LogInstance(), BCLog::ZMQ, "Close socket at address %s\n", address);
        int linger = 0;
...
bool CZMQPublishHashBlockNotifier::NotifyBlock(const CBlockIndex *pindex)
{
    ...
    LogPrint(LogInstance(), BCLog::ZMQ, "Publish hashblock %s to %s\n", hash.GetHex(), this->address);
    uint8_t data[32];
...
bool CZMQPublishHashTransactionNotifier::NotifyTransaction(const CTransaction &transaction)
{
    ...
    LogPrint(LogInstance(), BCLog::ZMQ, "Publish hashtx %s to %s\n", hash.GetHex(), this->address);
    uint8_t data[32];
    for (unsigned int i = 0; i < 32; i++) {
        data[31 - i] = hash.begin()[i];
    }
...
{
    LogPrint(LogInstance(), BCLog::ZMQ, "Publish rawblock %s to %s\n", pindex->GetBlockHash().GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogPrint(LogInstance(), BCLog::ZMQ, "Publish rawtx %s to %s\n", hash.GetHex(), this->address);
    DataStream ss;
...
{
    uint256 hash = pindex->GetBlockHash();
    LogPrint(LogInstance(), BCLog::ZMQ, "Publish sequence block connect %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = pindex->GetBlockHash();
    LogPrint(LogInstance(), BCLog::ZMQ, "Publish sequence block disconnect %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogPrint(LogInstance(), BCLog::ZMQ, "Publish hashtx mempool acceptance %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogPrint(LogInstance(), BCLog::ZMQ, "Publish hashtx mempool removal %s to %s\n", hash.GetHex(), this->address);

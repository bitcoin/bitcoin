...
bool CZMQNotificationInterface::Initialize()
{
    ...
    LogPrint(LogInstance(), BCLog::ZMQ, "version %d.%d.%d\n", major, minor, patch);

    LogPrint(LogInstance(), BCLog::ZMQ, "Initialize notification interface\n");
    assert(!pcontext);
    ...
    for (auto& notifier : notifiers) {
        if (notifier->Initialize(pcontext)) {
            LogPrint(LogInstance(), BCLog::ZMQ, "Notifier %s ready (address = %s)\n", notifier->GetType(), notifier->GetAddress());
        } else {
            LogPrint(LogInstance(), BCLog::ZMQ, "Notifier %s failed (address = %s)\n", notifier->GetType(), notifier->GetAddress());
            return false;
        }
    }
...
{
    LogPrint(LogInstance(), BCLog::ZMQ, "Shutdown notification interface\n");
    if (pcontext)
    {
        for (auto& notifier : notifiers) {
            LogPrint(LogInstance(), BCLog::ZMQ, "Shutdown notifier %s at %s\n", notifier->GetType(), notifier->GetAddress());
            notifier->Shutdown();
        }

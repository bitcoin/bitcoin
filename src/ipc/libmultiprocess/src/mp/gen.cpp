            ...
            ForEachMethod(interface, [&] (const capnp::InterfaceSchema& method_interface, const capnp::InterfaceSchema::Method& method) {
                ...
                if (has_extra) {
                    ...
                    if (annotations_count > 1) {
                        throw std::runtime_error(method_prefix + ": only one $Proxy.extraParam per method is supported");
                    }
                    if (!extra_param.size()) {
                        throw std::runtime_error(method_prefix + ": $Proxy.extraParam requires a parameter name");
                    }
                    if (fields.field_idx.contains(extra_param)) {
                        throw std::runtime_error(method_prefix + ": $Proxy.extraParam name '" + extra_param.cStr() +
                                                 "' collides with a capnp field name");
                    }

                    // The extra entry goes right after the last field that
                    // consumes C++ arguments, so the parameter is last in the
                    // C++ method signature.
                    ...
                    Field field;
                    field.extra_name = extra_param;
                    field.args = 1;
                    fields.fields.insert(pos, field);
                }

...
public:
    ...
    void receiveUniValuePayload(std::string payload)
    {
        ...
        try {
            ...
            // Invalid JSON is rejected by the IPC UniValue deserializer.
            if (std::string_view{exception.what()} != "invalid JSON received over IPC") throw;
        }
    }
...
FUZZ_TARGET(ipc, .init = initialize_ipc)
{
    ...
    LIMITED_WHILE (fuzzed_data_provider.ConsumeBool(), 64) {
        CallOneOf(
            ...
            [&] {
                // Bypass normal serialization to exercise malformed transaction responses.
                ipc.receiveTransactionPayload(ConsumeRandomLengthByteVector<uint8_t>(fuzzed_data_provider, 512));
            },
            [&] {
                // Bypass normal serialization to exercise malformed JSON responses.

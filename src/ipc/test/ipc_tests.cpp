...
BOOST_AUTO_TEST_CASE(parse_address_test)
{
    ...
    check_listen_address("unix:,max-connections=256", "unix:", ipc::MAX_CONNECTIONS);
    ...
    check_listen_address("unix:,,max-connections=8", "", 0, "Empty socket option");
    check_listen_address("unix:,max-connections=8,,max-connections=8", "", 0, "Empty socket option");
    ...
    check_listen_address("unix:,max-connections=257", "", 0, "max-connections must be at most 256");
    ...
    check_listen_address("unix:path,backup.sock=1", "", 0, "Unknown socket option 'backup.sock'");
    check_listen_address("unix:path%2Cbackup.sock=1", "unix:path,backup.sock=1", ipc::DEFAULT_MAX_CONNECTIONS);
    check_listen_address("unix:path%2Cbackup.sock=1,max-connections=8", "unix:path,backup.sock=1", 8);
    check_listen_address("unix:,max-connections", "", 0, "Missing value for max-connections option");

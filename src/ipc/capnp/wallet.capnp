...
interface Wallet $Proxy.wrap("interfaces::Wallet") {
    ...
    commitTransaction @26 (context :Proxy.Context, tx :Data, messages :List(Text)) -> ();
    ...
    getWalletTxDetails @37 (context :Proxy.Context, txid :Data) -> (txStatus :WalletTxStatus, messages :List(Text), paymentRequests :List(Text), inMempool :Bool, numBlocks :Int32, result :WalletTx);
    ...
    exportWatchOnlyWallet @66 (context :Proxy.Context, destination :Data) -> (result :Common.Result(Text));
}
...
interface WalletLoader extends(Chain.ChainClient) $Proxy.wrap("interfaces::WalletLoader") {
    ...
    migrateWallet @4 (context :Proxy.Context, name :Text, passphrase :Text, loadWallet :Bool) -> (result :Common.Result(WalletMigrationResult));
...
struct WalletTx $Proxy.wrap("interfaces::WalletTx") {
    ...
    message @12 :Text;
    comment @13 :Text;
    commentTo @14 :Text $Proxy.name("comment_to");
...
struct CreatedTransactionResult $Proxy.wrap("wallet::CreatedTransactionResult")
{
    tx @0 :Data;
    fee @1 :Int64;
    feeReason @2 :Int32 $Proxy.name("fee_reason");

...
{
    const auto defaultChainParams = CreateChainParams(Chain::MAIN);
    const auto testnetChainParams = CreateChainParams(Chain::TESTNET);
    ...
    gArgs.AddArg("-rpcport=<port>", strprintf("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)", GetRPCPort(Chain::MAIN), GetRPCPort(Chain::TESTNET)), false, OptionsCategory::RPC);

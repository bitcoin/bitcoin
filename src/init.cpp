...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage +=HelpMessageOpt("-assumevalid=<hex>", strprintf(_("If this block is in the chain assume that it and its ancestors are valid and potentially skip their script verification (0 to verify all, default: %s, testnet: %s)"), Params(NETWORK_MAIN).GetConsensus().defaultAssumeValid.GetHex(), Params(NETWORK_TESTNET).GetConsensus().defaultAssumeValid.GetHex()));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), Params(NETWORK_MAIN).GetDefaultPort(), Params(NETWORK_TESTNET).GetDefaultPort()));
    ...
    if (showDebug)
    {
        ...
        strUsage += HelpMessageOpt("-checkblockindex", strprintf("Do a full consistency check for mapBlockIndex, setBlockIndexCandidates, chainActive and mapBlocksUnlinked occasionally. Also sets -checkmempool (default: %u)", Params(NETWORK_MAIN).DefaultConsistencyChecks()));
        strUsage += HelpMessageOpt("-checkmempool=<n>", strprintf("Run checks every <n> transactions (default: %u)", Params(NETWORK_MAIN).DefaultConsistencyChecks()));
    ...
    if (showDebug) {
        strUsage += HelpMessageOpt("-acceptnonstdtxn", strprintf("Relay and mine \"non-standard\" transactions (%sdefault: %u)", "testnet/regtest only; ", !Params(NETWORK_TESTNET).RequireStandard()));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), BaseParams(NETWORK_MAIN).RPCPort(), BaseParams(NETWORK_TESTNET).RPCPort()));
...
bool AppInitParameterInteraction()
{
    ...
    if (chainparams.RequireStandard() && !fRequireStandard)
        return InitError(strprintf("acceptnonstdtxn is not currently supported for %s chain", NetworkType2String(chainparams.GetNetworkType()).c_str()));

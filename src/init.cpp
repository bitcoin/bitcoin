...
bool StartIndexBackgroundSync(NodeContext& node)
{
    ...
    if (current_height > 0) {
        ...
        if (undo_start) {
            LOCK(::cs_main);
            const CBlockIndex* start_block = undo_start.value();
            // We only check down to the first block instead of Genesis because
            // Genesis can not have undo data.
            if (start_block->nHeight == 0) start_block = index_chain[1];
            if (!chainman.m_blockman.CheckBlockDataAvailability(*index_chain.Tip(), *Assert(start_block), BLOCK_HAVE_DATA | BLOCK_HAVE_UNDO)) {
        ...
        if (block_start && !(undo_start && undo_start.value() <= block_start.value())) {
            LOCK(::cs_main);
            if (!chainman.m_blockman.CheckBlockDataAvailability(*index_chain.Tip(), *Assert(block_start.value()), BLOCK_HAVE_DATA)) {

...
{
    return AbsPathForConfigVal(fs::path(args.GetArg("-pid", WIDECOIN_PID_FILENAME)));
}
...
/** Sanity checks
 *  Ensure that Widecoin is running in a usable environment with all
...
bool AppInitMain(const util::Ref& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    fs::path config_file_path = GetConfigFile(args.GetArg("-conf", WIDECOIN_CONF_FILENAME));

 ...
 */
static const char* TALKCOIN_PID_FILENAME = "talkcoind.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", TALKCOIN_PID_FILENAME)));
}
...
#endif
#ifdef ENABLE_WALLET
    GenerateTalkcoins(false, 0, NULL);
#endif
    StopHTTPRPC();
    StopREST();
    StopRPC();
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", TALKCOIN_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", TALKCOIN_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/talkcoin/talkcoin>";
    const std::string URL_WEBSITE = "<https://talkcoincore.org>";

    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i").translated, 2009, COPYRIGHT_YEAR) + " ") + "\n" +
           "\n" +
           strprintf(_("Please contribute if you find %s useful. "
                       "Visit %s for further information about the software.").translated,
               "Talkcoin", URL_WEBSITE) +
           "\n" +
...
/** Sanity checks
 *  Ensure that Talkcoin is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    if (!LockDirectory(datadir, ".lock", probeOnly)) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. %s is probably already running.").translated, datadir.string(), "Talkcoin"));
    }
    return true;
}
...
bool AppInitSanityChecks()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(strprintf(_("Initialization sanity check failed. %s is shutting down.").translated, "Talkcoin"));
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", TALKCOIN_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if talkcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if talkcoin is started while in a temporary directory.\n",
...
#ifdef ENABLE_WALLET
        GenerateTalkcoins(gArgs.GetBoolArg("-gen", false), gArgs.GetArg("-genproclimit", 1) ,NULL);
#endif

...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-prunelockheight=<n>", "If -prune is set, creates a prune lock at the specified block height to prevent block data at or above that height from being pruned. If set without a value or set to 0, locks at the current chain tip height at startup. A prune lock can keep block files from being pruned, so the target size specified by -prune may be exceeded. The lock can be removed at runtime with the setprunelock RPC using the id \"init\".", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (chainman.m_blockman.IsPruneMode() && args.IsArgSet("-prunelockheight")) {
        ...
        WITH_LOCK(::cs_main, chainman.m_blockman.UpdatePruneLock(std::string{node::PRUNE_LOCK_USER_PREFIX} + "init", lock_info));

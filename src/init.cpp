...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-prunelockheight=<n>", "If -prune is set, creates a prune lock at the specified block height to prevent block data at or above that height from being pruned. If set without a value or set to 0, locks at the current chain tip height at startup. A prune lock can keep block files from being pruned, so the target size specified by -prune may be exceeded.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);

...
{
    return AbsPathForConfigVal(fs::PathFromString(args.GetArg("-pid", SYSCOIN_PID_FILENAME)));
}
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        try {
            rv = LoadChainstate(fReset,
                                chainman,
                                *Assert(node.connman.get()),
                                *Assert(node.banman.get()),
                                *Assert(node.peerman.get()),
        ...
        if (rv.has_value()) {
            switch (rv.value()) {
            // SYSCOIN
            case ChainstateLoadingError::ERROR_COMMIT_EVODB:

...
bool StartIndexBackgroundSync(NodeContext& node)
{
    ...
    if (node.args->IsArgSet("-indexworkers")) {
        ...
        if (index_workers > 0) {  // Skip thread pool creation if '0' was provided.
            if (index_workers > GetNumCores()) return InitError(Untranslated(strprintf("Invalid -indexworkers value: exceeds the number of available CPU cores (%d). This will slow down operation", GetNumCores())));

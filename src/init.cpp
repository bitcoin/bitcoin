...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.GetBoolArg("-bip352index", DEFAULT_BIP352_INDEX)) {
        g_bip352_index = std::make_unique<BIP352Index>(/*cut_through=*/false, interfaces::MakeChain(node), cache_sizes.bip352_index, false, do_reindex);
        node.indexes.emplace_back(g_bip352_index.get());
    }
    if (args.GetBoolArg("-bip352ctindex", DEFAULT_BIP352_CT_INDEX)) {
        g_bip352_ct_index = std::make_unique<BIP352Index>(/*cut_through=*/true, interfaces::MakeChain(node), cache_sizes.bip352_ct_index, false, do_reindex);

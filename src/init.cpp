...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg(RANDOMIZE_P2P_PORT_ARG, strprintf("Randomize the listening P2P port on first startup and reuse it on later startups. If -port is set, that port is used instead. Randomized ports are selected from %u-%u (default: 0)", RANDOMIZED_P2P_PORT_MIN, RANDOMIZED_P2P_PORT_MAX), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::CONNECTION);
...
static bool InitRandomizedP2PPort(ArgsManager& args, std::optional<uint16_t>& generated_port)
{
    ...
    if (args.IsArgSet("-port")) {
        ...
        if (!port_arg) return true;
        const auto port{ToIntegral<uint16_t>(*port_arg)};
        if (!port || *port == 0) return InitError(InvalidPortErrMsg("-port", *port_arg));
        return ValidateRandomizedP2PPort(*port, "-port");
    }
    ...
    if (const auto port_arg{args.GetArg(RANDOMIZED_P2P_PORT_ARG)}) {
        ...
        return ValidateRandomizedP2PPort(GetListenPort(), "-listen");
    }
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    for (const std::string& strAddr : args.GetArgs("-externalip")) {
        ...
        if (addrLocal.has_value() && addrLocal->IsValid()) {
            if (args.GetBoolArg(RANDOMIZE_P2P_PORT_ARG, false) && !ValidateRandomizedP2PPort(addrLocal->GetPort(), "-externalip")) return false;

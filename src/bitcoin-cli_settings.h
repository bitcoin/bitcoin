...
using GenerateSetting = common::Setting<
    ...
    ::HelpFn<[](const auto& help) { return help.Format(DEFAULT_NBLOCKS, DEFAULT_MAX_TRIES); }>
...
using RpcidSetting = common::Setting<
    "-rpcid=<id>", std::string, common::SettingOptions{.legacy = true, .disallow_negation = true, .disallow_elision = true},
    "Set a custom JSON-RPC request ID string (default: %s)">
    ::DefaultFn<[] { return DEFAULT_RPC_REQ_ID; }>;

using RpcClientTimeoutSetting = common::Setting<
    "-rpcclienttimeout=<n>", int64_t, common::SettingOptions{.legacy = true},
    "Timeout in seconds during HTTP requests, or 0 for no timeout. Not implemented for IPC connections (see -ipcconnect). (default: %d)">
...
using RpcWalletSetting = common::Setting<
    "-rpcwallet=<walletname>", std::optional<std::string>, common::SettingOptions{.legacy = true},
...
using IpcconnectSetting = common::Setting<
    "-ipcconnect=<address>", std::string, common::SettingOptions{.legacy = true},

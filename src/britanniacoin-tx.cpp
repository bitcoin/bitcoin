...
static void SetupBritanniaCoinTxArgs(ArgsManager &argsman)
{
...
{
    SetupBritanniaCoinTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " britanniacoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  britanniacoin-tx [options] <hex-tx> [commands]  Update hex-encoded britanniacoin transaction\n"
                "or:     britanniacoin-tx [options] -create [commands]   Create hex-encoded britanniacoin transaction\n"
                "\n";

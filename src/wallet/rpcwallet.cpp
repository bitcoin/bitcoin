...
extern bool CheckAliasInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs, bool fJustCheck, int nHeight, string &errorMessage, bool &bDestCheckFailed, const CAmount& nFees, bool dontaddtodb);
...
void SendMoneySyscoin(const vector<unsigned char> &vchAlias, const vector<unsigned char> &vchWitness, const CRecipient &aliasRecipient, CRecipient &aliasFeePlaceholderRecipient, vector<CRecipient> &vecSend, CWalletTx& wtxNew, CCoinControl* coinControl, bool fUseInstantSend=false, bool transferAlias = false)
{
	...
	// in * 180 + out * 33 + 10
	const CAmount nFees = ::minRelayTxFee.GetFee(wtxNew.vin.size()*180 + wtxNew.vout.size()*34 + 10);

...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a particl address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw runtime_error(
            ...
            "    \"solvable\" : true|false   (bool) Whether we know how to spend this output, ignoring the lack of keys\n"
            "  }\n"
            "  ,...\n"
            "]\n"
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    size_t nOutputs = IsHDWallet(pwalletMain) ? tx.vpout.size() : tx.vout.size();
    if (nOutputs == 0)

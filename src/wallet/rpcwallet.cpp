...
void SendMoneySyscoin(const vector<unsigned char> &vchAlias, const CRecipient &aliasRecipient, const CRecipient &aliasFeePlaceholderRecipient, vector<CRecipient> &vecSend, CWalletTx& wtxNew, CCoinControl* coinControl, bool useOnlyAliasPaymentToFund=true, bool transferAlias=false)
{
	...
	if (!pwalletMain->CreateTransaction(vecSend, wtxNew1, reservekey, nFeeRequired, nChangePosRet, strError, coinControl, false,true, useOnlyAliasPaymentToFund)) {
	...
	if(nBalance > 0)
	{
		...
		if(transferAlias && !aliasRecipient.scriptPubKey.empty())
		{
			...
			if (ExtractDestination(aliasRecipient.scriptPubKey, aliasDest)) 
			{
				...
				CRecipient recipient  = {scriptChangeOrig, nBalance-nFeeRequired, false};
		...
		if(nTotal > 0 || transferAlias)
		{
			...
			if(!bIsAliasPaymentFunded || transferAlias)
			{
				...
				aliasselectpaymentcoins(vchAlias, nRequiredPaymentFunds, feeOutPoints, bAreFeePlaceholdersFunded, nRequiredFeePlaceholderFunds, true, transferAlias);
	...
	if (!pwalletMain->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet, strError, coinControl, false,true, useOnlyAliasPaymentToFund)) {
        throw runtime_error(strError);
    }

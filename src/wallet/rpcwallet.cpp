...
void FundTransaction(CWallet& wallet, CMutableTransaction& tx, CAmount& fee_out, int& change_position, const UniValue& options, CCoinControl& coinControl, bool override_min_fee, const UniValue& solving_data)
{
    // Make sure the results are valid at least up to the most recent block
    // the user could have gotten from another RPC command prior to now
    wallet.BlockUntilSyncedToCurrentChain();
    ...
    if (!solving_data.isNull()) {
        if (solving_data.exists("pubkeys")) {
            ...
            for (unsigned int i = 0; i < pubkey_strs.size(); ++i) {
                const std::vector<unsigned char> data(ParseHex(pubkey_strs[i].get_str()));
        ...
        if (solving_data.exists("scripts")) {
            const UniValue script_strs = solving_data["scripts"].get_array();
            for (unsigned int i = 0; i < script_strs.size(); ++i) {
                std::vector<unsigned char> script_data(ParseHex(script_strs[i].get_str()));
        ...
        if (solving_data.exists("descriptors")) {
            const UniValue desc_strs = solving_data["descriptors"].get_array();
            for (unsigned int i = 0; i < desc_strs.size(); ++i) {
                ...
                if (!desc) {
                    throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Unable to parse descriptor %s: %s", desc_strs[i].get_str(), error));
                }
                desc->Expand(i, desc_out, scripts_temp, desc_out);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    FundTransaction(*pwallet, tx, fee, change_position, request.params[1], coin_control, /* override_min_fee */ true, request.params[3]);
...
static RPCHelpMan send()
{
    return RPCHelpMan{"send",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            FundTransaction(*pwallet, rawTx, fee, change_position, options, coin_control, /* override_min_fee */ false, options["solving_data"] /* solving_data */);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // be overridden by options.add_inputs.
    coin_control.m_add_inputs = rawTx.vin.size() == 0;
    FundTransaction(wallet, rawTx, fee, change_position, request.params[3], coin_control, /* override_min_fee */ true, request.params[5]);

...
UniValue loadwallet(const JSONRPCRequest& request)
{
    ...
    if (fParticlMode)
    {
        std::string walletName = wallet_file == "" ? "wallet.dat" : wallet_file;
...
UniValue createwallet(const JSONRPCRequest& request)
{
    ...
    if (fParticlMode)
    {
        temp_wallet = MakeUnique<CHDWallet>(wallet_name, WalletDatabase::Create(fs::absolute(wallet_name, GetWalletDir())));
        partWallet = temp_wallet.get();
    };
    std::shared_ptr<CWallet> const wallet = CWallet::CreateWalletFromFile(wallet_name, fs::absolute(wallet_name, GetWalletDir()), partWallet);
    if (!wallet) {

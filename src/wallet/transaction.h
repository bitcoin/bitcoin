...
public:
    ...
    void Serialize(Stream& s) const
    {
        ...
        s << TX_WITH_WITNESS(tx) << serializedHash << dummy_vector1 << serializedIndex << dummy_vector2 << string_values << msgs_reqs << fTimeReceivedIsTxTime << nTimeReceived << dummy_bool << dummy_bool;
    }
    ...
    void Unserialize(Stream& s)
    {
        ...
        std::map<std::string, std::string> string_values;
        std::vector<std::pair<std::string, std::string>> msgs_reqs;
        s >> TX_WITH_WITNESS(tx) >> serialized_block_hash >> dummy_vector1 >> serializedIndex >> dummy_vector2 >> string_values >> msgs_reqs >> fTimeReceivedIsTxTime >> nTimeReceived >> dummy_bool >> dummy_bool;

                                 ...
                                 CAmount& old_fee, CAmount& new_fee, CMutableTransaction& mtx, bool require_mine, const std::vector<CTxOut>& outputs, std::optional<uint32_t> original_change_index)
{
    ...
    if (original_change_index.has_value() && original_change_index.value() >= tx->GetOutputs().size()) {
    ...
    for (unsigned int i = 0; i < tx->GetInputs().size(); ++i) {
        const CTxIn& txin = tx->GetInputs().at(i);
        const Coin& coin = coins.at(txin.prevout);
    ...
    for (const auto& old_output : tx->GetOutputs()) {
    ...
    const auto& txouts = outputs.empty() ? tx->GetOutputs() : outputs;
    ...
    for (const auto& inputs : tx->GetInputs()) {

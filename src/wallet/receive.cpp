...
bool InputIsMine(const CWallet& wallet, const CTxIn& txin)
{
    ...
    if (prev && txin.prevout.n < prev->GetTx()->GetOutputs().size()) {
        return wallet.IsMine(prev->GetTx()->GetOutputs()[txin.prevout.n]);
    }
    return false;
}
                  ...
                  bool include_change)
{
    ...
    {
        const CTxOut& txout = wtx.GetTx()->GetOutputs()[i];
...
bool CachedTxIsTrusted(const CWallet& wallet, const CWalletTx& wtx, std::set<Txid>& trusted_parents)
{
    ...
    for (const CTxIn& txin : wtx.GetTx()->GetInputs())
    {
        ...
        const CTxOut& parentOut = parent->GetTx()->GetOutputs()[txin.prevout.n];
...
std::set< std::set<CTxDestination> > GetAddressGroupings(const CWallet& wallet)
{
    ...
    for (const auto& walletEntry : wallet.mapWallet)
    {
        ...
        if (wtx.GetTx()->GetInputs().size() > 0)
        {
            ...
            for (const CTxIn& txin : wtx.GetTx()->GetInputs())
            {
                ...
                if(!InputIsMine(wallet, txin)) /* If this input isn't mine, ignore it */
                    continue;
                if(!ExtractDestination(wallet.mapWallet.at(txin.prevout.hash).GetTx()->GetOutputs()[txin.prevout.n].scriptPubKey, address))
                    continue;
        ...
        for (const auto& txout : wtx.GetTx()->GetOutputs())

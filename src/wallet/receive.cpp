...
CAmount CachedTxGetImmatureCredit(const CWallet& wallet, const CWalletTx& wtx, bool avoid_reuse)
{
    ...
    if (wallet.IsTxImmatureCoinBase(wtx) && wtx.isConfirmed()) {
        return GetCachableAmount(wallet, wtx, CWalletTx::IMMATURE_CREDIT, avoid_reuse);
    }

    return 0;
}

CAmount CachedTxGetAvailableCredit(const CWallet& wallet, const CWalletTx& wtx, bool avoid_reuse)
{
    AssertLockHeld(wallet.cs_wallet);
    ...
    if (wtx.m_amounts[CWalletTx::AVAILABLE_CREDIT].IsCached(avoid_reuse)) {
...
Balance GetBalance(const CWallet& wallet, const int min_depth, bool avoid_reuse)
{
    Balance ret;
    {
        ...
        for (const auto& entry : wallet.mapWallet)
        {
            ...
            const CAmount tx_credit_mine{CachedTxGetAvailableCredit(wallet, wtx, avoid_reuse)};
            ...
            ret.m_mine_immature += CachedTxGetImmatureCredit(wallet, wtx, avoid_reuse);
        }
    }
    return ret;
}
...
std::map<CTxDestination, CAmount> GetAddressBalances(const CWallet& wallet)
{
...

    {
        ...
        for (const auto& walletEntry : wallet.mapWallet)
        {
            ...
            if (nDepth < (CachedTxIsFromMe(wallet, wtx) ? 0 : 1))
                continue;

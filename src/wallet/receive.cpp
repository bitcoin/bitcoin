...
CAmount OutputGetCredit(const CWallet& wallet, const CTxOut& txout)
{
    ...
    LOCK(wallet.cs_wallet);
    return (wallet.IsMine(txout) ? txout.nValue : CAmount{0});
}
...
CAmount OutputGetChange(const CWallet& wallet, const CTxOut& txout)
{
    ...
    if (!MoneyRange(txout.nValue))
        throw std::runtime_error(std::string(__func__) + ": value out of range");
    return (OutputIsChange(wallet, txout) ? txout.nValue : CAmount{0});
}
                  ...
                  bool include_change)
{
    ...
    if (nDebit > CAmount{0}) // debit>0 means we signed/sent this transaction
    {
...
std::map<CTxDestination, CAmount> GetAddressBalances(const CWallet& wallet)
{
...

    {
        ...
        for (const auto& [outpoint, txo] : wallet.GetTXOs()) {
            ...
            CAmount n = wallet.IsSpent(outpoint) ? CAmount{0} : txo.GetTxOut().nValue;

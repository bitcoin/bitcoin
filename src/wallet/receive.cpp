    ...
    EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (!wtx.m_cached_input_ownership.has_value()) {
        wtx.m_cached_input_ownership = GetInputOwnership(wallet, *wtx.GetTx());
    }
    ...
    EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        const CWalletTx* prev{wallet.GetWalletTx(txin.prevout.hash)};
        if (!prev || txin.prevout.n >= prev->GetTx()->vout.size()) return false;
        if (prev->GetTx()->vout[txin.prevout.n].nValue != 0) return false;
    }
    return true;
}
    ...
    EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (!wtx.m_cached_foreign_inputs_zero_value.has_value()) {
        wtx.m_cached_foreign_inputs_zero_value = AllForeignInputsKnownZeroValue(wallet, *wtx.GetTx());
    }
    ...
    EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    for (unsigned int i = 0; i < wtx.GetTx()->vout.size(); ++i)
    {
        ...
        //   1) the wallet contributed the total input value, so the output can be reported as sent

...
private:
     ...
     */
    bool AddToWalletIfInvolvingMe(const CTransactionRef& tx, const std::map<COutPoint, Coin>& spent_coins, const SyncTxState& state, bool rescanning_old_block) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    bool SyncTransaction(const CTransactionRef& tx, const SyncTxState& state, const std::map<COutPoint, Coin>& spent_coins = {}, bool rescanning_old_block = false) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);

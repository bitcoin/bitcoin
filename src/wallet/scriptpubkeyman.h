...
private:
    ...
    bool CanGetAddresses_() const EXCLUSIVE_LOCKS_REQUIRED(m_desc_mutex);
    ...
    //! Store the master private key and initialize the descriptor cache for a newly-created SPKM.
    void SetupDescriptorGeneration(WalletBatch& batch, const CExtKey& master_key) LOCKS_EXCLUDED(m_desc_mutex);

    WalletDescriptor m_wallet_descriptor GUARDED_BY(m_desc_mutex);
    void IncIndex() EXCLUSIVE_LOCKS_REQUIRED(m_desc_mutex);
    void DecIndex() EXCLUSIVE_LOCKS_REQUIRED(m_desc_mutex);
    void SetRangeEnd(int32_t end) EXCLUSIVE_LOCKS_REQUIRED(m_desc_mutex);
...
protected:
    ...
    void SetupDescriptor(WalletBatch& batch) LOCKS_EXCLUDED(m_desc_mutex);

...
public:
    ...
    {
        FeePerVSize feerate(ConsumeMoney(fuzzed_data_provider, /*max=*/CAmount{1'000'000}), fuzzed_data_provider.ConsumeIntegralInRange<unsigned int>(1000, 1000000));
...
FUZZ_TARGET(wallet_fees, .init = initialize_setup)
{
    ...
    CTxMemPool::Options mempool_opts{
        .incremental_relay_feerate = CFeeRate{ConsumeMoney(fuzzed_data_provider, CAmount{1'000'000})},
        .min_relay_feerate = CFeeRate{ConsumeMoney(fuzzed_data_provider, CAmount{1'000'000})},
        .dust_relay_feerate = CFeeRate{ConsumeMoney(fuzzed_data_provider, CAmount{1'000'000})}
    };
    ...
    auto target_feerate{CFeeRate{ConsumeMoney(fuzzed_data_provider, /*max=*/CAmount{1'000'000})}};

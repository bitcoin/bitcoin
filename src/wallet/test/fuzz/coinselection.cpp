...
static void GroupCoins(FuzzedDataProvider& fuzzed_data_provider, const std::vector<COutput>& coins, const CoinSelectionParams& coin_params, bool positive_only, std::vector<OutputGroup>& output_groups)
{
    ...
    for (auto& coin : coins) {
        if (!positive_only || (positive_only && coin.GetEffectiveValue() > CAmount{0})) {
        ...
        valid_outputgroup = !positive_only || output_group.GetSelectionAmount() > CAmount{0};
...
static CAmount CreateCoins(FuzzedDataProvider& fuzzed_data_provider, std::vector<COutput>& utxo_pool, CoinSelectionParams& coin_params, int& next_locktime)
{
    ...
    LIMITED_WHILE (fuzzed_data_provider.ConsumeBool(), 10000) {
        ...
        const CAmount amount{ConsumeMoney(fuzzed_data_provider, CAmount{1}, MAX_MONEY)};
...
FUZZ_TARGET(coin_grinder)
{
    ...
    const CAmount target{ConsumeMoney(fuzzed_data_provider, CAmount{1}, MAX_MONEY)};
    ...
    if (result_srd && result_srd->GetChange(CHANGE_LOWER, coin_params.m_change_fee) > CAmount{0}) { // exclude any srd solutions that don’t have change, err on excluding
    ...
    if (result_knapsack && result_knapsack->GetChange(CHANGE_LOWER, coin_params.m_change_fee) > CAmount{0}) { // exclude any knapsack solutions that don’t have change, err on excluding
...
FUZZ_TARGET(coin_grinder_is_optimal)
{
    ...
    LIMITED_WHILE (fuzzed_data_provider.ConsumeBool(), max_output_groups) {
        ...
        const CAmount eff_value{ConsumeMoney(fuzzed_data_provider, CAmount{1}, MAX_MONEY + CAmount(group_pos.size()) - max_spendable - CAmount{max_output_groups})};
    ...
    const CAmount target{ConsumeMoney(fuzzed_data_provider, CAmount{1}, std::max(CAmount{1}, max_spendable - coin_params.m_min_change_target))};
...
FUZZ_TARGET(bnb_finds_min_waste)
{
    ...
    coin_params.m_effective_feerate = CFeeRate{ConsumeMoney(fuzzed_data_provider, CAmount{10'000'000}), 1'000};
    coin_params.m_long_term_feerate = CFeeRate{ConsumeMoney(fuzzed_data_provider, CAmount{10'000'000}), 1'000};
    coin_params.m_discard_feerate = CFeeRate{ConsumeMoney(fuzzed_data_provider, CAmount{10'000'000}), 1'000};
    ...
    const auto dust{GetDustThreshold(CTxOut{/*nValueIn=*/CAmount{0}, change_out_script}, coin_params.m_discard_feerate)};
    coin_params.min_viable_change = std::max(change_spend_fee + CAmount{1}, dust);
    ...
    // Only choose targets below max_spendable
    const CAmount target{ConsumeMoney(fuzzed_data_provider, CAmount{1}, std::max(CAmount{1}, max_spendable - coin_params.m_cost_of_change))};
...
void FuzzCoinSelectionAlgorithm(std::span<const uint8_t> buffer) {
    ...
    if constexpr (Algorithm == CoinSelectionAlgorithm::BNB) {
        if (!coin_params.m_subtract_fee_outputs) {
            ...
            if (result_bnb) {
                result = *result_bnb;
                assert(result_bnb->GetChange(coin_params.min_viable_change, coin_params.m_change_fee) == CAmount{0});
    ...
    if constexpr (Algorithm == CoinSelectionAlgorithm::SRD) {
        ...
        if (result_srd) {
            ...
            assert(result_srd->GetChange(CHANGE_LOWER, coin_params.m_change_fee) > CAmount{0});
    ...
    if (new_total_balance > CAmount{0}) {
    ...
    if (manual_balance == CAmount{0}) return;

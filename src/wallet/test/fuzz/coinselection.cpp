...
static void GroupCoins(FuzzedDataProvider& fuzzed_data_provider, const std::vector<COutput>& coins, const CoinSelectionParams& coin_params, bool positive_only, std::vector<OutputGroup>& output_groups)
{
    ...
    for (auto& coin : coins) {
        if (!positive_only || (positive_only && coin.GetEffectiveValue() > 0*sats)) {
        ...
        valid_outputgroup = !positive_only || output_group.GetSelectionAmount() > 0*sats;
...
static CAmount CreateCoins(FuzzedDataProvider& fuzzed_data_provider, std::vector<COutput>& utxo_pool, CoinSelectionParams& coin_params, int& next_locktime)
{
    ...
    LIMITED_WHILE (fuzzed_data_provider.ConsumeBool(), 10000) {
        ...
        const CAmount amount{ConsumeMoney(fuzzed_data_provider, 1*sats, MAX_MONEY)};
...
FUZZ_TARGET(coin_grinder)
{
    ...
    const CAmount target{ConsumeMoney(fuzzed_data_provider, 1*sats, MAX_MONEY)};
    ...
    if (result_srd && result_srd->GetChange(CHANGE_LOWER, coin_params.m_change_fee) > 0*sats) { // exclude any srd solutions that don’t have change, err on excluding
    ...
    if (result_knapsack && result_knapsack->GetChange(CHANGE_LOWER, coin_params.m_change_fee) > 0*sats) { // exclude any knapsack solutions that don’t have change, err on excluding
...
FUZZ_TARGET(coin_grinder_is_optimal)
{
    ...
    LIMITED_WHILE (fuzzed_data_provider.ConsumeBool(), max_output_groups) {
        ...
        const CAmount eff_value{ConsumeMoney(fuzzed_data_provider, 1*sats, MAX_MONEY + CAmount(group_pos.size()) - max_spendable - CAmount{max_output_groups})};
    ...
    const CAmount target{ConsumeMoney(fuzzed_data_provider, 1*sats, std::max(1*sats, max_spendable - coin_params.m_min_change_target))};
...
FUZZ_TARGET(bnb_finds_min_waste)
{
    ...
    coin_params.m_effective_feerate = CFeeRate{ConsumeMoney(fuzzed_data_provider, 10'000'000*sats), 1'000};
    coin_params.m_long_term_feerate = CFeeRate{ConsumeMoney(fuzzed_data_provider, 10'000'000*sats), 1'000};
    coin_params.m_discard_feerate = CFeeRate{ConsumeMoney(fuzzed_data_provider, 10'000'000*sats), 1'000};
    ...
    const auto dust{GetDustThreshold(CTxOut{/*nValueIn=*/0*sats, change_out_script}, coin_params.m_discard_feerate)};
    coin_params.min_viable_change = std::max(change_spend_fee + 1*sats, dust);
    ...
    // Only choose targets below max_spendable
    const CAmount target{ConsumeMoney(fuzzed_data_provider, 1*sats, std::max(1*sats, max_spendable - coin_params.m_cost_of_change))};
...
void FuzzCoinSelectionAlgorithm(std::span<const uint8_t> buffer) {
    ...
    if constexpr (Algorithm == CoinSelectionAlgorithm::BNB) {
        if (!coin_params.m_subtract_fee_outputs) {
            ...
            if (result_bnb) {
                result = *result_bnb;
                assert(result_bnb->GetChange(coin_params.min_viable_change, coin_params.m_change_fee) == 0*sats);
    ...
    if constexpr (Algorithm == CoinSelectionAlgorithm::SRD) {
        ...
        if (result_srd) {
            ...
            assert(result_srd->GetChange(CHANGE_LOWER, coin_params.m_change_fee) > 0*sats);
    ...
    if (manual_balance == 0*sats) return;

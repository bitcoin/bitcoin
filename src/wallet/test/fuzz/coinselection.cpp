...
static CAmount CreateCoins(FuzzedDataProvider& fuzzed_data_provider, std::vector<COutput>& utxo_pool, CoinSelectionParams& coin_params, int& next_locktime)
{
    ...
    LIMITED_WHILE (fuzzed_data_provider.ConsumeBool(), 10000) {
        ...
        const CAmount amount{ConsumeMoney(fuzzed_data_provider, 1, MAX_MONEY)};
...
FUZZ_TARGET(coin_grinder)
{
    ...
    const CAmount target{ConsumeMoney(fuzzed_data_provider, 1, MAX_MONEY)};
...
FUZZ_TARGET(coin_grinder_is_optimal)
{
    ...
    LIMITED_WHILE (fuzzed_data_provider.ConsumeBool(), max_output_groups) {
        ...
        const CAmount eff_value{ConsumeMoney(fuzzed_data_provider, 1, MAX_MONEY + group_pos.size() - max_spendable - max_output_groups)};
    ...
    const CAmount target{ConsumeMoney(fuzzed_data_provider, 1, std::max(CAmount{1}, max_spendable - coin_params.m_min_change_target))};
...
FUZZ_TARGET(bnb_finds_min_waste)
{
    ...
    // Only choose targets below max_spendable
    const CAmount target{ConsumeMoney(fuzzed_data_provider, 1, std::max(CAmount{1}, max_spendable - coin_params.m_cost_of_change))};

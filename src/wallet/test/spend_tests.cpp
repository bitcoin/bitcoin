...
BOOST_FIXTURE_TEST_CASE(SubtractFee, TestChain100Setup)
{
    ...
    auto check_tx = [&wallet](CAmount leftover_input_amount) {
        ...
        CCoinControl coin_control;
        coin_control.m_feerate.emplace(CAmount{10000});
        ...
        BOOST_CHECK_GT(txr.fee, CAmount{0});
        return txr.fee;
    };

    // Send full input amount to recipient, check that only nonzero fee is
    // subtracted (to_reduce == fee).
    const CAmount fee{check_tx(CAmount{0})};
    ...
    BOOST_CHECK_EQUAL(fee, check_tx(CAmount{123}));
    ...
    BOOST_CHECK_EQUAL(fee, check_tx(fee + CAmount{123}));
}

...
BOOST_AUTO_TEST_CASE(bnb_test)
{
    for (int feerate : FEERATES) {
        ...
        TestBnBFail("Overshoot upper bound", utxo_pool, /*selection_target=*/4 * CENT - cs_params.m_cost_of_change - CAmount{1}, cs_params);
        ...
        AddCoins(doppelganger_pool, {1 * CENT + cs_params.m_cost_of_change + CAmount{17}}, cs_params);
        ...
        AddCoins(doppelganger_pool, {1 * CENT + cs_params.m_cost_of_change + CAmount{18}}, cs_params);
...
BOOST_AUTO_TEST_CASE(bnb_exhaustion_with_solution_test)
{
    ...
    BOOST_CHECK(result->GetSelectedEffectiveValue() > selection_target + CAmount{28});
...
BOOST_AUTO_TEST_CASE(bnb_feerate_sensitivity_test)
{
    ...
    const CoinSelectionParams high_feerate_params = init_cs_params(/*eff_feerate=*/CAmount{25'000});
...
BOOST_AUTO_TEST_CASE(srd_test)
{
    for (int feerate : FEERATES) {
        ...
        TestSRDSuccess("Select 21k sats", utxo_pool, /*selection_target=*/CAmount{21'000}, cs_params);
        TestSRDSuccess("Select 1 CENT", utxo_pool, /*selection_target=*/1 * CENT, cs_params);
        TestSRDSuccess("Select 3.125 CENT", utxo_pool, /*selection_target=*/CAmount{3'125'000}, cs_params);
        ...
        TestSRDFail("Undershoot minimum change by one sat", utxo_pool, /*selection_target=*/9 * CENT - cs_params.m_change_fee - CHANGE_LOWER + CAmount{1}, cs_params);
        TestSRDFail("Spend more than available", utxo_pool, /*selection_target=*/9 * CENT + CAmount{1}, cs_params);

...
static CoinSelectionParams init_cs_params(int eff_feerate = 5000)
{
    ...
    csp.m_change_fee = /*155 sats=*/csp.m_effective_feerate.GetFee(csp.change_output_size);
    csp.min_viable_change = /*204 sats=*/csp.m_discard_feerate.GetFee(csp.change_spend_size);
    csp.m_cost_of_change = /*204 + 155 sats=*/csp.min_viable_change + csp.m_change_fee;
...
BOOST_AUTO_TEST_CASE(bnb_feerate_sensitivity_test)
{
    ...
    CoinSelectionParams high_feerate_params = init_cs_params(/*eff_feerate=*/25'000);
...
{
    SelectionResult expected_result(CAmount(0), SelectionAlgorithm::SRD);

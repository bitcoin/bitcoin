...
static void addCoin(CoinsResult& coins,
                     ...
                     bool is_from_me,
                     CFeeRate fee_rate = CFeeRate(0*sats),
                     int depth = 6)
{
 ...
 CoinSelectionParams makeSelectionParams(FastRandomContext& rand, bool avoid_partial_spends)
{
    return CoinSelectionParams{
            ...
            /*effective_feerate=*/ CFeeRate(0*sats),
            /*long_term_feerate=*/ CFeeRate(0*sats),
            /*discard_feerate=*/ CFeeRate(0*sats),
            /*tx_noinputs_size=*/ 0,
...
BOOST_AUTO_TEST_CASE(outputs_grouping_tests)
{
    ...
    addCoin(group_verifier.coins_pool, *wallet, dest3, 1*sats, true, CFeeRate(100*sats));
    BOOST_CHECK(group_verifier.coins_pool.coins[OutputType::BECH32].back().GetEffectiveValue() <= 0*sats);
    ...
    addCoin(group_verifier.coins_pool, *wallet, dest4, 6 * COIN,
            /*is_from_me=*/false, CFeeRate(0*sats), /*depth=*/5);
    ...
    addCoin(group_verifier.coins_pool, *wallet, dest5, 6 * COIN,
            /*is_from_me=*/true, CFeeRate(0*sats), /*depth=*/0);

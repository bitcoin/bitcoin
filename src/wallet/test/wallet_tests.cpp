...
static void CheckValues(const CWallet& wallet, const Txid& txid, const TxValues& before) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    const CWalletTx& wtx = wallet.mapWallet.at(txid);
    const auto& tx = *wtx.GetTx();
    BOOST_CHECK_EQUAL(TxGetCredit(wallet, tx), before.credit);
    BOOST_CHECK_EQUAL(wallet.GetDebit(tx), before.debit);
    BOOST_CHECK_EQUAL(TxGetChange(wallet, tx), before.change);
    BOOST_CHECK_EQUAL(wallet.IsFromMe(tx), before.from_me);

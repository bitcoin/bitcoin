...
static std::shared_ptr<CWallet> TestLoadWallet(interfaces::Chain* chain)
{
    ...
    auto wallet = CWallet::Create(chain, "", gArgs, std::move(database), options.create_flags, error, warnings);
    if (chain) {
...
BOOST_FIXTURE_TEST_CASE(importmulti_rescan, TestChain100Setup)
{
    ...
    // after.
    {
        std::shared_ptr<CWallet> wallet = std::make_shared<CWallet>(m_node.chain.get(), "", gArgs, CreateDummyWalletDatabase());
...
public:
    ...
    {
        CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()));
        wallet = std::make_unique<CWallet>(m_node.chain.get(), "", gArgs, CreateMockWalletDatabase());
        {

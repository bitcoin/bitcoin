...
public:
    explicit ParallelFilterChecker(const ChainScanner::ScanContext& ctx, int start_height, const std::string& thread_name, int num_threads)
        : m_ctx(ctx), m_pool(thread_name), m_last_submitted_height(start_height - 1), m_max_tasks(num_threads * 2) {
...
ScanResult ChainScanner::Scan(const uint256& start_block, int start_height, std::optional<int> max_height,
    ...
    if (chain.hasBlockFilterIndex(BlockFilterType::BASIC)) {
        ...
        if (m_wallet.m_wallet_par > 1) {
            ctx.checker = std::make_unique<ParallelFilterChecker>(ctx, start_height,

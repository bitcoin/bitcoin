...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef& ptx, const std::map<COutPoint, Coin>& spent_coins, const SyncTxState& state, bool rescanning_old_block)
{
...
bool CWallet::SyncTransaction(const CTransactionRef& ptx, const SyncTxState& state, const std::map<COutPoint, Coin>& spent_coins, bool rescanning_old_block)
{
    if (!AddToWalletIfInvolvingMe(ptx, spent_coins, state, rescanning_old_block))
...
CWallet::ScanResult CWallet::ScanForWalletTransactions(const uint256& start_block, int start_height, std::optional<int> max_height, const WalletRescanReserver& reserver, const bool save_progress)
{
    ...
    while (!fAbortRescan && !chain().shutdownRequested()) {
        ...
        if (fetch_block) {
            ...
            CBlockUndo block_undo;
            // Find block
            FoundBlock found_block{FoundBlock().data(block).undoData(block_undo)};
            ...
            if (!block.IsNull()) {
                ...
                for (size_t posInBlock = 0; posInBlock < block.vtx.size(); ++posInBlock) {
                    ...
                    // Sync tx with wallet
                    SyncTransaction(block.vtx[posInBlock], TxStateConfirmed{block_hash, block_height, static_cast<int>(posInBlock)}, spent_coins, /*rescanning_old_block=*/true);
                }
...
{
    const std::map<OutputType, ScriptPubKeyMan*>& spk_managers = internal && type != OutputType::SILENT_PAYMENTS ? m_internal_spk_managers : m_external_spk_managers;
...
void CWallet::SetupDescriptorScriptPubKeyMans(WalletBatch& batch, const CExtKey& master_key)
{
    ...
    for (bool internal : {false, true}) {
        for (OutputType t : OUTPUT_TYPES) {
            if (t == OutputType::SILENT_PAYMENTS &&
                (internal == true || !IsWalletFlagSet(WALLET_FLAG_SILENT_PAYMENTS))) continue;
...
util::Result<std::reference_wrapper<DescriptorScriptPubKeyMan>> CWallet::AddWalletDescriptor(WalletDescriptor& desc, const FlatSigningProvider& signing_provider, const std::string& label, bool internal)
{
    ...
    if (!desc.descriptor->IsRange() && desc.descriptor->HasScripts() && desc.descriptor->GetOutputType() != OutputType::SILENT_PAYMENTS) {

...
void CWallet::SetSpentKeyState(WalletBatch& batch, const Txid& hash, unsigned int n, bool used, std::set<CTxDestination>& tx_destinations)
{
    ...
    CTxDestination dst;
    if (ExtractDestination(srctx->tx->GetOutputs()[n].scriptPubKey, dst)) {
        if (IsMine(dst)) {
...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& update_wtx, bool rescanning_old_block)
{
    ...
    if (wtx.IsCoinBase() && wtx.isInactive()) {
        ...
        while (!txs.empty()) {
            ...
            for (unsigned int i = 0; i < desc_tx->tx->GetOutputs().size(); ++i) {
...
void CWallet::UpdateTrucSiblingConflicts(const CWalletTx& parent_wtx, const Txid& child_txid, bool add_conflict) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet)
{
    ...
    for (long unsigned int i = 0; i < parent_wtx.tx->GetOutputs().size(); i++) {
...
bool CWallet::IsMine(const COutPoint& outpoint) const
{
    ...
    if (outpoint.n >= wtx->tx->GetOutputs().size()) {
        return false;
    }
    return IsMine(wtx->tx->GetOutputs()[outpoint.n]);
}
...
bool CWallet::SignTransaction(CMutableTransaction& tx) const
{
    ...
    for (auto& input : tx.vin) {
        ...
        if(mi == mapWallet.end() || input.prevout.n >= mi->second.tx->GetOutputs().size()) {
            return false;
        }
        const CWalletTx& wtx = mi->second;
        int prev_height = wtx.state<TxStateConfirmed>() ? wtx.state<TxStateConfirmed>()->confirmed_block_height : 0;
        coins[input.prevout] = Coin(wtx.tx->GetOutputs()[input.prevout.n], prev_height, wtx.IsCoinBase());
    }
...
util::Result<void> CWallet::RemoveTxs(WalletBatch& batch, std::vector<Txid>& txs_to_remove)
{
    ...
    batch.RegisterTxnListener({.on_commit=[&, erased_txs]() EXCLUSIVE_LOCKS_REQUIRED(cs_wallet) {
        ...
        for (const auto& it : erased_txs) {
            ...
            for (const auto& txin : it->second.tx->GetInputs()) {
            ...
            for (unsigned int i = 0; i < it->second.tx->GetOutputs().size(); ++i) {
...
void CWallet::MarkDestinationsDirty(const std::set<CTxDestination>& destinations) {
    for (auto& entry : mapWallet) {
        ...
        for (unsigned int i = 0; i < wtx.tx->GetOutputs().size(); i++) {
            CTxDestination dst;
            if (ExtractDestination(wtx.tx->GetOutputs()[i].scriptPubKey, dst) && destinations.contains(dst)) {
                wtx.MarkDirty();
                break;
            }
        }
...
{
    AssertLockHeld(cs_wallet);
    for (uint32_t i = 0; i < wtx.tx->GetOutputs().size(); ++i) {
        const CTxOut& txout = wtx.tx->GetOutputs().at(i);

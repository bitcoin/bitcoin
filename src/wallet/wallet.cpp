 ...
 */
void CWallet::UpgradeToGlobalHDKey(bool encryption_status)
{
    ...
    std::map<CKeyID, CKey> desc_keys;
    std::map<CKeyID, std::pair<CPubKey, std::vector<unsigned char>>> desc_crypt_keys;
    ...
    for (const auto& spkm : GetAllScriptPubKeyMans()) {
        ...
        desc_keys.merge(desc_spkm->GetKeys());
        desc_crypt_keys.merge(desc_spkm->GetCryptedKeys());
    }
    ...
    for (const auto& [xpub, info] : key_counts) {
        ...
        if (std::all_of(dtypes.begin(), dtypes.end(), [](const auto& p) { return p.second == 2;})) {
            ...
            if (!best_xpub || desc_time > best_time) {
                best_time = desc_time;
    ...
    if (best_xpub) {
        ...
        auto key_it = desc_keys.find(best_xpub->pubkey.GetID());
        // May not be in desc_keys as it could be encrypted
        if (key_it != desc_keys.end()) {
            key = key_it->second;
        }
        std::vector<unsigned char> crypted_key;
        // May not be in desc_crypt_keys as it could be unencrypted
        auto ckey_it = desc_crypt_keys.find(best_xpub->pubkey.GetID());
        if (ckey_it != desc_crypt_keys.end()) {
            crypted_key = ckey_it->second.second;
        }
        // Only one of key or crypted_key must be set at this point.
        // SetActiveHDKey will check this for us.
        Assume(SetActiveHDKey(*best_xpub, key, crypted_key));
    } else {
...
bool CWallet::ApplyMigrationData(MigrationData& data, bilingual_str& error)
{
    ...
    if (!IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
        ...
        if (data.master_key.key.IsValid()) {
            ...
            // Store the master key as our active hd key
            if (!SetActiveHDKey(data.master_key)) {
...
{
    AssertLockHeld(cs_wallet);
    if (!LoadActiveHDPubKey(xpub, key, crypted_key)) {
        return false;
    }
...
std::optional<CExtKey> CWallet::GetActiveHDPrivKey()
{
    ...
    CPubKey& pubkey = m_hd_key->xpub.pubkey;

    std::optional<CKey> key = std::visit(util::Overloaded{
        [&](const CKey& key) -> std::optional<CKey> {
            if (!Assume(!IsCrypted())) {
                return std::nullopt;
            }
            if (!Assume(key.IsValid())) {
                return std::nullopt;
            }
            return key;
        },
       [&](const std::vector<unsigned char> ckey) -> std::optional<CKey> {
            if (!Assume(IsCrypted())) {
                return std::nullopt;
            }
            if (IsLocked()) {
            ...
            if (!Assume(!ckey.empty())) {
                return std::nullopt;
            }
            CKey key;
            if (!Assume(DecryptKey(GetEncryptionKey(), ckey, pubkey, key))) {
                return std::nullopt;
            }
            return key;
        },
    ...
    CExtKey extkey(m_hd_key->xpub, *key);
    return extkey;
}
} // namespace wallet

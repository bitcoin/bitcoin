...
util::Result<void> CWallet::ApplyMigrationData(WalletBatch& local_wallet_batch, MigrationData& data)
{
    ...
    bool empty_local_wallet = data.desc_spkms.empty();
    ...
    if (!IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
        ...
        if (data.master_key.key.IsValid()) {
            ...
            // Setup with a new seed, only if we are actually migrating any spendable descriptor
...
bool DoMigration(CWallet& wallet, WalletContext& context, bilingual_str& error, MigrationResult& res, bool& empty_local_wallet) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    return RunWithinTxn(wallet.GetDatabase(), /*process_desc=*/"apply migration process", [&](WalletBatch& batch) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet){
        ...
        // wallet.
        empty_local_wallet = data->desc_spkms.empty();
        return true;
    });
}
...
util::Result<MigrationResult> MigrateLegacyToDescriptor(const std::string& wallet_name, const SecureString& passphrase, WalletContext& context)
{
    ...
    // Indicates if the current wallet ended up empty after migration.
    // Notes:

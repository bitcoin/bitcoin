...
util::ResultPtr<std::shared_ptr<CWallet>> CWallet::Create(WalletContext& context, const std::string& name, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags)
{
    ...
    if (nLoadWalletRet != DBErrors::LOAD_OK) {
        ...
        else if (nLoadWalletRet == DBErrors::NONCRITICAL_ERROR)
        {
            ...
            result.Update(util::Error{strprintf(_("Error loading %s: Wallet requires newer version of %s"), walletFile, PACKAGE_NAME)});
            return result;
        }
        ...
        {
            result.Update(util::Error{strprintf(_("Wallet needed to be rewritten: restart %s to complete"), PACKAGE_NAME)});
            return result;
        ...
        if ((wallet_creation_flags & WALLET_FLAG_EXTERNAL_SIGNER) || !(wallet_creation_flags & (WALLET_FLAG_DISABLE_PRIVATE_KEYS | WALLET_FLAG_BLANK_WALLET))) {
            ...
            if (walletInstance->IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
                ...
                for (auto spk_man : walletInstance->GetActiveScriptPubKeyMans()) {
                    if (!spk_man->SetupGeneration()) {
                        result.Update(util::Error{_("Unable to generate initial keys")});
                        return result;
                    }
                }
            }
    ...
    if (args.IsArgSet("-mintxfee")) {
        ...
        if (!min_tx_fee) {
            result.Update(util::Error{AmountErrMsg("mintxfee", args.GetArg("-mintxfee", ""))});
            return result;
    ...
    if (args.IsArgSet("-fallbackfee")) {
        ...
        if (!fallback_fee) {
            result.Update(util::Error{strprintf(_("Invalid amount for %s=<amount>: '%s'"), "-fallbackfee", args.GetArg("-fallbackfee", ""))});
            return result;
    ...
    if (args.IsArgSet("-discardfee")) {
        ...
        if (!discard_fee) {
            result.Update(util::Error{strprintf(_("Invalid amount for %s=<amount>: '%s'"), "-discardfee", args.GetArg("-discardfee", ""))});
            return result;
    ...
    if (args.IsArgSet("-paytxfee")) {
        ...
        if (!pay_tx_fee) {
            result.Update(util::Error{AmountErrMsg("paytxfee", args.GetArg("-paytxfee", ""))});
            return result;
    ...
    if (args.IsArgSet("-maxtxfee")) {
        ...
        if (!max_fee) {
            result.Update(util::Error{AmountErrMsg("maxtxfee", args.GetArg("-maxtxfee", ""))});
            return result;
    ...
    if (args.IsArgSet("-consolidatefeerate")) {
        if (std::optional<CAmount> consolidate_feerate = ParseMoney(args.GetArg("-consolidatefeerate", ""))) {
            ...
            result.Update(util::Error{AmountErrMsg("consolidatefeerate", args.GetArg("-consolidatefeerate", ""))});
            return result;
        }
    }
...
util::Result<MigrationResult> MigrateLegacyToDescriptor(const std::string& wallet_name, const SecureString& passphrase, WalletContext& context)
{
    ...
    if (auto wallet = GetWallet(context, wallet_name)) {
        ...
        if (!(RemoveWallet(context, wallet, /*load_on_start=*/std::nullopt) >> result)) {
            result.Update(util::Error{_("Unable to unload the wallet before migrating")});
            return result;
        }
    ...
    const auto& reload_wallet = [&](std::shared_ptr<CWallet>& to_reload) {
        ...
        to_reload.reset();
        if (auto wallet{LoadWallet(context, name, /*load_on_start=*/std::nullopt, options) >> result}) {
    ...
    if (success) {
        ...
        result->wallet = local_wallet.value();
        result->wallet_name = wallet_name;
        if (success && result->watchonly_wallet) {
            // Reload watchonly
            wallet_dirs.insert(fs::PathFromString(result->watchonly_wallet->GetDatabase().Filename()).parent_path());
            success = reload_wallet(result->watchonly_wallet);
        }
        if (success && result->solvables_wallet) {
            // Reload solvables
            wallet_dirs.insert(fs::PathFromString(result->solvables_wallet->GetDatabase().Filename()).parent_path());
            success = reload_wallet(result->solvables_wallet);
        }
    }
    if (!success) {
        ...
        // Restore the backup
        if (!(RestoreWallet(context, temp_backup_location, wallet_name, /*load_on_start=*/std::nullopt) >> result)) {

...
util::Result<MigrationResult> MigrateLegacyToDescriptor(std::shared_ptr<CWallet> local_wallet, const SecureString& passphrase, WalletContext& context, bool was_loaded)
{
    ...
    if (success) {
        ...
        if (!empty_local_wallet) { // Reload and set the main spendable wallet only if needed
        ...
        if (success && res.watchonly_wallet) {
            ...
            // When no spendable wallet is created, set the main wallet to the watch-only wallet
            if (res.wallet_name.empty()) {
                res.wallet_name = res.watchonly_wallet->GetName();
                res.wallet = std::move(res.watchonly_wallet);
        ...
        if (success && res.solvables_wallet) {
            ...
            // When no spendable neither watch-only wallets are created, set the main wallet to the solvables-only wallet
            if (res.wallet_name.empty()) {
                res.wallet_name = res.solvables_wallet->GetName();
                res.wallet = std::move(res.solvables_wallet);

    ...
    while (!fAbortRescan && !chain().shutdownRequested()) {
        ...
        if (fast_rescan_filter) {
            ...
            if (matches_block.has_value()) {
                if (*matches_block) {
                    LogPrint(LogInstance(), BCLog::SCAN, "Fast rescan: inspect block %d [%s] (filter matched)\n", block_height, block_hash.ToString());
                } else {
                ...
                LogPrint(LogInstance(), BCLog::SCAN, "Fast rescan: inspect block %d [%s] (WARNING: block filter not found!)\n", block_height, block_hash.ToString());
            }
        }

...
util::ResultPtr<std::shared_ptr<CWallet>, DatabaseError> CreateWallet(WalletContext& context, const std::string& name, std::optional<bool> load_on_start, DatabaseOptions& options)
{
    ...
    auto create{CWallet::CreateNew(context, name, std::move(database.value()), wallet_creation_flags) >> result};
    if (!create) {
...
util::ResultPtr<std::shared_ptr<CWallet>> CWallet::CreateNew(WalletContext& context, const std::string& name, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags)
{
...
util::Result<void> DoMigration(CWallet& wallet, WalletContext& context, MigrationResult& res) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (data->watch_descs.size() > 0 || data->solvable_descs.size() > 0) {
        ...
        if (data->watch_descs.size() > 0) {
            ...
            auto watchonly_wallet{CWallet::CreateNew(empty_context, wallet_name, std::move(database.value()), options.create_flags) >> result};
            if (!watchonly_wallet) {
        ...
        if (data->solvable_descs.size() > 0) {
            ...
            auto solvable_wallet{CWallet::CreateNew(empty_context, wallet_name, std::move(database.value()), options.create_flags) >> result};
            if (!solvable_wallet) {
                result.update(util::Error{_("Error: Failed to create new watchonly wallet")});
                return result;
            }
...
util::Result<MigrationResult> MigrateLegacyToDescriptor(std::shared_ptr<CWallet> local_wallet, const SecureString& passphrase, WalletContext& context)
{
...

    {
        ...
        if (HasLegacyRecords(*local_wallet)) {
            if (DoMigration(*local_wallet, context, res) >> result) {
                result.update({});
            }

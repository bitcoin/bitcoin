...
std::shared_ptr<CWallet> CreateWallet(WalletContext& context, const std::string& name, std::optional<bool> load_on_start, DatabaseOptions& options, DatabaseStatus& status, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (is_encrypted && (wallet_creation_flags & WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
    ...
    std::shared_ptr<CWallet> wallet = CWallet::CreateNew(context, name, std::move(database), wallet_creation_flags, is_encrypted, error, warnings);
    if (!wallet) {
...
std::shared_ptr<CWallet> CWallet::CreateNew(WalletContext& context, const std::string& name, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags, bool is_encrypted, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    interfaces::Chain* chain = context.chain;
...
{
    LOCK(cs_wallet);
    // We only want to call Setup if we are not blank,
    // have private keys (not having private keys implies blank),
    // or need to get descriptors from an external signer.
    if (!IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER) &&
...
bool DoMigration(CWallet& wallet, WalletContext& context, bilingual_str& error, MigrationResult& res) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (data->watch_descs.size() > 0 || data->solvable_descs.size() > 0) {
        ...
        if (data->watch_descs.size() > 0) {
            ...
            data->watchonly_wallet = CWallet::CreateNew(empty_context, wallet_name, std::move(database), options.create_flags, /*is_encrypted=*/false, error, warnings);
            if (!data->watchonly_wallet) {
        ...
        if (data->solvable_descs.size() > 0) {
            ...
            data->solvable_wallet = CWallet::CreateNew(empty_context, wallet_name, std::move(database), options.create_flags, /*is_encrypted=*/false, error, warnings);
            if (!data->solvable_wallet) {
                error = _("Error: Failed to create new watchonly wallet");
                return false;
            }

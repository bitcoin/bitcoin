...
std::vector<Txid> CWallet::GetMalleatedVariants(const CWalletTx& wtx)
{
    AssertLockHeld(cs_wallet);
    std::set<const CWalletTx*, WalletTxOrderComparator> txs;
    std::vector<Txid> out;

    // Coinbases cannot be malleated
    if (wtx.IsCoinBase()) return out;
    ...
    for (auto it = begin; it != end; ++it) {
        const CWalletTx* entry = GetWalletTx(it->second);
        if (!Assume(entry)) continue; // sanity-check: mapTxSpends has txs that are in mapWallet
        const bool is_self = entry == &wtx;
        found_self |= is_self;
        if (is_self || wtx.IsMalleation(*entry)) {
            Assume(txs.insert(entry).second);
        }
    }
    // wtx should always be found as this function is always called after AddToSpends
    Assert(found_self);
    out.reserve(txs.size());
    for (const CWalletTx* tx : txs) {
        out.push_back(tx->GetHash());
    }
    return out;
}
...
void CWallet::SyncMalleatedTxMetadata(WalletBatch& batch, const CWalletTx& wtx)
{
    ...
    const Txid& from_txid = *txs.begin();
    const CWalletTx* copyFrom = GetWalletTx(from_txid);
    ...
    for (const Txid& txid : txs) {
        if (txid == from_txid) continue;
        const auto& dst_it = m_txs_by_txid.find(txid);
...
bool CWallet::MarkReplaced(const Txid& originalHash, const Txid& newHash)
{
    ...
    for (const Txid& variant_txid : GetMalleatedVariants(wtx)) {
        if (variant_txid == originalHash) continue;
        WalletTxs::node_type variant_nh = m_txs_by_txid.extract(variant_txid);
        CWalletTx& variant = variant_nh.value();
        variant.m_replaced_by_txid = newHash;
        m_txs_by_txid.insert(std::move(variant_nh));
...
std::optional<WalletTxs::iterator> CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& update_wtx, bool rescanning_old_block)
{
    ...
    if (fInsertedNew) {
        ...
        SyncMalleatedTxMetadata(batch, const_wtx);
    }
    ...
    for (const auto& wtx : m_txs_by_pos) {
        ...
        if (data.watchonly_wallet) {
            ...
            if (data.watchonly_wallet->IsMine(*wtx.GetTx()) || data.watchonly_wallet->IsFromMe(*wtx.GetTx())) {
                ...
                watchonly_batch->WriteFullTx(*data.watchonly_wallet->GetWalletTx(hash));

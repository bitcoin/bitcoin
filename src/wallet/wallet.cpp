...
util::ResultPtr<std::shared_ptr<CWallet>, DatabaseStatus> RestoreWallet(WalletContext& context, const fs::path& backup_file, const std::string& wallet_name, std::optional<bool> load_on_start, bool load_after_restore)
{
    util::ResultPtr<std::shared_ptr<CWallet>, DatabaseStatus> result;
...
util::Result<MigrationResult> MigrateLegacyToDescriptor(const std::string& wallet_name, const SecureString& passphrase, WalletContext& context)
{
    ...
    return MigrateLegacyToDescriptor(std::move(local_wallet.value()), passphrase, context, was_loaded);
}
...
util::Result<MigrationResult> MigrateLegacyToDescriptor(std::shared_ptr<CWallet> local_wallet, const SecureString& passphrase, WalletContext& context, bool was_loaded)
{
    ...
    if (!success) {
        ...
        auto ptr_wallet{RestoreWallet(context, temp_backup_location, wallet_name, /*load_on_start=*/std::nullopt, /*load_after_restore=*/was_loaded) >> result};
        if (!ptr_wallet) {
        ...
        bool wallet_reloaded = ptr_wallet.value() != nullptr;

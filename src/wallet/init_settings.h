...
using AddressTypeSetting = common::Setting<
    ...
    ::HelpFn<[](const auto& help) { return help.Format(FormatOutputType(DEFAULT_ADDRESS_TYPE)); }>
    ::Category<OptionsCategory::WALLET>;
...
using DbLogSizeSetting = common::Setting<
    ...
    ::HelpFn<[](const auto& help) { return help.Format(DatabaseOptions().max_log_mb); }>
...
using PrivDbSetting = common::Setting<
    ...
    ::HelpFn<[](const auto& help) { return help.Format(!DatabaseOptions().use_shared_memory); }>
    ::Category<OptionsCategory::WALLET_DEBUG_TEST>;

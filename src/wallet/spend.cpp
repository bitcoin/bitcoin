                                            ...
                                            const CoinSelectionParams& coin_selection_params)
{
    ...
    // calculateIndividualBumpFees() either returns an entry per requested outpoint or gives up
    // entirely, so an empty result for a non-empty request means the calculation failed.
                                         ...
                                         const CoinFilterParams& params)
{
    ...
    if (feerate.has_value()) {
        ...
        // calculateIndividualBumpFees() either returns an entry per requested outpoint or gives
        // up entirely, so an empty result for a non-empty request means the calculation failed.
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (coin_control.m_allow_other_inputs) {
        ...
        available_coins = std::move(*res_available_coins);
    }

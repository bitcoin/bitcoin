...
TxSize CalculateMaximumSignedTxSize(const CTransaction &tx, const CWallet *wallet, const CCoinControl* coin_control)
{
    ...
    for (const CTxIn& input : tx.GetInputs()) {
        ...
        if (mi != wallet->mapWallet.end()) {
            assert(input.prevout.n < mi->second.GetTx()->GetOutputs().size());
            txouts.emplace_back(mi->second.GetTx()->GetOutputs().at(input.prevout.n));
...
const CTxOut& FindNonChangeParentOutput(const CWallet& wallet, const COutPoint& outpoint)
{
    ...
    while (OutputIsChange(wallet, ptx->GetOutputs()[n]) && ptx->GetInputs().size() > 0) {
        ...
        if (!it || it->GetTx()->GetOutputs().size() <= prevout.n ||
            !wallet.IsMine(it->GetTx()->GetOutputs()[prevout.n])) {
            break;
        }
        ptx = it->GetTx().get();
        n = prevout.n;

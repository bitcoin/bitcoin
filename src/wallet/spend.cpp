...
{
    CAmount total_deniabilization_fee = 0;
    CAmount current_total_value = total_value;
    unsigned int current_num_utxos = num_utxos;
    unsigned int current_num_deniabilization_txs = 1;
    unsigned int current_deniabilization_cycles = deniabilization_cycles;
    for (;;) {
        float deniabilization_probability = CalculateDeniabilizationProbability(current_deniabilization_cycles);
        // convert to integer percent to truncate and check for zero probability
        unsigned int deniabilization_probability_percent = deniabilization_probability * 100;
        if (deniabilization_probability_percent == 0) {
            break;
        }

        CAmount deniabilization_fee = CalculateDeniabilizationTxFee(shared_script, current_total_value, current_num_utxos, fee_rate) * current_num_deniabilization_txs;
        // if it's worthwhile to do further deniabilizations then add them to the estimate
        if (total_deniabilization_fee == 0 || IsDeniabilizationWorthwhile(current_total_value, total_deniabilization_fee + deniabilization_fee)) {
            total_deniabilization_fee += deniabilization_fee;
        } else {
            break;
        }

        // for the estimate we assume the amount will be split in half for the next deniabilization cycle
        current_total_value /= NUM_DENIABILIZATION_OUTPUTS;
        current_num_utxos = 1;
        current_num_deniabilization_txs = NUM_DENIABILIZATION_OUTPUTS;
        current_deniabilization_cycles++;
    }
    return total_deniabilization_fee;
}
...
std::pair<unsigned int, bool> CalculateDeniabilizationCycles(CWallet& wallet, const COutPoint& outpoint)
{
    ...
    std::pair<unsigned int, bool> result = std::make_pair(0, false);
    COutPoint current_outpoint = outpoint;

    for (;;) {
        auto walletTx = wallet.GetWalletTx(current_outpoint.hash);
        if (!walletTx) {
            break;
        }
        auto tx = walletTx->tx;
        ...
        Assert(current_outpoint.n < tx->vout.size());
        unsigned int n = 0;
        for (const auto& txOut : tx->vout) {
            if (wallet.IsMine(txOut) == ISMINE_NO) {
                Assert(n != current_outpoint.n);
                break;
            }
            n++;
        }
        ...
        if (tx->vin.size() > 1) {
            result.first += deniabilizationCycles;
            break;
        }

        const auto& txIn = tx->vin[0];
        // now calculate the deniabilization cycles of the input
        result.first += deniabilizationCycles;
        current_outpoint = txIn.prevout;
    }
    return result;
};
...
void SpoofTransactionFingerprint(CMutableTransaction& tx, FastRandomContext& rng_fast, const std::optional<bool>& signal_bip125_rbf)
{
    ...
    if (fingerprint.standardVersion) {
        // By default "Core" uses transactions with version 2
        Assert(tx.version == 2);
    } else {
        tx.version = 1;
    }
...
util::Result<CreatedTransactionResult> CreateDeniabilizationTransaction(CWallet& wallet, const std::set<COutPoint>& inputs, const std::optional<OutputType>& opt_output_type, unsigned int confirm_target, unsigned int deniabilization_cycles, bool sign, bool& insufficient_amount)
{
    ...
    for (int recipient_index = 0; recipient_index < num_recipients; recipient_index++) {
        ...
        if (!lastRecipient) {
            ...
            if (random_range > 0) {
                random_amount = FastRandomContext().randrange(random_range);

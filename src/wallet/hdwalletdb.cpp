...
{
    return Read(std::make_pair(std::string("aki"), ki), op, nFlags);
};

bool CHDWalletDB::WriteAnonKeyImage(const CCmpPubKey &ki, const COutPoint &op)
{
    return Write(std::make_pair(std::string("aki"), ki), op, true);
};

bool CHDWalletDB::EraseAnonKeyImage(const CCmpPubKey &ki)
{
    return Erase(std::make_pair(std::string("aki"), ki));
};
...
{
    char c = 't';
    return Write(std::make_pair(std::string("lao"), op), c, true);
};

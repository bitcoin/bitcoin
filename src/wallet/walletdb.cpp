...
static DBErrors LoadLegacyWalletRecords(CWallet* pwallet, WalletBatch& batch, int last_client) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    if (pwallet->IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
        if (HasLegacyRecords(*pwallet, *batch.m_batch)) {
        ...
        if (keyMeta.nVersion >= CKeyMetadata::VERSION_WITH_HDDATA && !keyMeta.hd_seed_id.IsNull() && keyMeta.hdKeypath.size() > 0) {
            ...
            if (keyMeta.hdKeypath != "s" && keyMeta.hdKeypath != "m") {
                ...
                if (keyMeta.has_key_origin && path != keyMeta.key_origin.path) {
                    ...
                    // The expected path pattern is m/0'/a'/i' and this string is written to hdKeypath, but the bad derivation path is m/0'/a'/i-1'/0'/a'/i'
                    // Thus we can check for bad derivation by looking for:
                    // - 6 indexes
                    // - path[5] == path[2] + 1
                    // - path[0:1] == path[3:4]
    ...
    if (!updated_metas.empty()) {
        ...
        for (const auto& pubkey : updated_metas) {
            const auto& it = spkm->mapKeyMetadata.find(pubkey.GetID());
            if (it != spkm->mapKeyMetadata.end() && !batch.WriteKeyMetadata(it->second, pubkey, true)) {

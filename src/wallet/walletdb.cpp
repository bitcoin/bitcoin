...
{
    if (!batch.TxnBegin()) {
        LogPrint(LogInstance(), BCLog::WALLETDB, "Error: cannot create db txn for %s\n", process_desc);
        return false;
    }

    // Run procedure
    if (!func(batch)) {
        LogPrint(LogInstance(), BCLog::WALLETDB, "Error: %s failed\n", process_desc);
        batch.TxnAbort();
        return false;
    }

    if (!batch.TxnCommit()) {
        LogPrint(LogInstance(), BCLog::WALLETDB, "Error: cannot commit db txn for %s\n", process_desc);
        return false;
    }

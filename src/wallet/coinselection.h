...
public:
          ...
          from_me{from_me}
    {
        if (feerate) {
            ...
            fee = input_bytes < 0 ? CAmount{0} : feerate.value().GetFee(input_bytes);
        ...
        : COutput(outpoint, txout, depth, input_bytes, solvable, safe, time, from_me)
    {
        ...
        assert((input_bytes < 0 && fees == CAmount{0}) || (input_bytes > 0 && fees >= CAmount{0}));
        fee = fees;
        effective_value = txout.nValue - fee.value();
    }
...
struct OutputGroup
{
    ...
    CFeeRate m_long_term_feerate{CAmount{0}};
    /** Indicate that we are subtracting the fee from outputs.

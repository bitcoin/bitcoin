...
MinimumFeeRateResult GetMinimumFeeRate(const CWallet& wallet, const CCoinControl& coin_control)
{
    ...
    */
    ...
    if (fee_rate == CFeeRate(CAmount{0})) {
        ...
        if (wallet.m_fallback_fee == CFeeRate(CAmount{0})) return {fee_rate, FeeReason::FALLBACK, std::nullopt};
    }
...
CFeeRate GetDiscardRate(const CWallet& wallet)
{
    ...
    auto discard_rate = res ? CFeeRate(res->feerate) : CFeeRate(CAmount{0});
    // Don't let discard_rate be greater than longest possible fee estimate if we get a valid fee estimate
    discard_rate = (discard_rate == CFeeRate(CAmount{0})) ? wallet.m_discard_rate : std::min(discard_rate, wallet.m_discard_rate);

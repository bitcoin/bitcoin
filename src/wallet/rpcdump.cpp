...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "importprivkey \"flowprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"flowprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
void ImportAddress(CWallet*, const CFlowAddress& address, const std::string& strLabel);
void ImportScript(CWallet* const pwallet, const CScript& script, const std::string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(pwallet, CFlowAddress(CScriptID(script)), strLabel);
    } else {
...
void ImportAddress(CWallet* const pwallet, const CFlowAddress& address, const std::string& strLabel)
{
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Flow address or script");
    }
...
UniValue importpubkey(const JSONRPCRequest& request)
{
    ...
    ImportAddress(pwallet, CFlowAddress(pubKey.GetID()), strLabel);
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The flow address for the private key\n"
            "\nResult:\n"
    ...
    return CFlowSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Flow %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CFlowAddress(keyid).ToString();
        CKey key;
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CFlowSecret(key).ToString(), strTime);
...
UniValue ProcessImport(CWallet * const pwallet, const UniValue& data, const int64_t timestamp)
{
    try {
        ...
        if (isP2SH) {
            ...
            CFlowAddress redeemAddress = CFlowAddress(CScriptID(redeemScript));
            ...
            if (pubKeys.size() && keys.size() == 0) {
                ...
                CFlowAddress pubKeyAddress = CFlowAddress(pubKey.GetID());
                ...
                // Consistency check.
                if (isScript) {
                    CFlowAddress scriptAddress;
                    CTxDestination destination;

                    if (ExtractDestination(script, destination)) {
                        scriptAddress = CFlowAddress(destination);

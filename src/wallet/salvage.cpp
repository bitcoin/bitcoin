...
util::Result<void> RecoverDatabaseFile(const ArgsManager& args, const fs::path& file_path)
{
    ...
    if (ret != 0)
    {
        result.Update(util::Error{Untranslated(strprintf("Failed to rename %s to %s", filename, newFilename))});
        return result;
    }
    ...
    if (ret != 0 && ret != DB_VERIFY_BAD) {
        result.Update(util::Error{Untranslated(strprintf("Salvage: Database salvage failed with result %d.", ret))});
        return result;
    }
    ...
    {
        result.Update(util::Error{Untranslated(strprintf("Salvage(aggressive) found no records in %s.", newFilename))});
        return result;
    }
    ...
    if (ret > 0) {
        pdbCopy->close(0);
        result.Update(util::Error{Untranslated(strprintf("Cannot create database file %s", filename))});
        return result;
    }
    ...
    for (KeyValPair& row : salvagedData)
    {
        ...
        if (!fReadOK)
        {
            result.AddWarning(Untranslated(strprintf("WARNING: WalletBatch::Recover skipping %s: %s", strType, strErr)));
            continue;
        }

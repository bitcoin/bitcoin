        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            if (pwallet->IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
                ...
                if (seed_bytes.has_value()) {
                    ...
                    bool ok = keyman.AddHDKey(batch, master_key, master_xpub);
                    CHECK_NONFATAL(ok);
                    keyman.SetActiveHDKey(master_xpub);
                } else {
                ...
                if (flush_key_pool) {
                    std::optional<CExtKey> active_hd_key;
                    {
                        LOCK(keyman.cs_keyman);
                        active_hd_key = keyman.GetActiveHDKey();
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    std::optional<CExtKey> active_hd_key;
    {
        ...
        // Make sure that keyman has an active hd key
        if (keyman.GetActiveHDPubKey() == std::nullopt) {
    ...
    for (bool internal : internals) {
        ...
        ids.push_back(pwallet->SetupDescriptorScriptPubKeyMan(*output_type, internal));
    }

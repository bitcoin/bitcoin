    ...
    return RPCHelpMan{
        ...
        HELP_REQUIRING_PASSPHRASE,
                ...
                RPCExamples{
                    ...
                    "\nSend 0.1 SNAIL with a confirmation target of 6 blocks in economical fee estimate mode using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"donation\" \"sean's outpost\" false true 6 economical") +
                    "\nSend 0.1 SNAIL with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB, subtract fee from amount, BIP125-replaceable, using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"drinks\" \"room77\" true true null \"unset\" null 1.1") +
                    "\nSend 0.2 SNAIL with a confirmation target of 6 blocks in economical fee estimate mode using named arguments\n"
                    + HelpExampleCli("-named sendtoaddress", "address=\"" + EXAMPLE_ADDRESS[0] + "\" amount=0.2 conf_target=6 estimate_mode=\"economical\"") +
                    "\nSend 0.5 SNAIL with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!pwallet->chain().rpcEnableDeprecated("settxfee")) {
        ...
        "\nTo use settxfee restart snailcoind with -deprecatedrpc=settxfee.");
    }
...
RPCHelpMan send()
{
    return RPCHelpMan{
        ...
        RPCExamples{""
        "\nSend 0.1 SNAIL with a confirmation target of 6 blocks in economical fee estimate mode\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.1}' 6 economical\n") +
        "Send 0.2 SNAIL with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB using positional arguments\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" 1.1\n") +
        "Send 0.2 SNAIL with a fee rate of 1 " + CURRENCY_ATOM + "/vB using the options argument\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" null '{\"fee_rate\": 1}'\n") +
        "Send 0.3 SNAIL with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::NUM, "changepos", "The position of the added change output, or -1"},
                                ...
                                RPCExamples{
                            "\nCreate a PSBT with automatically picked inputs that sends 0.5 SNAIL to an address and has a fee rate of 2 sat/vB:\n"

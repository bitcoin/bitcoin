...
WalletTx MakeWalletTx(CWallet& wallet, const CWalletTx& wtx)
{
    ...
    WalletTx result;
    result.tx = wtx.GetTx();
    result.txin_is_mine.reserve(result.tx->GetInputs().size());
    for (const auto& txin : result.tx->GetInputs()) {
        result.txin_is_mine.emplace_back(InputIsMine(wallet, txin));
    }
    result.txout_is_mine.reserve(result.tx->GetOutputs().size());
    result.txout_address.reserve(result.tx->GetOutputs().size());
    result.txout_address_is_mine.reserve(result.tx->GetOutputs().size());
    for (const auto& txout : result.tx->GetOutputs()) {
    ...
    EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    result.lock_time = wtx.GetTx()->GetLockTime();
...
{
    WalletTxOut result;
    result.txout = wtx.GetTx()->GetOutputs()[n];
    result.time = wtx.GetTxTime();

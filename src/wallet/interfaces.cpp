...
public:
    ...
    std::unique_ptr<Wallet> createWallet(const std::string& name, const SecureString& passphrase, uint64_t wallet_creation_flags, bilingual_str& error, std::vector<bilingual_str>& warnings) override
    {
        ...
        return MakeWallet(CreateWallet(*m_context.chain, name, true /* load_on_start */, gArgs, options, status, error, warnings));
    }
    std::unique_ptr<Wallet> loadWallet(const std::string& name, bilingual_str& error, std::vector<bilingual_str>& warnings) override
    {
        ...
        return MakeWallet(LoadWallet(*m_context.chain, name, true /* load_on_start */, gArgs, options, status, error, warnings));
    }

...
static void ErrorLogCallback(void* arg, int code, const char* msg)
{
    ...
    assert(arg == nullptr);
    LogPrintf(LogInstance(), "SQLite Error. Code: %d. Message: %s\n", code, msg);
}
...
static int TraceSqlCallback(unsigned code, void* context, void* param1, void* param2)
{
    ...
    if (code == SQLITE_TRACE_STMT) {
        ...
        LogPrintf(LogInstance(), "[%s] SQLite Statement: %s\n", db->Filename(), expanded ? expanded : sqlite3_sql(stmt));
                                ...
                                const std::string& description)
{
    ...
    if (res != SQLITE_OK) {
        LogPrintf(LogInstance(), "Unable to bind %s to statement: %s\n", description, sqlite3_errstr(res));
    ...
    : WalletDatabase(), m_mock(mock), m_dir_path(fs::PathToString(dir_path)), m_file_path(fs::PathToString(file_path)), m_write_semaphore(1), m_use_unsafe_sync(options.use_unsafe_sync)
{
    {
        LOCK(g_sqlite_mutex);
        LogPrintf(LogInstance(), "Using SQLite Version %s\n", SQLiteDatabaseVersion());
...
void SQLiteDatabase::Cleanup() noexcept
{
    ...
    if (--g_sqlite_count == 0) {
        ...
        if (ret != SQLITE_OK) {
            LogPrintf(LogInstance(), "SQLiteDatabase: Failed to shutdown SQLite: %s\n", sqlite3_errstr(ret));
        }
    }
}
...
void SQLiteDatabase::Open()
{
    ...
    if (m_db == nullptr) {
        ...
        if (LogAcceptCategory(LogInstance(), BCLog::WALLETDB, BCLog::Level::Trace)) {
           ret = sqlite3_trace_v2(m_db, SQLITE_TRACE_STMT, TraceSqlCallback, this);
           if (ret != SQLITE_OK) {
               LogPrintf(LogInstance(), "Failed to enable SQL tracing for %s\n", Filename());
           }
        }
    }
    ...
    if (m_use_unsafe_sync) {
        ...
        LogPrintf(LogInstance(), "WARNING SQLite is configured to not wait for data to be flushed to disk. Data loss and corruption may occur.\n");
...
bool SQLiteDatabase::Backup(const std::string& dest) const
{
    ...
    if (!backup) {
        LogPrintf(LogInstance(), "%s: Unable to begin backup: %s\n", __func__, sqlite3_errmsg(m_db));
        sqlite3_close(db_copy);
        return false;
    }
    ...
    if (res != SQLITE_DONE) {
        LogPrintf(LogInstance(), "%s: Unable to backup: %s\n", __func__, sqlite3_errstr(res));
...
void SQLiteBatch::Close()
{
    ...
    if (m_txn) {
        if (TxnAbort()) {
            LogPrintf(LogInstance(), "SQLiteBatch: Batch closed unexpectedly without the transaction being explicitly committed or aborted\n");
        } else {
            ...
            LogPrintf(LogInstance(), "SQLiteBatch: Batch closed and failed to abort transaction, resetting db connection..\n");
        }
    }
    ...
    for (const auto& [stmt_prepared, stmt_description] : statements) {
        ...
        if (res != SQLITE_OK) {
            LogPrintf(LogInstance(), "SQLiteBatch: Batch closed but could not finalize %s statement: %s\n",
...
bool SQLiteBatch::ReadKey(DataStream&& key, DataStream& value)
{
    ...
    if (res != SQLITE_ROW) {
        if (res != SQLITE_DONE) {
            ...
            LogPrintf(LogInstance(), "%s: Unable to execute statement: %s\n", __func__, sqlite3_errstr(res));
        }
...
DatabaseCursor::Status SQLiteCursor::Next(DataStream& key, DataStream& value)
{
    ...
    if (res != SQLITE_ROW) {
        LogPrintf(LogInstance(), "%s: Unable to execute cursor step: %s\n", __func__, sqlite3_errstr(res));
        return Status::FAIL;
    }
...
SQLiteCursor::~SQLiteCursor()
{
    ...
    if (res != SQLITE_OK) {
        LogPrintf(LogInstance(), "%s: cursor closed but could not finalize cursor statement: %s\n",
                  __func__, sqlite3_errstr(res));
    }
}
...
bool SQLiteBatch::TxnBegin()
{
    ...
    if (res != SQLITE_OK) {
        LogPrintf(LogInstance(), "SQLiteBatch: Failed to begin the transaction\n");
        m_database.m_write_semaphore.post();
    } else {
        m_txn = true;
    }
...
bool SQLiteBatch::TxnCommit()
{
    ...
    if (res != SQLITE_OK) {
        LogPrintf(LogInstance(), "SQLiteBatch: Failed to commit the transaction\n");
    } else {
        m_txn = false;
        m_database.m_write_semaphore.post();
    }
...
bool SQLiteBatch::TxnAbort()
{
    ...
    if (res != SQLITE_OK) {
        LogPrintf(LogInstance(), "SQLiteBatch: Failed to abort the transaction\n");
    } else {
        m_txn = false;
        m_database.m_write_semaphore.post();
    }

...
constexpr bool WAIT_FOR_DEBUGGER_ENABLE{true};
#else
constexpr bool WAIT_FOR_DEBUGGER_ENABLE{false};
#endif
...
{
    if constexpr (!WAIT_FOR_DEBUGGER_ENABLE) {
        return EXIT_SUCCESS;
    }
...
#else
        fputs("Platform doesn't implement WAIT_FOR_DEBUGGER/-waitfordebugger.\n", stderr);
        return EXIT_FAILURE;
#endif // platform
        if (attached) {
            return EXIT_SUCCESS;

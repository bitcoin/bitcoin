...
public:
    ...
    TxMempoolInfo info(const TransactionIdentifier auto& txid) const
    {
        LOCK(cs);
        auto i{GetIter(txid)};
    ...
    TxMempoolInfo info_for_relay(const TransactionIdentifier auto& txid, uint64_t last_sequence) const
    {
        LOCK(cs);
        auto i{GetIter(txid)};

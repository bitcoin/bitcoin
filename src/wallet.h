...
public:
    ...
    int64_t GetGameunitsXBalance() const;
    ...
    bool SendGameunitsToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendAnonToGameunits(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    int64_t GetGameunitsXDebit(const CTxIn& txin) const;
    int64_t GetGameunitsXCredit(const CTxOut& txout) const;
    ...
    int64_t GetDebit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txin.IsAnonInput())
            {
                nDebit += GetGameunitsXDebit(txin);
            } else
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetGameunitsXCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nGAMEUNITS, int64_t& nGameunitsX) const
    {
        nGAMEUNITS = 0;
        nGameunitsX = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nGameunitsX += GetGameunitsXCredit(txout);
            } else
                nGAMEUNITS += GetCredit(txout);
            if (!MoneyRange(nGAMEUNITS)
                || !MoneyRange(nGameunitsX))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
    };
...
public:
    ...
    mutable bool fAvailableGameunitsXCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableGameunitsXCreditCached;
    
    mutable int64_t nCredGAMEUNITSCached;
    mutable int64_t nCredGameunitsXCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableGameunitsXCreditCached = false;
        
        nCredGAMEUNITSCached = 0;
        nCredGameunitsXCached = 0;
        ...
        nAvailableGameunitsXCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredGAMEUNITS, int64_t& nCredGameunitsX, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredGAMEUNITS = nCredGameunitsX = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredGAMEUNITSCached = 0;
            nCredGameunitsXCached = 0;
            pwallet->GetCredit(*this, nCredGAMEUNITSCached, nCredGameunitsXCached);
            fCreditSplitCached = true;
        };
        
        nCredGAMEUNITS = nCredGAMEUNITSCached;
        nCredGameunitsX = nCredGameunitsXCached;
        return true;
    }
    ...
    int64_t GetAvailableGameunitsXCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableGameunitsXCreditCached)
            return nAvailableGameunitsXCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableGameunitsXCredit() : value out of range");
            };
        };
        
        nAvailableGameunitsXCreditCached = nCredit;
        fAvailableGameunitsXCreditCached = true;
        return nCredit;
    };

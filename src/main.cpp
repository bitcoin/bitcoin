...
void MarkBlockAsInFlight(NodeId nodeid, const uint256 &hash)
{
    ...
    QueuedBlock newentry = {
        hash, GetTimeMicros(), state->nBlocksInFlight
    };
...
void ProcessBlockAvailability(NodeId nodeid)
{
    ...
    if (state->hashLastUnknownBlock != 0)
    {
        ...
        {
            if (state->pindexBestKnownBlock == NULL || itOld->second->nChainWork >=
...
void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip ||
        (pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork +
            (chainActive.Tip()->GetBlockWork() * 6)))
    {
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("InvalidChainFound:  current best=%s  height=%d  log2_work=%.8g  date=%s\n",
        chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), log(chainActive.Tip()->nChainWork.getdouble(
                )) / log(2.0),
...
void static InvalidBlockFound(CBlockIndex* pindex, const CValidationState &state)
{
    ...
    if (state.IsInvalid(nDoS))
    {
        ...
        {
            CBlockReject reject = {
                state.GetRejectCode(), state.GetRejectReason(), pindex->GetBlockHash()
            };
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    LogPrint("bench", "      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs]\n",
        ...
        0.001 *
        (nTime1 - nTimeStart) / block.vtx.size(), nInputs <= 1 ? 0 : 0.001 * (nTime1 - nTimeStart) / (nInputs - 1),
    ...
    LogPrint("bench", "    - Verify %u txins: %.2fms (%.3fms/txin) [%.2fs]\n", nInputs - 1, 0.001 *
        (nTime2 - nTimeStart), nInputs <= 1 ? 0 : 0.001 * (nTime2 - nTimeStart) / (nInputs - 1), nTimeVerify * 0.000001);
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f\n",
        chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), log(chainActive.Tip()->nChainWork.getdouble(
                )) / log(2.0), (unsigned long)chainActive.Tip()->nChainTx,
        DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
...
bool ActivateBestChain(CValidationState &state, CBlock* pblock)
{
    ...
    do
    {
        ...
        bool fInitialDownload;
        {
            ...
            if (!ActivateBestChainStep(state, pindexMostWork, pblock && pblock->GetHash() ==
                    pindexMostWork->GetBlockHash() ? pblock : NULL))
            {
                return false;
            }
        ...
        CBlockIndex::IsSuperMajority(2, pindex->pprev, Params().EnforceBlockUpgradeMajority()))
    {
            ...
            !std::equal(expect.begin(), expect.end(), block.vtx[0].vin[0].scriptSig.begin()))
        {
            ...
            return state.DoS(100, error(
                    "AcceptBlock() : block height mismatch in coinbase"), REJECT_INVALID, "bad-cb-height");
        }
    }
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
...

    {
        ...
        {
            return state.Invalid(error("ProcessBlock() : already have block %d %s", mapBlockIndex[hash]->nHeight,
                    hash.ToString()), 0, "duplicate");
        }

        if (mapOrphanBlocks.count(hash))
        {
            return state.Invalid(error("ProcessBlock() : already have block (orphan) %s",
                    hash.ToString()), 0, "duplicate");
        }
...
bool CVerifyDB::VerifyDB(CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (nCheckLevel >= 3 && pindex == pindexState && (coins.GetCacheSize() + pcoinsTip->GetCacheSize()) <=
            nCoinCacheSize)
        {
            bool fClean = true;
    ...
    if (nCheckLevel >= 4)
    {
        ...
        while (pindex != chainActive.Tip())
        {
            ...
            uiInterface.ShowProgress(_("Verifying blocks..."),
                std::max(1,
                    std::min(99, 100 -
                        (int)(((double)(chainActive.Height() - pindex->nHeight)) / (double)nCheckDepth * 50))));
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "getblocks")
    {
        ...
        int nLimit = 500;
        LogPrint("net", "getblocks %d to %s limit %d from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop ==
            uint256(
                0) ? "end" : hashStop.ToString(), nLimit, pfrom->id);
...
std::string CBlockFileInfo::ToString() const
{
    return strprintf("CBlockFileInfo(blocks=%u, size=%u, heights=%u...%u, time=%s...%s)", nBlocks, nSize, nHeightFirst,
        nHeightLast, DateTimeStrFormat("%Y-%m-%d",
            nTimeFirst), DateTimeStrFormat("%Y-%m-%d", nTimeLast));
}

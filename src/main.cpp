...
struct CMainSignals
{
    // Notifies listeners of updated transaction data (transaction, and optionally the block it is
    // found in.
...
void RegisterValidationInterface(CValidationInterface* pwalletIn)
{
    g_signals.SyncTransaction.connect(
        boost::bind(&CValidationInterface::SyncTransaction, pwalletIn, _1, _2));
    g_signals.EraseTransaction.connect(
        boost::bind(&CValidationInterface::EraseFromWallet, pwalletIn, _1));
...
void UpdatePreferredDownload(CNode* node, CNodeState* state)
{
    ...
    state->fPreferredDownload =
        (!node->fInbound || node->fWhitelisted) && !node->fOneShot && !node->fClient;
...
void MarkBlockAsInFlight(NodeId nodeid, const uint256& hash, CBlockIndex* pindex = NULL)
{
    ...
    list<QueuedBlock>::iterator it =
        state->vBlocksInFlight.insert(state->vBlocksInFlight.end(), newentry);
    state->nBlocksInFlight++;
                              ...
                              NodeId& nodeStaller)
{
    ...
    // linked block we have in common with this peer. The +1 is so we can detect stalling, namely if
    // we would be able to
    ...
    while (pindexWalk->nHeight < nMaxHeight) {
        // Read up to 128 (or more, if more blocks than that are needed) successors of pindexWalk
        // (towards
        // pindexBestKnownBlock) into vToFetch. We fetch 128, because CBlockIndex::GetAncestor may
        // be as expensive
        // as iterating over ~100 CBlockIndex* entries anyway.
        int nToFetch =
            std::min(nMaxHeight - pindexWalk->nHeight, std::max<int>(count - vBlocks.size(), 128));
            ...
            if (pindex->nStatus & BLOCK_HAVE_DATA) {
                ...
                if (pindex->nHeight > nWindowEnd) {
                    ...
                    if (vBlocks.size() == 0 && waitingfor != nodeid) {
                        // We aren't able to fetch anything, but we would be if the download window
                        // was one larger.
...
void EraseOrphansFor(NodeId peer)
{
    ...
    while (iter != mapOrphanTransactions.end()) {
        map<uint256, COrphanTx>::iterator maybeErase =
...
bool CheckTransaction(const CTransaction& tx, CValidationState& state)
{
    ...
    if (tx.vin.empty())
        return state.DoS(10,
                         error("CheckTransaction() : vin empty"),
                         REJECT_INVALID,
                         "bad-txns-vin-empty");
    if (tx.vout.empty())
        return state.DoS(10,
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) > MAX_BLOCK_SIZE)
        return state.DoS(100,
                         error("CheckTransaction() : size limits failed"),
                         REJECT_INVALID,
                         "bad-txns-oversize");
                        ...
                        bool fRejectInsaneFee)
{
...

    {
        ...
        CAmount nValueIn = 0;
        {
            ...
            // do all inputs exist?
            // Note that this does not check for the presence of actual outputs (see the next check
            // for that),
            ...
            // we have all inputs cached now, so switch back to dummy, so we don't need to keep lock
            // on mempool
...
bool GetTransaction(const uint256& hash, CTransaction& txOut, uint256& hashBlock, bool fAllowSlow)
{
    CBlockIndex* pindexSlow = NULL;
    {
        ...
        if (fAllowSlow) { // use coin database to locate block that contains transaction, and scan
                          // it
            int nHeight = -1;
            {
    ...
    if (pindexBestForkTip ||
        (pindexBestInvalid &&
         pindexBestInvalid->nChainWork >
             chainActive.Tip()->nChainWork + (chainActive.Tip()->GetBlockWork() * 6))) {
        ...
        if (pindexBestForkTip) {
            ...
            LogPrintf("CheckForkWarningConditions: Warning: Found invalid chain at least ~6 blocks "
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    // or a chain that is entirely longer than ours and invalid (note that this should be detected
    // by both)
    // We define it this way because it allows us to only store the highest fork tip (+ base) which
    // meets
...
bool CScriptCheck::operator()() const
{
    ...
    if (!VerifyScript(
            scriptSig, scriptPubKey, nFlags, CachingSignatureChecker(*ptxTo, nIn, cacheStore)))
        return error("CScriptCheck() : %s:%d VerifySignature failed",
                     ...
                     bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        // specially with outsEmpty.
        {
            ...
            // No network rules currently depend on the version here, so an inconsistency is
            // harmless
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (undo.nHeight != 0) {
                    // undo data contains height: this is the last output of the prevout tx being
                    // spent
                    if (!coins->IsPruned())
                        fClean =
                            fClean &&
                    ...
                    if (coins->IsPruned())
                        fClean = fClean && error("DisconnectBlock() : undo data adding output to "
                                                 "missing transaction");
                }
                if (coins->IsAvailable(out.n))
                    fClean = fClean &&
                             error("DisconnectBlock() : undo data overwriting existing output");
                  ...
                  bool fJustCheck)
{
    ...
    // UTC.
    // Now that the whole chain is irreversibly beyond that time it is applied to all blocks except
    // the
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        UpdateCoins(
            tx, state, view, i == 0 ? undoDummy : blockundo.vtxundo.back(), pindex->nHeight);
...
bool static WriteChainState(CValidationState& state, bool forceWrite = false)
{
    ...
    if (forceWrite || pcoinsTip->GetCacheSize() > nCoinCacheSize ||
        (!IsInitialBlockDownload() && GetTimeMicros() > nLastWrite + 600 * 1000000)) {
...
bool static ConnectTip(CValidationState& state, CBlockIndex* pindexNew, CBlock* pblock)
{
    ...
    // Emit this signal after the SyncWithWallets signals as the wallet relies on that everything up
    // to this point has been synced
    if ((chainActive.Height() % 20160) == 0 ||
        ((chainActive.Height() % 144) == 0 && !IsInitialBlockDownload()))
...
static CBlockIndex* FindMostWorkChain()
{
    do {
        ...
        // Check whether all blocks on the path between the currently active chain and the candidate
        // are valid.
        // Just going until the active chain is an optimization, as we know all blocks in it are
        // valid already.
                                  ...
                                  CBlock* pblock)
{
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        // Don't iterate the entire list of potential improvements toward the best tip, as we likely
        // only need
        ...
        BOOST_REVERSE_FOREACH (CBlockIndex* pindexConnect, vpindexToConnect) {
            if (!ConnectTip(state,
                ...
                // Delete all entries in setBlockIndexCandidates that are worse than our new current
                // block.
                // Note that we can't delete the current block itself, as we may need to return to
                // it later in case a
                ...
                // Either the current tip or a successor of it we're working towards is left in
                // setBlockIndexCandidates.
                assert(!setBlockIndexCandidates.empty());
                if (!pindexOldTip || chainActive.Tip()->nChainWork > pindexOldTip->nChainWork) {
                    // We're in a better position than we were. Return temporarily to release the
                    // lock.
                    fContinue = false;
                    break;
                }
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    pindexNew->nChainWork =
        (pindexNew->pprev ? pindexNew->pprev->nChainWork : 0) + pindexNew->GetBlockWork();
                  ...
                  bool fKnown = false)
{
    ...
    if (!fKnown) {
        ...
        unsigned int nNewChunks =
            (vinfoBlockFile[nFile].nSize + BLOCKFILE_CHUNK_SIZE - 1) / BLOCKFILE_CHUNK_SIZE;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (fCheckMerkleRoot) {
        ...
        if (mutated)
            return state.DoS(100,
                             error("CheckBlock() : duplicate transaction"),
                             REJECT_INVALID,
                             "bad-txns-duplicate",
                             true);
    }
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        if ((!Params().SkipProofOfWorkCheck()) &&
            ...
            return state.DoS(100,
                             error("%s : incorrect proof of work", __func__),
                             REJECT_INVALID,
                             "bad-diffbits");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    try {
        ...
        if (dbp != NULL)
            blockPos = *dbp;
        if (!FindBlockPos(
                state, blockPos, nBlockSize + 8, nHeight, block.GetBlockTime(), dbp != NULL))
...
uint256 CPartialMerkleTree::ExtractMatches(std::vector<uint256>& vMatch)
{
    ...
    if (nTransactions >
        MAX_BLOCK_SIZE / 60) // 60 is the lower bound for the size of a serialized CTransaction
        return 0;
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH (const PAIRTYPE(int, CBlockIndex*)&item, vSortedByHeight) {
        CBlockIndex* pindex = item.second;
        pindex->nChainWork =
            (pindex->pprev ? pindex->pprev->nChainWork : 0) + pindex->GetBlockWork();
        if (pindex->nStatus & BLOCK_HAVE_DATA) {
            if (pindex->pprev) {
...
bool CVerifyDB::VerifyDB(CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (pindexFailure)
        return error("VerifyDB() : *** coin database inconsistencies found (last %i blocks, %i "
                     "good transactions before that)\n",
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            uiInterface.ShowProgress(
                ...
                std::max(1,
                         std::min(99,
                                  100 - (int)(((double)(chainActive.Height() - pindex->nHeight)) /
...
bool InitBlockIndex()
{
    ...
    if (!fReindex) {
        try {
            ...
            // Force a chainstate write so that when we VerifyDB in a moment, it doesnt check stale
            // data
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos* dbp)
{
    ...
    try {
        ...
        CBufferedFile blkdat(
            fileIn, 2 * MAX_BLOCK_SIZE, MAX_BLOCK_SIZE + 8, SER_DISK, CLIENT_VERSION);
...
string GetWarnings(string strFor)
{
    ...
    if (!CLIENT_VERSION_IS_RELEASE)
        strStatusBar = _("This is a pre-release test build - use at your own risk - do not use for "
    ...
    if (fLargeWorkForkFound) {
        nPriority = 2000;
        strStatusBar = strRPC = _("Warning: The network does not appear to fully agree! Some "
                                  "miners appear to be experiencing issues.");
    } else if (fLargeWorkInvalidChainFound) {
        nPriority = 2000;
        strStatusBar = strRPC = _("Warning: We do not appear to fully agree with our peers! You "
                ...
                if (mi != mapBlockIndex.end()) {
                    ...
                    if (pcheckpoint && nHeight < pcheckpoint->nHeight) {
                        if (!chainActive.Contains(mi->second)) {
                            LogPrintf("ProcessGetData(): ignoring request for old block that isn't "
                                      "in the main chain\n");
                        } else {
                            send = true;
                        }
                ...
                if (send) {
                    ...
                    else // MSG_FILTERED_BLOCK)
                    {
                        ...
                        if (pfrom->pfilter) {
                            ...
                            // the block the client did not see
                            // This avoids hurting performance by pointlessly requiring a round-trip
                            // Note that there is currently no way for a node to request any single
                            // transactions we didnt send here -
                           ...
                           int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex &&
                    ...
                    // First request the headers preceeding the announced block. In the normal
                    // fully-synced
                    // case where a new block is announced that succeeds the current tip (no
                    // reorganization),
                    // there are no such headers.
                    // Secondly, and only when we are close to being synced, we request the
                    // announced block directly,
                    // to avoid an extra round-trip. Note that we must *first* ask for the headers,
                    // so by the
                    // time the block arrives, the header chain leading up to it is already
                    // validated. Not
                    // doing this will result in the received block being rejected as an orphan in
                    // case it is
                    ...
                    if (chainActive.Tip()->GetBlockTime() >
                        ...
                        // Mark block as in flight already, even though the actual "getdata" message
                        // only goes out
        ...
        LogPrint("net",
                 ...
                 hashStop == uint256(0) ? "end" : hashStop.ToString(),
                 nLimit,
                 pfrom->id);
        ...
        if (AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs)) {
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++) {
                ...
                for (set<uint256>::iterator mi = itByPrev->second.begin();
                    ...
                    // Use a dummy CValidationState so someone can't setup nodes to counter-DoS
                    // based on orphan
                    // resolution (that is, feeding people an invalid transaction based on LegitTxX
                    // in order to get
        ...
        if (state.IsInvalid(nDoS)) {
            LogPrint("mempool",
                     ...
                     state.GetRejectReason());
            pfrom->PushMessage(
                "reject", strCommand, state.GetRejectCode(), state.GetRejectReason(), inv.hash);
            if (nDoS > 0)
    ...
    else if (strCommand == "headers" && !fImporting &&
             !fReindex) // Ignore headers received while importing
    {
        std::vector<CBlockHeader> headers;

        // Bypass the normal CBlock deserialization, as we don't want to risk deserializing 2000
        // full blocks.
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            // TODO: optimize: if pindexLast is an ancestor of chainActive.Tip or pindexBestHeader,
            // continue
            // from there instead.
            LogPrint("net",
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (nChecksum != hdr.nChecksum) {
            LogPrintf("ProcessMessages(%s, %u bytes) : CHECKSUM ERROR nChecksum=%08x "
                      "hdr.nChecksum=%08x\n",
                      strCommand,
                      nMessageSize,
                      nChecksum,
        ...
        try {
            ...
            if (strstr(e.what(), "end of data")) {
                ...
                LogPrintf("ProcessMessages(%s, %u bytes) : Exception '%s' caught, normally caused "
...
{
    {
        ...
        TRY_LOCK(cs_main,
                 lockMain); // Acquire cs_main for IsInitialBlockDownload() and CNodeState()
        if (!lockMain)
            return true;
        ...
        bool fFetch = state.fPreferredDownload || (nPreferredDownload == 0 && !pto->fClient &&
                                                   !pto->fOneShot); // Download if this is a nice
                                                                    // peer, or we have no nice
                                                                    // peers and this one might do.
        ...
        if (!pto->fDisconnect && state.nStallingSince &&
            ...
            // Stalling only triggers when the block download window cannot move. During normal
            // steady state,
            // the download window should be much larger than the to-be-downloaded set of blocks, so
            // disconnection

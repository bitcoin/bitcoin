                        ...
                        bool* pfMissingInputs, bool fRejectAbsurdFee)
{
...

    {
        ...
        size_t nConflictingSize = 0;
        {
            ...
            {
                bool fFssRbf = GetBoolArg("-fssrbf", false);

                if (fFssRbf)
                {
                    // Allow transactions to opt-out of FSS-RBF (if set) by
                    // setting nSequence < maxint-1 on one of the inputs.
                    //
                    // This allows those who want the advantages of full-RBF to
                    // still benefit, while allowing recipients to easily
                    // detect replacable transactions.
                    //
                    // maxint-1 is picked to still allow use of nLockTime by
                    // FSS-RBF transactions. Triggering this behavior on just
                    // one input rather than all is chosen for the sake of
                    // multiple-party transactions in the spirit of the
                    // original purpose of nSequence - read it as "one of the
                    // parties hasn't yet decided their tx is final"
                    BOOST_FOREACH(const CTxIn txin, tx.vin)
                    {
                ...
                if (fFssRbf)
                {
                    ...
                    for (unsigned int j = 0; j < txConflicting.vout.size(); j++)
                    {
                        ...
                        // Outputs of the conflicting tx must be unspent.
                        COutPoint outpoint(hashConflicting, j);

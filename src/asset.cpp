			...
			if (!theAssetAllocation.listSendingAllocationAmounts.empty()) {
				...
				for (auto& amountTuple : theAssetAllocation.listSendingAllocationAmounts) {
					...
					if (!bSanityCheck) {
						...
						if (!passetallocationdb->WriteAssetAllocation(receiverAllocation, amountTuple.second, dbAsset, INT64_MAX, receiverAllocation.vchAlias, fJustCheck, nHeight))
						{
				...
				for (unsigned int i = 0; i < theAssetAllocation.listSendingAllocationInputs.size(); i++) {
					...
					if (!bSanityCheck) {
						...
						if (!passetallocationdb->WriteAssetAllocation(receiverAllocation, rangeTotals[i], dbAsset, INT64_MAX, receiverAllocation.vchAlias, fJustCheck, nHeight))
						{

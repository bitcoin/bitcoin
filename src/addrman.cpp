...
void AddrManImpl::Unserialize(Stream& s_)
{
    ...
    if (!restore_bucketing) {
        LogPrint(LogInstance(), BCLog::ADDRMAN, "Bucketing method was updated, re-bucketing addrman entries from disk\n");
    }
    ...
    if (nLost + nLostUnk > 0) {
        LogPrint(LogInstance(), BCLog::ADDRMAN, "addrman lost %i new and %i tried addresses due to collisions or invalid addresses\n", nLostUnk, nLost);
    }
...
void AddrManImpl::ClearNew(int nUBucket, int nUBucketPos)
{
    ...
    if (vvNew[nUBucket][nUBucketPos] != -1) {
        ...
        LogPrint(LogInstance(), BCLog::ADDRMAN, "Removed %s from new[%i][%i]\n", infoDelete.ToStringAddrPort(), nUBucket, nUBucketPos);
...
void AddrManImpl::MakeTried(AddrInfo& info, int nId)
{
    ...
    if (vvTried[nKBucket][nKBucketPos] != -1) {
        ...
        LogPrint(LogInstance(), BCLog::ADDRMAN, "Moved %s from tried[%i][%i] to new[%i][%i] to make space\n",
...
bool AddrManImpl::AddSingle(const CAddress& addr, const CNetAddr& source, std::chrono::seconds time_penalty)
{
    ...
    if (vvNew[nUBucket][nUBucketPos] != nId) {
        ...
        if (fInsert) {
            ...
            LogPrint(LogInstance(), BCLog::ADDRMAN, "Added %s%s to new[%i][%i]\n",
...
bool AddrManImpl::Good_(const CService& addr, bool test_before_evict, NodeSeconds time)
{
    ...
    if (test_before_evict && (vvTried[tried_bucket][tried_bucket_pos] != -1)) {
        ...
        LogPrint(LogInstance(), BCLog::ADDRMAN, "Collision with %s while attempting to move %s to tried table. Collisions=%d\n",
        ...
        MakeTried(info, nId);
        const auto mapped_as{m_netgroupman.GetMappedAS(addr)};
        LogPrint(LogInstance(), BCLog::ADDRMAN, "Moved %s%s to tried[%i][%i]\n",
...
bool AddrManImpl::Add_(const std::vector<CAddress>& vAddr, const CNetAddr& source, std::chrono::seconds time_penalty)
{
    ...
    if (added > 0) {
        LogPrint(LogInstance(), BCLog::ADDRMAN, "Added %i addresses (of %i) from %s: %i tried, %i new\n", added, vAddr.size(), source.ToStringAddr(), nTried, nNew);
    }
    return added > 0;
}
...
std::pair<CAddress, NodeSeconds> AddrManImpl::Select_(bool new_only, std::optional<Network> network) const
{
    ...
    while (1) {
        ...
        if (insecure_rand.randbits(30) < chance_factor * info.GetChance() * (1 << 30)) {
            LogPrint(LogInstance(), BCLog::ADDRMAN, "Selected %s from %s\n", info.ToStringAddrPort(), search_tried ? "tried" : "new");
...
std::vector<CAddress> AddrManImpl::GetAddr_(size_t max_addresses, size_t max_pct, std::optional<Network> network, const bool filtered) const
{
    ...
    LogPrint(LogInstance(), BCLog::ADDRMAN, "GetAddr returned %d random addresses\n", addresses.size());
    return addresses;
}
            ...
            if (!info_new.IsValid()) { // id_new may no longer map to a valid address
                ...
                if (current_time - info_old.m_last_success < ADDRMAN_REPLACEMENT) {
                    ...
                    if (current_time - info_old.m_last_try > 60s) {
                        LogPrint(LogInstance(), BCLog::ADDRMAN, "Replacing %s with %s in tried table\n", info_old.ToStringAddrPort(), info_new.ToStringAddrPort());
                    ...
                    LogPrint(LogInstance(), BCLog::ADDRMAN, "Unable to test; replacing %s with %s in tried table anyway\n", info_old.ToStringAddrPort(), info_new.ToStringAddrPort());
                    Good_(info_new, false, current_time);
                    erase_collision = true;
                }
...
void AddrManImpl::Check() const
{
    ...
    if (err) {
        LogPrintf(LogInstance(), "ADDRMAN CONSISTENCY CHECK FAILED!!! err=%i\n", err);
        assert(false);
    }
}

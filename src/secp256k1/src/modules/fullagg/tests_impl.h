...
/* Checks that the tagged hashes initialized with fixed midstates have the
 * expected state (i.e. the hardcoded midstates match the BIP0459 tags). */
static void fullagg_sha256_tag_test(void) {
    secp256k1_sha256 sha;
    const secp256k1_hash_ctx *hash_ctx = &CTX->hash_ctx;
    {
        /* "BIP0459/aux" */
        static const unsigned char tag[] = { 'B', 'I', 'P', '0', '4', '5', '9', '/', 'a', 'u', 'x' };
        secp256k1_fullagg_sha256_tagged_aux(&sha);
        test_sha256_tag_midstate(hash_ctx, &sha, tag, sizeof(tag));
    }
    {
        /* "BIP0459/nonce" */
        static const unsigned char tag[] = { 'B', 'I', 'P', '0', '4', '5', '9', '/', 'n', 'o', 'n', 'c', 'e' };
        secp256k1_fullagg_sha256_tagged_nonce(&sha);
        test_sha256_tag_midstate(hash_ctx, &sha, tag, sizeof(tag));
    }
    {
        /* "BIP0459/noncecoef" */
        static const unsigned char tag[] = { 'B', 'I', 'P', '0', '4', '5', '9', '/', 'n', 'o', 'n', 'c', 'e', 'c', 'o', 'e', 'f' };
        secp256k1_fullagg_sha256_tagged_noncecoef(&sha);
        test_sha256_tag_midstate(hash_ctx, &sha, tag, sizeof(tag));
    }
    {
        /* "BIP0459/sig" */
        static const unsigned char tag[] = { 'B', 'I', 'P', '0', '4', '5', '9', '/', 's', 'i', 'g' };

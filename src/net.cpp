...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        char intClient[16];
        char intPort[6];
        string strDesc = "cosbycoin " + FormatFullVersion();
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  cosbycoin is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    Generatecosbycoins(fGeneratecosbycoins, pwalletMain);
}
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[3] > 0) printf("ThreadcosbycoinMiner still running\n");

...
std::vector<CAddress> CConnman::GetAddressesUncached(size_t max_addresses, size_t max_pct, std::optional<Network> network, const bool filtered) const
{
...
std::vector<CAddress> CConnman::GetAddresses(CNode& requestor, size_t max_addresses, size_t max_pct)
{
    ...
    if (cache_entry.m_cache_entry_expiration < current_time) { // If emplace() added new one it has expiration 0.
        cache_entry.m_addrs_response_cache = GetAddressesUncached(max_addresses, max_pct, /*network=*/std::nullopt);
...
{
    const std::vector<CAddress> v4_addrs{GetAddressesUncached(/*max_addresses=*/ 0, /*max_pct=*/ 0, Network::NET_IPV4, /*filtered=*/ false)};

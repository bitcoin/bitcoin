...
static std::vector<CAddress> ConvertSeeds(const std::vector<uint8_t> &vSeedsIn)
{
    ...
    while (!s.eof()) {
        ...
        LogPrint(LogInstance(), BCLog::NET, "Added hardcoded seed: %s\n", addr.ToStringAddrPort());
...
std::optional<CService> GetLocalAddrForPeer(CNode& node)
{
    ...
    if (addrLocal.IsRoutable()) {
        LogPrint(LogInstance(), BCLog::NET, "Advertising address %s to peer=%d\n", addrLocal.ToStringAddrPort(), node.GetId());
        return addrLocal;
    }
...
bool AddLocal(const CService& addr_, int nScore)
{
    ...
    LogPrintf(LogInstance(), "AddLocal(%s,%i)\n", addr.ToStringAddrPort(), nScore);
...
{
    LOCK(g_maplocalhost_mutex);
    LogPrintf(LogInstance(), "RemoveLocal(%s)\n", addr.ToStringAddrPort());
...
static CAddress GetBindAddress(const Sock& sock)
{
    ...
    if (!sock.GetSockName((struct sockaddr*)&sockaddr_bind, &sockaddr_bind_len)) {
        ...
        LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Warning, "getsockname failed\n");
    }
    return addr_bind;
}
...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, ConnectionType conn_type, bool use_v2transport)
{
    ...
    if (pszDest == nullptr) {
        ...
        if (pnode)
        {
            LogPrintf(LogInstance(), "Failed to open new connection, already connected\n");
            return nullptr;
        }
    }

    LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Debug, "trying %s connection %s lastseen=%.1fhrs\n",
        ...
        if (!resolved.empty()) {
            ...
            for (const auto& r : resolved) {
                ...
                if (!addrConnect.IsValid()) {
                    LogPrint(LogInstance(), BCLog::NET, "Resolver returned invalid address %s for %s\n", addrConnect.ToStringAddrPort(), pszDest);
                    return nullptr;
                }
                ...
                CNode* pnode = FindNode(static_cast<CService>(addrConnect));
                if (pnode) {
                    LogPrintf(LogInstance(), "Not opening a connection to %s, already connected to %s\n", pszDest, addrConnect.ToStringAddrPort());
                    return nullptr;
                }
    ...
    for (auto& target_addr: connect_to) {
        if (target_addr.IsValid()) {
            ...
            if (target_addr.IsI2P() && use_proxy) {
                ...
                LogPrintLevel(LogInstance(), BCLog::PROXY, BCLog::Level::Debug, "Using proxy: %s to connect to %s\n", proxy.ToString(), target_addr.ToStringAddrPort());
...
void CNode::CloseSocketDisconnect()
{
    ...
    LOCK(m_sock_mutex);
    if (m_sock) {
        LogPrint(LogInstance(), BCLog::NET, "disconnecting peer=%d\n", id);
        m_sock.reset();
    }
...
void CNode::SetAddrLocal(const CService& addrLocalIn) {
    ...
    if (addrLocal.IsValid()) {
        LogError(LogInstance(), "Addr local already set for node: %i. Refusing to change from %s to %s\n", id, addrLocal.ToStringAddrPort(), addrLocalIn.ToStringAddrPort());
    } else {
...
int V1Transport::readHeader(Span<const uint8_t> msg_bytes)
{
    ...
    try {
        ...
        LogPrint(LogInstance(), BCLog::NET, "Header error: Unable to deserialize, peer=%d\n", m_node_id);
        return -1;
    }

    // Check start string, network magic
    if (hdr.pchMessageStart != m_magic_bytes) {
        LogPrint(LogInstance(), BCLog::NET, "Header error: Wrong MessageStart %s received, peer=%d\n", HexStr(hdr.pchMessageStart), m_node_id);
        return -1;
    }

    // reject messages larger than MAX_SIZE or MAX_PROTOCOL_MESSAGE_LENGTH
    if (hdr.nMessageSize > MAX_SIZE || hdr.nMessageSize > MAX_PROTOCOL_MESSAGE_LENGTH) {
        LogPrint(LogInstance(), BCLog::NET, "Header error: Size too large (%s, %u bytes), peer=%d\n", SanitizeString(hdr.GetCommand()), hdr.nMessageSize, m_node_id);
        return -1;
    }
...
CNetMessage V1Transport::GetReceivedMessage(const std::chrono::microseconds time, bool& reject_message)
{
    ...
    if (memcmp(hash.begin(), hdr.pchChecksum, CMessageHeader::CHECKSUM_SIZE) != 0) {
        LogPrint(LogInstance(), BCLog::NET, "Header error: Wrong checksum (%s, %u bytes), expected %s was %s, peer=%d\n",
        ...
        LogPrint(LogInstance(), BCLog::NET, "Header error: Invalid message type (%s, %u bytes), peer=%d\n",
...
bool V2Transport::ProcessReceivedKeyBytes() noexcept
{
    ...
    if (!m_initiating && m_recv_buffer.size() >= OFFSET + MATCH.size()) {
        if (std::equal(MATCH.begin(), MATCH.end(), m_recv_buffer.begin() + OFFSET)) {
            LogPrint(LogInstance(), BCLog::NET, "V2 transport error: V1 peer with wrong MessageStart %s\n",
...
bool V2Transport::ProcessReceivedGarbageBytes() noexcept
{
    ...
    if (m_recv_buffer.size() >= BIP324Cipher::GARBAGE_TERMINATOR_LEN) {
        if (MakeByteSpan(m_recv_buffer).last(BIP324Cipher::GARBAGE_TERMINATOR_LEN) == m_cipher.GetReceiveGarbageTerminator()) {
            ...
            LogPrint(LogInstance(), BCLog::NET, "V2 transport error: missing garbage terminator, peer=%d\n", m_nodeid);
            return false;
        } else {
...
bool V2Transport::ProcessReceivedPacketBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == BIP324Cipher::LENGTH_LEN) {
        ...
        if (m_recv_len > MAX_CONTENTS_LEN) {
            LogPrint(LogInstance(), BCLog::NET, "V2 transport error: packet too large (%u bytes), peer=%d\n", m_recv_len, m_nodeid);
            return false;
        }
        ...
        if (!ret) {
            LogPrint(LogInstance(), BCLog::NET, "V2 transport error: packet decryption failure (%u bytes), peer=%d\n", m_recv_len, m_nodeid);
            return false;
        }
...
CNetMessage V2Transport::GetReceivedMessage(std::chrono::microseconds time, bool& reject_message) noexcept
{
    ...
    if (msg_type) {
        ...
        LogPrint(LogInstance(), BCLog::NET, "V2 transport error: invalid message type (%u bytes contents), peer=%d\n", m_recv_decode_buffer.size(), m_nodeid);
        reject_message = true;
    }
...
void V2Transport::MarkBytesSent(size_t bytes_sent) noexcept
{
    ...
    if (m_send_state == SendState::AWAITING_KEY && m_send_pos == 0 && bytes_sent > 0) {
        LogPrint(LogInstance(), BCLog::NET, "start sending v2 handshake to peer=%d\n", m_nodeid);
    }
        ...
        if (nBytes > 0) {
            ...
            if (nBytes < 0) {
                ...
                if (nErr != WSAEWOULDBLOCK && nErr != WSAEMSGSIZE && nErr != WSAEINTR && nErr != WSAEINPROGRESS) {
                    LogPrint(LogInstance(), BCLog::NET, "socket send error for peer=%d: %s\n", node.GetId(), NetworkErrorString(nErr));
...
bool CConnman::AttemptToEvictConnection()
{
    ...
    for (CNode* pnode : m_nodes) {
        if (pnode->GetId() == *node_id_to_evict) {
            LogPrint(LogInstance(), BCLog::NET, "selected %s connection for eviction peer=%d; disconnecting\n", pnode->ConnectionTypeAsString(), pnode->GetId());
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    if (!sock) {
        ...
        if (nErr != WSAEWOULDBLOCK) {
            LogPrintf(LogInstance(), "socket error accept failed: %s\n", NetworkErrorString(nErr));
        }
        return;
    }

    if (!addr.SetSockAddr((const struct sockaddr*)&sockaddr)) {
        LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Warning, "Unknown socket family\n");
    } else {
                                            ...
                                            const CAddress& addr)
{
    ...
    if (!fNetworkActive) {
        LogPrint(LogInstance(), BCLog::NET, "connection from %s dropped: not accepting new connections\n", addr.ToStringAddrPort());
        return;
    }

    if (!sock->IsSelectable()) {
        LogPrintf(LogInstance(), "connection from %s dropped: non-selectable socket\n", addr.ToStringAddrPort());
        return;
    }
    ...
    if (sock->SetSockOpt(IPPROTO_TCP, TCP_NODELAY, &on, sizeof(on)) == SOCKET_ERROR) {
        LogPrint(LogInstance(), BCLog::NET, "connection from %s: unable to set TCP_NODELAY, continuing anyway\n",
                 addr.ToStringAddrPort());
    }
    ...
    {
        LogPrint(LogInstance(), BCLog::NET, "connection from %s dropped (banned)\n", addr.ToStringAddrPort());
        return;
    }
    ...
    {
        LogPrint(LogInstance(), BCLog::NET, "connection from %s dropped (discouraged)\n", addr.ToStringAddrPort());
        return;
    }
    ...
    if (nInbound >= m_max_inbound)
    {
        if (!AttemptToEvictConnection()) {
            ...
            LogPrint(LogInstance(), BCLog::NET, "failed to find an eviction candidate - connection dropped (full)\n");
            return;
        }
    }
    ...
    LogPrint(LogInstance(), BCLog::NET, "connection from %s accepted\n", addr.ToStringAddrPort());
        ...
        if (!fNetworkActive) {
            ...
            for (CNode* pnode : m_nodes) {
                if (!pnode->fDisconnect) {
                    LogPrint(LogInstance(), BCLog::NET, "Network not active, dropping peer=%d\n", pnode->GetId());
                    pnode->fDisconnect = true;
                }
            }
        }
        ...
        for (CNode* pnode : nodes_copy)
        {
            if (pnode->fDisconnect)
            {
                ...
                if (pnode->m_transport->ShouldReconnectV1()) {
                    ...
                    LogPrint(LogInstance(), BCLog::NET, "retrying with v1 transport protocol for peer=%d\n", pnode->GetId());
                }
...
bool CConnman::InactivityCheck(const CNode& node) const
{
    ...
    if (last_recv.count() == 0 || last_send.count() == 0) {
        LogPrint(LogInstance(), BCLog::NET, "socket no message in first %i seconds, %d %d peer=%d\n", count_seconds(m_peer_connect_timeout), last_recv.count() != 0, last_send.count() != 0, node.GetId());
        return true;
    }

    if (now > last_send + TIMEOUT_INTERVAL) {
        LogPrint(LogInstance(), BCLog::NET, "socket sending timeout: %is peer=%d\n", count_seconds(now - last_send), node.GetId());
        return true;
    }

    if (now > last_recv + TIMEOUT_INTERVAL) {
        LogPrint(LogInstance(), BCLog::NET, "socket receive timeout: %is peer=%d\n", count_seconds(now - last_recv), node.GetId());
        return true;
    }

    if (!node.fSuccessfullyConnected) {
        LogPrint(LogInstance(), BCLog::NET, "version handshake timeout peer=%d\n", node.GetId());
        return true;
    }
    ...
    for (CNode* pnode : nodes) {
        ...
        if (recvSet || errorSet)
        {
            ...
            else if (nBytes == 0)
            {
                ...
                if (!pnode->fDisconnect) {
                    LogPrint(LogInstance(), BCLog::NET, "socket closed for peer=%d\n", pnode->GetId());
                }
                pnode->CloseSocketDisconnect();
            }
            else if (nBytes < 0)
            {
                ...
                {
                    if (!pnode->fDisconnect) {
                        LogPrint(LogInstance(), BCLog::NET, "socket recv error for peer=%d: %s\n", pnode->GetId(), NetworkErrorString(nErr));
                    }
                    pnode->CloseSocketDisconnect();
                }
            }
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    if (gArgs.IsArgSet("-seednode")) {
        ...
        LogPrintf(LogInstance(), "-seednode enabled. Trying the provided seeds for %d seconds before defaulting to the dnsseeds.\n", SEEDNODE_TIMEOUT.count());
        while (!interruptNet) {
            ...
            if (NodeClock::now() > start + SEEDNODE_TIMEOUT) {
                LogPrintf(LogInstance(), "Couldn't connect to enough peers via seed nodes. Handing fetch logic to the DNS seeds.\n");
                break;
            }

            outbound_connection_count = GetFullOutboundConnCount();
            if (outbound_connection_count >= TARGET_OUTBOUND_CONNECTIONS) {
                LogPrintf(LogInstance(), "P2P peers available. Finished fetching data from seed nodes.\n");
                break;
            }
        }
    }
    ...
    if (outbound_connection_count < TARGET_OUTBOUND_CONNECTIONS || seeds_right_now) {
        ...
        for (const std::string& seed : seeds) {
            if (seeds_right_now == 0) {
                ...
                if (addrman.Size() > 0) {
                    LogPrintf(LogInstance(), "Waiting %d seconds before querying DNS seeds.\n", seeds_wait_time.count());
                    ...
                    while (to_wait.count() > 0) {
                        ...
                        if (GetFullOutboundConnCount() >= TARGET_OUTBOUND_CONNECTIONS) {
                            if (found > 0) {
                                LogPrintf(LogInstance(), "%d addresses found from DNS seeds\n", found);
                                LogPrintf(LogInstance(), "P2P peers available. Finished DNS seeding.\n");
                            } else {
                                LogPrintf(LogInstance(), "P2P peers available. Skipped DNS seeding.\n");
                            }
                            return;
                        }
                    }
            ...
            if (!fNetworkActive) {
                LogPrintf(LogInstance(), "Waiting for network to be reactivated before querying DNS seeds.\n");
                do {
                    if (!interruptNet.sleep_for(std::chrono::seconds{1})) return;
                } while (!fNetworkActive);
            }

            LogPrintf(LogInstance(), "Loading addresses from DNS seed %s\n", seed);
        ...
        LogPrintf(LogInstance(), "Skipping DNS seeds. Enough peers have been found\n");
    }
}
...
void CConnman::DumpAddresses()
{
    ...
    LogPrint(LogInstance(), BCLog::NET, "Flushed %d addresses to peers.dat  %dms\n",
...
void CConnman::SetTryNewOutboundPeer(bool flag)
{
    ...
    LogPrint(LogInstance(), BCLog::NET, "setting try another outbound peer=%s\n", flag ? "true" : "false");
}

void CConnman::StartExtraBlockRelayPeers()
{
    LogPrint(LogInstance(), BCLog::NET, "enabling extra block-relay-only peers\n");
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect)
{
    ...
    if (!add_fixed_seeds) {
        LogPrintf(LogInstance(), "Fixed seeds are disabled\n");
    }
    ...
    while (!interruptNet)
    {
        ...
        if (add_fixed_seeds && !fixed_seed_networks.empty()) {
            ...
            if (GetTime<std::chrono::seconds>() > start + std::chrono::minutes{1}) {
                ...
                LogPrintf(LogInstance(), "Adding fixed seeds as 60 seconds have passed and addrman is empty for at least one reachable network\n");
            }
            ...
            // Perform cheap checks before locking a mutex.
                ...
                if (m_added_node_params.empty()) {
                    add_fixed_seeds_now = true;
                    LogPrintf(LogInstance(), "Adding fixed seeds as -dnsseed=0 (or IPv4/IPv6 connections are disabled via -onlynet) and neither -addnode nor -seednode are provided\n");
                }
            }
            ...
            if (add_fixed_seeds_now) {
                ...
                LogPrintf(LogInstance(), "Added %d fixed seeds from reachable networks.\n", seed_addrs.size());
            }
        }
        ...
        while (!interruptNet)
        {
            if (anchor && !m_anchors.empty()) {
                ...
                addrConnect = addr;
                LogPrint(LogInstance(), BCLog::NET, "Trying to make an anchor connection to %s\n", addrConnect.ToStringAddrPort());
                break;
            }
            ...
            if (AddedNodesContain(addr)) {
                LogPrintLevel(LogInstance(), BCLog::NET, BCLog::Level::Debug, "Not making automatic %s%s connection to %s peer selected for manual (addnode) connection%s\n",
        ...
        if (addrConnect.IsValid()) {
            if (fFeeler) {
                ...
                LogPrint(LogInstance(), BCLog::NET, "Making feeler connection to %s\n", addrConnect.ToStringAddrPort());
            }

            if (preferred_net != std::nullopt) LogPrint(LogInstance(), BCLog::NET, "Making network specific connection to %s on %s.\n", addrConnect.ToStringAddrPort(), GetNetworkName(preferred_net.value()));
...
bool CConnman::BindListenPort(const CService& addrBind, bilingual_str& strError, NetPermissionFlags permissions)
{
    ...
    LogPrintf(LogInstance(), "Bound to %s\n", addrBind.ToStringAddrPort());
    ...
    if (getifaddrs(&myaddrs) == 0)
    {
        for (struct ifaddrs* ifa = myaddrs; ifa != nullptr; ifa = ifa->ifa_next)
        {
            ...
            else if (ifa->ifa_addr->sa_family == AF_INET6)
            {
                ...
                if (AddLocal(addr, LOCAL_IF))
                    LogPrintf(LogInstance(), "%s: IPv6 %s: %s\n", __func__, ifa->ifa_name, addr.ToStringAddr());
            }
        }
        freeifaddrs(myaddrs);
    }
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    if (m_use_addrman_outgoing) {
        ...
        LogPrintf(LogInstance(), "%i block-relay-only anchors will be tried for connections.\n", m_anchors.size());
    }
    ...
    if (!gArgs.GetBoolArg("-dnsseed", DEFAULT_DNSSEED))
        LogPrintf(LogInstance(), "DNS seeding disabled\n");
    else
...
bool CConnman::DisconnectNode(const std::string& strNode)
{
    ...
    if (CNode* pnode = FindNode(strNode)) {
        LogPrint(LogInstance(), BCLog::NET, "disconnect by address%s matched peer=%d; disconnecting\n", (fLogIPs ? strprintf("=%s", strNode) : ""), pnode->GetId());
        pnode->fDisconnect = true;
        return true;
    }
    return false;
...
bool CConnman::DisconnectNode(const CSubNet& subnet)
{
    ...
    for (CNode* pnode : m_nodes) {
        if (subnet.Match(pnode->addr)) {
            LogPrint(LogInstance(), BCLog::NET, "disconnect by subnet%s matched peer=%d; disconnecting\n", (fLogIPs ? strprintf("=%s", subnet.ToString()) : ""), pnode->GetId());
            pnode->fDisconnect = true;
            disconnected = true;
        }
    }
...
bool CConnman::DisconnectNode(NodeId id)
{
    ...
    for(CNode* pnode : m_nodes) {
        if (id == pnode->GetId()) {
            LogPrint(LogInstance(), BCLog::NET, "disconnect by id peer=%d; disconnecting\n", pnode->GetId());
            pnode->fDisconnect = true;
            return true;
        }
    }
      ...
      m_i2p_sam_session{std::move(node_opts.i2p_sam_session)}
{
    ...
    if (fLogIPs) {
        LogPrint(LogInstance(), BCLog::NET, "Added connection to %s peer=%d\n", m_addr_name, id);
    } else {
        LogPrint(LogInstance(), BCLog::NET, "Added connection peer=%d\n", id);
    }
}
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    LogPrint(LogInstance(), BCLog::NET, "sending %s (%d bytes) peer=%d\n", msg.m_type, nMessageSize, pnode->GetId());

...
namespace benchmark {
//! Height of generated blocks above the selected chain's genesis block.
inline constexpr int GENERATED_BLOCK_HEIGHT{1};
...
struct ScriptRecipe {
    ...
    size_t tx_count{0}; //!< Target non-coinbase count. Zero picks a seeded count; overweight blocks are trimmed.
    double geometric_base_prob{0.5}; //!< Controls geometric distribution for vin/vout/witness sizes.
    bool include_witness{true}; //!< Generate witness stacks and a coinbase witness commitment.
};

//! mostly legacy outputs, useful for pre-SegWit baselines
inline constexpr ScriptRecipe LEGACY_RECIPE{
    .anchor_prob = 0.00,
    .multisig_prob = 0.05,
...
inline constexpr ScriptRecipe WITNESS_RECIPE{
    .anchor_prob = 0.001,
    .multisig_prob = 0.005,

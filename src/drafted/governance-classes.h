 ...
 *
 *   // network
 *   CSyscoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.syscoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CSyscoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CSyscoinNetwork(UniValue objIn)
    {
...
// // can be under: SyscoinNetwork
// //   -- signature requirements : Key1(User)
// class CSyscoinNetworkVariable : public CGovernanceObject
// {
// private:

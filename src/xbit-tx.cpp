...
static void SetupXBitTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " xbit-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  xbit-tx [options] <hex-tx> [commands]  Update hex-encoded xbit transaction\n" +
            "or:     xbit-tx [options] -create [commands]   Create hex-encoded xbit transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded xbit transaction

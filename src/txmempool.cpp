...
std::vector<CTxMemPoolEntry::CTxMemPoolEntryRef> CTxMemPool::GetChildren(const CTxMemPoolEntry& entry) const
{
    ...
    auto iter = mapNextTx.lower_bound(COutPoint(Txid::FromUint256(entry.GetTx().GetHash()), 0));
...
{
    auto ancestors = m_txgraph->GetAncestors(entry, true);
    setEntries ret;
    if (ancestors.size() > 0) {
    ...
    for (const auto& parent : staged_parents) {
        auto parent_ancestors = m_txgraph->GetAncestors(*parent, true);
...
{
    m_txgraph = MakeTxGraph(m_opts.limits.cluster_count, m_opts.limits.cluster_size_vbytes * WITNESS_SCALE_FACTOR, 10'000);
}
...
{
    auto descendants = m_txgraph->GetDescendants(*entryit);
...
void CTxMemPool::removeRecursive(CTxMemPool::txiter to_remove, MemPoolRemovalReason reason)
{
    ...
    auto descendants = m_txgraph->GetDescendants(*to_remove);
    for (auto tx: descendants) {
    ...
    if (origit != mapTx.end()) {
        ...
        while (iter != mapNextTx.end() && iter->first->hash == origTx.GetHash()) {
            if (!visited(iter->second)) {
                ...
                auto descendants = m_txgraph->GetDescendants(*(iter->second));
...
bool CTxMemPool::CompareMiningScoreWithTopology(const uint256& hasha, const uint256& hashb, bool wtxid)
{
    ...
    return m_txgraph->CompareMainOrder(*i, *j) == std::strong_ordering::less ? true : false;
}
...
std::vector<CTxMemPool::indexed_transaction_set::const_iterator> CTxMemPool::GetSortedScoreWithTopology() const
{
    ...
    std::sort(iters.begin(), iters.end(), [this](const CTxMemPool::indexed_transaction_set::const_iterator& a, const CTxMemPool::indexed_transaction_set::const_iterator& b) {
        ...
        return this->m_txgraph->CompareMainOrder(*a, *b) == std::strong_ordering::less;
    });
    return iters;
}
...
size_t CTxMemPool::DynamicMemoryUsage() const {
    ...
    return memusage::MallocUsage(sizeof(CTxMemPoolEntry) + 9 * sizeof(void*)) * mapTx.size() + memusage::DynamicUsage(mapNextTx) + memusage::DynamicUsage(mapDeltas) + memusage::DynamicUsage(txns_randomized) + cachedInnerUsage;
}
...
{
    auto descendants = m_txgraph->GetDescendants(entry);
...
void CTxMemPool::GetTransactionAncestry(const uint256& txid, size_t& ancestors, size_t& clustersize, size_t* const ancestorsize, CAmount* const ancestorfees) const {
    ...
    if (it != mapTx.end()) {
        ...
        clustersize = m_txgraph->GetCluster(*it).size();
    }
}

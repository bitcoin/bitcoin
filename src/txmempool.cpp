...
static void UpdateAncestorsOf(P& pool, bool add, typename P::txiter it, const typename P::setEntries& setAncestors) EXCLUSIVE_LOCKS_REQUIRED(pool.cs)
{
    ...
    for (auto& ancestorIt : setAncestors) {
        pool.Modify(const_cast<typename P::txiter&>(ancestorIt), update_descendant_state(updateSize, updateFee, updateCount));
    }
}
...
void TxPoolLayer::UpdateAncestorsOf(bool add, const TxPoolLayer::txiter& it, setEntries& setAncestors)
{
    return ::UpdateAncestorsOf(*this, add, it, setAncestors);
}
...
void TxPoolLayer::UpdateEntryForAncestors(TxPoolLayer::txiter& it, const setEntries& setAncestors)
{
    return ::UpdateEntryForAncestors(*this, it, setAncestors);
}
...
static void UpdateForRemoveFromMempool(P& pool, const typename P::setEntries& entriesToRemove, bool updateDescendants) EXCLUSIVE_LOCKS_REQUIRED(pool.cs)
{
    ...
    if (updateDescendants) {
        ...
        for (const auto& removeIt : entriesToRemove) {
            ...
            for (auto& dit : setDescendants) {
                pool.Modify(const_cast<typename P::txiter&>(dit), update_ancestor_state(modifySize, modifyFee, -1, modifySigOps));
            }
        }
    }
    for (auto& removeIt : entriesToRemove) {
        ...
        pool.UpdateAncestorsOf(false, const_cast<typename P::txiter&>(removeIt), setAncestors);
    }
...
void TxPoolLayer::UpdateForRemoveFromMempool(setEntries& entriesToRemove, bool updateDescendants)
{
    return ::UpdateForRemoveFromMempool(*this, entriesToRemove, updateDescendants);
}
...
void TxPoolLayer::addUnchecked(const CTxMemPoolEntry& entry, TxPoolLayer::setEntries& setAncestors, bool /* validFeeEstimate */)
{
...
void TxPoolLayer::check() const
{
    ...
    for (const CTxMemPoolEntry* e : all_txs) {
        ...
        for (const auto& p : GetMemPoolParents(it)) {
            setParentCheck.erase(p);
        }
        assert(setParentCheck.size() == 0);

        setEntries setChildrenCheck;
        for (unsigned i{0}; i <= tx.vout.size(); ++i) {
            if (const CTransaction* c_tx = GetConflictTx(COutPoint{tx.GetHash(), i})) {
                setChildrenCheck.insert(*GetIter(c_tx->GetHash()));
            }
        }
        for (const auto& c : GetMemPoolChildren(it)) {
            setChildrenCheck.erase(c);
        }
        assert(setChildrenCheck.size() == 0);
    }
}
...
const CTxMemPoolEntry& TxPoolLayer::GetEntry(const txiter& it)
{
    if (it.m_variant.which() == 0) return *boost::get<txiter_nested>(it.m_variant)._i;
    return CTxMemPool::GetEntry(boost::get<CTxMemPool::txiter>(it.m_variant));
}

void TxPoolLayer::EraseTx(TxPoolLayer::txiter it)
{
    if (it.m_variant.which() == 0) {
        const uint256& hash{GetEntry(it).GetTx().GetHash()};
        if (m_tx_pool.exists(hash)) {
            m_cache_removed.emplace(hash);
        }
        m_cache_added.erase(boost::get<txiter_nested>(it.m_variant)._i);
    } else {
        m_cache_removed.emplace(GetEntry(it).GetTx().GetHash());
    m_cache_added.erase(GetEntry(it).GetTx().GetHash()); ////////////// TODO
    }
}
...
boost::optional<TxPoolLayer::txiter> TxPoolLayer::GetIter(const uint256& txid) const
{
    ...
    if (it_cache != m_cache_added.end()) {
        return TxPoolLayer::txiter{txiter_nested{it_cache}};
    }
...
TxPoolLayer::setEntries TxPoolLayer::VariantIteratorSet(const CTxMemPool::setEntries& set)
{
    setEntries ret;
    for (const auto& i : set) {
        ret.emplace(txiter{i});
    }
    return ret;
}
...
{
    if (it.m_variant.which() == 1) {
        auto tx_pool_it = boost::get<CTxMemPool::txiter>(it.m_variant);
        it = Insert(GetEntry(it)).first;
        if (m_cache_map_links.find(it) == m_cache_map_links.end()) {
            auto p = VariantIteratorSet(m_tx_pool.GetMemPoolParents(tx_pool_it));
            auto c = VariantIteratorSet(m_tx_pool.GetMemPoolChildren(tx_pool_it));
            auto ret = m_cache_map_links.emplace(it, TxLinks{std::move(p), std::move(c)});
            assert(ret.second);
        }
    }
    m_cache_added.modify(boost::get<txiter_nested>(it.m_variant)._i, update_object);
}
...
void TxPoolLayer::UpdateChild(const TxPoolLayer::txiter& entry, const TxPoolLayer::txiter& child, bool add)
{
    ...
    if (it == m_cache_map_links.end()) {
        auto tx_pool_it = boost::get<CTxMemPool::txiter>(entry.m_variant);
        auto p = VariantIteratorSet(m_tx_pool.GetMemPoolParents(tx_pool_it));
        auto c = VariantIteratorSet(m_tx_pool.GetMemPoolChildren(tx_pool_it));
        auto ret = m_cache_map_links.emplace(entry, TxLinks{std::move(p), std::move(c)});
        assert(ret.second);
        it = ret.first;
    }
    if (add) {
        ...
        it->second.children.erase(child);
    }
}
...
void TxPoolLayer::UpdateParent(const TxPoolLayer::txiter& entry, const TxPoolLayer::txiter& parent, bool add)
{
    ...
    if (add) {
        ...
        it->second.parents.erase(parent);
    }
}
...
TxPoolLayer::setEntries TxPoolLayer::GetMemPoolParents(const txiter& entry) const
{
    if (entry.m_variant.which() == 0) {
        const auto it = m_cache_map_links.find(entry);
        assert(it != m_cache_map_links.end());
        return it->second.parents;
    }

    return VariantIteratorSet(m_tx_pool.GetMemPoolParents(boost::get<CTxMemPool::txiter>(entry.m_variant)));
}
...
TxPoolLayer::setEntries TxPoolLayer::GetMemPoolChildren(const txiter& entry) const
{
    ...
    return VariantIteratorSet(m_tx_pool.GetMemPoolChildren(boost::get<CTxMemPool::txiter>(entry.m_variant)));
}

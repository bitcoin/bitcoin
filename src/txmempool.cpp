...
util::Result<std::pair<std::vector<FeeFrac>, std::vector<FeeFrac>>> CTxMemPool::CTxMemPoolChangeSet::CalculateChunksForRBF()
{
    LOCK(m_pool->cs);
...
std::vector<TxEntry::TxEntryRef> CTxMemPool::CTxMemPoolChangeSet::CalculateParentsOf(const CTransactionRef& tx)
{
    std::vector<TxEntry::TxEntryRef> ret;
...
bool CTxMemPool::CTxMemPoolChangeSet::CheckMemPoolPolicyLimits()
{
    LOCK(m_pool->cs);

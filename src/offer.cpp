...
UniValue offernew(const UniValue& params, bool fHelp) {
	...
	CreateRecipient(scriptPubKeyAlias, alias.vchAlias, alias.vchAliasPeg, chainActive.Tip()->nHeight, aliasRecipient);
...
UniValue offeracceptfeedback(const UniValue& params, bool fHelp) {
   ...
   // gather & validate inputs
	...
	CreateRecipient(scriptPubKey, recipient);
	CreateRecipient(scriptPubKeyAlias, theAlias.vchAlias, theAlias.vchAliasPeg, chainActive.Tip()->nHeight, recipientAlias);
	...
	coinControl.fAllowOtherInputs = false;
	coinControl.fAllowWatchOnly = false;
	SendMoneySyscoin(vchLinkAlias, recipientAlias, vecSend, wtx, &coinControl);
	UniValue res(UniValue::VARR);
	UniValue signParams(UniValue::VARR);
	signParams.push_back(EncodeHexTx(wtx));
	const UniValue &resSign = tableRPC.execute("syscoinsignrawtransaction", signParams);
...
UniValue offeracceptacknowledge(const UniValue& params, bool fHelp) {
   ...
   // gather & validate inputs
	...
	CreateRecipient(scriptPubKeyAlias, sellerAlias.vchAlias, sellerAlias.vchAliasPeg, chainActive.Tip()->nHeight, recipientAlias);
	...
	coinControl.fAllowOtherInputs = false;
	coinControl.fAllowWatchOnly = false;
	SendMoneySyscoin(sellerAlias.vchAlias, recipientAlias, vecSend, wtx, &coinControl);
	UniValue res(UniValue::VARR);
	UniValue signParams(UniValue::VARR);
	signParams.push_back(EncodeHexTx(wtx));
	const UniValue &resSign = tableRPC.execute("syscoinsignrawtransaction", signParams);

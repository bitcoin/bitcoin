...
private:
    ...
    void UpdateChunk(const SetType& chunk, TxIdx query, TxIdx chunk_rep, const SetInfo<SetType>& dep_change) noexcept
    {
        ...
        for (auto tx_idx : chunk) {
            ...
            auto active_child_deps = std::span{tx_data.child_deps}.first(tx_data.child_deps_active);
...
public:
    ...
    void SanityCheck(const DepGraph<SetType>& depgraph) const
    {
        ...
        //
        assert(m_transactions == depgraph.Positions());
        for (auto tx_idx : m_transactions) {
            ...
            // Verify list of child dependencies. The fact that the first child_deps_active ones
            // are active has been verified above already.
        ...
        for (size_t i = 0; i < m_suboptimal_chunks.size(); ++i) {
            ...
            assert(m_tx_data[tx_idx].suboptimal != 0);
        ...
        for (auto tx_idx : m_transactions) {
            const auto& tx_data = m_tx_data[tx_idx];
            assert((tx_data.suboptimal != 0) == suboptimal[tx_idx]);
        }

        // Verify m_nonminimal_chunks.
        SetType nonminimal;
        for (size_t i = 0; i < m_nonminimal_chunks.size(); ++i) {
            auto [chunk_rep, pivot_tx, direction] = m_nonminimal_chunks[i];
            assert(m_transactions[chunk_rep]);
            assert(m_transactions[pivot_tx]);
            const auto& chunk_data = m_tx_data[chunk_rep];
            assert(chunk_data.chunk_rep == chunk_rep);
            assert(chunk_data.chunk_setinfo.transactions[pivot_tx]);
            assert(!nonminimal[chunk_rep]);
            nonminimal.Set(chunk_rep);
        }
    }
};

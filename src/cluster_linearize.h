 ...
 *     deactivated.
 *   - Otherwise (the number of self-merges so far is not 2 mod 3), a uniformly random dependency
 *     within the chunk among those with maximal gain is deactivated, if any with strictly positive
 *     gain exist.
 *
...
private:
     ...
     *  upwards ([0]) and downwards ([1]) direction. */
...
public:
    ...
    bool OptimizeStep() noexcept
    {
        while (!m_suboptimal_chunks.empty()) {
            ...
            // adversarially-constructed clusters reliably make bad choices, every 3rd attempt to
            // split the same cluster uses the random strategy.
            const bool use_max_gain = (m_self_merges[chunk_idx] % 3) != 2;
    ...
    bool MinimizeStep() noexcept
    {
        ...
        if (candidate_tiebreak == 0) {
            if (!second_stage) m_nonminimal_chunks.emplace_back(chunk_idx, pivot_idx, flags ^ 3);
            return true;
        }
        ...
        // dependency that was just activated).

...
void BanMapFromJson(const UniValue& bans_json, banmap_t& bans)
{
    for (const auto& ban_entry_json : bans_json.getValues()) {
        ...
        if (version != CBanEntry::CURRENT_VERSION) {
            LogPrintf(LogInstance(), "Dropping entry with unknown version (%s) from ban list\n", version);
            continue;
        }
        const auto& subnet_str = ban_entry_json[BANMAN_JSON_ADDR_KEY].get_str();
        const CSubNet subnet{LookupSubNet(subnet_str)};
        if (!subnet.IsValid()) {
            LogPrintf(LogInstance(), "Dropping entry with unparseable address or subnet (%s) from ban list\n", subnet_str);
            continue;
        }

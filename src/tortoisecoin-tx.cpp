...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " tortoisecoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  tortoisecoin-tx [options] <hex-tx> [commands]  Update hex-encoded tortoisecoin transaction\n"
                "or:     tortoisecoin-tx [options] -create [commands]   Create hex-encoded tortoisecoin transaction\n"
                "\n";

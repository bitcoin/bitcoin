        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                if (!CheckProofOfWork(pindexNew->GetBlockHash(), pindexNew->nBits, consensusParams)) {
                    LogError(LogInstance(), "%s: CheckProofOfWork failed: %s\n", __func__, pindexNew->ToString());
                    return false;
                }

                pcursor->Next();
            } else {
                LogError(LogInstance(), "%s: failed to read value\n", __func__);
                return false;
            }
        } else {
            break;
    ...
    ChainstateManager& chainman)
{
    ...
    LogPrintf(LogInstance(), "[%s] Prune (Manual): prune_height=%d removed %d blk/rev pairs\n",
    ...
    ChainstateManager& chainman)
{
    ...
    LogPrint(LogInstance(), BCLog::PRUNE, "[%s] target=%dMiB actual=%dMiB diff=%dMiB min_height=%d max_prune_height=%d removed %d blk/rev pairs\n",
...
bool BlockManager::LoadBlockIndex(const std::optional<uint256>& snapshot_blockhash)
{
    ...
    if (snapshot_blockhash) {
        ...
        LogPrintf(LogInstance(), "[snapshot] set nChainTx=%d for %s\n", au_data.nChainTx, snapshot_blockhash->ToString());
    } else {
    ...
    for (CBlockIndex* pindex : vSortedByHeight) {
        ...
        if (previous_index && pindex->nHeight > previous_index->nHeight + 1) {
            LogError(LogInstance(), "%s: block index is non-contiguous, index of height %d missing\n", __func__, previous_index->nHeight + 1);
            return false;
        }
...
bool BlockManager::LoadBlockIndexDB(const std::optional<uint256>& snapshot_blockhash)
{
    ...
    LogPrintf(LogInstance(), "%s: last block file = %i\n", __func__, max_blockfile_num);
    for (int nFile = 0; nFile <= max_blockfile_num; nFile++) {
        m_block_tree_db->ReadBlockFileInfo(nFile, m_blockfile_info[nFile]);
    }
    LogPrintf(LogInstance(), "%s: last block file info: %s\n", __func__, m_blockfile_info[max_blockfile_num].ToString());
    ...
    LogPrintf(LogInstance(), "Checking all blk files are present...\n");
    ...
    if (m_have_pruned) {
        LogPrintf(LogInstance(), "LoadBlockIndexDB(): Block files have previously been pruned\n");
    }
...
void BlockManager::CleanupBlockRevFiles() const
{
    ...
    LogPrintf(LogInstance(), "Removing unusable blk?????.dat and rev?????.dat files for -reindex with -prune\n");
...
bool BlockManager::UndoWriteToDisk(const CBlockUndo& blockundo, FlatFilePos& pos, const uint256& hashBlock) const
{
    ...
    if (fileout.IsNull()) {
        LogError(LogInstance(), "%s: OpenUndoFile failed\n", __func__);
        return false;
    }
    ...
    if (fileOutPos < 0) {
        LogError(LogInstance(), "%s: ftell failed\n", __func__);
        return false;
    }
...
bool BlockManager::UndoReadFromDisk(CBlockUndo& blockundo, const CBlockIndex& index) const
{
    ...
    if (pos.IsNull()) {
        LogError(LogInstance(), "%s: no undo data available\n", __func__);
        return false;
    }
    ...
    try {
        ...
        LogError(LogInstance(), "%s: Deserialize or I/O error - %s\n", __func__, e.what());
        return false;
    }

    // Verify checksum
    if (hashChecksum != verifier.GetHash()) {
        LogError(LogInstance(), "%s: Checksum mismatch\n", __func__);
        return false;
    }
...
void BlockManager::UnlinkPrunedFiles(const std::set<int>& setFilesToPrune) const
{
    ...
    for (std::set<int>::iterator it = setFilesToPrune.begin(); it != setFilesToPrune.end(); ++it) {
        ...
        if (removed_blockfile || removed_undofile) {
            LogPrint(LogInstance(), BCLog::BLOCKSTORAGE, "Prune: %s deleted blk/rev (%05u)\n", __func__, *it);
        }
    }
}
...
FlatFilePos BlockManager::FindNextBlockPos(unsigned int nAddSize, unsigned int nHeight, uint64_t nTime)
{
    ...
    if (!m_blockfile_cursors[chain_type]) {
        ...
        LogPrint(LogInstance(), BCLog::BLOCKSTORAGE, "[%s] initializing blockfile cursor to %s\n", chain_type, new_cursor);
    }
    ...
    if (nFile != last_blockfile) {
        LogPrint(LogInstance(), BCLog::BLOCKSTORAGE, "Leaving block file %i: %s (onto %i) (height %i)\n",
        ...
        if (!FlushBlockFile(last_blockfile, /*fFinalize=*/true, finalize_undo)) {
            LogPrintLevel(LogInstance(), BCLog::BLOCKSTORAGE, BCLog::Level::Warning,
...
bool BlockManager::WriteBlockToDisk(const CBlock& block, FlatFilePos& pos) const
{
    ...
    if (fileout.IsNull()) {
        LogError(LogInstance(), "WriteBlockToDisk: OpenBlockFile failed\n");
        return false;
    }
    ...
    long fileOutPos = ftell(fileout.Get());
    if (fileOutPos < 0) {
        LogError(LogInstance(), "WriteBlockToDisk: ftell failed\n");
        return false;
    }
    pos.nPos = (unsigned int)fileOutPos;
...
bool BlockManager::WriteUndoDataForBlock(const CBlockUndo& blockundo, BlockValidationState& state, CBlockIndex& block)
{
    ...
    if (block.GetUndoPos().IsNull()) {
        ...
        if (!FindUndoPos(state, block.nFile, _pos, ::GetSerializeSize(blockundo) + 40)) {
            LogError(LogInstance(), "ConnectBlock(): FindUndoPos failed\n");
            return false;
        }
        ...
        if (_pos.nFile < cursor.file_num && static_cast<uint32_t>(block.nHeight) == m_blockfile_info[_pos.nFile].nHeightLast) {
            ...
            if (!FlushUndoFile(_pos.nFile, true)) {
                LogPrintLevel(LogInstance(), BCLog::BLOCKSTORAGE, BCLog::Level::Warning, "Failed to flush undo file %05i\n", _pos.nFile);
            }
...
bool BlockManager::ReadBlockFromDisk(CBlock& block, const FlatFilePos& pos) const
{
    ...
    if (filein.IsNull()) {
        LogError(LogInstance(), "ReadBlockFromDisk: OpenBlockFile failed for %s\n", pos.ToString());
        return false;
    }
    ...
    try {
        ...
        LogError(LogInstance(), "%s: Deserialize or I/O error - %s at %s\n", __func__, e.what(), pos.ToString());
        return false;
    }

    // Check the header
    if (!CheckProofOfWork(block.GetHash(), block.nBits, GetConsensus())) {
        LogError(LogInstance(), "ReadBlockFromDisk: Errors in block header at %s\n", pos.ToString());
        return false;
    }

    // Signet only: check block solution
    if (GetConsensus().signet_blocks && !CheckSignetBlockSolution(block, GetConsensus())) {
        LogError(LogInstance(), "ReadBlockFromDisk: Errors in block solution at %s\n", pos.ToString());
        return false;
    }
...
bool BlockManager::ReadBlockFromDisk(CBlock& block, const CBlockIndex& index) const
{
    ...
    if (block.GetHash() != index.GetBlockHash()) {
        LogError(LogInstance(), "ReadBlockFromDisk(CBlock&, CBlockIndex*): GetHash() doesn't match index for %s at %s\n",
...
bool BlockManager::ReadRawBlockFromDisk(std::vector<uint8_t>& block, const FlatFilePos& pos) const
{
    ...
    if (hpos.nPos < 8) {
        LogError(LogInstance(), "%s: OpenBlockFile failed for %s\n", __func__, pos.ToString());
        return false;
    }
    ...
    try {
        ...
        if (blk_start != GetParams().MessageStart()) {
            LogError(LogInstance(), "%s: Block magic mismatch for %s: %s versus expected %s\n", __func__, pos.ToString(),
                         HexStr(blk_start),
        ...
        if (blk_size > MAX_SIZE) {
            LogError(LogInstance(), "%s: Block data is larger than maximum deserialization size for %s: %s versus %s\n", __func__, pos.ToString(),
                         blk_size, MAX_SIZE);
            return false;
        }
        ...
        LogError(LogInstance(), "%s: Read from block file failed: %s for %s\n", __func__, e.what(), pos.ToString());
        return false;
    }
...
FlatFilePos BlockManager::SaveBlockToDisk(const CBlock& block, int nHeight)
{
    ...
    if (blockPos.IsNull()) {
        LogError(LogInstance(), "%s: FindNextBlockPos failed\n", __func__);
        return FlatFilePos();
    }
...
void ImportBlocks(ChainstateManager& chainman, std::vector<fs::path> vImportFiles)
{
    ...
    if (!chainman.m_blockman.m_blockfiles_indexed) {
        ...
        while (true) {
            ...
            LogPrintf(LogInstance(), "Reindexing block file blk%05u.dat...\n", (unsigned int)nFile);
            chainman.LoadExternalBlockFile(file, &pos, &blocks_with_unknown_parent);
            if (chainman.m_interrupt) {
                LogPrintf(LogInstance(), "Interrupt requested. Exit %s\n", __func__);
                return;
            }
            nFile++;
        }
        ...
        LogPrintf(LogInstance(), "Reindexing finished\n");
    ...
    for (const fs::path& path : vImportFiles) {
        ...
        if (!file.IsNull()) {
            LogPrintf(LogInstance(), "Importing blocks file %s...\n", fs::PathToString(path));
            ...
            LogPrintf(LogInstance(), "Warning: Could not open blocks file %s\n", fs::PathToString(path));
        }
    }

...
PSBTAnalysis AnalyzePSBT(PartiallySignedTransaction psbtx)
{
    ...
    for (unsigned int i = 0; i < psbtx.tx->vin.size(); ++i) {
        ...
        if (!PSBTInputSignedAndVerified(psbtx, i, &txdata)) {
            ...
            bool complete = SignPSBTInput(DUMMY_SIGNING_PROVIDER, psbtx, i, &txdata, std::nullopt, &outdata) == PSBTResult::OK;
    ...
    if (calc_fee) {
        ...
        for (unsigned int i = 0; i < psbtx.tx->vin.size(); ++i) {
            ...
            if (SignPSBTInput(DUMMY_SIGNING_PROVIDER, psbtx, i, nullptr, std::nullopt) != PSBTResult::OK || !psbtx.GetInputUTXO(newcoin.out, i)) {
                success = false;
                break;
            } else {

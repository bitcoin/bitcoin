...
{
    // Make sure the capacity is within bounds
    Assume(capacity > 0 && capacity <= MAX_SKETCH_CAPACITY);

    Minisketch sketch = node::MakeMinisketch32(capacity);

    for (const auto& wtxid : m_local_set) {
        sketch.Add(ComputeShortID(wtxid));
    }

    return sketch;
}

Minisketch TxReconciliationState::ComputeExtendedSketch(uint32_t extended_capacity) const
{
    ...
    // instead of computing the lower order elements again. This may need upstream
    // changes to minisketch.
    Minisketch sketch = node::MakeMinisketch32(extended_capacity);

    // We don't have to recompute short IDs here.
    for (const auto& [short_id, wtxid] : m_short_id_mapping_snapshot) {
        sketch.Add(short_id);
    }
    return sketch;
}
...
std::pair<std::vector<uint32_t>, std::vector<Wtxid>> TxReconciliationState::GetRelevantIDsFromShortIDs(const std::vector<uint64_t>& diff, bool from_snapshot) const
{
    ...
    for (const auto& diff_short_id : diff) {
        // Make sure we are within the 32-bit range.
        Assume(diff_short_id <= std::numeric_limits<uint32_t>::max());
...
private:
    ...
    std::variant<HandleSketchResult, ReconciliationError> HandleInitialSketch(TxReconciliationState& peer_state, const NodeId peer_id, const std::vector<uint8_t>& skdata) const
    {
    ...
    std::variant<HandleSketchResult, ReconciliationError> HandleSketchExtension(TxReconciliationState& peer_state, const NodeId peer_id, const std::vector<uint8_t>& skdata)
    {
        ...
        auto working_skdata = std::vector<uint8_t>(skdata);
        ...
        std::erase_if(txs_to_announce, [&peer_state](const auto& x) {
    ...
    void RespondToInitialRequest(TxReconciliationState& peer_state, const NodeId peer_id, std::vector<uint8_t>& skdata) const
    {
        Assume(!peer_state.m_we_initiate);
        Assume(peer_state.m_phase == ReconciliationPhase::INIT_REQUESTED);
        ...
        if (peer_state.m_remote_set_size > 0 && !peer_state.m_local_set.empty()) {
            sketch_capacity = peer_state.EstimateSketchCapacity(peer_state.m_local_set.size());
            // EstimateSketchCapacity estimated difference starts at 1.
            Assume(sketch_capacity > 0);
            skdata = peer_state.ComputeBaseSketch(sketch_capacity).Serialize();
        }
    ...
    void RespondToExtensionRequest(TxReconciliationState& peer_state, NodeId peer_id, std::vector<uint8_t>& skdata) const
    {
        Assume(!peer_state.m_we_initiate);
        Assume(peer_state.m_phase == ReconciliationPhase::EXT_REQUESTED);
        Assume(peer_state.m_capacity_snapshot > 0);
        // Update local reconciliation state for the peer.
        peer_state.m_phase = ReconciliationPhase::EXT_RESPONDED;
        ...
        uint32_t extended_capacity = peer_state.m_capacity_snapshot * 2;
        Minisketch sketch = peer_state.ComputeExtendedSketch(extended_capacity);
        Assume(sketch);
...
public:
    ...
    std::optional<ReconciliationError> HandleReconciliationRequest(NodeId peer_id, uint16_t peer_recon_set_size, uint16_t peer_q) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        // The set size only feeds our sketch capacity estimate. Cap it at the largest set we would
        // ever keep ourselves, so a peer cannot claim an inflated size to make us answer a tiny
        // request with a maximum-capacity sketch. Clamping rather than rejecting keeps us
        // interoperable with implementations whose own set limit is higher than ours.
        peer_state->m_remote_set_size = std::min(peer_recon_set_size, static_cast<uint16_t>(MAX_RECONSET_SIZE));
        peer_state->m_phase = ReconciliationPhase::INIT_REQUESTED;
    ...
    bool ShouldRespondToReconciliationRequest(NodeId peer_id, std::vector<uint8_t>& skdata) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        ReconciliationPhase incoming_phase = peer_state->m_phase;
        if (incoming_phase == ReconciliationPhase::INIT_REQUESTED) {
            RespondToInitialRequest(*peer_state, peer_id, skdata);
            return true;
        } else if (incoming_phase == ReconciliationPhase::EXT_REQUESTED) {
            RespondToExtensionRequest(*peer_state, peer_id, skdata);
            return true;
        } else {
            return false;
        }

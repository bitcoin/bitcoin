...
private:
    ...
    std::variant<HandleSketchResult, ReconciliationError> HandleInitialSketch(TxReconciliationState& recon_state, const NodeId peer_id, const std::vector<uint8_t>& skdata)
    {
        Assume(recon_state.m_we_initiate);
        Assume(recon_state.m_phase == ReconciliationPhase::INIT_REQUESTED);
        ...
        if (!remote_sketch.has_value() || !local_sketch.has_value()) {
            ...
            txs_to_announce = recon_state.GetAllTransactions(/*from_snapshot=*/false);
            ...
            if (local_sketch.value().Merge(remote_sketch.value()).Decode(differences)) {
                ...
                std::tie(txs_to_request, txs_to_announce) = recon_state.GetRelevantIDsFromShortIDs(differences, /*from_snapshot=*/false);
                // Update local reconciliation state for the peer.
                recon_state.Clear();
                ...
                recon_state.m_phase = ReconciliationPhase::EXT_REQUESTED;
    ...
    std::variant<HandleSketchResult, ReconciliationError> HandleSketchExtension(TxReconciliationState& recon_state, const NodeId peer_id, const std::vector<uint8_t>& skdata)
    {
        Assume(recon_state.m_we_initiate);
        Assume(recon_state.m_phase == ReconciliationPhase::EXT_REQUESTED);
        ...
        size_t extended_sketch_size = recon_state.m_remote_sketch_snapshot.size() + skdata.size();
        ...
        if (local_sketch.Merge(remote_sketch).Decode(differences)) {
            ...
            // Identify locally/remotely missing transactions.
            std::tie(txs_to_request, txs_to_announce) = recon_state.GetRelevantIDsFromShortIDs(differences, /*from_snapshot=*/true);
            ...
            txs_to_announce = recon_state.GetAllTransactions(/*from_snapshot=*/true);
        ...
        txs_to_announce.erase(std::remove_if(txs_to_announce.begin(), txs_to_announce.end(), [recon_state](const auto&x) {
            return recon_state.m_announced_while_reconciling.contains(x);
        }), txs_to_announce.end());
    ...
    void RespondToInitialRequest(TxReconciliationState& recon_state, const NodeId peer_id, std::vector<uint8_t>& skdata)
    {
        ...
        recon_state.m_phase = ReconciliationPhase::INIT_RESPONDED;
    ...
    void RespondToExtensionRequest(TxReconciliationState& recon_state, NodeId peer_id, std::vector<uint8_t>& skdata)
    {
        ...
        // Update local reconciliation state for the peer.
        recon_state.m_phase = ReconciliationPhase::EXT_RESPONDED;
        ...
        LogDebug(BCLog::TXRECONCILIATION, "Responding with a sketch extension to reconciliation initiated by peer=%d.\n", peer_id);
    }
...
public:
                                                    ...
                                                    uint64_t remote_salt) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        if (recon_state == m_states.end()) return ReconciliationError::NOT_FOUND;
    ...
    bool IsPeerNextToReconcileWith(NodeId peer_id, std::chrono::microseconds now) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        if (m_next_recon_request <= now && m_queue.front() == peer_id) {
            ...
            if (recon_state.m_phase == ReconciliationPhase::NONE) ScheduleNextReconRequest(now);
            return true;
        }
    ...
    std::variant<ReconCoefficients, ReconciliationError> InitiateReconciliationRequest(NodeId peer_id) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        AssertLockNotHeld(m_txreconciliation_mutex);
        LOCK(m_txreconciliation_mutex);
        if (!GetRegisteredPeerState(peer_id)) return ReconciliationError::NOT_FOUND;

        auto& recon_state = std::get<TxReconciliationState>(m_states.find(peer_id)->second);
        if (!recon_state.m_we_initiate) return ReconciliationError::WRONG_ROLE;

        // Shortcut if the peer hasn't completed the previous reconciliation cycle
        if (recon_state.m_phase != ReconciliationPhase::NONE) return ReconciliationError::WRONG_PHASE;
    ...
    bool ShouldRespondToReconciliationRequest(NodeId peer_id, std::vector<uint8_t>& skdata, bool send_trickle) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        ReconciliationPhase incoming_phase = recon_state.m_phase;
        if (incoming_phase == ReconciliationPhase::INIT_REQUESTED && send_trickle) {
            RespondToInitialRequest(recon_state, peer_id, skdata);
            return true;
        } else if (incoming_phase == ReconciliationPhase::EXT_REQUESTED) {
    ...
    std::variant<HandleSketchResult, ReconciliationError> HandleSketch(NodeId peer_id, const std::vector<uint8_t>& skdata) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        ReconciliationPhase cur_phase = recon_state.m_phase;
        if (cur_phase == ReconciliationPhase::INIT_REQUESTED) {
            return HandleInitialSketch(recon_state, peer_id, skdata);
        } else if (cur_phase == ReconciliationPhase::EXT_REQUESTED) {
            return HandleSketchExtension(recon_state, peer_id, skdata);
        } else {
    ...
    std::optional<ReconciliationError> HandleExtensionRequest(NodeId peer_id) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        auto& recon_state = std::get<TxReconciliationState>(m_states.find(peer_id)->second);
        if (recon_state.m_we_initiate) return ReconciliationError::WRONG_ROLE;
        if (recon_state.m_phase != ReconciliationPhase::INIT_RESPONDED) return ReconciliationError::WRONG_PHASE;
        ...
        EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        if (recon_state.m_phase != ReconciliationPhase::INIT_RESPONDED &&
            recon_state.m_phase != ReconciliationPhase::EXT_RESPONDED) return ReconciliationError::WRONG_PHASE;
        ...
        if (recon_result) {
            remote_missing = recon_state.GetWtxidsFromShortIDs(remote_missing_short_ids);
        } else {
            ...
            remote_missing = recon_state.GetAllTransactions(/*from_snapshot=*/true);
        }

        // Filter out transactions received from the peer while reconciling.
        remote_missing.erase(std::remove_if(remote_missing.begin(), remote_missing.end(), [recon_state](const auto&x) {
            return recon_state.m_announced_while_reconciling.contains(x);
        }), remote_missing.end());

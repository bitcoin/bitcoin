...
void TxReconciliationState::GetRelevantIDsFromShortIDs(const std::vector<uint64_t>& diff, bool from_snapshot,
                                                       // returning values
...
private:
    ...
    bool HandleInitialSketch(TxReconciliationState& recon_state, const NodeId peer_id, const std::vector<uint8_t>& skdata,
                             // returning values
                             std::vector<uint32_t>& txs_to_request, std::vector<Wtxid>& txs_to_announce, std::optional<bool>& result)
    {
        ...
        if (!remote_sketch.has_value() || !local_sketch.has_value()) {
            ...
            if (local_sketch.value().Merge(remote_sketch.value()).Decode(differences)) {
                ...
                recon_state.GetRelevantIDsFromShortIDs(differences, /*from_snapshot=*/false, txs_to_request, txs_to_announce);
                // Update local reconciliation state for the peer.
                recon_state.Clear();
    ...
    bool HandleSketchExtension(TxReconciliationState& recon_state, const NodeId peer_id, const std::vector<uint8_t>& skdata,
                               // returning values
                               std::vector<uint32_t>& txs_to_request, std::vector<Wtxid>& txs_to_announce, std::optional<bool>& result)
    {
        Assume(recon_state.m_we_initiate);
        ...
        if (extended_sketch_size % BYTES_PER_SKETCH_CAPACITY != 0) return false;
        ...
        if (local_sketch.Merge(remote_sketch).Decode(differences)) {
            ...
            // Identify locally/remotely missing transactions.
            recon_state.GetRelevantIDsFromShortIDs(differences, /*from_snapshot=*/true, txs_to_request, txs_to_announce);
...
public:
    ...
    std::optional<std::pair<uint16_t, uint16_t>> InitiateReconciliationRequest(NodeId peer_id) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        if (recon_state.m_phase != ReconciliationPhase::NONE) return std::nullopt;
    ...
    void HandleExtensionRequest(NodeId peer_id) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        auto& recon_state = std::get<TxReconciliationState>(m_states.find(peer_id)->second);
        if (recon_state.m_phase != ReconciliationPhase::INIT_RESPONDED) return;
                            ...
                            std::vector<Wtxid>& remote_missing) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        if (recon_state.m_phase != ReconciliationPhase::INIT_RESPONDED &&
            recon_state.m_phase != ReconciliationPhase::EXT_RESPONDED) return false;

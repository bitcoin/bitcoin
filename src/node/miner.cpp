...
static BlockCreateOptions ClampOptions(BlockCreateOptions options, const MiningArgs mining_args)
{
    ...
    options.block_max_weight = std::clamp<size_t>(mining_args.nBlockMaxWeight, options.block_reserved_weight, MAX_BLOCK_WEIGHT);
    return options;
}
...
BlockAssembler::BlockAssembler(Chainstate& chainstate,
    ...
    : chainparams{chainstate.m_chainman.GetParams()},
      ...
      m_options{ClampOptions(options, m_mining_args)}
{
}
...
{
    if (nBlockWeight + package_feerate.size >= m_options.block_max_weight) {
        return false;
    }
...
void BlockAssembler::addChunks()
{
    ...
    while (selected_transactions.size() > 0) {
        ...
        if (chunk_feerate.fee < m_mining_args.blockMinFeeRate.GetFee(chunk_feerate_vsize.size)) {

...
void MiniMiner::BuildMockTemplate(std::optional<CFeeRate> target_feerate)
{
    ...
    while (!m_entries_by_txid.empty()) {
        ...
        m_linearize_result.size_per_feerate.emplace_back(CFeeRate{ancestor_package_fee, static_cast<int32_t>(ancestor_package_size)}, static_cast<uint32_t>(ancestor_package_size));

...
public:
    ...
    {
        if (!m_node.fee_estimator || (!m_node.fee_estimator->block_policy_estimator)) return {};
        return m_node.fee_estimator->block_policy_estimator.value()->estimateSmartFee(num_blocks, calc, conservative);
    }
    unsigned int estimateMaxBlocks() override
    {
        if (!m_node.fee_estimator || (!m_node.fee_estimator->block_policy_estimator)) return 0;
        return m_node.fee_estimator->block_policy_estimator.value()->HighestTargetTracked(FeeEstimateHorizon::LONG_HALFLIFE);
    }

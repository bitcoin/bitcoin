...
static constexpr uint32_t STORAGE_HEADER_SIZE{std::tuple_size_v<MessageStartChars> + sizeof(unsigned int)};

/** Total overhead when writing undo data: header (8 bytes) plus checksum (32 bytes) */
static constexpr uint32_t UNDO_DATA_DISK_OVERHEAD{STORAGE_HEADER_SIZE + uint256::size()};
...
private:
     ...
     * separator fields (STORAGE_HEADER_SIZE).
     */

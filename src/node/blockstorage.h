...
public:
    ...
    util::Expected<void, kernel::FatalError> WriteBlockUndo(const CBlockUndo& blockundo, CBlockIndex& block)
        EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
     ...
     */
    util::Expected<FlatFilePos, kernel::FatalError> WriteBlock(const CBlock& block, int nHeight) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);

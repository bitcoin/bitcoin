...
public:
    ...
    std::vector<Wtxid> GetAllTransactions(bool snapshot) const
    {
        return snapshot ? std::vector<Wtxid>(m_local_set_snapshot.begin(), m_local_set_snapshot.end()) :
    ...
    void GetRelevantIDsFromShortIDs(const std::vector<uint64_t>& diff,
                                    // returning values
                                    std::vector<uint32_t>& local_missing, std::vector<Wtxid>& remote_missing, bool from_snapshot = false) const
    {
     ...
     */
    std::vector<Wtxid> GetWtxidsFromShortIDs(const std::vector<uint32_t>& remote_missing_short_ids, bool from_snapshot = false) const
    {
...
private:
                             ...
                             std::vector<uint32_t>& txs_to_request, std::vector<Wtxid>& txs_to_announce, std::optional<bool>& result)
    {
        ...
        if (!recon_state.m_local_set.empty() && remote_sketch_capacity > 0) {
        ...
        if (remote_sketch_capacity == 0 || !remote_sketch.has_value() || !local_sketch.has_value()) {
...
public:
    ...
    bool HandleSketch(NodeId peer_id, const std::vector<uint8_t>& skdata,
                      // returning values
                      std::vector<uint32_t>& txs_to_request, std::vector<Wtxid>& txs_to_announce, std::optional<bool>& result) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        AssertLockNotHeld(m_txreconciliation_mutex);
        LOCK(m_txreconciliation_mutex);
        if (!GetRegisteredPeerState(peer_id)) return false;
    ...
    bool FinalizeInitByThem(NodeId peer_id, bool recon_result,
        const std::vector<uint32_t>& remote_missing_short_ids, std::vector<Wtxid>& remote_missing) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        AssertLockNotHeld(m_txreconciliation_mutex);
        LOCK(m_txreconciliation_mutex);
        if (!GetRegisteredPeerState(peer_id)) return false;

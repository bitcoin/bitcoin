    ...
    const ChainstateLoadOptions& options) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    for (Chainstate* chainstate : chainman.GetAll()) {
        LogPrintf(LogInstance(), "Initializing chainstate %s\n", chainstate->ToString());
                                    ...
                                    const ChainstateLoadOptions& options)
{
    if (!chainman.AssumedValidBlock().IsNull()) {
        LogPrintf(LogInstance(), "Assuming ancestors of block %s have valid signatures.\n", chainman.AssumedValidBlock().GetHex());
    } else {
        LogPrintf(LogInstance(), "Validating signatures for all blocks.\n");
    }
    LogPrintf(LogInstance(), "Setting nMinimumChainWork=%s\n", chainman.MinimumChainWork().GetHex());
    if (chainman.MinimumChainWork() < UintToArith256(chainman.GetConsensus().nMinimumChainWork)) {
        LogPrintf(LogInstance(), "Warning: nMinimumChainWork set below default value of %s\n", chainman.GetConsensus().nMinimumChainWork.GetHex());
    }
    if (chainman.m_blockman.GetPruneTarget() == BlockManager::PRUNE_TARGET_MANUAL) {
        LogPrintf(LogInstance(), "Block pruning enabled.  Use RPC call pruneblockchain(height) to manually prune block and undo files.\n");
    } else if (chainman.m_blockman.GetPruneTarget()) {
        LogPrintf(LogInstance(), "Prune configured to target %u MiB on disk for block and undo files.\n", chainman.m_blockman.GetPruneTarget() / 1024 / 1024);
    }
    ...
    if (has_snapshot && options.wipe_chainstate_db) {
        LogPrintf(LogInstance(), "[snapshot] deleting snapshot chainstate due to reindexing\n");
    ...
    if (snapshot_completion == SnapshotCompletionResult::SKIPPED) {
        ...
        LogPrintf(LogInstance(), "[snapshot] cleaning up unneeded background chainstate, then reinitializing\n");

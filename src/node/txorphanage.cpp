...
void TxOrphanageImpl::LimitOrphans()
{
    ...
    for (const auto& [nodeid, entry] : m_peer_orphanage_info) {
        ...
        if (ByRatio{dos_score} > ByRatio{FeeFrac{1*sats, 1}}) {
    ...
    do {
        ...
        Assume(ByRatio{dos_score} > ByRatio{FeeFrac(1*sats, 1)});
        ...
        const auto& dos_threshold = heap_peer_dos.empty() ? FeeFrac{1*sats, 1} : heap_peer_dos.front().second;

...
private:
    ...
    bool IsUnique(const Wtxid& wtxid) const;
...
public:
    ...
    unsigned int CountAnnouncements() const override;
    unsigned int CountUniqueOrphans() const override;
    unsigned int AnnouncementsFromPeer(NodeId peer) const override;
    int64_t UsageFromPeer(NodeId peer) const override;
    unsigned int MaxGlobalAnnouncements() const override;
    unsigned int ReservedPeerUsage() const override;
    unsigned int MaxPeerAnnouncements() const override;
    unsigned int MaxGlobalUsage() const override;
...
unsigned int TxOrphanageImpl::MaxGlobalAnnouncements() const { return m_max_global_announcements; }
/** Constant, decided on initialization. */
unsigned int TxOrphanageImpl::ReservedPeerUsage() const { return m_reserved_usage_per_peer; }
 ...
 * exceeded. */
unsigned int TxOrphanageImpl::MaxPeerAnnouncements() const { return m_max_global_announcements / std::max<unsigned int>(m_peer_orphanage_info.size(), 1); }
...
unsigned int TxOrphanageImpl::MaxGlobalUsage() const { return m_reserved_usage_per_peer * std::max<unsigned int>(m_peer_orphanage_info.size(), 1); }

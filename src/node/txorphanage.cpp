...
void TxOrphanageImpl::LimitOrphans()
{
    ...
    for (const auto& [nodeid, entry] : m_peer_orphanage_info) {
        ...
        if (ByRatio{dos_score} > ByRatio{FeeFrac{CAmount{1}, 1}}) {
    ...
    do {
        ...
        Assume(ByRatio{dos_score} > ByRatio{FeeFrac(CAmount{1}, 1)});
        ...
        const auto& dos_threshold = heap_peer_dos.empty() ? FeeFrac{CAmount{1}, 1} : heap_peer_dos.front().second;

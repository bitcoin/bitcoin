...
public:
    ...
    *   removed), return the Cluster it is in. Otherwise, return nullptr. */
    ...
    /** If cluster is not in the top level, copy it there, and return a pointer to it. This has no
...
void Cluster::ApplyRemovals(TxGraphImpl& graph, std::span<GraphIndex>& to_remove) noexcept
{
    ...
    if (todo.None()) {
        ...
        if (IsOptimal(/*after_split=*/true)) {
            quality = QualityLevel::NEEDS_SPLIT_OPTIMAL;
        } else if (IsAcceptable(/*after_split=*/true)) {
...
void TxGraphImpl::ApplyRemovals(int level) noexcept
{
    ...
    Assume(size_t(level) == m_clustersets.size() - 1);

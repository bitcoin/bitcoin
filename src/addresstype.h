...
/** P2SKH (Pay-to-Schnorr-Key-Hash) destination: OP_2 <hash160(P.x)>.
 *  The 20-byte program holds RIPEMD160(SHA256(P.x)) of the spending key. */
struct SchnorrKeyHash : public BaseHash<uint160>
{
    SchnorrKeyHash() : BaseHash() {}
    explicit SchnorrKeyHash(const uint160& hash) : BaseHash(hash) {}
};
 ...
 *  * SchnorrKeyHash: TxoutType::SCHNORR_KEYHASH destination (P2SKH address)
 *  * PayToAnchor: TxoutType::ANCHOR destination (P2A address)
 *  * WitnessUnknown: TxoutType::WITNESS_UNKNOWN destination (P2W??? address)
 *  A CTxDestination is the internal data type encoded in a bitcoin address
 */
using CTxDestination = std::variant<CNoDestination, PubKeyDestination, PKHash, ScriptHash, WitnessV0ScriptHash, WitnessV0KeyHash, WitnessV1Taproot, SchnorrKeyHash, PayToAnchor, WitnessUnknown>;

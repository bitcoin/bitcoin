    ...
    : m_db_context{std::make_unique<LevelDBContext>()}, m_name{fs::PathToString(params.path.stem())}, m_path{params.path}, m_is_memory{params.memory_only}
{
...

    {
        ...
        std::vector<uint8_t> key_bytes(Obfuscation::SIZE_BYTES, '\000');
        ...
        if (key_bytes.size() != Obfuscation::SIZE_BYTES) {
        ...
        m_obfuscation = MakeByteSpan(key_bytes).first<Obfuscation::SIZE_BYTES>();
    }
}

...
static void HandleError(const leveldb::Status& status)
{
    ...
    LogPrintf(LogInstance(), "You can use -debug=leveldb to get more complete diagnostic messages\n");
...
public:
    ...
    void Logv(const char * format, va_list ap) override {
            if (!LogAcceptCategory(LogInstance(), BCLog::LEVELDB, BCLog::Level::Debug)) {
                return;
            }
            char buffer[500];
...
#endif
    LogPrint(LogInstance(), BCLog::LEVELDB, "LevelDB using max_open_files=%d (default=%d)\n",
    ...
    : m_db_context{std::make_unique<LevelDBContext>()}, m_name{fs::PathToString(params.path.stem())}, m_path{params.path}, m_is_memory{params.memory_only}
{
    ...
    if (params.memory_only) {
        ...
        if (params.wipe_data) {
            LogPrintf(LogInstance(), "Wiping LevelDB in %s\n", fs::PathToString(params.path));
            leveldb::Status result = leveldb::DestroyDB(fs::PathToString(params.path), DBContext().options);
            HandleError(result);
        }
        TryCreateDirectories(params.path);
        LogPrintf(LogInstance(), "Opening LevelDB in %s\n", fs::PathToString(params.path));
    }
    ...
    HandleError(status);
    LogPrintf(LogInstance(), "Opened LevelDB successfully\n");

    if (params.options.force_compact) {
        LogPrintf(LogInstance(), "Starting database compaction of %s\n", fs::PathToString(params.path));
        DBContext().pdb->CompactRange(nullptr, nullptr);
        LogPrintf(LogInstance(), "Finished database compaction of %s\n", fs::PathToString(params.path));
    }
    ...
    if (!key_exists && params.obfuscate && IsEmpty()) {
        ...
        LogPrintf(LogInstance(), "Wrote new obfuscate key for %s: %s\n", fs::PathToString(params.path), HexStr(obfuscate_key));
    }

    LogPrintf(LogInstance(), "Using obfuscation key for %s: %s\n", fs::PathToString(params.path), HexStr(obfuscate_key));
}
...
{
    const bool log_memory = LogAcceptCategory(LogInstance(), BCLog::LEVELDB, BCLog::Level::Debug);
    double mem_before = 0;
    if (log_memory) {
    ...
    if (log_memory) {
        ...
        LogPrint(LogInstance(), BCLog::LEVELDB, "WriteBatch memory usage: db=%s, before=%.1fMiB, after=%.1fMiB\n",
...
size_t CDBWrapper::DynamicMemoryUsage() const
{
    ...
    if (!DBContext().pdb->GetProperty("leveldb.approximate-memory-usage", &memory) || !(parsed = ToIntegral<size_t>(memory))) {
        LogPrint(LogInstance(), BCLog::LEVELDB, "Failed to get approximate-memory-usage property\n");
        return 0;
    }
    return parsed.value();
}
...
std::optional<std::string> CDBWrapper::ReadImpl(Span<const std::byte> key) const
{
    ...
    if (!status.ok()) {
        if (status.IsNotFound())
            return std::nullopt;
        LogPrintf(LogInstance(), "LevelDB read failure: %s\n", status.ToString());
        HandleError(status);
    }
    return strValue;
}

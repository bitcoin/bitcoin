 ...
 */
static void HandleError(const BCLog::Source& log, const leveldb::Status& status)
{
    if (status.ok())
        return;
...
public:
    CBitcoinLevelDBLogger(const BCLog::Source& log) : m_log{log} {}
...
static void SetMaxOpenFiles(const BCLog::Source& log, leveldb::Options *options) {
...
static leveldb::Options GetOptions(const BCLog::Source& log, size_t nCacheSize)
{
...
CDBWrapper::CDBWrapper(BCLog::Logger& logger, const DBParams& params)
    : m_log{BCLog::LEVELDB, logger}, m_db_context{std::make_unique<LevelDBContext>()}, m_name{fs::PathToString(params.path.stem())}, m_path{params.path}, m_is_memory{params.memory_only}
{
    ...
    if (params.memory_only) {
        ...
        LogInfo(m_log, "Opening LevelDB in %s\n", fs::PathToString(params.path));
    }
    ...
    if (params.options.force_compact) {
        LogInfo(m_log, "Starting database compaction of %s\n", fs::PathToString(params.path));
        DBContext().pdb->CompactRange(nullptr, nullptr);
        LogInfo(m_log, "Finished database compaction of %s\n", fs::PathToString(params.path));
    }
    ...
    if (!key_exists && params.obfuscate && IsEmpty()) {
        ...
        LogInfo(m_log, "Wrote new obfuscate key for %s: %s\n", fs::PathToString(params.path), HexStr(obfuscate_key));
    }

    LogInfo(m_log, "Using obfuscation key for %s: %s\n", fs::PathToString(params.path), HexStr(obfuscate_key));
}
...
std::optional<std::string> CDBWrapper::ReadImpl(Span<const std::byte> key) const
{
    ...
    if (!status.ok()) {
        if (status.IsNotFound())
            return std::nullopt;
        LogInfo(m_log, "LevelDB read failure: %s\n", status.ToString());
        HandleError(m_log, status);
    }
    return strValue;
}

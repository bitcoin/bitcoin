...
// was reached
int const DONT_STORE_MAINNET_STATE_UNTIL = 0;

#define TEST_ECO_PROPERTY_1 (0x80000003UL)

// increment this value to force a refresh of the state (similar to --omnistartclean)
#define DB_VERSION 8
...
extern std::map<uint32_t, int64_t> global_balance_money GUARDED_BY(cs_tally);
//! Reserved balances of wallet propertiess
extern std::map<uint32_t, int64_t> global_balance_reserved GUARDED_BY(cs_tally);
//! Vector containing a list of properties relative to the wallet
extern std::set<uint32_t> global_wallet_property_list GUARDED_BY(cs_tally);
...
extern std::unordered_map<std::string, CMPTally> mp_tally_map GUARDED_BY(cs_tally);

...
std::map<string, CMPTally> walletCache;
...
int WalletCacheUpdate()
{
    ...
    for(std::map<string, CMPTally>::iterator my_it = mp_tally_map.begin(); my_it != mp_tally_map.end(); ++my_it) {
        ...
        std::map<std::string, CMPTally>::iterator search_it = walletCache.find(address);
        if (search_it == walletCache.end()) { // cache miss, new address
            ++numChanges;
            changedAddresses.insert(address);
            walletCache.insert(std::make_pair(address,tally));

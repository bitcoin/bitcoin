...
int populateRPCTransactionObject(const uint256& txid, Object *txobj, std::string filterAddress, bool extendedDetails, std::string extendedDetailsFilter)
{
            ...
            if (0 < parseRC) {
                ...
                if (0<numberOfPurchases) {
                    for(int purchaseNumber = 1; purchaseNumber <= numberOfPurchases; purchaseNumber++) {
                        ...
                        if (!bIsMine) bIsMine = IsMyAddress(seller);
                        if (!filterAddress.empty()) {
                            if ((buyer != filterAddress) && (seller != filterAddress)) return -1; // return negative rc if filtering & no match
                        }
                ...
                if (0<=mp_obj.step1()) {
                    ...
                    if (!filterAddress.empty()) {
                        if ((senderAddress != filterAddress) && (refAddress != filterAddress)) return -1; // return negative rc if filtering & no match
                    }
                    isMPTx = true;
    ...
    if (isMPTx) {
        ...
        if (!bIsMine) bIsMine = IsMyAddress(refAddress);
        txobj->push_back(Pair("txid", wtxid.GetHex()));
        ...
        // add extended details if requested
        if (extendedDetails) {
            if (MSC_TYPE_SEND_TO_OWNERS == MPTxTypeInt) {
                Array receiveArray;
                uint64_t tmpAmount = 0;
                uint64_t stoFee = 0;
                s_stolistdb->getRecipients(wtxid, extendedDetailsFilter, &receiveArray, &tmpAmount, &stoFee);
            ...
            if (MSC_TYPE_METADEX == MPTxTypeInt) {
                if (mdex_action == 1) {
                    ...
                    int64_t totalSold = 0;
                    t_tradelistdb->getMatchingTrades(wtxid, propertyId, &tradeArray, &totalSold, &totalBought);
                    std::string statusText = MetaDEx_getStatus(wtxid, propertyId, amount, totalSold, totalBought);
                    txobj->push_back(Pair("status", statusText));
                    if((statusText == "cancelled") || (statusText == "cancelled part filled")) txobj->push_back(Pair("canceltxid", p_txlistdb->findMetaDExCancel(wtxid).GetHex()));
                    txobj->push_back(Pair("matches", tradeArray));
                } else {
                    Array cancelArray;
                    int numberOfCancels = p_txlistdb->getNumberOfMetaDExCancels(wtxid);
                    if (0<numberOfCancels) {
                        for(int refNumber = 1; refNumber <= numberOfCancels; refNumber++) {
                            Object cancelTx;
                            string strValue = p_txlistdb->getKeyValue(wtxid.ToString() + "-C" + to_string(refNumber));
                            if (strValue.empty()) continue;
                            std::vector<std::string> vstr;
                            boost::split(vstr, strValue, boost::is_any_of(":"), token_compress_on);
                            if (vstr.size() != 3) {
                                file_log("TXListDB Error - trade cancel number of tokens is not as expected (%s)\n", strValue);
                                continue;
                            }
    ...
    for (CWallet::TxItems::reverse_iterator it = txOrdered.rbegin(); it != txOrdered.rend(); ++it) {
        ...
        if (pwtx != 0) {
            ...
            for(uint32_t i = 0; i<vecReceipts.size(); i++) {
                ...
                if (svstr.size() != 4) {
                    file_log("STODB Error - number of tokens is not as expected (%s)\n", vecReceipts[i]);
                    continue;
                }

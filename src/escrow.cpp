...
UniValue escrowrefund(const UniValue& params, bool fHelp) {
	...
	scriptPubKeyOrigSeller << CScript::EncodeOP_N(OP_ESCROW_REFUND) << vchEscrow << vchFromString("0") << vchHashEscrow << OP_2DROP << OP_2DROP;
...
void BuildFeedbackJson(const CEscrow& escrow, UniValue& oFeedback) {
	...
	const string &id = stringFromVch(escrow.vchEscrow) + CFeedback::FeedbackUserToString(escrow.feedback.nFeedbackUserTo);

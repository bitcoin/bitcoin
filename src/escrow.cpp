...
bool CheckEscrowInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs, const std::vector<std::vector<unsigned char> > &vvchAliasArgs, bool fJustCheck, int nHeight, string &errorMessage, bool dontaddtodb) {
	...
	if(fJustCheck)
	{
		...
		for (unsigned int i = 0; i < tx.vin.size(); i++) {
			...
			if (foundAlias)
				break;
			else if (!foundAlias && IsAliasOp(pop) && vvch.size() >= 2 && vvchAliasArgs.size() >= 2 && theEscrow.vchLinkAlias == vvch[0] && vvchAliasArgs[1] == vvch[1])
			{
				foundAlias = true;
				prevAliasOp = pop;
	...
	else
	{
		...
		if (!fJustCheck && pescrowdb->ReadISLock(serializedEscrow.vchEscrow, bSendLocked) && bSendLocked) {
			...
			if (theEscrow.txHash != tx.GetHash())
			{
				...
				if (op != OP_ESCROW_ACTIVATE && !pescrowdb->ReadLastEscrow(serializedEscrow.vchEscrow, theEscrow)) {
					theEscrow.SetNull();
				}
				if (!dontaddtodb) {
				...
				if (!dontaddtodb) {
					if (fDebug)
						LogPrintf("CONNECTED ESCROW: op=%s escrow=%s hash=%s height=%d fJustCheck=%d POW IS\n",
							...
							nHeight,
							fJustCheck ? 1 : 0);
					if (!pescrowdb->Write(make_pair(std::string("escrowp"), serializedEscrow.vchEscrow), theEscrow))
					{
	...
	if (op != OP_ESCROW_ACTIVATE)
	{
		...
		if (op == OP_ESCROW_BID) {
			...
			theEscrow.vchLinkAlias = serializedEscrow.vchLinkAlias;
			// write escrow bid
			if (!dontaddtodb)
			{
		...
		else if (op == OP_ESCROW_REFUND)
		{
			...
			if (serializedEscrow.vchLinkAlias != theEscrow.vchSellerAlias && serializedEscrow.vchLinkAlias != theEscrow.vchArbiterAlias)
			{
		...
		else if (op == OP_ESCROW_FEEDBACK)
		{
			...
			if (serializedEscrow.feedback.nFeedbackUserFrom == FEEDBACKBUYER && serializedEscrow.vchLinkAlias != theEscrow.vchBuyerAlias)
			{
				errorMessage = "SYSCOIN_ESCROW_CONSENSUS_ERROR: ERRCODE: 4064 - " + _("Only buyer can leave this feedback");
				return true;
			}
			if (serializedEscrow.feedback.nFeedbackUserFrom == FEEDBACKSELLER && serializedEscrow.vchLinkAlias != vchSellerAlias)
			{
				errorMessage = "SYSCOIN_ESCROW_CONSENSUS_ERROR: ERRCODE: 4065 - " + _("Only seller can leave this feedback");
				return true;
			}
			if (serializedEscrow.feedback.nFeedbackUserFrom == FEEDBACKARBITER && serializedEscrow.vchLinkAlias != theEscrow.vchArbiterAlias)
			{
...
UniValue escrowbid(const UniValue& params, bool fHelp) {
	...
	theEscrow.vchLinkAlias = bidderalias.vchAlias;
...
UniValue escrowacknowledge(const UniValue& params, bool fHelp) {
	...
	coinControl.fAllowOtherInputs = false;
	coinControl.fAllowWatchOnly = false;
	SendMoneySyscoin(escrow.vchLinkAlias, vchWitness, aliasRecipient, aliasPaymentRecipient, vecSend, wtx, &coinControl);
	UniValue res(UniValue::VARR);
	res.push_back(EncodeHexTx(wtx));
	return res;
...
UniValue escrowrelease(const UniValue& params, bool fHelp) {
	...
	escrow.vchLinkAlias = theAlias.vchAlias;
	vector<unsigned char> data;

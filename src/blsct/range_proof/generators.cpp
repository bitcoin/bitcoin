...
GeneratorsFactory<T>::GeneratorsFactory()
{
    ...
    const Point default_G = m_generator_deriver.Derive(Point::GetBasePoint(), 0, default_token_id);
    ...
    for (size_t i = 0; i < Config::m_max_input_value_vec_len; ++i) {
        ...
        Point hi = m_generator_deriver.Derive(default_G, base_index + 1, default_token_id);
        Point gi = m_generator_deriver.Derive(default_G, base_index + 2, default_token_id);
        m_Hi.value().Add(hi);
        m_Gi.value().Add(gi);
    }
...
Generators<T> GeneratorsFactory<T>::GetInstance(const TokenId& token_id)
{
    ...
    if (GeneratorsFactory<T>::m_G_cache.count(token_id) == 0) {
        const Point G = m_generator_deriver.Derive(GeneratorsFactory<T>::m_H.value(), 0, token_id);

 ...
 *
 * TxGraph implicitly maintains an associated total ordering on its transactions (its linearization)
...
public:
    ...
    /** Get pointers to all transactions in the which arg is in. The transactions will be returned
     *  in graph order. Returns {} if arg does not exist in the graph. */
    virtual std::vector<Ref*> GetCluster(const Ref& arg) noexcept = 0;
    /** Get pointers to all ancestors of the specified transaction, in unspecified order. The
     *  graph must not be oversized. Returns {} if arg does not exist in the graph. */
    virtual std::vector<Ref*> GetAncestors(const Ref& arg) noexcept = 0;
    /** Get pointers to all descendants of the specified transaction, in unspecified order. The
    *   graph must not be oversized. Returns {} if arg does not exist in the graph. */

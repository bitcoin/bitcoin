 ...
 *
 * For each graph, TxGraph implicitly maintains an associated total ordering on its transactions
...
public:
    ...
    /** Get pointers to all ancestors of the specified transaction, in unspecified order. If
     *  main_only is false and a staging graph exists, it is queried; otherwise the main graph is
     *  queried. The queried graph must not be oversized. Returns {} if arg does not exist in the
     *  graph. */
    virtual std::vector<Ref*> GetAncestors(const Ref& arg, bool main_only = false) noexcept = 0;
    /** Get pointers to all descendants of the specified transaction, in unspecified order. If
     *  main_only is false and a staging graph exists, it is queried; otherwise the main graph is
     *  queried. The queried graph must not be oversized. Returns {} if arg does not exist in the
     *  graph. */
     ...
     *  BlockBuilder created now. The chunk is returned in reverse-topological order, so every
     *  element is preceded by all its descendants. If the graph is empty, {} is returned. */

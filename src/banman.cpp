...
void BanMan::LoadBanlist()
{
    ...
    if (m_ban_db.Read(m_banned)) {
        ...
        LogPrint(LogInstance(), BCLog::NET, "Loaded %d banned node addresses/subnets  %dms\n", m_banned.size(),
                 Ticks<std::chrono::milliseconds>(SteadyClock::now() - start));
    } else {
        LogPrintf(LogInstance(), "Recreating the banlist database\n");
        m_banned = {};
        m_is_dirty = true;
    }
}
...
void BanMan::DumpBanlist()
{
    ...
    LogPrint(LogInstance(), BCLog::NET, "Flushed %d banned node addresses/subnets to disk  %dms\n", banmap.size(),
             Ticks<std::chrono::milliseconds>(SteadyClock::now() - start));
}
...
void BanMan::SweepBanned()
{
    ...
    while (it != m_banned.end()) {
        ...
        if (!sub_net.IsValid() || now > ban_entry.nBanUntil) {
            ...
            m_is_dirty = true;
            notify_ui = true;
            LogPrint(LogInstance(), BCLog::NET, "Removed banned node address/subnet: %s\n", sub_net.ToString());
        } else {
            ++it;
        }
    }

...
public:
    ...
    public:
         ...
         * @param[out] net Target network, will only be set if `true` is returned.
         * @param[out] proxy Optional proxy, will only be set if `true` is returned (could be `nullopt`).
         * @retval true The network was selected and `net` and `proxy` were set.
         * @retval false No network could be selected, `net` and `proxy` were not set.
         */
        bool PickNetwork(Network& net, std::optional<Proxy>& proxy) const;
         ...
         */
        void NumToOpenSub(size_t n) EXCLUSIVE_LOCKS_REQUIRED(!m_num_to_open_mutex);

...
static bool ExecuteWitnessScript(const Span<const valtype>& stack_span, const CScript& witness_script, unsigned int flags, SigVersion sigversion, const BaseSignatureChecker& checker, ScriptExecutionData& execdata, ScriptError* serror)
{
    ...
    if (!EvalScript(stack, witness_script, flags, checker, sigversion, execdata, serror)) return false;

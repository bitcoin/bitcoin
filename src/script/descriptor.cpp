...
public:
    ...
    // NOLINTNEXTLINE(misc-no-recursion)
    virtual bool ToStringHelper(const SigningProvider* arg, std::string& out, const StringType type, bool& has_priv_key, const DescriptorCache* cache = nullptr) const
    {
    ...
    {
        bool has_priv_key;
        bool ret = ToStringHelper(&arg, out, StringType::NORMALIZED, has_priv_key, cache);
        out = AddChecksum(out);
        return ret;
    }
    ...
    // Process miniscript expressions.
    {
        ...
        if (node) {
            ...
            if (!node->IsSane() || node->IsNotSatisfiable()) {
                ...
                bool has_priv_key;
                error = insane_node->ToString(parser, has_priv_key);

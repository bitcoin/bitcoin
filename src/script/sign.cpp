...
TransactionSignatureCreator::TransactionSignatureCreator(const CTransaction* txToIn, unsigned int nInIn, const std::vector<uint8_t>& amountIn, int nHashTypeIn) : BaseSignatureCreator(), txTo(txToIn), nIn(nInIn), nHashType(nHashTypeIn), amount(amountIn), checker(txTo, nIn, amountIn) {}
                     ...
                     std::vector<valtype>& ret, txnouttype& whichTypeRet, SigVersion sigversion)
{
    ...
    case TX_SCRIPTHASH256:
        ...
        if (vSolutions[0].size() == 32)
            ...
            return false;
        if (provider.GetCScript(idScript, scriptRet)) {

...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKeyIn, SignatureData& sigdata)
{
    ...
    bool solved = SignStep(provider, creator, fromPubKeyOut, result, whichType, SigVersion::BASE, sigdata);
    bool P2SH = false;
    CScript subscript;
    ...
    // Test solution
    sigdata.complete = solved && VerifyScript(sigdata.scriptSig, fromPubKeyOut, &sigdata.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, creator.Checker());

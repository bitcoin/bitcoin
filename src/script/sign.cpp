...
bool SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, const SignOptions& options, std::map<int, bilingual_str>& input_errors)
{
    ...
    ScriptError serror{SCRIPT_ERR_OK};
    if (txdata.m_spent_outputs_ready && !VerifyCISATransaction(CTransaction(mtx), txdata.m_spent_outputs, STANDARD_SCRIPT_VERIFY_FLAGS, txdata, &serror)) {
        for (unsigned int i = 0; i < mtx.vin.size(); ++i) {
            if (AggregatedCISAMarker(txdata.m_spent_outputs[i], mtx.vin[i].scriptWitness)) input_errors[i] = Untranslated(ScriptErrorString(serror));
        }
    }

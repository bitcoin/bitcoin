...
struct ScriptTraceFrame {
    ...
    std::optional<ScriptError> script_error;
};
...
struct ScriptTraceScope {
    ...
    ~ScriptTraceScope()
    {
        Emit(ScriptTraceFrameKind::End, /*exec=*/true, /*opcode=*/0, m_error ? std::optional{*m_error} : std::nullopt);
    }

private:
    void Emit(ScriptTraceFrameKind kind, bool exec, uint8_t opcode, std::optional<ScriptError> error) const
    {
